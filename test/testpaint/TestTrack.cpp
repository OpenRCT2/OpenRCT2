...
public:
    ...
    virtual void ApplyTo(uint8 rideType, uint8 trackType, int variant, rct_map_element * mapElement,
                         rct_map_element * surfaceElement, rct_ride * ride, rct_ride_entry * rideEntry) abstract;
};
...
public:
    ...
    virtual void ApplyTo(uint8 rideType, uint8 trackType, int variant, rct_map_element * mapElement,
                         rct_map_element * surfaceElement, rct_ride * ride, rct_ride_entry * rideEntry)
    {
        if (variant == 0)
        {
...
static void CallOriginal(uint8 rideType, uint8 trackType, uint8 direction, uint8 trackSequence, uint16 height,
                         rct_map_element * mapElement)
{
    ...
    RCT2_CALLPROC_X(0x006C4934, rideType, (int)trackDirectionList, direction, height, (int)mapElement,
                    rideIndex * sizeof(rct_ride), trackSequence);
}

static void CallNew(uint8 rideType, uint8 trackType, uint8 direction, uint8 trackSequence, uint16 height,
                    rct_map_element * mapElement)
{
...
static uint8 TestTrackElementPaintCalls(uint8 rideType, uint8 trackType, uint8 trackSequence, std::string * error)
{
    ...
    for (auto && arguments : argumentPermutations)
    {
        ...
        for (size_t filterIndex = 0; filterIndex < activeFilters.size(); ++filterIndex)
        {
            ...
            activeFilters[filterIndex]->ApplyTo(rideType, trackType, variant, &mapElement, &surfaceElement, &(gRideList[0]),
                                                gRideEntries[0]);
        }
        ...
        for (int currentRotation = 0; currentRotation < 4; currentRotation++)
        {
            ...
            for (int direction = 0; direction < 4; direction++)
            {
                ...
                std::string caseName = String::Format("%srotation:%d direction:%d trackSequence:%d]", baseCaseName.c_str(),
                                                      currentRotation, direction, trackSequence);
                ...
                {
                    *error += String::Format("Call counts don't match (was %d, expected %d). %s\n", newCalls.size(),
                                             oldCalls.size(), caseName.c_str());
                    sucess = false;
                }
...
static uint8 TestTrackElementSegmentSupportHeight(uint8 rideType, uint8 trackType, uint8 trackSequence, std::string * error)
{
    ...
    for (int direction = 0; direction < 4; direction++)
    {
        ...
        std::vector<SegmentSupportCall> newCalls = SegmentSupportHeightCall::getSegmentCalls(gSupportSegments, direction);
        ...
        if (referenceCall.height != -1)
        {
            if (gSupport.height != referenceCall.height)
            {
                *error += String::Format(
                    ...
                    referenceCall.height - height, gSupport.height - height, direction, state.c_str());
                return TEST_FAILED;
            }
        }
            ...
            if (gSupport.slope != referenceCall.slope)
            {
                *error +=
                    String::Format("General support slopes didn't match. (expected 0x%02X, actual: 0x%02X) [direction:%d] %s\n",
                                   referenceCall.slope, gSupport.slope, direction, state.c_str());
                return TEST_FAILED;
            }
        }
    }
...
static uint8 TestTrackElementSideTunnels(uint8 rideType, uint8 trackType, uint8 trackSequence, std::string * error)
{
    ...
    if (!SideTunnelCall::TunnelCallsLineUp(tileTunnelCalls))
    {
        ...
        *error += String::Format("Original tunnel calls don\'t line up. Skipping tunnel validation [trackSequence:%d].\n",
                                 trackSequence);
...
static uint8 TestTrackElementVerticalTunnels(uint8 rideType, uint8 trackType, uint8 trackSequence, std::string * error)
{
    ...
    {
        *error += String::Format("Original vertical tunnel height is inconsistent, skipping test. [trackSequence:%d]\n",
                                 trackSequence);
        return TEST_SUCCESS;
    }
    ...
    for (int direction = 0; direction < 4; direction++)
    {
        ...
        if (gVerticalTunnelHeight != referenceHeight)
        {
            ...
            {
                *error +=
                    String::Format("Expected no tunnel. Actual: %d [trackSequence:%d]\n", gVerticalTunnelHeight, trackSequence);
                return TEST_FAILED;
            }

            *error += String::Format("Expected vertical tunnel height to be `%s`, was `%s`. [trackSequence:%d direction:%d]\n",
                                     Printer::PrintHeightOffset((referenceHeight * 16), height).c_str(),
                                     Printer::PrintHeightOffset((gVerticalTunnelHeight * 16), height).c_str(), trackSequence,
                                     direction);
