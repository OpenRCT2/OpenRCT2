...
TEST_F(PlayTests, SecondGuestInQueueShouldNotRideIfNoFunds)
{
    ...
    bool matched = updateUntil(*gs, 1000, [&]() { return richGuest->State == PeepState::PEEP_STATE_QUEUING; });
    ASSERT_TRUE(matched);
    ...
    matched = updateUntil(*gs, 1000, [&]() { return poorGuest->State == PeepState::PEEP_STATE_QUEUING; });
    ASSERT_TRUE(matched);
    ...
    matched = updateUntil(*gs, 10000, [&]() {
        enteredTheRide |= poorGuest->State == PeepState::PEEP_STATE_ON_RIDE;
        return poorGuest->State == PeepState::PEEP_STATE_WALKING || enteredTheRide;
    });
...
TEST_F(PlayTests, CarRideWithOneCarOnlyAcceptsTwoGuests)
{
    ...
    auto guestIsOnRide = [](auto* g) { return g->State == PeepState::PEEP_STATE_ON_RIDE; };
