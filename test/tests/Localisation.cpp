...
TEST_F(Localisation, RCT2_to_UTF8_UK)
{
    ...
    auto expected = reinterpret_cast<const utf8*>(u8"The quick brown fox");
...
TEST_F(Localisation, RCT2_to_UTF8_JP)
{
    ...
    auto expected = reinterpret_cast<const utf8*>(u8"ファストブラウンフォックス");
...
TEST_F(Localisation, RCT2_to_UTF8_PL)
{
    ...
    auto expected = reinterpret_cast<const utf8*>(u8"Górska Dołki Tęczowy");
    auto actual = rct2_to_utf8(input, RCT2LanguageId::EnglishUK);
    ASSERT_EQ(expected, actual);
}
...
TEST_F(Localisation, RCT2_to_UTF8_ZH_TW_PREMATURE_END)
{
    ...
    auto expected = reinterpret_cast<const utf8*>(u8"小型車輛在鐵道上振動搖");
    auto actual = rct2_to_utf8(input, RCT2LanguageId::ChineseTraditional);
    ASSERT_EQ(expected, actual);
}
...
{
    auto input = reinterpret_cast<const utf8*>(u8"à l'époque était");
...
{
    auto input = reinterpret_cast<const utf8*>(u8"$: 快速的棕色狐狸");
...
{
    auto input = reinterpret_cast<const utf8*>(u8"Górska Dołki Tęczowy");
