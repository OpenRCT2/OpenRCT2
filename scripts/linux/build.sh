...
pushd build
	...
	then
		...
		docker run -v "$PARENT":"$PARENT" -w "$PARENT"/build -i -t openrct2/openrct2:64bit-only bash -c "cmake ../ -DBUILD_TESTING=YES $OPENRCT2_CMAKE_OPTS && ninja $OPENRCT_MAKE_OPTS && ctest --output-on-failure"
	...
	then
		...
		chmod g+s "$(pwd)"
		# CMAKE and MAKE opts from environment
		docker run -v "$PARENT":"$PARENT" -w "$PARENT"/build -i -t openrct2/openrct2:ubuntu_amd64 bash -c "cmake ../ -DBUILD_TESTING=YES $OPENRCT2_CMAKE_OPTS && ninja $OPENRCT_MAKE_OPTS install && ctest --output-on-failure"
	elif [[ $TARGET == "windows" ]]
	then
		PARENT=$(readlink -f ../)
		chmod a+rwx "$(pwd)"
