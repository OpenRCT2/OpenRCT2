        ...
        else if (strcmp(argv[0], "set") == 0)
        {
            ...
            if (strcmp(argv[1], "type") == 0)
            {
                ...
                else
                {
                    ...
                    int res                = game_do_command(0, (type << 8) | 1, 0, (RIDE_SETTING_RIDE_TYPE << 8) | ride_index,
        ...
        else if (strcmp(argv[0], "location") == 0)
        {
            ...
            if (w != NULL)
            {
                ...
                get_map_coordinates_from_pos(w->viewport->view_width / 2, w->viewport->view_height / 2,
                                             VIEWPORT_INTERACTION_MASK_TERRAIN, &mapCoord.x, &mapCoord.y, &interactionType,
                                             &mapElement, NULL);
                mapCoord.x -= 16;
                mapCoord.x /= 32;
                mapCoord.y -= 16;
...
static int cc_set(const utf8 ** argv, int argc)
{
    ...
    if (argc > 1)
    {
        ...
        {
            gGuestInitialCash =
                clamp(MONEY((int)double_val[0], ((int)(double_val[0] * 100)) % 100), MONEY(0, 0), MONEY(1000, 0));
        ...
        else if (strcmp(argv[0], "construction_rights_cost") == 0 && invalidArguments(&invalidArgs, double_valid[0]))
        {
            gConstructionRightsPrice =
                clamp(MONEY((int)double_val[0], ((int)(double_val[0] * 100)) % 100), MONEY(0, 0), MONEY(200, 0));
        ...
        else if (strcmp(argv[0], "cheat_sandbox_mode") == 0 && invalidArguments(&invalidArgs, int_valid[0]))
        {
            ...
            {
                if (game_do_command(0, GAME_COMMAND_FLAG_APPLY, CHEAT_SANDBOXMODE, (int_val[0] != 0), GAME_COMMAND_CHEAT, 0,
                                    0) != MONEY32_UNDEFINED)
                {
        ...
        else if (strcmp(argv[0], "cheat_disable_clearance_checks") == 0 && invalidArguments(&invalidArgs, int_valid[0]))
        {
            ...
            {
                if (game_do_command(0, GAME_COMMAND_FLAG_APPLY, CHEAT_DISABLECLEARANCECHECKS, (int_val[0] != 0),
                                    GAME_COMMAND_CHEAT, 0, 0) != MONEY32_UNDEFINED)
                {
                    // Change it locally so it shows the accurate value in the
        ...
        else if (strcmp(argv[0], "cheat_disable_support_limits") == 0 && invalidArguments(&invalidArgs, int_valid[0]))
        {
            ...
            {
                if (game_do_command(0, GAME_COMMAND_FLAG_APPLY, CHEAT_DISABLESUPPORTLIMITS, (int_val[0] != 0),
                                    GAME_COMMAND_CHEAT, 0, 0) != MONEY32_UNDEFINED)
                {
                    // Change it locally so it shows the accurate value in the
...
utf8 * console_window_table[] = { "object_selection", "inventions_list", "scenario_options",
                                  "options",          "themes",          "title_sequences" };
...
console_command console_command_table[] = {
    ...
    { "reset_user_strings", cc_reset_user_strings,
      "Resets all user-defined strings, to fix incorrectly occurring 'Chosen name in use already' errors.",
      "reset_user_strings" },
    { "fix_banner_count", cc_fix_banner_count,
      "Fixes incorrectly appearing 'Too many banners' error by marking every banner entry without a map element as null.",
      "fix_banner_count" },
