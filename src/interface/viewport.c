...
void center_2d_coordinates(int x, int y, int z, int * out_x, int * out_y, rct_viewport * viewport)
{
    ...
    rct_xyz16 coord_3d = {.x = x, .y = y, .z = z };
...
*  w:      esi
*/
void viewport_create(rct_window * w, int x, int y, int width, int height, int zoom, int center_x, int center_y, int center_z,
                     char flags, sint16 sprite)
{
...
static void viewport_shift_pixels(rct_drawpixelinfo * dpi, rct_window * w, rct_viewport * viewport, sint16 x_diff,
                                  sint16 y_diff)
{
...
static void viewport_paint_weather_gloom(rct_drawpixelinfo * dpi)
{
    ...
    if (gloom != 0)
    {
        gfx_filter_rect(dpi, dpi->x, dpi->y, dpi->width + dpi->x - 1, dpi->height + dpi->y - 1,
...
void screen_pos_to_map_pos(sint16 * x, sint16 * y, int * direction)
{
    ...
    if (dist_from_center_x > 8 && dist_from_center_x < 24 && dist_from_center_y > 8 && dist_from_center_y < 24)
    {
        my_direction = 4;
    }
    else
...
void viewport_set_visibility(uint8 mode)
{
    ...
    if (window != NULL)
    {
        ...
        { // Set all these flags to 0, and invalidate if any were active
            uint32 mask = VIEWPORT_FLAG_UNDERGROUND_INSIDE | VIEWPORT_FLAG_SEETHROUGH_RIDES | VIEWPORT_FLAG_SEETHROUGH_SCENERY |
                          VIEWPORT_FLAG_SEETHROUGH_PATHS | VIEWPORT_FLAG_INVISIBLE_SUPPORTS | VIEWPORT_FLAG_LAND_HEIGHTS |
                          VIEWPORT_FLAG_TRACK_HEIGHTS | VIEWPORT_FLAG_PATH_HEIGHTS | VIEWPORT_FLAG_INVISIBLE_PEEPS |
 ...
 * viewport: edi
 */
void get_map_coordinates_from_pos(int screenX, int screenY, int flags, sint16 * x, sint16 * y, int * interactionType,
                                  rct_map_element ** mapElement, rct_viewport ** viewport)
{
    ...
    if (y != NULL)
        *y = _unk9AC14E;
    if (mapElement != NULL)
        *mapElement = _interaction_element;
}
...
void screen_get_map_xy(int screenX, int screenY, sint16 * x, sint16 * y, rct_viewport ** viewport)
{
    ...
    get_map_coordinates_from_pos(screenX, screenY, VIEWPORT_INTERACTION_MASK_TERRAIN, &my_x, &my_y, &interactionType, NULL,
                                 &myViewport);
