    ...
    if (entryIndex >= 0x10000)
    {
        ...
        if (!(rideEntry->flags & RIDE_ENTRY_FLAG_SEPARATE_RIDE))
        {
            for (i = 0; i < 128; i++)
            {
                ...
                if (rideEntry2->ride_type[0] == base_ride_type || rideEntry2->ride_type[1] == base_ride_type ||
        ...
        set_format_arg(0, rct_string_id, ((rideEntry->flags & RIDE_ENTRY_FLAG_SEPARATE_RIDE_NAME))
                                             ? rideEntry->name
                                             : RideNaming[base_ride_type].name);
        if (!gSilentResearch)
        {
...
void research_remove_non_separate_vehicle_types()
{
    ...
    {
    loopBeginning:
        if (researchItem != gResearchItems && researchItem->entryIndex != RESEARCHED_ITEMS_SEPARATOR &&
            researchItem->entryIndex != RESEARCHED_ITEMS_END && researchItem->entryIndex >= 0x10000)
        {
            ...
            if (!(rideEntry->flags & (RIDE_ENTRY_FLAG_SEPARATE_RIDE | RIDE_ENTRY_FLAG_SEPARATE_RIDE_NAME)))
            {
                ...
                do
                {
                    if (researchItem2->entryIndex != RESEARCHED_ITEMS_SEPARATOR && researchItem2->entryIndex >= 0x10000)
                    {
