...
static const uint32 fadeSprites[] = {
    SPR_NONE, SPR_FADE_1, SPR_FADE_2, SPR_FADE_3, SPR_FADE_4, SPR_FADE_5, SPR_FADE_6, SPR_FADE_7,
};
 ...
 *  rct2: 0x0067A690
 */
static void FASTCALL gfx_bmp_sprite_to_buffer(uint8 * palette_pointer, uint8 * unknown_pointer, uint8 * source_pointer,
                                              uint8 * dest_pointer, rct_g1_element * source_image, rct_drawpixelinfo * dest_dpi,
                                              int height, int width, int image_type)
{
                                                   ...
                                                   uint8 * unknown_pointer)
{
    ...
    if (dpi->zoom_level != 0 && (g1_source->flags & G1_FLAG_HAS_ZOOM_SPRITE))
    {
        ...
        gfx_draw_sprite_palette_set_software(&zoomed_dpi, image_type | (image_element - g1_source->zoomed_offset), x >> 1,
                                             y >> 1, palette_pointer, unknown_pointer);
        return;
    }
    ...
    if (g1_source->flags & G1_FLAG_RLE_COMPRESSION)
    {
        ...
        gfx_rle_sprite_to_buffer(g1_source->offset, dest_pointer, palette_pointer, dpi, image_type, source_start_y, height,
                                 source_start_x, width);
        return;
    }
    ...
    {
        gfx_bmp_sprite_to_buffer(palette_pointer, unknown_pointer, source_pointer, dest_pointer, g1_source, dpi, height, width,
                                 image_type);
    }
    return;
}
