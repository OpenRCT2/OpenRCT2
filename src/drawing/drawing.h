...
#define SPRITE_ID_PALETTE_COLOUR_2(primaryId, secondaryId)                                                                     \
    (IMAGE_TYPE_REMAP_2_PLUS | IMAGE_TYPE_REMAP | ((primaryId << 19) | (scondaryId << 24)))
...
void sub_68371D();
void FASTCALL gfx_rle_sprite_to_buffer(const uint8 * RESTRICT source_bits_pointer, uint8 * RESTRICT dest_bits_pointer,
                                       const uint8 * RESTRICT palette_pointer, const rct_drawpixelinfo * RESTRICT dpi,
                                       int image_type, int source_y_start, int height, int source_x_start, int width);
...
void FASTCALL gfx_draw_sprite_palette_set_software(rct_drawpixelinfo * dpi, int image_id, int x, int y, uint8 * palette_pointer,
                                                   uint8 * unknown_pointer);
...
void gfx_draw_string_left_clipped(rct_drawpixelinfo * dpi, rct_string_id format, void * args, int colour, int x, int y,
                                  int width);
int gfx_draw_string_left_wrapped(rct_drawpixelinfo * dpi, void * args, int x, int y, int width, rct_string_id format,
                                 int colour);
...
void gfx_draw_string_centred_clipped(rct_drawpixelinfo * dpi, rct_string_id format, void * args, int colour, int x, int y,
                                     int width);
void draw_string_centred_underline(rct_drawpixelinfo * dpi, rct_string_id format, void * args, int colour, int x, int y);
int gfx_draw_string_centred_wrapped(rct_drawpixelinfo * dpi, void * args, int x, int y, int width, rct_string_id format,
                                    int colour);
...
void gfx_draw_string_centred_wrapped_partial(rct_drawpixelinfo * dpi, int x, int y, int width, int colour, rct_string_id format,
                                             void * args, int ticks);
void gfx_draw_string_with_y_offsets(rct_drawpixelinfo * dpi, const utf8 * text, int colour, int x, int y,
                                    const sint8 * yOffsets, bool forceSpriteFont);
