...
{
    if (Source_Sample::format.format != format.format || Source_Sample::format.channels != format.channels ||
        Source_Sample::format.freq != format.freq)
    {
        SDL_AudioCVT cvt;
        if (SDL_BuildAudioCVT(&cvt, Source_Sample::format.format, Source_Sample::format.channels, Source_Sample::format.freq,
...
void Mixer::Init(const char * device)
{
    ...
    for (int i = 0; i < (int)Util::CountOf(css1sources); i++)
    {
        ...
        {
            source_sample->Convert(
                format); // convert to audio output format, saves some cpu usage but requires a bit more memory, optional
    ...
    if (channel.source && channel.source->Length() > 0 && !channel.done)
    {
        ...
        do
        {
            ...
            if (channel.offset < channel.source->Length())
            {
                ...
                {
                    if (SDL_BuildAudioCVT(&cvt, streamformat.format, streamformat.channels, streamformat.freq,
                                          Mixer::format.format, Mixer::format.channels, Mixer::format.freq) == -1)
                    {
                        break;
                    }
                    mustconvert = true;
                ...
                if (mustconvert)
                {
                    // tofix: there seems to be an issue with converting audio using SDL_ConvertAudio in the callback vs
                ...
                if (rate != 1 && format.format == AUDIO_S16SYS)
                {
                    ...
                    speex_resampler_process_interleaved_int(channel.resampler, (const spx_int16_t *)tomix,
                                                            (spx_uint32_t *)&in_len, (spx_int16_t *)effectbuffer,
