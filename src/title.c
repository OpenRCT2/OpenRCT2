...
static const uint8 _magicMountainScript[] = {
    LOADMM(),          LOCATION(210, 112), WAIT(13),          ROTATE(1), LOCATION(210, 112), WAIT(14),
    ROTATE(3),         LOCATION(167, 180), WAIT(12),          ROTATE(1), LOCATION(155, 189), WAIT(12),
    LOCATION(106, 39), WAIT(12),           LOCATION(182, 50), WAIT(12),  ROTATE(3),          LOCATION(209, 47),
    WAIT(12),          ROTATE(1),          LOCATION(159, 93), WAIT(12),  RESTART(),
};
...
static void title_do_next_script_opcode()
{
    ...
    case TITLE_SCRIPT_LOAD:
    {
        ...
        if (title_load_park(path))
        {
            ...
            gTitleScriptSave =
                gConfigTitleSequences.presets[gCurrentPreviewTitleSequence].commands[gTitleScriptCommand].saveIndex;
        }
        else
        {
            log_error("Failed to load: \"%s\" for the title sequence.", path);
            script_opcode = *_currentScript;
            while (script_opcode != TITLE_SCRIPT_LOADMM && script_opcode != TITLE_SCRIPT_LOAD &&
                   script_opcode != TITLE_SCRIPT_RESTART && script_opcode != TITLE_SCRIPT_END)
            {
                title_skip_opcode();
                script_opcode = *_currentScript;
            }
    ...
    case TITLE_SCRIPT_LOADRCT1:
        ...
        {
            script_opcode = *_currentScript;
            while (script_opcode != TITLE_SCRIPT_LOADRCT1 && script_opcode != TITLE_SCRIPT_RESTART &&
                   script_opcode != TITLE_SCRIPT_END)
            {
                title_skip_opcode();
                script_opcode = *_currentScript;
            }
...
bool title_refresh_sequence()
{
    ...
    log_error("Failed to load title sequence, hasLoad: %i, hasWait4seconds: %i, hasRestart: %i, hasInvalidSave: %i", hasLoad,
              hasWait, hasRestart, hasInvalidSave);
    window_error_open(STR_ERR_FAILED_TO_LOAD_TITLE_SEQUENCE,
                      (!hasWait && hasRestart) ? STR_TITLE_EDITOR_ERR_RESTART_REQUIRES_WAIT : STR_NONE);
    _scriptNoLoadsSinceRestart = 1;
    if (_loadedScript != _magicMountainScript)
        SafeFree(_loadedScript);
    _scriptCurrentPreset         = 0;
