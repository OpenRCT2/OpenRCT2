...
static void sub_68F41A(rct_peep *peep, int index)
{
	...
	if ((index & 0x1FF) == (RCT2_GLOBAL(RCT2_ADDRESS_CURRENT_TICKS, uint32) & 0x1FF)){
		...
		if (peep->flags & PEEP_FLAGS_EXPLODE && peep->x != (sint16)0x8000){
			audio_sound_play_panned(SOUND_CRASH, 0x8001, peep->x, peep->y, peep->z);
...
int peep_update_action(sint16* x, sint16* y, sint16* xy_distance, rct_peep* peep){
	...
	*xy_distance = x_delta + y_delta;
	...
	audio_sound_play_panned(sound_id, 0x8001, peep->x, peep->y, peep->z);
...
void peep_update_sprite_type(rct_peep* peep)
{
	if (
		...
		if (
			...
			bl = 1;
			audio_sound_play_panned(SOUND_BALLOON_POP, 0x8001, peep->x, peep->y, peep->z);
		}
...
static void peep_update_ride_sub_state_20(rct_peep* peep){
	...
	audio_sound_play_panned(SOUND_TOILET_FLUSH, 0x8001, peep->x, peep->y, peep->z);
...
void peep_applause()
{
	...
	// Play applause noise
	audio_sound_play_panned(SOUND_APPLAUSE, RCT2_GLOBAL(RCT2_ADDRESS_SCREEN_WIDTH, uint16) / 2, 0, 0, 0);
}
...
static void peep_spend_money(rct_peep *peep, money16 *peep_expend_type, money32 amount)
{
	...
	audio_sound_play_panned(SOUND_PURCHASE, 0x8001, peep->x, peep->y, peep->z);
}
...
static void peep_on_exit_ride(rct_peep *peep, int rideIndex)
{
	...
	if (peep_really_liked_ride(peep, ride)) {
		...
		if (laugh < 3) {
			audio_sound_play_panned(SOUND_LAUGH_1 + laugh, 0x8001, peep->x, peep->y, peep->z);
		}
	}
