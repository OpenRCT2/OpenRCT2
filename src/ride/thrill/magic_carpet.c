...
static void paint_magic_carpet_frame(uint8 plane, uint8 direction, rct_xyz16 offset, rct_xyz16 bbOffset, rct_xyz16 bbSize)
{
    uint32 imageId;
    if (direction & 1)
    {
    ...
    if (plane == PLANE_BACK)
    {
        sub_98197C(imageId, (sint8)offset.x, (sint8)offset.y, bbSize.x, bbSize.y, 127, offset.z, bbOffset.x, bbOffset.y,
                   bbOffset.z, get_current_rotation());
    }
    else
    {
        sub_98199C(imageId, (sint8)offset.x, (sint8)offset.y, bbSize.x, bbSize.y, 127, offset.z, bbOffset.x, bbOffset.y,
                   bbOffset.z, get_current_rotation());
    }
}

static void paint_magic_carpet_pendulum(uint8 plane, uint32 swingImageId, uint8 direction, rct_xyz16 offset, rct_xyz16 bbOffset,
                                        rct_xyz16 bbSize)
{
    ...
    imageId |= gTrackColours[SCHEME_TRACK];
    sub_98199C(imageId, (sint8)offset.x, (sint8)offset.y, bbSize.x, bbSize.y, 127, offset.z, bbOffset.x, bbOffset.y, bbOffset.z,
               get_current_rotation());
}

static void paint_magic_carpet_vehicle(rct_ride * ride, uint8 direction, uint32 swingImageId, rct_xyz16 offset,
                                       rct_xyz16 bbOffset, rct_xyz16 bbSize)
{
    ...
    {
        imageColourFlags =
            0xA0000000 | (ride->vehicle_colours[0].trim_colour << 24) | (ride->vehicle_colours[0].body_colour << 19);
    }
    ...
    sub_98199C(vehicleImageId | imageColourFlags, (sint8)offset.x, (sint8)offset.y, bbSize.x, bbSize.y, 127, offset.z,
...
static void paint_magic_carpet(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height, rct_map_element * mapElement)
{
    ...
    {
    case 3:
        paint_magic_carpet_structure(ride, direction, -48, height);
        break;
    case 0:
        paint_magic_carpet_structure(ride, direction, -16, height);
        break;
    case 2:
        paint_magic_carpet_structure(ride, direction, 16, height);
        break;
