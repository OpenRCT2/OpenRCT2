                                      ...
                                      const rct_ride_entry_vehicle * vehicleEntry)
{
    ...
    if (unk_140E9A8->zoom_level < 2)
    {
        if (vehicle->num_peeps > 0)
        {
            ...
            image_id = (baseImage_id + ((((imageDirection / 8) + 0) & 3) * 3)) | (vehicle->peep_tshirt_colours[0] << 19) |
                       (vehicle->peep_tshirt_colours[1] << 24) | 0xA0000000;
            sub_98199C(image_id, 0, 0, 16, 16, 41, z, -5, -5, z + 1, rotation);
            if (vehicle->num_peeps > 2)
            {
                image_id = (baseImage_id + ((((imageDirection / 8) + 1) & 3) * 3)) | (vehicle->peep_tshirt_colours[2] << 19) |
                           (vehicle->peep_tshirt_colours[3] << 24) | 0xA0000000;
                sub_98199C(image_id, 0, 0, 16, 16, 41, z, -5, -5, z + 1, rotation);
            }
            if (vehicle->num_peeps > 4)
            {
                image_id = (baseImage_id + ((((imageDirection / 8) + 2) & 3) * 3)) | (vehicle->peep_tshirt_colours[4] << 19) |
                           (vehicle->peep_tshirt_colours[5] << 24) | 0xA0000000;
                sub_98199C(image_id, 0, 0, 16, 16, 41, z, -5, -5, z + 1, rotation);
            }
            if (vehicle->num_peeps > 6)
            {
                image_id = (baseImage_id + ((((imageDirection / 8) + 3) & 3) * 3)) | (vehicle->peep_tshirt_colours[6] << 19) |
                           (vehicle->peep_tshirt_colours[7] << 24) | 0xA0000000;
                sub_98199C(image_id, 0, 0, 16, 16, 41, z, -5, -5, z + 1, rotation);
            }
        }
    }
...
/** rct2: 0x006FD1F8 */
static void paint_launched_freefall_base(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height,
                                         rct_map_element * mapElement)
{
    ...
    track_paint_util_paint_fences(edges, position, mapElement, ride, gTrackColours[SCHEME_TRACK], height,
...
/** rct2: 0x006FD208 */
static void paint_launched_freefall_tower_section(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height,
                                                  rct_map_element * mapElement)
{
    if (trackSequence == 1)
    {
