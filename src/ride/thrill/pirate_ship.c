...
static const uint8 track_map_1x5[][5] = {
    { 0, 1, 2, 3, 4 }, { 0, 4, 3, 2, 1 }, { 0, 4, 3, 2, 1 }, { 0, 1, 2, 3, 4 },
};
...
static const pirate_ship_bound_box pirate_ship_data[] = {
    { 31, 16, 1, 8 }, { 16, 31, 8, 1 }, { 31, 16, 1, 8 }, { 16, 31, 8, 1 },
};
...
static void paint_pirate_ship_structure(rct_ride * ride, uint8 direction, sint8 axisOffset, uint16 height)
{
    ...
    sub_98197C(imageId, xOffset, yOffset, bounds.length_x, bounds.length_y, 80, height, bounds.offset_x, bounds.offset_y,
               height, get_current_rotation());

    imageId = baseImageId | imageColourFlags;
    sub_98199C(imageId, xOffset, yOffset, bounds.length_x, bounds.length_y, 80, height, bounds.offset_x, bounds.offset_y,
               height, get_current_rotation());
    ...
    if (dpi->zoom_level <= 1 && ride->lifecycle_flags & RIDE_LIFECYCLE_ON_TRACK && vehicle != NULL)
    {
        ...
        while (peep < 16)
        {
            ...
            imageColourFlags =
                vehicle->peep_tshirt_colours[peep] << 19 | vehicle->peep_tshirt_colours[peep + 1] << 24 | 0xA0000000;
            imageId = (baseImageId + frameNum) | imageColourFlags;
            sub_98199C(imageId, xOffset, yOffset, bounds.length_x, bounds.length_y, 80, height, bounds.offset_x,
...
static void paint_pirate_ship(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height, rct_map_element * mapElement)
{
    ...
    if (direction & 1)
    {
        if (relativeTrackSequence != 1 && relativeTrackSequence != 4)
        {
            ...
            {
                imageId =
                    (hasFence ? SPR_STATION_PLATFORM_FENCED_NW_SE : SPR_STATION_PLATFORM_NW_SE) | gTrackColours[SCHEME_TRACK];
            }
    ...
    else
    {
        if (relativeTrackSequence != 1 && relativeTrackSequence != 4)
        {
            ...
            {
                imageId =
                    (hasFence ? SPR_STATION_PLATFORM_FENCED_SW_NE : SPR_STATION_PLATFORM_SW_NE) | gTrackColours[SCHEME_TRACK];
            }
    ...
    {
    case 1:
        paint_pirate_ship_structure(ride, direction, 64, height);
        break;
    case 2:
        paint_pirate_ship_structure(ride, direction, 32, height);
        break;
    case 0:
        paint_pirate_ship_structure(ride, direction, 0, height);
        break;
    case 3:
        paint_pirate_ship_structure(ride, direction, -32, height);
        break;
