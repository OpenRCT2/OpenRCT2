...
/** rct2: 0x0097C3AF */
const uint8 gRideClassifications[255] = { RIDE_CLASS_RIDE,
                                          RIDE_CLASS_RIDE,
                                          RIDE_CLASS_RIDE,
                                          RIDE_CLASS_RIDE,
                                          RIDE_CLASS_RIDE,
...
static const int RideInspectionInterval[] = { 10, 20, 30, 45, 60, 120, 0, 0 };
...
bool sub_6CA2DF(int * trackType, int * trackDirection, int * rideIndex, int * edxRS16, int * x, int * y, int * z,
                int * properties);
...
bool track_block_get_next_from_zero(sint16 x, sint16 y, sint16 z_start, uint8 rideIndex, uint8 direction_start,
                                    rct_xy_element * output, int * z, int * direction)
{
    ...
    do
    {
        ...
        uint8 nextRotation = ((nextTrackCoordinate->rotation_begin + (mapElement->type & MAP_ELEMENT_DIRECTION_MASK)) &
                              MAP_ELEMENT_DIRECTION_MASK) |
...
bool track_block_get_next(rct_xy_element * input, rct_xy_element * output, int * z, int * direction)
{
    ...
    uint8 directionStart =
        ((trackCoordinate->rotation_end + rotation) & MAP_ELEMENT_DIRECTION_MASK) | (trackCoordinate->rotation_end & (1 << 2));
                                        ...
                                        track_begin_end * outTrackBeginEnd)
{
    ...
    do
    {
        ...
        uint8 nextRotation = ((nextTrackCoordinate->rotation_end + (mapElement->type & MAP_ELEMENT_DIRECTION_MASK)) &
                              MAP_ELEMENT_DIRECTION_MASK) |
        ...
        nextRotation = ((nextTrackCoordinate->rotation_begin + (mapElement->type & MAP_ELEMENT_DIRECTION_MASK)) &
                        MAP_ELEMENT_DIRECTION_MASK) |
                       (nextTrackCoordinate->rotation_begin & (1 << 2));
        ...
        outTrackBeginEnd->begin_z += get_track_def_from_ride(ride, mapElement->properties.track.type)->z - nextTrackBlock->z;
...
bool track_block_get_previous(int x, int y, rct_map_element * mapElement, track_begin_end * outTrackBeginEnd)
{
    ...
    rotation = ((trackCoordinate->rotation_begin + rotation) & MAP_ELEMENT_DIRECTION_MASK) |
...
void ride_get_status(int rideIndex, rct_string_id * formatSecondary, int * argument)
{
    ...
    if (ride->mode == RIDE_MODE_RACE && !(ride->lifecycle_flags & RIDE_LIFECYCLE_PASS_STATION_NO_STOPPING) &&
        ride->race_winner != 0xFFFF && (GET_PEEP(ride->race_winner))->sprite_identifier == SPRITE_IDENTIFIER_PEEP)
    {
 ...
 * bp : flags
 */
int sub_6C683D(int * x, int * y, int * z, int direction, int type, uint16 extra_params, rct_map_element ** output_element,
               uint16 flags)
{
    rct_map_element * mapElement        = map_get_first_element_at(*x / 32, *y / 32);
...
void ride_restore_provisional_entrance_or_exit()
{
    ...
    {
        game_do_command(_unkF440BF.x, (GAME_COMMAND_FLAG_APPLY | GAME_COMMAND_FLAG_ALLOW_DURING_PAUSED | GAME_COMMAND_FLAG_5 |
                                       GAME_COMMAND_FLAG_GHOST) |
                                          (_unkF440BF.direction << 8),
                        _unkF440BF.y, _currentRideIndex | (gRideEntranceExitPlaceType << 8),
                        GAME_COMMAND_PLACE_RIDE_ENTRANCE_OR_EXIT, _unkF440C4, 0);
    }
}
...
void ride_remove_provisional_entrance_or_exit()
{
    if (_currentTrackSelectionFlags & TRACK_SELECTION_FLAG_ENTRANCE_OR_EXIT)
    {
        game_do_command(_unkF440BF.x, (GAME_COMMAND_FLAG_5 | GAME_COMMAND_FLAG_ALLOW_DURING_PAUSED | GAME_COMMAND_FLAG_APPLY),
                        _unkF440BF.y, _currentRideIndex, GAME_COMMAND_REMOVE_RIDE_ENTRANCE_OR_EXIT, _unkF440C4, 0);
    }
}
...
void ride_remove_provisional_track_piece()
{
    ...
    else
    {
        ...
        {
            game_do_command(next_track.x, 105 | ((direction & 3) << 8), next_track.y,
                            next_track.element->properties.track.type |
                                ((next_track.element->properties.track.sequence & 0x0F) << 8),
...
void ride_construction_set_default_next_piece()
{
    ...
    case RIDE_CONSTRUCTION_STATE_FRONT:
        ...
        _currentTrackLiftHill =
            ((mapElement->type & 0x80) && slope != TRACK_SLOPE_DOWN_25 && slope != TRACK_SLOPE_DOWN_60) != 0;
        break;
...
static int ride_modify_entrance_or_exit(rct_map_element * mapElement, int x, int y)
{
    ...
    sub_6C9627();
    if (_rideConstructionState != RIDE_CONSTRUCTION_STATE_ENTRANCE_EXIT || !(gInputFlags & INPUT_FLAG_TOOL_ACTIVE) ||
...
static void ride_breakdown_update(int rideIndex)
{
    ...
    {
        int totalDowntime = ride->downtime_history[0] + ride->downtime_history[1] + ride->downtime_history[2] +
                            ride->downtime_history[3] + ride->downtime_history[4] + ride->downtime_history[5] +
                            ride->downtime_history[6] + ride->downtime_history[7];
...
rct_peep * ride_get_assigned_mechanic(rct_ride * ride)
{
    ...
    if (ride->lifecycle_flags & RIDE_LIFECYCLE_BROKEN_DOWN)
    {
        if (ride->mechanic_status == RIDE_MECHANIC_STATUS_HEADING || ride->mechanic_status == 3 || ride->mechanic_status == 4)
        {
...
#define MAKE_TUNEID_LIST(...)                                                                                                  \
    (uint8[])                                                                                                                  \
    {                                                                                                                          \
        (countof(((uint8[]){ __VA_ARGS__ }))), __VA_ARGS__                                                                     \
    }
...
static void ride_measurement_update(rct_ride_measurement * measurement)
{
    ...
    if (trackType == 216 || trackType == TRACK_ELEM_CABLE_LIFT_HILL || trackType == TRACK_ELEM_25_DEG_UP_TO_FLAT ||
        trackType == TRACK_ELEM_60_DEG_UP_TO_FLAT || trackType == TRACK_ELEM_DIAG_25_DEG_UP_TO_FLAT ||
...
static void ride_entrance_exit_connected(rct_ride * ride, int ride_idx)
{
    for (int i = 0; i < 4; ++i)
    {
        uint16 station_start = ride->station_starts[i], entrance = ride->entrances[i], exit = ride->exits[i];
...
void ride_set_map_tooltip(rct_map_element * mapElement)
{
    ...
    {
        if (mapElement->properties.track.type == 2 || mapElement->properties.track.type == 3 ||
...
#define INIT_MUSIC_INFO(path_id, offset, length, unknown)                                                                      \
    (rct_ride_music_info[])                                                                                                    \
    {                                                                                                                          \
        length, offset, path_id, unknown                                                                                       \
    }
        ...
        if (!gGameSoundsOff && gConfigSound.ride_music_enabled && !(gScreenFlags & 1))
        {
                 ...
                 ride_music_params < gRideMusicParamsListEnd; ride_music_params++)
            {
                if (ride_music_params->ride_id != (uint8)-1)
                {
                    rct_ride_music * ride_music = &gRideMusicList[0];
                    int              channel    = 0;
                    while (ride_music_params->ride_id != ride_music->ride_id ||
                           ride_music_params->tune_id != ride_music->tune_id)
                    {
                        if (ride_music->ride_id == (uint8)-1)
                        {
...
static int ride_mode_check_valid_station_numbers(rct_ride * ride)
{
    ...
    {
        if (ride->station_starts[station_index] != 0xFFFF)
            no_stations++;
    }
 ...
 *  rct2: 0x006DD90D
 */
static rct_vehicle * vehicle_create_car(int rideIndex, int vehicleEntryIndex, int carIndex, int vehicleIndex, int x, int y,
                                        int z, int * remainingDistance, rct_map_element * mapElement)
{
    registers regs = { 0 };
 ...
 *  rct2: 0x006DD84C
 */
static train_ref vehicle_create_train(int rideIndex, int x, int y, int z, int vehicleIndex, int * remainingDistance,
                                      rct_map_element * mapElement)
{
    rct_ride * ride = get_ride(rideIndex);
    ...
    for (int carIndex = 0; carIndex < ride->num_cars_per_train; carIndex++)
    {
        ...
        rct_vehicle * car =
            vehicle_create_car(rideIndex, vehicle, carIndex, vehicleIndex, x, y, z, remainingDistance, mapElement);
        if (carIndex == 0)
        {
            train.head = car;
        }
...
static bool ride_initialise_cable_lift_track(rct_ride * ride, bool isApplying)
{
    ...
    track_circuit_iterator it;
    track_circuit_iterator_begin(&it, (rct_xy_element){.x = x, .y = y, .element = mapElement });
...
static int ride_is_valid_for_test(int rideIndex, int goingToBeOpen, int isApplying)
{
    ...
    if (!ride_type_has_flag(ride->type, RIDE_TYPE_FLAG_13) && !(ride->lifecycle_flags & RIDE_LIFECYCLE_ON_TRACK))
    {
    ...
    if ((RideData4[ride->type].flags & RIDE_TYPE_FLAG4_10) &&
...
void ride_set_name(int rideIndex, const char * name)
{
    ...
    game_do_command(1, (rideIndex << 8) | 1, 0, name_buffer[0].as_int, GAME_COMMAND_SET_RIDE_NAME, name_buffer[2].as_int,
                    name_buffer[1].as_int);
    game_do_command(2, (rideIndex << 8) | 1, 0, name_buffer[3].as_int, GAME_COMMAND_SET_RIDE_NAME, name_buffer[5].as_int,
                    name_buffer[4].as_int);
    game_do_command(0, (rideIndex << 8) | 1, 0, name_buffer[6].as_int, GAME_COMMAND_SET_RIDE_NAME, name_buffer[8].as_int,
                    name_buffer[7].as_int);
}
...
int ride_get_refund_price(int ride_id)
{
    ...
    while (map_element_iterator_next(&it))
    {
        ...
        {
            money32 removePrice =
                game_do_command(x, GAME_COMMAND_FLAG_APPLY | (rotation << 8), y,
                                type | ((it.element->properties.track.sequence & 0xF) << 8), GAME_COMMAND_REMOVE_TRACK, z, 0);
        ...
        refundPrice += game_do_command(x, GAME_COMMAND_FLAG_APPLY | GAME_COMMAND_FLAG_2 | (0 << 8), y, ride_id | (2 << 8),
                                       GAME_COMMAND_SET_MAZE_TRACK, z, 0);

        refundPrice += game_do_command(x, GAME_COMMAND_FLAG_APPLY | GAME_COMMAND_FLAG_2 | (1 << 8), y + 16, ride_id | (2 << 8),
                                       GAME_COMMAND_SET_MAZE_TRACK, z, 0);

        refundPrice += game_do_command(x + 16, GAME_COMMAND_FLAG_APPLY | GAME_COMMAND_FLAG_2 | (2 << 8), y + 16,
                                       ride_id | (2 << 8), GAME_COMMAND_SET_MAZE_TRACK, z, 0);

        refundPrice += game_do_command(x + 16, GAME_COMMAND_FLAG_APPLY | GAME_COMMAND_FLAG_2 | (3 << 8), y, ride_id | (2 << 8),
                                       GAME_COMMAND_SET_MAZE_TRACK, z, 0);
        map_element_iterator_restart_for_tile(&it);
    }
...
foundRideEntry:
    ...
    *outRideIndex = rideIndex;
    ...
    {
        *outRideColour =
            ride_get_random_colour_preset_index(type) | (ride_get_unused_preset_vehicle_colour(type, subType) << 8);
    }
        ...
        if (*ebx & GAME_COMMAND_FLAG_APPLY)
        {
            ...
            *ebx = ride_get_refund_price(ride_id);
            ...
            FOR_ALL_GUESTS(spriteIndex, peep)
            {
                ...
                peep->rides_been_on[ride_id_offset] &=
                    ~(1 << ride_id_bit); // clear ride from potentially being in rides_been_on
...
void set_vehicle_type_image_max_sizes(rct_ride_entry_vehicle * vehicle_type, int num_images)
{
    ...
    rct_drawpixelinfo dpi = {
        .bits = (uint8 *)bitmap, .x = -100, .y = -100, .width = 200, .height = 200, .pitch = 0, .zoom_level = 0
    };
...
money32 ride_get_entrance_or_exit_price(int rideIndex, int x, int y, int direction, int dh, int di)
{
    ...
    money32 result = game_do_command(
        x, (GAME_COMMAND_FLAG_APPLY | GAME_COMMAND_FLAG_ALLOW_DURING_PAUSED | GAME_COMMAND_FLAG_5 | GAME_COMMAND_FLAG_GHOST) |
               (direction << 8),
        y, rideIndex | (dh << 8), GAME_COMMAND_PLACE_RIDE_ENTRANCE_OR_EXIT, di, 0);
 ...
 *  rct2: 0x006CCF70
 */
void ride_get_entrance_or_exit_position_from_screen_position(int screenX, int screenY, int * outX, int * outY,
                                                             int * outDirection)
{
    short             mapX, mapY;
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            if (mapX >= 0 && mapY >= 0 && mapX < (256 * 32) && mapY < (256 * 32))
            {
                ...
                do
                {
                    ...
                    if (FlatRideTrackSequenceProperties[mapElement->properties.track.type]
    ...
    case RIDE_SET_VEHICLES_COMMAND_TYPE_RIDE_ENTRY:
        ...
        if (!gCheatsDisableTrainLengthLimit)
        {
            ride->proposed_num_cars_per_train =
                clamp(rideEntry->min_cars_in_train, ride->proposed_num_cars_per_train, rideEntry->max_cars_in_train);
        }
        break;
    default:
...
void sub_6CB945(int rideIndex)
{
    ...
    if (ride->type != RIDE_TYPE_MAZE)
    {
        for (uint8 stationId = 0; stationId < 4; ++stationId)
        {
            ...
            rct_xyz16 location = {.x = (ride->station_starts[stationId] & 0xFF) * 32,
    ...
    *locationList++ = 0xFFFF;
    ...
    for (; *locationList != 0xFFFF; locationList++)
    {
        ...
        rct_xy16 location = {.x = (*locationList & 0xFF) * 32, .y = ((*locationList >> 8) & 0xFF) * 32 };
...
static void maze_entrance_hedge_removal(int x, int y, rct_map_element * mapElement)
{
    ...
    do
    {
        if (mapElement->type != MAP_ELEMENT_TYPE_TRACK)
            continue;
        if (mapElement->properties.track.ride_index != rideIndex)
            continue;
        if (mapElement->base_height != z)
...
static money32 place_ride_entrance_or_exit(sint16 x, sint16 y, sint16 z, uint8 direction, uint8 flags, uint8 rideIndex,
                                           uint8 station_num, uint8 is_exit)
{
    ...
    else
    {
        ...
        if (requires_remove)
        {
            money32 success = game_do_command(remove_x, flags, remove_y, rideIndex, GAME_COMMAND_REMOVE_RIDE_ENTRANCE_OR_EXIT,
                                              station_num, 0);
...
{
    *ebx = place_ride_entrance_or_exit(*eax & 0xFFFF, *ecx & 0xFFFF, *edx & 0xFF, (*ebx >> 8) & 0xFF, *ebx & 0xFF, *edx & 0xFF,
                                       *edi & 0xFF, (*edx >> 8) & 0xFF);
}
...
{
    *ebx = remove_ride_entrance_or_exit(*eax & 0xFFFF, *ecx & 0xFFFF, *edx & 0xFF, *edi & 0xFF, *ebx & 0xFF);
}
...
{
    uint32 sum = ride->num_customers[0] + ride->num_customers[1] + ride->num_customers[2] + ride->num_customers[3] +
                 ride->num_customers[4] + ride->num_customers[5] + ride->num_customers[6] + ride->num_customers[7] +
                 ride->num_customers[8] + ride->num_customers[9];
    return sum;
}
...
rct_map_element * get_station_platform(int x, int y, int z, int z_tolerance)
{
    ...
    if (mapElement != NULL)
    {
        do
        {
            ...
            if (z - z_tolerance > mapElement->base_height || z + z_tolerance < mapElement->base_height)
            {
