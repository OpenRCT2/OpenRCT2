...
/** rct2: 0x00993EDC */
static const uint8 maze_segment_mirror_map[] = { 5, 4, 2, 7, 1, 0, 14, 3, 13, 12, 10, 15, 9, 8, 6, 11 };
 ...
 *  rct2: 0x006D0964
 */
static int track_design_place_scenery(rct_td6_scenery_element * scenery_start, uint8 rideIndex, int originX, int originY,
                                      int originZ)
{
        ...
        for (rct_td6_scenery_element * scenery = scenery_start; (scenery->scenery_object.flags & 0xFF) != 0xFF; scenery++)
        {
                ...
                case OBJECT_TYPE_SMALL_SCENERY:
                    ...
                    if (!(!(small_scenery->small_scenery.flags & SMALL_SCENERY_FLAG_FULL_TILE) &&
                        ...
                        (small_scenery->small_scenery.flags &
                         (SMALL_SCENERY_FLAG9 | SMALL_SCENERY_FLAG24 | SMALL_SCENERY_FLAG25)))
                    {
                        bh &= 0x3F;
                    }

                    z = (scenery->z * 8 + originZ) / 8;
                    game_do_command(mapCoord.x, 0x69 | bh << 8, mapCoord.y, (entry_index << 8) | z, GAME_COMMAND_REMOVE_SCENERY,
                                    0, 0);
                    break;
                case OBJECT_TYPE_LARGE_SCENERY:
                    z = (scenery->z * 8 + originZ) / 8;
                    game_do_command(mapCoord.x, 0x69 | (((rotation + scenery->flags) & 0x3) << 8), mapCoord.y, z,
                                    GAME_COMMAND_REMOVE_LARGE_SCENERY, 0, 0);
                    break;
                case OBJECT_TYPE_WALLS:
                    z = (scenery->z * 8 + originZ) / 8;
                    game_do_command(mapCoord.x, 0x69, mapCoord.y, (z << 8) | ((rotation + scenery->flags) & 0x3),
                                    GAME_COMMAND_REMOVE_FENCE, 0, 0);
                    break;
                case OBJECT_TYPE_PATHS:
                    z = (scenery->z * 8 + originZ) / 8;
                ...
                _trackDesignPlaceOperation == PTD_OPERATION_GET_COST)
            {
                ...
                case OBJECT_TYPE_SMALL_SCENERY:
                    ...
                    cost = game_do_command(mapCoord.x, bl | (entry_index << 8), mapCoord.y,
                                           quadrant | (scenery->primary_colour << 8), GAME_COMMAND_PLACE_SCENERY,
                                           rotation | (scenery->secondary_colour << 16), z);
                ...
                case OBJECT_TYPE_LARGE_SCENERY:
                    ...
                    cost = game_do_command(mapCoord.x, bl | (rotation << 8), mapCoord.y,
                ...
                case OBJECT_TYPE_WALLS:
                    ...
                    cost = game_do_command(mapCoord.x, bl | (entry_index << 8), mapCoord.y,
                                           rotation | (scenery->primary_colour << 8), GAME_COMMAND_PLACE_FENCE, z,
                ...
                case OBJECT_TYPE_PATHS:
                    ...
                    if (mode == 0)
                    {
                        ...
                        cost                   = game_do_command(mapCoord.x, bl | (bh << 8), mapCoord.y, z | (entry_index << 8),
    ...
    for (; maze->all != 0; maze++)
    {
            ...
            _trackDesignPlaceOperation == PTD_OPERATION_4 || _trackDesignPlaceOperation == PTD_OPERATION_GET_COST)
        {
            ...
            case 0x08:
                ...
                {
                    cost = game_do_command(mapCoord.x, bl | rotation << 8, mapCoord.y, rideIndex,
            ...
            case 0x80:
                ...
                {
                    cost = game_do_command(mapCoord.x, bl | rotation << 8, mapCoord.y, rideIndex | (1 << 8),
                                           GAME_COMMAND_PLACE_RIDE_ENTRANCE_OR_EXIT, 0, 0);
                }
                if (cost != MONEY32_UNDEFINED)
                {
            ...
            default:
                ...
                cost = game_do_command(mapCoord.x, bl | (maze_entry & 0xFF) << 8, mapCoord.y, rideIndex | (maze_entry & 0xFF00),
...
static bool track_design_place_ride(rct_track_td6 * td6, sint16 x, sint16 y, sint16 z, uint8 rideIndex)
{
    ...
    for (; track->type != 0xFF; track++)
    {
        ...
        case PTD_OPERATION_GET_COST:
        {
            ...
            uint32 edi = ((track->flags & 0x0F) << 17) | ((track->flags & 0x0F) << 28) | (((track->flags >> 4) & 0x03) << 24) |
                         (tempZ & 0xFFFF);
    ...
    for (; entrance->z != -1; entrance++)
    {
        ...
        case PTD_OPERATION_GET_COST:
        {
            ...
            if (_trackDesignPlaceOperation != PTD_OPERATION_1)
            {
                rct_xy16          tile        = { x + TileDirectionDelta[rotation].x, y + TileDirectionDelta[rotation].y };
                ...
                do
                {
                    ...
                    gGameCommandErrorTitle = STR_RIDE_CONSTRUCTION_CANT_CONSTRUCT_THIS_HERE;
                    money32 cost           = game_do_command(x, bl | (rotation << 8), y, rideIndex | (isExit << 8),
                                                   GAME_COMMAND_PLACE_RIDE_ENTRANCE_OR_EXIT, di, 0);
                    _trackDesignPlaceCost += cost;
...
int sub_6D01B3(rct_track_td6 * td6, uint8 bl, uint8 rideIndex, int x, int y, int z)
{
    ...
    {
        if (!track_design_place_scenery(scenery, rideIndex, gTrackPreviewOrigin.x, gTrackPreviewOrigin.y,
...
static money32 place_track_design(sint16 x, sint16 y, sint16 z, uint8 flags, uint8 * outRideIndex)
{
    *outRideIndex = 255;
    ...
    if (entryIndex != 0xFF)
    {
        game_do_command(0, GAME_COMMAND_FLAG_APPLY | (2 << 8), 0, rideIndex | (entryIndex << 8), GAME_COMMAND_SET_RIDE_VEHICLES,
                        0, 0);
    }

    game_do_command(0, GAME_COMMAND_FLAG_APPLY | (td6->ride_mode << 8), 0, rideIndex | (0 << 8), GAME_COMMAND_SET_RIDE_SETTING,
                    0, 0);
    ...
    game_do_command(0, GAME_COMMAND_FLAG_APPLY | (td6->operation_setting << 8), 0, rideIndex | (4 << 8),
                    GAME_COMMAND_SET_RIDE_SETTING, 0, 0);
    game_do_command(0, GAME_COMMAND_FLAG_APPLY | ((td6->lift_hill_speed_num_circuits & 0x1F) << 8), 0, rideIndex | (8 << 8),
                    GAME_COMMAND_SET_RIDE_SETTING, 0, 0);
    ...
    game_do_command(0, GAME_COMMAND_FLAG_APPLY | (num_circuits << 8), 0, rideIndex | (9 << 8), GAME_COMMAND_SET_RIDE_SETTING, 0,
                    0);
...
{
    *ebx = place_maze_design(*ebx & 0xFF, *edx & 0xFF, ((*ebx >> 8) & 0xFF) | (((*edx >> 8) & 0xFF) << 8), *eax & 0xFFFF,
                             *ecx & 0xFFFF, *edi & 0xFFFF);
}
...
static map_backup * track_design_preview_backup_map()
{
    ...
    if (backup != NULL)
    {
        memcpy(backup->map_elements, gMapElements, sizeof(backup->map_elements));
        memcpy(backup->tile_pointers, gMapElementTilePointers, sizeof(backup->tile_pointers));
...
static void track_design_preview_restore_map(map_backup * backup)
{
    memcpy(gMapElements, backup->map_elements, sizeof(backup->map_elements));
    memcpy(gMapElementTilePointers, backup->tile_pointers, sizeof(backup->tile_pointers));
