...
static void paint_merry_go_round_structure(uint8 rideIndex, uint8 direction, sint8 xOffset, sint8 yOffset, uint16 height)
{
    ...
    if (dpi->zoom_level == 0 && ride->lifecycle_flags & RIDE_LIFECYCLE_ON_TRACK && vehicle != NULL)
    {
        ...
        for (int peep = 0; peep <= 14; peep += 2)
        {
            ...
            imageColourFlags =
                vehicle->peep_tshirt_colours[peep] << 19 | vehicle->peep_tshirt_colours[peep + 1] << 24 | 0x0A0000000;
            imageId = (baseImageId + 32 + imageOffset) | imageColourFlags;
            sub_98199C(imageId, xOffset, yOffset, 24, 24, 48, height, xOffset + 16, yOffset + 16, height,
 ...
 * rct2: 0x00761B0C
 */
static void paint_merry_go_round(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height,
                                 rct_map_element * mapElement)
{
    ...
    {
    case 1:
        paint_merry_go_round_structure(rideIndex, direction, 32, 32, height);
        break;
    case 3:
        paint_merry_go_round_structure(rideIndex, direction, 32, -32, height);
        break;
    case 5:
        paint_merry_go_round_structure(rideIndex, direction, 0, -32, height);
        break;
    case 6:
        paint_merry_go_round_structure(rideIndex, direction, -32, 32, height);
        break;
    case 7:
        ...
        break;
    case 8:
        paint_merry_go_round_structure(rideIndex, direction, -32, 0, height);
        break;
    }
