...
static void paint_circus_show_tent(uint8 rideIndex, uint8 direction, sint8 al, sint8 cl, uint16 height)
{
    ...
    sub_98197C(imageId | imageColourFlags, al, cl, 24, 24, 47, height + 3, al + 16, cl + 16, height + 3,
...
static void paint_circus_show(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height, rct_map_element * mapElement)
{
    ...
    track_paint_util_paint_fences(edges, position, mapElement, ride, gTrackColours[SCHEME_SUPPORTS], height, fenceSpritesRope,
                                  get_current_rotation());

    switch (trackSequence)
    {
    case 1:
        paint_circus_show_tent(rideIndex, direction, 32, 32, height);
        break;
    case 3:
        paint_circus_show_tent(rideIndex, direction, 32, -32, height);
        break;
    case 5:
        paint_circus_show_tent(rideIndex, direction, 0, -32, height);
        break;
    case 6:
        paint_circus_show_tent(rideIndex, direction, -32, 32, height);
        break;
    case 7:
        ...
        break;
    case 8:
        paint_circus_show_tent(rideIndex, direction, -32, 0, height);
        break;
    }
