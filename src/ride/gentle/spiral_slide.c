...
static void spiral_slide_paint_tile_right(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height,
                                          rct_map_element * mapElement)
{
    uint32 image_id = 0;

    if (direction == 0)
        image_id = SPIRAL_SLIDE_RIGHT_R0 | gTrackColours[SCHEME_TRACK];
    if (direction == 1)
        image_id = SPIRAL_SLIDE_RIGHT_R1 | gTrackColours[SCHEME_TRACK];
    if (direction == 2)
        image_id = SPIRAL_SLIDE_RIGHT_R2 | gTrackColours[SCHEME_TRACK];
    if (direction == 3)
        image_id = SPIRAL_SLIDE_RIGHT_R3 | gTrackColours[SCHEME_TRACK];

    sub_98197C(image_id, 16, 16, 16, 16, 108, height, 16, 0, height + 3, get_current_rotation());
}

static void spiral_slide_paint_tile_left(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height,
                                         rct_map_element * mapElement)
{
    uint32 image_id = 0;

    if (direction == 0)
        image_id = SPIRAL_SLIDE_LEFT_R0 | gTrackColours[SCHEME_TRACK];
    if (direction == 1)
        image_id = SPIRAL_SLIDE_LEFT_R1 | gTrackColours[SCHEME_TRACK];
    if (direction == 2)
        image_id = SPIRAL_SLIDE_LEFT_R2 | gTrackColours[SCHEME_TRACK];
    if (direction == 3)
        image_id = SPIRAL_SLIDE_LEFT_R3 | gTrackColours[SCHEME_TRACK];

    sub_98197C(image_id, 16, 16, 16, 16, 108, height, 0, 16, height + 3, get_current_rotation());
}

static void spiral_slide_paint_tile_front(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height,
                                          rct_map_element * mapElement)
{
    uint32 image_id = 0;
    ...
    if (dpi->zoom_level == 0 && ride->slide_in_use != 0)
    {
        ...
        if (slide_progress < 46)
        {
            ...
            rct_xyz8  boundingBox = {.z = 108 };
            rct_xyz16 boundingBoxOffset = {.z = height + 3 };
            ...
            sub_98199C(image_id, 16, 16, boundingBox.x, boundingBox.y, boundingBox.z, height, boundingBoxOffset.x,
...
static const uint32 spiral_slide_fence_sprites[] = {
    SPIRAL_SLIDE_FENCE_TOP_RIGHT, SPIRAL_SLIDE_FENCE_BOTTOM_RIGHT, SPIRAL_SLIDE_FENCE_BOTTOM_LEFT, SPIRAL_SLIDE_FENCE_TOP_LEFT,
};
