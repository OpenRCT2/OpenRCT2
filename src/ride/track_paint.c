...
void top_spin_paint_vehicle(sint8 al, sint8 cl, uint8 rideIndex, uint8 direction, int height, rct_map_element* mapElement) {
	...
	assert(armRotation < sizeof(SeatPositionOffset));
	switch (direction) {
	case 0:
		seatCoords.x -= SeatPositionOffset[armRotation];
		break;
	case 1:
		seatCoords.y += SeatPositionOffset[armRotation];
		break;
	case 2:
		seatCoords.x += SeatPositionOffset[armRotation];
		break;
