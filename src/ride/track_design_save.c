...
void track_design_save_toggle_map_element(int interactionType, int x, int y, rct_map_element * mapElement)
{
    ...
    else
    {
        ...
        {
            window_error_open(STR_SAVE_TRACK_SCENERY_UNABLE_TO_SELECT_ADDITIONAL_ITEM_OF_SCENERY,
 ...
 *  rct2: 0x006D2FA7
 */
static void track_design_save_push_map_element_desc(rct_object_entry * entry, int x, int y, int z, uint8 flags,
                                                    uint8 primaryColour, uint8 secondaryColour)
{
...
static void track_design_save_pop_map_element(int x, int y, rct_map_element * mapElement)
{
    ...
    if (removeIndex != SIZE_MAX)
    {
        ...
        {
            memmove(&_trackSavedMapElements[removeIndex], &_trackSavedMapElements[removeIndex + 1],
 ...
 *  rct2: 0x006D2FDD
 */
static void track_design_save_pop_map_element_desc(rct_object_entry * entry, int x, int y, int z, uint8 flags,
                                                   uint8 primaryColour, uint8 secondaryColour)
{
    size_t removeIndex = SIZE_MAX;
    ...
    if (removeIndex != SIZE_MAX)
    {
        ...
        if (remainingNumItems > 0)
        {
            memmove(&_trackSavedMapElementsDesc[removeIndex], &_trackSavedMapElementsDesc[removeIndex + 1],
...
static bool track_design_save_to_td6_for_tracked_ride(uint8 rideIndex, rct_track_td6 * td6)
{
    ...
    if (track_block_get_previous(trackElement.x, trackElement.y, trackElement.element, &trackBeginEnd))
    {
        ...
        do
        {
            rct_xy_element lastGood = {.element = trackBeginEnd.begin_element,
                                       .x       = trackBeginEnd.begin_x,
                                       .y       = trackBeginEnd.begin_y };

            if (!track_block_get_previous(trackBeginEnd.end_x, trackBeginEnd.end_y, trackBeginEnd.begin_element,
                                          &trackBeginEnd))
            {
    ...
    do
    {
        ...
        if (track->type == TRACK_ELEM_LEFT_VERTICAL_LOOP || track->type == TRACK_ELEM_RIGHT_VERTICAL_LOOP)
        {
            td6->flags |= (1 << 7);
        }

        if (track->type == TRACK_ELEM_LEFT_TWIST_DOWN_TO_UP || track->type == TRACK_ELEM_RIGHT_TWIST_DOWN_TO_UP ||
            track->type == TRACK_ELEM_LEFT_TWIST_UP_TO_DOWN || track->type == TRACK_ELEM_RIGHT_TWIST_UP_TO_DOWN)
        {
            td6->flags |= (1 << 17);
        }

        if (track->type == TRACK_ELEM_LEFT_BARREL_ROLL_UP_TO_DOWN || track->type == TRACK_ELEM_RIGHT_BARREL_ROLL_UP_TO_DOWN ||
            track->type == TRACK_ELEM_LEFT_BARREL_ROLL_DOWN_TO_UP || track->type == TRACK_ELEM_RIGHT_BARREL_ROLL_DOWN_TO_UP)
        {
            td6->flags |= (1 << 29);
        }

        if (track->type == TRACK_ELEM_HALF_LOOP_UP || track->type == TRACK_ELEM_HALF_LOOP_DOWN)
        {
            td6->flags |= (1 << 18);
        }

        if (track->type == TRACK_ELEM_LEFT_CORKSCREW_UP || track->type == TRACK_ELEM_RIGHT_CORKSCREW_UP ||
            track->type == TRACK_ELEM_LEFT_CORKSCREW_DOWN || track->type == TRACK_ELEM_RIGHT_CORKSCREW_DOWN)
        {
            td6->flags |= (1 << 19);
        }
        ...
        if (track->type == TRACK_ELEM_LEFT_LARGE_HALF_LOOP_UP || track->type == TRACK_ELEM_RIGHT_LARGE_HALF_LOOP_UP ||
            track->type == TRACK_ELEM_RIGHT_LARGE_HALF_LOOP_DOWN || track->type == TRACK_ELEM_LEFT_LARGE_HALF_LOOP_DOWN)
        {
            td6->flags |= (1u << 31);
        }
        ...
        if (RideData4[ride->type].flags & RIDE_TYPE_FLAG4_3 && trackElement.element->properties.track.colour & (1 << 2))
        {
            flags |= (1 << 6);
        }
...
bool track_design_save_to_file(const utf8 * path)
{
    ...
    if (td6->type == RIDE_TYPE_MAZE)
    {
        auto_buffer_write(&td6Buffer, td6->maze_elements,
                          track_design_get_maze_elements_count(td6) * sizeof(rct_td6_maze_element));
        auto_buffer_write(&td6Buffer, &EndMarkerForMaze, sizeof(EndMarkerForMaze));
    }
    else
    {
        auto_buffer_write(&td6Buffer, td6->track_elements,
                          track_design_get_track_elements_count(td6) * sizeof(rct_td6_track_element));
        auto_buffer_write(&td6Buffer, &EndMarker, sizeof(EndMarker));
        auto_buffer_write(&td6Buffer, td6->entrance_elements,
                          track_design_get_entrance_elements_count(td6) * sizeof(rct_td6_entrance_element));
        auto_buffer_write(&td6Buffer, &EndMarker, sizeof(EndMarker));
    }
    auto_buffer_write(&td6Buffer, td6->scenery_elements,
                      track_design_get_scenery_elements_count(td6) * sizeof(rct_td6_scenery_element));
    auto_buffer_write(&td6Buffer, &EndMarker, sizeof(EndMarker));
