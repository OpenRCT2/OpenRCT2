 ...
 */
static paint_struct * sub_9819_c(uint32 image_id, rct_xyz16 offset, rct_xyz16 boundBoxSize, rct_xyz16 boundBoxOffset,
                                 uint8 rotation)
{
...
static void paint_arrange_structs_helper(paint_struct * ps_next, uint16 ax, uint8 flag)
{
    ...
    while (true)
    {
        while (true)
        {
            ...
            if (ps_next == NULL)
                return;
            if (ps_next->var_1B & (1 << 7))
                return;
            if (ps_next->var_1B & (1 << 0))
                break;
            ps = ps_next;
        }
        ...
        bound_box initialBBox = {.x     = ps_next->bound_box_x,
        ...
        while (true)
        {
            ...
            {
            case 0:
                if (initialBBox.z_end >= ps_next->bound_box_z && initialBBox.y_end >= ps_next->bound_box_y &&
                    initialBBox.x_end >= ps_next->bound_box_x &&
                    !(initialBBox.z < ps_next->bound_box_z_end && initialBBox.y < ps_next->bound_box_y_end &&
                      initialBBox.x < ps_next->bound_box_x_end))
                    yes = 1;
                break;
            case 1:
                if (initialBBox.z_end >= ps_next->bound_box_z && initialBBox.y_end >= ps_next->bound_box_y &&
