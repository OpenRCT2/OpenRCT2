...
// 6B8172:
static void scenery_multiple_paint_supports(uint8 direction, uint16 height, rct_map_element * mapElement, uint32 dword_F4387C,
...
static void scenery_multiple_sign_paint_line(const utf8 * str, rct_large_scenery_text * text, int textImage, int textColour,
                                             uint8 direction, int y_offset)
{
...
static const boundbox s98E3C4[] = { { 3, 3, 26, 26 },  { 17, 17, 12, 12 }, { 17, 3, 12, 12 }, { 17, 3, 12, 26 },
                                    { 3, 3, 12, 12 },  { 3, 3, 26, 26 },   { 3, 3, 28, 12 },  { 3, 3, 26, 26 },
                                    { 3, 17, 12, 12 }, { 3, 17, 26, 12 },  { 3, 3, 26, 26 },  { 3, 3, 26, 26 },
                                    { 3, 3, 12, 28 },  { 3, 3, 26, 26 },   { 3, 3, 26, 26 },  { 3, 3, 26, 26 },
                                    { 1, 1, 30, 30 } };
...
void scenery_multiple_paint(uint8 direction, uint16 height, rct_map_element * mapElement)
{
    ...
    image_id |= ((mapElement->properties.scenerymultiple.colour[0] & 0x1F) << 19) |
                ((mapElement->properties.scenerymultiple.colour[1] & 0x1F) << 24) | 0xA0000000;
    rct_xyz16 boxlength;
    rct_xyz16 boxoffset;
    ...
    boxlength.z = ah;
    sub_98197C(image_id, 0, 0, boxlength.x, boxlength.y, ah, height, boxoffset.x, boxoffset.y, boxoffset.z,
    ...
    if (entry->large_scenery.flags & 0x4)
    {
        ...
        uint32 bannerIndex = (mapElement->type & 0xC0) | ((mapElement->properties.scenerymultiple.colour[0] & 0xE0) >> 2) |
        ...
        if (text->var_C & 0x1)
        {
            ...
            while ((codepoint = utf8_get_next(fitStrPtr, &fitStrPtr)) != 0)
            {
                ...
                scenery_multiple_sign_paint_line(str, entry->large_scenery.text, entry->large_scenery.text_image, textColour,
                                                 direction, y_offset - height);
            ...
            if (text->var_C & 0x2)
            {
                ...
                if (width > text->max_width)
                {
                    ...
                    for (int i = 0; i < 2; i++)
                    {
                        ...
                        } while (w <= text->max_width && (dst = utf8_write_codepoint(dst, codepoint)) && (srcold = src) &&
                                 (codepoint = utf8_get_next(src, (const utf8 **)&src)));
                        src = srcold;
                        ...
                        scenery_multiple_sign_paint_line(str1, entry->large_scenery.text, entry->large_scenery.text_image,
                                                         textColour, direction, y_offset);
                        y_offset += (scenery_multiple_sign_get_glyph(text, 'A')->height + 1) * 2;
                    }
                }
                else
                {
                    scenery_multiple_sign_paint_line(signString, entry->large_scenery.text, entry->large_scenery.text_image,
                                                     textColour, direction, y_offset);
                }
            }
            else
    ...
    sub_98199C(scrolling_text_setup(stringId, scroll, scrollMode), 0, 0, 1, 1, 21, height + 25, boxoffset.x, boxoffset.y,
               boxoffset.z, get_current_rotation());
