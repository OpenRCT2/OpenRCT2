...
static void sub_68B3FB(int x, int y)
{
    ...
    if ((gMapSelectFlags & MAP_SELECT_FLAG_ENABLE_ARROW) && gPaintMapPosition.x == gMapSelectArrowPosition.x &&
    ...
    for (int y = 0; y < 3; y++)
    {
        for (int x = 0; x < 3; x++)
        {
            ...
            int            xOffset = y * 10;
            int            yOffset = -22 + x * 10;
            paint_struct * ps = sub_98197C(5504 | imageColourFlats, xOffset, yOffset, 10, 10, 1, segmentHeight, xOffset + 1,
                                           yOffset + 16, segmentHeight, get_current_rotation());
            if (ps != NULL)
            {
