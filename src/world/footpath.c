...
const rct_xy16 word_981D6C[4] = { { -1, 0 }, { 0, 1 }, { 1, 0 }, { 0, -1 } };
...
/** rct2: 0x0098D7EC */
static const uint8 byte_98D7EC[] = { 207, 159, 63, 111 };
...
static money32 footpath_element_insert(int type, int x, int y, int z, int slope, int flags, uint8 pathItemType)
{
    ...
    if (!gCheatsDisableClearanceChecks &&
        !map_can_construct_with_clear_at(x, y, z, zHigh, &map_place_non_scenery_clear_func, bl, flags, &gFootpathPrice))
        return MONEY32_UNDEFINED;
...
static money32 footpath_element_update(int x, int y, rct_map_element * mapElement, int type, int flags, uint8 pathItemType)
{
    ...
    {
        if (!(flags & GAME_COMMAND_FLAG_GHOST) && footpath_element_get_path_scenery(mapElement) == pathItemType &&
            !(mapElement->flags & MAP_ELEMENT_FLAG_BROKEN))
        {
        ...
        if (pathItemType != 0)
        {
            ...
            if (!(unk6 & (PATH_BIT_FLAG_JUMPING_FOUNTAIN_WATER | PATH_BIT_FLAG_JUMPING_FOUNTAIN_SNOW)) &&
                (mapElement->properties.path.edges & 0x0F) == 0x0F)
            {
...
static void remove_banners_at_element(int x, int y, rct_map_element * mapElement)
{
    while (!map_element_is_last_for_tile(mapElement++))
    {
        if (map_element_get_type(mapElement) == MAP_ELEMENT_TYPE_PATH)
            return;
        else if (map_element_get_type(mapElement) != MAP_ELEMENT_TYPE_BANNER)
            continue;

        game_do_command(x, 1, y, mapElement->base_height | mapElement->properties.banner.position << 8,
...
{
    *ebx = footpath_place_from_track((*edx >> 8) & 0xFF, *eax & 0xFFFF, *ecx & 0xFFFF, *edx & 0xFF,
                                     ((*ebx >> 13) & 0x3) | ((*ebx >> 10) & 0x4), (*ebx >> 8) & 0xF, *ebx & 0xFF);
}
...
money32 footpath_provisional_set(int type, int x, int y, int z, int slope)
{
    ...
    cost = footpath_place(type, x, y, z, slope, GAME_COMMAND_FLAG_GHOST | GAME_COMMAND_FLAG_5 | GAME_COMMAND_FLAG_4 |
                                                    GAME_COMMAND_FLAG_ALLOW_DURING_PAUSED | GAME_COMMAND_FLAG_APPLY);
...
void footpath_provisional_remove()
{
    if (gFootpathProvisionalFlags & PROVISIONAL_PATH_FLAG_1)
    {
        ...
        footpath_remove(gFootpathProvisionalPosition.x, gFootpathProvisionalPosition.y, gFootpathProvisionalPosition.z,
 ...
 *		mapElement: edx
 */
void footpath_get_coordinates_from_pos(int screenX, int screenY, int * x, int * y, int * direction,
                                       rct_map_element ** mapElement)
{
    ...
    get_map_coordinates_from_pos(screenX, screenY, VIEWPORT_INTERACTION_MASK_FOOTPATH, &position.x, &position.y,
                                 &interactionType, &myMapElement, &viewport);
    if (interactionType != VIEWPORT_INTERACTION_ITEM_FOOTPATH ||
        !(viewport->flags & (VIEWPORT_FLAG_UNDERGROUND_INSIDE | VIEWPORT_FLAG_HIDE_BASE | VIEWPORT_FLAG_HIDE_VERTICAL)))
    {
        get_map_coordinates_from_pos(screenX, screenY, VIEWPORT_INTERACTION_MASK_FOOTPATH & VIEWPORT_INTERACTION_MASK_TERRAIN,
                                     &position.x, &position.y, &interactionType, &myMapElement, &viewport);
 ...
 * mapElement: edx
 */
void footpath_bridge_get_info_from_pos(int screenX, int screenY, int * x, int * y, int * direction,
                                       rct_map_element ** mapElement)
{
    ...
    rct_xy16 map_pos = { 0 };
    get_map_coordinates_from_pos(screenX, screenY, VIEWPORT_INTERACTION_MASK_RIDE, &map_pos.x, &map_pos.y, &interactionType,
                                 mapElement, &viewport);
    *x = map_pos.x;
    *y = map_pos.y;
    ...
    get_map_coordinates_from_pos(screenX, screenY, VIEWPORT_INTERACTION_MASK_RIDE & VIEWPORT_INTERACTION_MASK_FOOTPATH &
                                                       VIEWPORT_INTERACTION_MASK_TERRAIN,
                                 &map_pos.x, &map_pos.y, &interactionType, mapElement, &viewport);
    *x = map_pos.x;
    *y = map_pos.y;
...
static bool sub_footpath_disconnect_queue_from_path(int x, int y, rct_map_element * mapElement, int action, int direction)
{
    ...
    if (otherMapElement != NULL && !footpath_element_is_queue(otherMapElement))
    {
        ...
        if (action > 0)
        {
            ...
            if (action >= 2)
                mapElement->properties.path.type |= direction;
        }
        else if (action < 0)
        {
            mapElement->properties.path.edges |= (1 << direction);
...
static bool footpath_disconnect_queue_from_path(int x, int y, rct_map_element * mapElement, int action)
{
    ...
    {
        if ((action < 0) && (direction == (mapElement->properties.path.type & 3)))
            continue;
...
static bool footpath_is_queue_connected_to_path(int x, int y, rct_map_element * mapElement, int direction)
{
    ...
    if (footpath_element_is_queue(mapElement))
        return false;
    if (mapElement->properties.path.edges & ((1 << direction) ^ 2))
        return true;
    return false;
}
 ...
 *  rct2: 0x006A6D7E
 */
static void loc_6A6D7E(int initialX, int initialY, int z, int direction, rct_map_element * initialMapElement, int flags,
    ...
    else
    {
        ...
        do
        {
            ...
            case MAP_ELEMENT_TYPE_PATH:
                ...
                {
                    if (!footpath_element_is_sloped(mapElement) ||
                        footpath_element_get_slope_direction(mapElement) != (direction ^ 2))
                    {
                        return;
                    }
                    goto loc_6A6F1F;
                    ...
                    if (entrance_has_direction(mapElement, ((direction - mapElement->type) & 3) ^ 2))
                    {
                        if (query)
                        {
                            neighbour_list_push(neighbourList, 8, direction, mapElement->properties.entrance.ride_index,
                                                mapElement->properties.entrance.index);
                        }
                        else
                        {
...
static void loc_6A6C85(int x, int y, int direction, rct_map_element * mapElement, int flags, bool query,
            ...
            if (neighbourList.items[i].ride_index != 255)
            {
                ...
                else if (rideIndex != neighbourList.items[i].ride_index)
                {
                    neighbour_list_remove(&neighbourList, i);
                         ...
                         entranceIndex != neighbourList.items[i].entrance_index && neighbourList.items[i].entrance_index != 255)
                {
                    neighbour_list_remove(&neighbourList, i);
                }
            }
 ...
 */
static int footpath_is_connected_to_map_edge_recurse(int x, int y, int z, int direction, int flags, int level,
                                                     int distanceFromJunction, int junctionTolerance)
{
    rct_map_element * mapElement;
    ...
    do
    {
        ...
        if (!(flags & (1 << 7)))
        {
            if (mapElement[1].type == MAP_ELEMENT_TYPE_BANNER)
            {
                for (int i = 1; i < 4; i++)
                {
                    if (map_element_is_last_for_tile(&mapElement[i - 1]))
                        break;
                    if (mapElement[i].type != MAP_ELEMENT_TYPE_BANNER)
                        break;
...
searchFromFootpath:
    ...
    if (edges == 0)
    {
        ...
        return footpath_is_connected_to_map_edge_recurse(x, y, z, direction, flags, level, distanceFromJunction + 1,
                                                         junctionTolerance);
    }
    else
    {
...
void footpath_update_path_wide_flags(int x, int y)
{
    ...
    do
    {
        ...
        {
            if ((F3EFA5 & 2) && (pathList[0] != NULL) && (!footpath_element_is_wide(pathList[0])) &&
            ...
            if ((F3EFA5 & 0x20) && (pathList[6] != NULL) && (!footpath_element_is_wide(pathList[6])) &&
        ...
        {
            if ((F3EFA5 & 2) && (pathList[2] != NULL) && (true || !footpath_element_is_wide(pathList[2])) &&
                ((pathList[2]->properties.path.edges & 0xC) == 0xC) && (pathList[1] != NULL) &&
             ...
             * in combination with reset tiles.
             * Short circuit the logic appropriately. */
            if ((F3EFA5 & 0x20) && (pathList[4] != NULL) && (true || !footpath_element_is_wide(pathList[4])) &&
                ((pathList[4]->properties.path.edges & 9) == 9) && (pathList[5] != NULL) &&
...
void footpath_remove_edges_at(int x, int y, rct_map_element * mapElement)
{
    ...
    for (int direction = 0; direction < 4; direction++)
    {
        ...
        z0 = z1 - 2;
        footpath_remove_edges_towards(x + TileDirectionDelta[direction].x, y + TileDirectionDelta[direction].y, z0, z1,
                                      direction, footpath_element_is_queue(mapElement));
    }
