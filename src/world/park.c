...
*  rct2: 0x006EC847
*/
void invalidate_viewport_tile(int x, int y, int z0, int z1)
{
	return;
...
void park_remove_entrance_segment(int x, int y, int z)
{
	...
	do {
		...
		invalidate_viewport_tile(x, y, mapElement->base_height * 8, mapElement->clearance_height * 8);
	...
	case 2:
		if ((ownership & (OWNERSHIP_OWNED | OWNERSHIP_CONSTRUCTION_RIGHTS_OWNED)) != 0) { // If the land or construction rights are already owned
			...
			if ((flags & 1) != 0) {
				...
				baseHeight *= 8;
				invalidate_viewport_tile(x, y, baseHeight, baseHeight + 16);
			}
			...
			if ((ownership & (OWNERSHIP_OWNED | OWNERSHIP_CONSTRUCTION_RIGHTS_OWNED)) != 0) { // If the land or construction rights are already owned
				...
				*ebx = 0x80000000;
				...
				if ((*ebx & 1) != 0) {
					...
					uint16 baseHeight = TILE_MAP_ELEMENT_POINTER(tile_idx)->base_height;
					baseHeight *= 8;
					invalidate_viewport_tile(x, y2, baseHeight, baseHeight + 16);
				}
