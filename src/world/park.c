...
void park_init()
{
    ...
    gResearchPriorities = (1 << RESEARCH_CATEGORY_TRANSPORT) | (1 << RESEARCH_CATEGORY_GENTLE) |
                          (1 << RESEARCH_CATEGORY_ROLLERCOASTER) | (1 << RESEARCH_CATEGORY_THRILL) |
                          (1 << RESEARCH_CATEGORY_WATER) | (1 << RESEARCH_CATEGORY_SHOP) | (1 << RESEARCH_CATEGORY_SCENERYSET);
...
{
    return DECRYPT_MONEY(gCashEncrypted) + gParkValue - gBankLoan;
}
...
int park_get_entrance_index(int x, int y, int z)
{
    ...
    for (i = 0; i < 4; i++)
    {
        if (x == gParkEntranceX[i] && y == gParkEntranceY[i] && z == gParkEntranceZ[i])
        {
            return i;
        }
    }
...
void game_command_remove_park_entrance(int * eax, int * ebx, int * ecx, int * edx, int * esi, int * edi, int * ebp)
{
    ...
    // Left post
    park_remove_entrance_segment(x + TileDirectionDelta[direction].x, y + TileDirectionDelta[direction].y, z);
...
void park_set_name(const char * name)
{
    ...
    game_do_command(1, GAME_COMMAND_FLAG_APPLY, 0, *((int *)(name + 0)), GAME_COMMAND_SET_PARK_NAME, *((int *)(name + 8)),
                    *((int *)(name + 4)));
    game_do_command(2, GAME_COMMAND_FLAG_APPLY, 0, *((int *)(name + 12)), GAME_COMMAND_SET_PARK_NAME, *((int *)(name + 20)),
                    *((int *)(name + 16)));
    game_do_command(0, GAME_COMMAND_FLAG_APPLY, 0, *((int *)(name + 24)), GAME_COMMAND_SET_PARK_NAME, *((int *)(name + 32)),
                    *((int *)(name + 28)));
}
...
void game_command_buy_land_rights(int * eax, int * ebx, int * ecx, int * edx, int * esi, int * edi, int * ebp)
{
    ...
    *ebx = map_buy_land_rights((*eax & 0xFFFF), (*ecx & 0xFFFF), (*edi & 0xFFFF), (*ebp & 0xFFFF), (*edx & 0xFF00) >> 8, flags);
...
void park_remove_ghost_entrance()
{
    if (gParkEntranceGhostExists)
    {
        ...
        game_do_command(gParkEntranceGhostPosition.x, 40 | GAME_COMMAND_FLAG_APPLY, gParkEntranceGhostPosition.y,
                        gParkEntranceGhostPosition.z, GAME_COMMAND_REMOVE_PARK_ENTRANCE, 0, 0);
    }
}
...
money32 park_place_ghost_entrance(int x, int y, int z, int direction)
{
    ...
    result = game_do_command(x, 104 | GAME_COMMAND_FLAG_APPLY | (direction << 8), y, z, GAME_COMMAND_PLACE_PARK_ENTRANCE, 0, 0);
