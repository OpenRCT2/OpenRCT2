 ...
 */
const rct_xy16 TileDirectionDelta[] = { { -32, 0 },   { 0, +32 },   { +32, 0 },   { 0, -32 },
...
int map_element_height(int x, int y)
{
    ...
    uint32 height =
        ((mapElement->properties.surface.terrain & MAP_ELEMENT_WATER_HEIGHT_MASK) << 20) | (mapElement->base_height << 3);
...
void game_command_remove_large_scenery(int * eax, int * ebx, int * ecx, int * edx, int * esi, int * edi, int * ebp)
{
    ...
    rct_xyz16           firstTile = {.x = scenery_entry->large_scenery.tiles[tileIndex].x_offset,
    ...
    for (int i = 0; scenery_entry->large_scenery.tiles[i].x_offset != -1; i++)
    {
        ...
        rct_xyz16 currentTile = {.x = scenery_entry->large_scenery.tiles[i].x_offset,
...
void game_command_set_large_scenery_colour(int * eax, int * ebx, int * ecx, int * edx, int * esi, int * edi, int * ebp)
{
    ...
    rct_xyz16 baseTile = {.x = scenery_entry->large_scenery.tiles[tileIndex].x_offset,
                          .y = scenery_entry->large_scenery.tiles[tileIndex].y_offset,
                          .z = (base_height * 8) - scenery_entry->large_scenery.tiles[tileIndex].z_offset };
    ...
    for (int i = 0; scenery_entry->large_scenery.tiles[i].x_offset != -1; ++i)
    {
        ...
        {
            rct_map_element * mapElement =
                map_get_large_scenery_segment(currentTile.x, currentTile.y, base_height, map_element_direction, i);
...
{
    *ebx = map_clear_scenery((sint16)(*eax & 0xFFFF), (sint16)(*ecx & 0xFFFF), (sint16)(*edi & 0xFFFF), (sint16)(*ebp & 0xFFFF),
                             *edx, *ebx & 0xFF);
}
...
static money32 map_change_surface_style(int x0, int y0, int x1, int y1, uint8 surfaceStyle, uint8 edgeStyle, uint8 flags)
{
    ...
    for (int x = x0; x <= x1; x += 32)
    {
        for (int y = y0; y <= y1; y += 32)
        {
            ...
            {
                uint8 cur_terrain =
                    ((mapElement->type & MAP_ELEMENT_DIRECTION_MASK) << 3) | (mapElement->properties.surface.terrain >> 5);
            ...
            if (edgeStyle != 0xFF)
            {
                uint8 currentEdge = ((mapElement->type & 0x80) >> 4) | (mapElement->properties.surface.slope >> 5);
...
{
    *ebx = map_change_surface_style((sint16)(*eax & 0xFFFF), (sint16)(*ecx & 0xFFFF), (sint16)(*edi & 0xFFFF),
                                    (sint16)(*ebp & 0xFFFF), *edx & 0xFF, (*edx & 0xFF00) >> 8, *ebx & 0xFF);
}
...
{
    *ebx = map_set_land_height(*ebx & 0xFF, *eax & 0xFFFF, *ecx & 0xFFFF, *edx & 0xFF, (*edx >> 8) & 0xFF, *edi >> 5);
}
    ...
    for (int yi = ay; yi <= by; yi += 32)
    {
        for (int xi = ax; xi <= bx; xi += 32)
        {
            ...
            if (map_element != NULL)
            {
                ...
                {
                    uint8 newStyle =
                        map_element_raise_styles[selectionType][map_element->properties.surface.slope & MAP_ELEMENT_SLOPE_MASK];
                    if (newStyle & 0x20)
                    { // needs to be raised
                        height += 2;
                        newStyle &= ~0x20;
        ...
        for (int xi = ax; xi <= bx; xi += 32)
        {
            ...
            if (map_element != NULL)
            {
                ...
                if (height >= max_height)
                {
                    ...
                    uint8 newStyle =
                        map_element_lower_styles[selectionType][map_element->properties.surface.slope & MAP_ELEMENT_SLOPE_MASK];
                    if (newStyle & 0x20)
                    { // needs to be lowered
                        height -= 2;
                        newStyle &= ~0x20;
...
{
    *ebx = raise_land(*ebx, *eax, *ecx, map_element_height(*eax, *ecx), (sint16)(*edx & 0xFFFF), (sint16)(*ebp & 0xFFFF),
                      *edx >> 16, *ebp >> 16, *edi & 0xFFFF);
}
...
{
    *ebx = lower_land(*ebx, *eax, *ecx, map_element_height(*eax, *ecx), (sint16)(*edx & 0xFFFF), (sint16)(*ebp & 0xFFFF),
                      *edx >> 16, *ebp >> 16, *edi & 0xFFFF);
}
...
static money32 smooth_land(int flags, int centreX, int centreY, int mapLeft, int mapTop, int mapRight, int mapBottom,
                           int command)
{
    // Cap bounds to map
...
{
    *ebx = raise_water((sint16)(*eax & 0xFFFF), (sint16)(*ecx & 0xFFFF), (sint16)(*edi & 0xFFFF), (sint16)(*ebp & 0xFFFF),
                       (uint8)*ebx);
}
...
{
    *ebx = lower_water((sint16)(*eax & 0xFFFF), (sint16)(*ecx & 0xFFFF), (sint16)(*edi & 0xFFFF), (sint16)(*ebp & 0xFFFF),
                       (uint8)*ebx);
}
...
void game_command_remove_fence(int * eax, int * ebx, int * ecx, int * edx, int * esi, int * edi, int * ebp)
{
    ...
    if (!(flags & GAME_COMMAND_FLAG_GHOST) && !(gScreenFlags & SCREEN_FLAGS_SCENARIO_EDITOR) && !gCheatsSandboxMode &&
        !map_is_location_owned(x, y, base_height * 8))
    {
        *ebx = MONEY32_UNDEFINED;
        return;
    }
...
void game_command_place_scenery(int * eax, int * ebx, int * ecx, int * edx, int * esi, int * edi, int * ebp)
{
    ...
    if (scenery_entry->small_scenery.flags & SMALL_SCENERY_FLAG_FULL_TILE ||
        !(scenery_entry->small_scenery.flags & SMALL_SCENERY_FLAG9))
    {
    ...
    if (!gCheatsDisableClearanceChecks && (scenery_entry->small_scenery.flags & SMALL_SCENERY_FLAG_REQUIRE_FLAT_SURFACE) &&
        !supportsRequired && !isOnWater && (surface_element->properties.surface.slope & 0x1F))
    {
    ...
    if (!gCheatsDisableSupportLimits && !(scenery_entry->small_scenery.flags & SMALL_SCENERY_FLAG18) && supportsRequired)
    {
    ...
    if (!(scenery_entry->small_scenery.flags & SMALL_SCENERY_FLAG24))
    {
        if (scenery_entry->small_scenery.flags & SMALL_SCENERY_FLAG9 &&
            scenery_entry->small_scenery.flags & SMALL_SCENERY_FLAG_FULL_TILE)
        {
    ...
    if (!gCheatsDisableClearanceChecks &&
        !map_can_construct_with_clear_at(x, y, zLow, zHigh, &map_place_scenery_clear_func, unk_bl | collisionQuadrants, flags,
                                         &clearCost))
    {
        *ebx = MONEY32_UNDEFINED;
        return;
 ...
 *  rct2: 0x006E5CBA
 */
static bool map_place_fence_check_obstruction_with_track(rct_scenery_entry * wall, int x, int y, int z0, int z1, int edge,
...
{
    rct_xyz16 position = {.x = *eax & 0xFFFF, .y = *ecx & 0xFFFF, .z = *edi & 0xFFFF };
    ...
    if (!(gScreenFlags & SCREEN_FLAGS_SCENARIO_EDITOR) && !(flags & GAME_COMMAND_FLAG_7) && !gCheatsSandboxMode)
    {
...
money32 map_place_fence(int type, int x, int y, int z, int edge, int primaryColour, int secondaryColour, int tertiaryColour,
                        int flags)
{
...
void game_command_place_large_scenery(int * eax, int * ebx, int * ecx, int * edx, int * esi, int * edi, int * ebp)
{
    ...
    for (rct_large_scenery_tile * tile = scenery_entry->large_scenery.tiles; tile->x_offset != -1; tile++)
    {
        ...
        rct_xy16 curTile = {.x = tile->x_offset, .y = tile->y_offset };
    ...
    for (rct_large_scenery_tile *tile = scenery_entry->large_scenery.tiles; tile->x_offset != -1; tile++, tile_num++)
    {
        ...
        if (!gCheatsDisableClearanceChecks &&
            !map_can_construct_with_clear_at(curTile.x, curTile.y, zLow, zHigh, &map_place_scenery_clear_func, bl, flags,
                                             &supportsCost))
        {
            *ebx = MONEY32_UNDEFINED;
            return;
        ...
        if (flags & GAME_COMMAND_FLAG_APPLY)
        {
            ...
            new_map_element->properties.scenerymultiple.type = (tile_num << 10) | entry_index;
 ...
 *	bl = bl
 */
int map_can_construct_with_clear_at(int x, int y, int zLow, int zHigh, CLEAR_FUNC * clearFunc, uint8 bl, uint8 flags,
                                    money32 * price)
{
    gMapGroundFlags = ELEMENT_IS_1;
    ...
    do
    {
        if (map_element_get_type(map_element) != MAP_ELEMENT_TYPE_SURFACE)
        {
            if (zLow < map_element->clearance_height && zHigh > map_element->base_height &&
                !(map_element->flags & MAP_ELEMENT_FLAG_GHOST))
            {
...
void map_restore_provisional_elements()
{
    ...
    {
        gFootpathProvisionalFlags &= ~PROVISIONAL_PATH_FLAG_1;
        footpath_provisional_set(gFootpathProvisionalType, gFootpathProvisionalPosition.x, gFootpathProvisionalPosition.y,
...
int map_element_get_banner_index(rct_map_element * mapElement)
{
    ...
    case MAP_ELEMENT_TYPE_SCENERY_MULTIPLE:
        ...
        return (mapElement->type & MAP_ELEMENT_QUADRANT_MASK) |
...
static void clear_element_at(int x, int y, rct_map_element ** elementPtr)
{
    ...
    case MAP_ELEMENT_TYPE_FENCE:
        ...
        game_do_command(x, GAME_COMMAND_FLAG_APPLY, y,
                        (element->type & MAP_ELEMENT_DIRECTION_MASK) | (element->base_height << 8), GAME_COMMAND_REMOVE_FENCE,
                        0, 0);
        break;
    case MAP_ELEMENT_TYPE_SCENERY_MULTIPLE:
        gGameCommandErrorTitle = STR_CANT_REMOVE_THIS;
        game_do_command(x, (GAME_COMMAND_FLAG_APPLY) | ((element->type & MAP_ELEMENT_DIRECTION_MASK) << 8), y,
    ...
    case MAP_ELEMENT_TYPE_BANNER:
        gGameCommandErrorTitle = STR_CANT_REMOVE_THIS;
        game_do_command(x, GAME_COMMAND_FLAG_APPLY, y,
                        (element->base_height) | ((element->properties.banner.position & 3) << 8), GAME_COMMAND_REMOVE_BANNER,
                        0, 0);
        break;
    default:
...
bool map_large_scenery_get_origin(int x, int y, int z, int direction, int sequence, int * outX, int * outY, int * outZ,
                                  rct_map_element ** outElement)
{
    rct_map_element *        mapElement;
    rct_scenery_entry *      sceneryEntry;
    rct_large_scenery_tile * tile;
...
{
    *ebx = place_park_entrance(*ebx & 0xFF, *eax & 0xFFFF, *ecx & 0xFFFF, *edx & 0xFFFF, (*ebx >> 8) & 0xFF);
}
...
void game_command_set_sign_style(int * eax, int * ebx, int * ecx, int * edx, int * esi, int * edi, int * ebp)
{
    ...
    { // small sign
        ...
        map_element->flags &= 0x9F;
        map_element->properties.fence.item[1] = mainColour | ((textColour & 0x7) << 5);
    ...
    { // large sign
        ...
        if (!sign_set_colour(banner->x * 32, banner->y * 32, mapElement->base_height, mapElement->type & 3,
                             mapElement->properties.scenerymultiple.type >> 10, mainColour, textColour))
        {
            *ebx = MONEY32_UNDEFINED;
            return;
        }
...
rct_map_element * map_get_track_element_at_of_type_seq(int x, int y, int z, int trackType, int sequence)
{
    ...
    do
    {
        ...
        if (mapElement->properties.track.type != trackType)
            continue;
        if ((mapElement->properties.track.sequence & 0x0F) != sequence)
            continue;
