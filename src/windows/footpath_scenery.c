...
static uint64 window_footpath_scenery_enabled_widgets = {
	(1ULL << WIDX_CLOSE) |
	(1ULL << WIDX_RULE_LIST) |
	(1ULL << WIDX_CLEAR_RULES) |
	(1ULL << WIDX_OBJECT) |
	(1ULL << WIDX_OBJECT_DROPDOWN) |
	(1ULL << WIDX_ADD)
};
...
void window_footpath_scenery_open()
{
	...
	window = window_bring_to_front_by_class(WC_FOOTPATH_SCENERY);
	if (window != NULL)
		return;
...
static void window_footpath_scenery_path_scenery_dialog(rct_window *w, rct_widget *widget, int widgetIndex)
{
	...
	window_dropdown_show_image(
		...
		50,
		50,
		gAppropriateImageDropdownItemsPerRow[numSceneryTypes]
	);
	return;
}
...
static void window_footpath_scenery_list_scrollgetsize(rct_window *w, int scrollIndex, int *width, int *height)
{
	...
	*height = w->no_list_items * 10;
	...
	top = *height - window_footpath_scenery_widgets[WIDX_RULE_LIST].bottom + window_footpath_scenery_widgets[WIDX_RULE_LIST].top + 21;
	if (top < 0)
		top = 0;
...
const rct_scenery_entry* window_footpath_scenery_get_next_scenery()
{
	...
	return _footpath_scenery_selected_listEntry->sceneEntry;
}
