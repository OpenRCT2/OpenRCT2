...
static rct_widget window_cheats_rides_widgets[] = {
	...
	{ WWT_CHECKBOX,			2,		XPL(0),					OWPL,					YPL(12),		OHPL(12),		STR_CHEAT_ALLOW_RIDE_ENTRANCE_ROTATION,	STR_NONE }, 						// Allow ride entrances to be rotated
	{ WIDGETS_END },
};
...
static void window_cheats_rides_mouseup(rct_window *w, int widgetIndex)
{
	...
	case WIDX_SHOW_VEHICLES_FROM_OTHER_TRACK_TYPES:
		...
		break;
	case WIDX_ALLOW_ROTATING_ENTRANCES:
		game_do_command(0, GAME_COMMAND_FLAG_APPLY, CHEAT_ROTATINGENTRANCES, 0, GAME_COMMAND_CHEAT, 0, 0);
		break;
	}
}
...
static void window_cheats_invalidate(rct_window *w)
{
	...
	case WINDOW_CHEATS_PAGE_RIDES:
		...
		widget_set_checkbox_value(w, WIDX_ALLOW_ROTATING_ENTRANCES, gCheatsRotatingEntrances);
		break;
	}
