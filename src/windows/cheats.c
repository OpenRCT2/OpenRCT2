...
static rct_widget window_cheats_rides_widgets[] = {
	...
	{ WWT_CHECKBOX,			1,		XPL(0),					WPL(1),					YPL(5),			HPL(5),			STR_CHEAT_BUILD_IN_PAUSE_MODE,		STR_CHEAT_BUILD_IN_PAUSE_MODE_TIP }, 	// Build in pause mode
	{ WWT_CHECKBOX,			1,		XPL(0),					WPL(1),					YPL(6),			HPL(6),			STR_CHEAT_DISABLE_BRAKES_FAILURE,	STR_CHEAT_DISABLE_BRAKES_FAILURE_TIP }, // Disable brakes failure
	{ WWT_CHECKBOX,			1,		XPL(0),					WPL(1),					YPL(7),			HPL(7),			STR_CHEAT_DISABLE_BREAKDOWNS,		STR_CHEAT_DISABLE_BREAKDOWNS_TIP }, 	// Disable all breakdowns
	{ WWT_CHECKBOX,			1,		XPL(0),					WPL(1),					YPL(8),			HPL(8),			STR_CHEAT_UNLOCK_OPERATING_LIMITS,	STR_CHEAT_UNLOCK_OPERATING_LIMITS_TIP },// 410 km/h lift hill etc.
	{ WWT_CHECKBOX,			1,		XPL(0),					WPL(1),					YPL(9),			HPL(9),			STR_CHEAT_ENABLE_CHAIN_LIFT_ON_ALL_TRACK,		STR_CHEAT_ENABLE_CHAIN_LIFT_ON_ALL_TRACK_TIP },	// Enable chain lift on all track
	{ WWT_GROUPBOX,			1,		XPL(0) - GROUP_SPACE,	WPL(1) + GROUP_SPACE,	YPL(10) + 5,	HPL(14) + 5,	STR_ACTION_UNSTABLE_CHEAT,						STR_NONE }, 									// Unstable cheats groupbox
	{ WWT_CHECKBOX,			1,		XPL(0),					WPL(1),					YPL(11),		HPL(11),		STR_CHEAT_SHOW_VEHICLES_FROM_OTHER_TRACK_TYPES,	STR_NONE }, 									// Show vehicles from other track types
	{ WWT_CHECKBOX,			1,		XPL(0),					WPL(1),					YPL(12),		HPL(12),		STR_CHEAT_SHOW_ALL_OPERATING_MODES,				STR_NONE }, 									// Show all operating modes
	{ WWT_CHECKBOX,			1,		XPL(0),					WPL(1),					YPL(13),		HPL(13),		STR_CHEAT_DISABLE_TRAIN_LENGTH_LIMIT,			STR_CHEAT_DISABLE_TRAIN_LENGTH_LIMIT_TIP },		// Disable train length limits
	{ WWT_CHECKBOX,			1,		XPL(0),					WPL(1),					YPL(14),		HPL(14),		STR_CHEAT_ALLOW_ARBITRARY_RIDE_TYPE_CHANGES,	STR_CHEAT_ALLOW_ARBITRARY_RIDE_TYPE_CHANGES_TIP },	// Allow arbitrary ride type changes
	{ WIDGETS_END },
};
...
static uint64 window_cheats_page_enabled_widgets[] = {
	...
	(1ULL << WIDX_CLOSE) | (1ULL << WIDX_TAB_1) | (1ULL << WIDX_TAB_2) | (1ULL << WIDX_TAB_3) | (1ULL << WIDX_TAB_4) | 
		(1ULL << WIDX_HIGH_MONEY) | (1ULL << WIDX_CLEAR_LOAN),
	
	//Guest window
	(1ULL << WIDX_CLOSE) | (1ULL << WIDX_TAB_1) | (1ULL << WIDX_TAB_2) | (1ULL << WIDX_TAB_3) | (1ULL << WIDX_TAB_4) | (1ULL << WIDX_GUEST_PARAMETERS_GROUP) |
		//"Set guests' parameters" group
		(1ULL << WIDX_GUEST_HAPPINESS_MIN) | (1ULL << WIDX_GUEST_HAPPINESS_MAX) | 
		(1ULL << WIDX_GUEST_ENERGY_MIN) | (1ULL << WIDX_GUEST_ENERGY_MAX) |
		(1ULL << WIDX_GUEST_HUNGER_MIN) | (1ULL << WIDX_GUEST_HUNGER_MAX) | 
		(1ULL << WIDX_GUEST_THIRST_MIN) | (1ULL << WIDX_GUEST_THIRST_MAX) |
		(1ULL << WIDX_GUEST_NAUSEA_MIN) | (1ULL << WIDX_GUEST_NAUSEA_MAX) | 
		(1ULL << WIDX_GUEST_NAUSEA_TOLERANCE_MIN) | (1ULL << WIDX_GUEST_NAUSEA_TOLERANCE_MAX) |
		(1ULL << WIDX_GUEST_BATHROOM_MIN) | (1ULL << WIDX_GUEST_BATHROOM_MAX) | 
		(1ULL << WIDX_GUEST_RIDE_INTENSITY_LESS_THAN_15) | (1ULL << WIDX_GUEST_RIDE_INTENSITY_MORE_THAN_1) |
		(1ULL << WIDX_GUEST_IGNORE_RIDE_INTENSITY) | 
		(1ULL << WIDX_DISABLE_VANDALISM) |
		(1ULL << WIDX_DISABLE_LITTERING) |
		//"Give all guest" group
		(1ULL << WIDX_GIVE_ALL_GUESTS_GROUP) | 
		(1ULL << WIDX_GIVE_GUESTS_MONEY) | (1ULL << WIDX_GIVE_GUESTS_PARK_MAPS) |
		(1ULL << WIDX_GIVE_GUESTS_BALLOONS) | (1ULL << WIDX_GIVE_GUESTS_UMBRELLAS) | 
		//Extra features
		(1ULL << WIDX_TRAM_GUESTS) | (1ULL << WIDX_REMOVE_ALL_GUESTS) | 
		(1ULL << WIDX_EXPLODE_GUESTS) |
		 
	//Park window
	(1ULL << WIDX_CLOSE) | (1ULL << WIDX_TAB_1) | (1ULL << WIDX_TAB_2) | (1ULL << WIDX_TAB_3) | (1ULL << WIDX_TAB_4) |
		//"General" Group
		(1ULL << WIDX_OPEN_CLOSE_PARK) | (1ULL << WIDX_PARK_PARAMETERS) | 
		(1ULL << WIDX_SANDBOX_MODE) | (1ULL << WIDX_RESET_DATE) |
		(1ULL << WIDX_UNLOCK_ALL_PRICES) | 
		(1ULL << WIDX_FORCE_PARK_RATING) | (1ULL << WIDX_INCREASE_PARK_RATING) | (1ULL << WIDX_DECREASE_PARK_RATING),
		(1ULL << WIDX_WIN_SCENARIO) | (1ULL << WIDX_HAVE_FUN) |
		(1ULL << WIDX_NEVERENDING_MARKETING) |
		//"Climate" Group
		(1ULL << WIDX_FREEZE_CLIMATE) |
		(1ULL << WIDX_WEATHER) | (1ULL << WIDX_WEATHER_DROPDOWN_BUTTON) | 
		//"Staff" Group
		(1ULL << WIDX_CLEAR_GRASS) | (1ULL << WIDX_MOWED_GRASS) |
		(1ULL << WIDX_WATER_PLANTS) | (1ULL << WIDX_FIX_VANDALISM) | 
		(1ULL << WIDX_REMOVE_LITTER) |
		(1ULL << WIDX_DISABLE_PLANT_AGING) |
		(1ULL << WIDX_NORMAL_STAFF) | (1ULL << WIDX_FAST_STAFF) |

	//Rides Window
	(1ULL << WIDX_CLOSE) | (1ULL << WIDX_TAB_1) | (1ULL << WIDX_TAB_2) | (1ULL << WIDX_TAB_3) | (1ULL << WIDX_TAB_4) | 
		(1ULL << WIDX_RENEW_RIDES) |
		(1ULL << WIDX_FIX_ALL) | (1ULL << WIDX_MAKE_DESTRUCTIBLE) |
		(1ULL << WIDX_RESET_CRASH_STATUS) | (1ULL << WIDX_10_MINUTE_INSPECTIONS) |
		(1ULL << WIDX_BUILD_IN_PAUSE_MODE) |
		(1ULL << WIDX_DISABLE_BRAKES_FAILURE) |
		(1ULL << WIDX_DISABLE_ALL_BREAKDOWNS) |
		(1ULL << WIDX_FAST_LIFT_HILL) | 
		//"Unstable Cheats" group
...
static void window_cheats_rides_mouseup(rct_window *w, int widgetIndex)
{
	...
	case WIDX_DISABLE_TRAIN_LENGTH_LIMITS:
		game_do_command(0, GAME_COMMAND_FLAG_APPLY, CHEAT_DISABLETRAINLENGTHLIMIT, 0, GAME_COMMAND_UNSTABLE_CHEAT, 0, 0);
