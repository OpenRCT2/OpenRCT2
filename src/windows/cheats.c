...
static rct_widget window_cheats_guests_widgets[] = {
	...
	{ WWT_CHECKBOX,			2,		XPL(0),					OWPL, 					YPL(12),		OHPL(12),		STR_CHEAT_IGNORE_RAIN,				STR_NONE },								// guests ignore rain
	{ WWT_GROUPBOX,			1,		XPL(0) - GROUP_SPACE,	WPL(1) + GROUP_SPACE,	YPL(14),		HPL(15.5),		STR_CHEAT_GIVE_ALL_GUESTS,			STR_NONE },								// Guests parameters group frame
...
static void window_cheats_guests_mouseup(rct_window *w, int widgetIndex)
{
	...
	case WIDX_DISABLE_VANDALISM:
		...
		window_invalidate(w);
		break;
	case WIDX_GUEST_IGNORE_RAIN:
		gConfigCheat.ignore_rain_guest ^= 1;
		config_save_default();
		window_invalidate(w);
		break;
	}
...
static void window_cheats_invalidate(rct_window *w)
{
	...
	case WINDOW_CHEATS_PAGE_GUESTS:
		...
		widget_set_checkbox_value(w, WIDX_GUEST_IGNORE_RAIN, gConfigCheat.ignore_rain_guest);
		break;
