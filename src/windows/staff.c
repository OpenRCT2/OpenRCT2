...
rct_widget * window_staff_page_widgets[] = { window_staff_overview_widgets, window_staff_options_widgets,
...
// 0x992AEC
static rct_window_event_list window_staff_overview_events = { window_staff_overview_close,
...
// 0x992B5C
static rct_window_event_list window_staff_options_events = { NULL,
...
// 0x992BCC
static rct_window_event_list window_staff_stats_events = { NULL,
...
static rct_window_event_list * window_staff_page_events[] = { &window_staff_overview_events, &window_staff_options_events,
                                                              &window_staff_stats_events };

uint32 window_staff_page_enabled_widgets[] = { (1 << WIDX_CLOSE) | (1 << WIDX_TAB_1) | (1 << WIDX_TAB_2) | (1 << WIDX_TAB_3) |
                                                   (1 << WIDX_PICKUP) | (1 << WIDX_PATROL) | (1 << WIDX_RENAME) |
                                                   (1 << WIDX_LOCATE) | (1 << WIDX_FIRE),

                                               (1 << WIDX_CLOSE) | (1 << WIDX_TAB_1) | (1 << WIDX_TAB_2) | (1 << WIDX_TAB_3) |
                                                   (1 << WIDX_CHECKBOX_1) | (1 << WIDX_CHECKBOX_2) | (1 << WIDX_CHECKBOX_3) |
                                                   (1 << WIDX_CHECKBOX_4) | (1 << WIDX_COSTUME_BTN),
...
void window_staff_overview_mouseup(rct_window * w, int widgetIndex)
{
    ...
    case WIDX_FIRE:
        ...
        break;
    case WIDX_RENAME:
        window_text_input_open(w, widgetIndex, STR_STAFF_TITLE_STAFF_MEMBER_NAME, STR_STAFF_PROMPT_ENTER_NAME,
                               peep->name_string_idx, peep->id, 32);
        break;
    }
}
...
void window_staff_overview_tool_down(rct_window * w, int widgetIndex, int x, int y)
{
    if (widgetIndex == WIDX_PICKUP)
    {
        ...
        game_command_callback = game_command_callback_pickup_staff;
        game_do_command(w->number, GAME_COMMAND_FLAG_APPLY, 2, mapElement->base_height, GAME_COMMAND_PICKUP_STAFF, dest_x,
                        dest_y);
    }
...
void window_staff_overview_text_input(rct_window * w, int widgetIndex, char * text)
{
    ...
    game_do_command(2, GAME_COMMAND_FLAG_APPLY, w->number, *((int *)(text + 12)), GAME_COMMAND_SET_STAFF_NAME,
                    *((int *)(text + 20)), *((int *)(text + 16)));
    game_do_command(0, GAME_COMMAND_FLAG_APPLY, w->number, *((int *)(text + 24)), GAME_COMMAND_SET_STAFF_NAME,
