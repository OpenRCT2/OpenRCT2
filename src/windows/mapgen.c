...
static rct_widget * window_mapgen_page_widgets[] = { window_mapgen_base_widgets, window_mapgen_random_widgets,
...
static rct_window_event_list window_mapgen_base_events = { NULL,
...
static rct_window_event_list window_mapgen_random_events = { NULL,
...
static rct_window_event_list window_mapgen_simplex_events = { NULL,
...
static rct_window_event_list * window_mapgen_page_events[] = { &window_mapgen_base_events, &window_mapgen_random_events,
...
static uint32 window_mapgen_page_enabled_widgets[] = {
    (1 << WIDX_CLOSE) | (1 << WIDX_TAB_1) | (1 << WIDX_TAB_2) | (1 << WIDX_TAB_3) | (1 << WIDX_GENERATE) |
        (1 << WIDX_MAP_SIZE) | (1 << WIDX_MAP_SIZE_UP) | (1 << WIDX_MAP_SIZE_DOWN) | (1 << WIDX_BASE_HEIGHT) |
        (1 << WIDX_BASE_HEIGHT_UP) | (1 << WIDX_BASE_HEIGHT_DOWN) | (1 << WIDX_WATER_LEVEL) | (1 << WIDX_WATER_LEVEL_UP) |
        (1 << WIDX_WATER_LEVEL_DOWN) | (1 << WIDX_FLOOR_TEXTURE) | (1 << WIDX_WALL_TEXTURE),

    (1 << WIDX_CLOSE) | (1 << WIDX_TAB_1) | (1 << WIDX_TAB_2) | (1 << WIDX_TAB_3) | (1 << WIDX_GENERATE) |
        (1 << WIDX_RANDOM_TERRAIN) | (1 << WIDX_PLACE_TREES),

    (1 << WIDX_CLOSE) | (1 << WIDX_TAB_1) | (1 << WIDX_TAB_2) | (1 << WIDX_TAB_3) | (1 << WIDX_GENERATE) |
        (1 << WIDX_SIMPLEX_LABEL) | (1 << WIDX_SIMPLEX_LOW) | (1 << WIDX_SIMPLEX_LOW_UP) | (1 << WIDX_SIMPLEX_LOW_DOWN) |
        (1 << WIDX_SIMPLEX_HIGH) | (1 << WIDX_SIMPLEX_HIGH_UP) | (1 << WIDX_SIMPLEX_HIGH_DOWN) | (1 << WIDX_SIMPLEX_BASE_FREQ) |
        (1 << WIDX_SIMPLEX_BASE_FREQ_UP) | (1 << WIDX_SIMPLEX_BASE_FREQ_DOWN) | (1 << WIDX_SIMPLEX_OCTAVES) |
        (1 << WIDX_SIMPLEX_OCTAVES_UP) | (1 << WIDX_SIMPLEX_OCTAVES_DOWN) | (1 << WIDX_SIMPLEX_MAP_SIZE) |
        (1 << WIDX_SIMPLEX_MAP_SIZE_UP) | (1 << WIDX_SIMPLEX_MAP_SIZE_DOWN) | (1 << WIDX_SIMPLEX_WATER_LEVEL) |
        (1 << WIDX_SIMPLEX_WATER_LEVEL_UP) | (1 << WIDX_SIMPLEX_WATER_LEVEL_DOWN) | (1 << WIDX_SIMPLEX_FLOOR_TEXTURE) |
        (1 << WIDX_SIMPLEX_WALL_TEXTURE)
};

static uint32 window_mapgen_page_hold_down_widgets[] = {
    (1 << WIDX_MAP_SIZE_UP) | (1 << WIDX_MAP_SIZE_DOWN) | (1 << WIDX_BASE_HEIGHT_UP) | (1 << WIDX_BASE_HEIGHT_DOWN) |
        (1 << WIDX_WATER_LEVEL_UP) | (1 << WIDX_WATER_LEVEL_DOWN),

    0,

    (1 << WIDX_SIMPLEX_LOW_UP) | (1 << WIDX_SIMPLEX_LOW_DOWN) | (1 << WIDX_SIMPLEX_HIGH_UP) | (1 << WIDX_SIMPLEX_HIGH_DOWN) |
        (1 << WIDX_SIMPLEX_BASE_FREQ_UP) | (1 << WIDX_SIMPLEX_BASE_FREQ_DOWN) | (1 << WIDX_SIMPLEX_OCTAVES_UP) |
        (1 << WIDX_SIMPLEX_OCTAVES_DOWN) | (1 << WIDX_SIMPLEX_MAP_SIZE_UP) | (1 << WIDX_SIMPLEX_MAP_SIZE_DOWN) |
        (1 << WIDX_SIMPLEX_WATER_LEVEL_UP) | (1 << WIDX_SIMPLEX_WATER_LEVEL_DOWN)
};
...
rct_window * window_mapgen_open()
{
    ...
    if (w == NULL)
    {
        w           = window_create_centred(300, 200, window_mapgen_page_events[0], WC_MAPGEN, WF_10);
        w->number   = 0;
        w->frame_no = 0;
    }
...
static void window_mapgen_base_mouseup(rct_window * w, int widgetIndex)
{
    ...
    case WIDX_BASE_HEIGHT:
        ...
        window_text_input_open(w, WIDX_BASE_HEIGHT, STR_BASE_HEIGHT, STR_ENTER_BASE_HEIGHT, STR_FORMAT_INTEGER,
                               (_baseHeight - 12) / 2, 3);
        break;
    case WIDX_WATER_LEVEL:
        TextInputDescriptionArgs[0] = (WATERLEVEL_MIN - 12) / 2;
        TextInputDescriptionArgs[1] = (WATERLEVEL_MAX - 12) / 2;
        window_text_input_open(w, WIDX_WATER_LEVEL, STR_WATER_LEVEL, STR_ENTER_WATER_LEVEL, STR_FORMAT_INTEGER,
                               (_waterLevel - 12) / 2, 3);
        break;
    }
}
...
static void window_mapgen_base_dropdown(rct_window * w, int widgetIndex, int dropdownIndex)
{
    ...
    case WIDX_FLOOR_TEXTURE:
        ...
        type = (dropdownIndex == -1) ? _floorTexture : (uint32)gDropdownItemsArgs[dropdownIndex] - SPR_FLOOR_TEXTURE_GRASS;
    ...
    case WIDX_WALL_TEXTURE:
        ...
        type = (dropdownIndex == -1) ? _wallTexture : (uint32)gDropdownItemsArgs[dropdownIndex] - SPR_WALL_TEXTURE_ROCK;
...
static void window_mapgen_base_paint(rct_window * w, rct_drawpixelinfo * dpi)
{
    ...
    gfx_draw_string_left(dpi, STR_RESOLUTION_X_BY_Y, mapSizeArgs, w->colours[1], w->x + w->widgets[WIDX_MAP_SIZE].left + 1,
                         w->y + w->widgets[WIDX_MAP_SIZE].top + 1);

    arg = (_baseHeight - 12) / 2;
    gfx_draw_string_left(dpi, STR_COMMA16, &arg, w->colours[1], w->x + w->widgets[WIDX_BASE_HEIGHT].left + 1,
                         w->y + w->widgets[WIDX_BASE_HEIGHT].top + 1);

    arg = (_waterLevel - 12) / 2;
    gfx_draw_string_left(dpi, STR_COMMA16, &arg, w->colours[1], w->x + w->widgets[WIDX_WATER_LEVEL].left + 1,
...
static void window_mapgen_simplex_mouseup(rct_window * w, int widgetIndex)
{
    ...
    case WIDX_SIMPLEX_MAP_SIZE:
        ...
        TextInputDescriptionArgs[1] = MAXIMUM_MAP_SIZE_PRACTICAL;
        // Practical map size is 2 lower than the technical map size
        window_text_input_open(w, WIDX_SIMPLEX_MAP_SIZE, STR_MAP_SIZE_2, STR_ENTER_MAP_SIZE, STR_FORMAT_INTEGER, _mapSize - 2,
                               4);
        break;
    case WIDX_GENERATE:
        mapgenSettings.mapSize = _mapSize;
...
static void window_mapgen_simplex_paint(rct_window * w, rct_drawpixelinfo * dpi)
{
    ...
    gfx_draw_string_left(dpi, STR_MAPGEN_SIMPLEX_NOISE_LOW_, NULL, COLOUR_BLACK, w->x + 5,
                         w->y + w->widgets[WIDX_SIMPLEX_LOW].top + 1);
    gfx_draw_string_left(dpi, STR_MAPGEN_SIMPLEX_NOISE_HIGH, NULL, COLOUR_BLACK, w->x + 5,
                         w->y + w->widgets[WIDX_SIMPLEX_HIGH].top + 1);
    gfx_draw_string_left(dpi, STR_MAPGEN_SIMPLEX_NOISE_BASE_FREQUENCY, NULL, COLOUR_BLACK, w->x + 5,
                         w->y + w->widgets[WIDX_SIMPLEX_BASE_FREQ].top + 1);
    gfx_draw_string_left(dpi, STR_MAPGEN_SIMPLEX_NOISE_OCTAVES, NULL, COLOUR_BLACK, w->x + 5,
    ...
    gfx_draw_string_left(dpi, STR_WATER_LEVEL_LABEL, NULL, COLOUR_BLACK, w->x + 5,
                         w->y + w->widgets[WIDX_SIMPLEX_WATER_LEVEL].top + 1);

    gfx_draw_string_left(dpi, STR_COMMA16, &_simplex_low, w->colours[1], w->x + w->widgets[WIDX_SIMPLEX_LOW].left + 1,
                         w->y + w->widgets[WIDX_SIMPLEX_LOW].top + 1);
    gfx_draw_string_left(dpi, STR_COMMA16, &_simplex_high, w->colours[1], w->x + w->widgets[WIDX_SIMPLEX_HIGH].left + 1,
                         w->y + w->widgets[WIDX_SIMPLEX_HIGH].top + 1);
    gfx_draw_string_left(dpi, STR_WINDOW_OBJECTIVE_VALUE_RATING, &_simplex_base_freq, w->colours[1],
                         w->x + w->widgets[WIDX_SIMPLEX_BASE_FREQ].left + 1, w->y + w->widgets[WIDX_SIMPLEX_BASE_FREQ].top + 1);
    gfx_draw_string_left(dpi, STR_COMMA16, &_simplex_octaves, w->colours[1], w->x + w->widgets[WIDX_SIMPLEX_OCTAVES].left + 1,
                         w->y + w->widgets[WIDX_SIMPLEX_OCTAVES].top + 1);

    // The practical map size is 2 lower than the technical map size
    uint16 mapSizeArgs[2] = { _mapSize - 2, _mapSize - 2 };
    gfx_draw_string_left(dpi, STR_RESOLUTION_X_BY_Y, mapSizeArgs, w->colours[1],
                         w->x + w->widgets[WIDX_SIMPLEX_MAP_SIZE].left + 1, w->y + w->widgets[WIDX_SIMPLEX_MAP_SIZE].top + 1);

    arg = (_waterLevel - 12) / 2;
    gfx_draw_string_left(dpi, STR_COMMA16, &arg, w->colours[1], w->x + w->widgets[WIDX_SIMPLEX_WATER_LEVEL].left + 1,
                         w->y + w->widgets[WIDX_SIMPLEX_WATER_LEVEL].top + 1);
}
