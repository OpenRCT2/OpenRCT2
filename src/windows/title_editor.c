...
static rct_widget window_title_editor_widgets[] = {
    ...
    { WWT_DROPDOWN_BUTTON, 1, 10, 100, 82, 93, STR_TITLE_EDITOR_ACTION_CREATE,
      STR_TITLE_EDITOR_ACTION_CREATE_SEQUENCE_TIP }, // Create button
    { WWT_DROPDOWN_BUTTON, 1, 10, 100, 82 + 20, 93 + 20, STR_TITLE_EDITOR_ACTION_DUPLICATE,
      STR_TITLE_EDITOR_ACTION_DUPLICATE_SEQUENCE_TIP }, // Duplicate button
    { WWT_DROPDOWN_BUTTON, 1, 110, 200, 82, 93, STR_TRACK_MANAGE_DELETE,
      STR_TITLE_EDITOR_ACTION_DELETE_SEQUENCE_TIP }, // Delete button
    { WWT_DROPDOWN_BUTTON, 1, 210, 300, 82, 93, STR_TRACK_MANAGE_RENAME,
      STR_TITLE_EDITOR_ACTION_RENAME_SEQUENCE_TIP }, // Rename button

    // Saves Tab
    { WWT_DROPDOWN_BUTTON, 1, BX, BX + BW - 1, BY, BH, STR_TITLE_EDITOR_ACTION_ADD, STR_TITLE_EDITOR_ACTION_ADD_TIP }, // Add
    { WWT_DROPDOWN_BUTTON, 1, BX, BX + BW - 1, BY + (BS * 1), BH + (BS * 1), STR_TITLE_EDITOR_ACTION_REMOVE,
      STR_TITLE_EDITOR_ACTION_REMOVE_TIP }, // Remove
    { WWT_DROPDOWN_BUTTON, 1, BX, BX + BW - 1, BY + (BS * 2), BH + (BS * 2), STR_TRACK_MANAGE_RENAME,
      STR_TITLE_EDITOR_ACTION_RENAME_TIP }, // Rename
    { WWT_DROPDOWN_BUTTON, 1, BX, BX + BW - 1, BY + (BS * 3), BH + (BS * 3), STR_TITLE_EDITOR_ACTION_LOAD,
      STR_TITLE_EDITOR_ACTION_LOAD_TIP }, // Load

    // Script Tab
    { WWT_DROPDOWN_BUTTON, 1, BX, BX + BW - 1, BY, BH, STR_TITLE_EDITOR_ACTION_INSERT,
      STR_TITLE_EDITOR_ACTION_INSERT_TIP }, // Insert
    { WWT_DROPDOWN_BUTTON, 1, BX, BX + BW - 1, BY + (BS * 1), BH + (BS * 1), STR_TITLE_EDITOR_ACTION_EDIT,
      STR_TITLE_EDITOR_ACTION_EDIT_TIP }, // Edit
    { WWT_DROPDOWN_BUTTON, 1, BX, BX + BW - 1, BY + (BS * 2), BH + (BS * 2), STR_TRACK_MANAGE_DELETE,
      STR_TITLE_EDITOR_ACTION_DELETE_TIP }, // Delete
    //{ WWT_DROPDOWN_BUTTON,	1,	BX,		BX+BW-1,BY+(BS*3),	BH+(BS*3),	STR_TITLE_EDITOR_ACTION_RELOAD,
    //STR_TITLE_EDITOR_ACTION_RELOAD_TIP }, // Reload
    { WWT_DROPDOWN_BUTTON, 1, BX, BX + BW - 1, BY + (BS * 3), BH + (BS * 3), STR_TITLE_EDITOR_ACTION_SKIP_TO,
      STR_TITLE_EDITOR_ACTION_SKIP_TO_TIP }, // Skip to

    { WWT_DROPDOWN_BUTTON, 1, BX, BX + BW / 2 - 1, BY + (BS * 5), BH + (BS * 5), STR_DOWN,
      STR_TITLE_EDITOR_ACTION_MOVE_DOWN_TIP }, // Move down
    { WWT_DROPDOWN_BUTTON, 1, BX + BW / 2, BX + BW - 1, BY + (BS * 5), BH + (BS * 5), STR_UP,
      STR_TITLE_EDITOR_ACTION_MOVE_UP_TIP }, // Move up

    { WWT_IMGBTN, 1, BX, BX + BW / 4 - 1, WH - 32, WH - 16, SPR_G2_TITLE_RESTART,
      STR_TITLE_EDITOR_ACTION_REPLAY_TIP }, // Replay
    { WWT_IMGBTN, 1, BX + BW / 4, BX + BW / 2 - 1, WH - 32, WH - 16, SPR_G2_TITLE_STOP,
      STR_TITLE_EDITOR_ACTION_STOP_TIP }, // Stop
    { WWT_IMGBTN, 1, BX + BW / 2, BX + BW * 3 / 4 - 1, WH - 32, WH - 16, SPR_G2_TITLE_PLAY,
      STR_TITLE_EDITOR_ACTION_PLAY_TIP },                                                                               // Play
...
static int window_title_editor_tab_animation_loops[]   = { 64, 1, 28 };
static int window_title_editor_tab_animation_divisor[] = { 4, 1, 4 };
static int window_title_editor_tab_sprites[]           = { SPR_TAB_RIDE_0, SPR_FLOPPY, SPR_TAB_STATS_0 };
...
static void window_title_editor_draw_tab_images(rct_drawpixelinfo * dpi, rct_window * w)
{
    ...
    for (int i = 0; i < WINDOW_TITLE_EDITOR_TAB_COUNT; i++)
    {
        ...
        gfx_draw_sprite(dpi, sprite_idx, w->x + w->widgets[WIDX_TITLE_EDITOR_PRESETS_TAB + i].left + x,
...
void window_title_editor_open(int tab)
{
    ...
    window->enabled_widgets =
        (1 << WIDX_TITLE_EDITOR_CLOSE) | (1 << WIDX_TITLE_EDITOR_PRESETS_TAB) | (1 << WIDX_TITLE_EDITOR_SAVES_TAB) |
        ...
        (1 << WIDX_TITLE_EDITOR_ADD) | (1 << WIDX_TITLE_EDITOR_REMOVE) | (1 << WIDX_TITLE_EDITOR_RENAME) |
...
static void window_title_editor_mouseup(rct_window * w, int widgetIndex)
{
    ...
    playing       = (gCurrentTitleSequence == gCurrentPreviewTitleSequence) &&
              ((gScreenFlags & SCREEN_FLAGS_TITLE_DEMO) == SCREEN_FLAGS_TITLE_DEMO);
    ...
    case WIDX_TITLE_EDITOR_NEW_BUTTON:
        ...
        if (!commandEditorOpen)
            ...
            window_text_input_open(w, widgetIndex, STR_TITLE_EDITOR_ACTION_DUPLICATE, STR_TITLE_EDITOR_ENTER_NAME_FOR_SEQUENCE,
                                   STR_NONE, 0, 64);
        break;
    case WIDX_TITLE_EDITOR_DUPLICATE_BUTTON:
        ...
        if (!commandEditorOpen)
            window_text_input_open(w, widgetIndex, STR_TITLE_EDITOR_ACTION_DUPLICATE, STR_TITLE_EDITOR_ENTER_NAME_FOR_SEQUENCE,
                                   STR_STRING, (uintptr_t)&gConfigTitleSequences.presets[gCurrentTitleSequence].name, 64);
        break;
    ...
    case WIDX_TITLE_EDITOR_RENAME_BUTTON:
        ...
        commandEditorOpen *= 2;
        if (!defaultPreset && !commandEditorOpen)
            window_text_input_open(w, widgetIndex, STR_TRACK_MANAGE_RENAME, STR_TITLE_EDITOR_ENTER_NAME_FOR_SEQUENCE,
                                   STR_STRING, (uintptr_t)&gConfigTitleSequences.presets[gCurrentTitleSequence].name, 64);
        break;
        ...
        if (!defaultPreset && !playing && !commandEditorOpen)
        {
            if (w->selected_list_item != -1)
                window_text_input_open(
                    ...
                    (uintptr_t)gConfigTitleSequences.presets[gCurrentTitleSequence].saves[w->selected_list_item],
                    TITLE_SEQUENCE_MAX_SAVE_LENGTH - 1);
        }
        break;
    case WIDX_TITLE_EDITOR_LOAD:
        if (w->selected_list_item != -1)
        {
            ...
            safe_strcat_path(path, gConfigTitleSequences.presets[gCurrentTitleSequence].saves[w->selected_list_item],
                             sizeof(path));
            game_load_save(path);
    ...
    case WIDX_TITLE_EDITOR_EDIT:
        ...
        {
            if (w->selected_list_item != -1 &&
                w->selected_list_item < gConfigTitleSequences.presets[gCurrentTitleSequence].num_commands)
...
void window_title_editor_scrollgetsize(rct_window * w, int scrollIndex, int * width, int * height)
{
    ...
    *height = lineCount * ROW_HEIGHT;
    i       = *height - window_title_editor_widgets[WIDX_TITLE_EDITOR_LIST].bottom +
...
void window_title_editor_invalidate(rct_window * w)
{
    ...
    if (!playing)
        ...
        w->disabled_widgets &= ~((1 << WIDX_TITLE_EDITOR_REPLAY) | (1 << WIDX_TITLE_EDITOR_STOP) |
                                 (1 << WIDX_TITLE_EDITOR_SKIP) | (1 << WIDX_TITLE_EDITOR_SKIP_TO));
}
...
void window_title_editor_paint(rct_window * w, rct_drawpixelinfo * dpi)
{
    ...
    case WINDOW_TITLE_EDITOR_TAB_PRESETS:
        ...
        gfx_draw_string_left(dpi, STR_TITLE_SEQUENCE, NULL, w->colours[1], w->x + 10,
                             w->y + window_title_editor_widgets[WIDX_TITLE_EDITOR_PRESETS].top + 1);
        gfx_draw_string_left_clipped(dpi, STR_STRING, gCommonFormatArgs, w->colours[1],
...
void window_title_editor_scrollpaint(rct_window * w, rct_drawpixelinfo * dpi, int scrollIndex)
{
        ...
        for (int i = 0; i < title->num_saves; i++, y += ROW_HEIGHT)
        {
            ...
            if (i == w->selected_list_item)
            {
                ...
                gfx_fill_rect(dpi, x, y, x + SCROLL_WIDTH + 100, y + ROW_HEIGHT - 1, ColourMapA[w->colours[1]].dark);
                     ...
                     (i == gTitleScriptSave && inTitle && gCurrentTitleSequence == gCurrentPreviewTitleSequence))
            {
                hover = true;
            ...
            {
                gfx_fill_rect(dpi, x, y, x + SCROLL_WIDTH + 100, y + ROW_HEIGHT - 1,
    ...
    else if (w->selected_tab == WINDOW_TITLE_EDITOR_TAB_SCRIPT)
    {
        for (int i = 0; i < title->num_commands; i++, y += ROW_HEIGHT)
        {
            ...
            else if (i == (int)_window_title_editor_highlighted_index ||
                     (i == gTitleScriptCommand && inTitle && gCurrentTitleSequence == gCurrentPreviewTitleSequence))
            {
                hover = true;
                gfx_fill_rect(dpi, x, y, x + SCROLL_WIDTH + 100, y + ROW_HEIGHT - 1, ColourMapA[w->colours[1]].mid_dark);
            }
