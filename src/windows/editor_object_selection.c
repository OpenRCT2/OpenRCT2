...
static rct_window_event_list window_editor_object_selection_events = { window_editor_object_selection_close,
...
static void visible_list_refresh(rct_window * w)
{
    ...
    for (int i = 0; i < numObjects; i++)
    {
        ...
        if (objectType == w->selected_tab && !(selectionFlags & OBJECT_SELECTION_FLAG_6) && filter_source(item) &&
...
void window_editor_object_selection_open()
{
    ...
    window->enabled_widgets = (1 << WIDX_ADVANCED) | (1 << WIDX_INSTALL_TRACK) | (1 << WIDX_FILTER_DROPDOWN) |
                              ...
                              (1 << WIDX_LIST_SORT_TYPE) | (((uint32)1) << WIDX_LIST_SORT_RIDE);
...
static void window_editor_object_selection_mouseup(rct_window * w, int widgetIndex)
{
    ...
    case WIDX_FILTER_STRING_BUTTON:
        // window_text_input_open(w, widgetIndex, STR_OBJECT_SEARCH, STR_OBJECT_SEARCH_DESC, STR_STRING, (uint32)_filter_string,
        // 40);
...
void window_editor_object_selection_mousedown(int widgetIndex, rct_window * w, rct_widget * widget)
{
    ...
    case WIDX_FILTER_DROPDOWN:
        ...
        window_dropdown_show_text(w->x + widget->left, w->y + widget->top, widget->bottom - widget->top + 1,
                                  w->colours[widget->colour], DROPDOWN_FLAG_STAY_OPEN, num_items);
...
static void window_editor_object_selection_invalidate(rct_window * w)
{
    ...
    if (ridePage)
    {
        w->enabled_widgets |= (1 << WIDX_FILTER_RIDE_TAB_ALL) | (1 << WIDX_FILTER_RIDE_TAB_TRANSPORT) |
                              (1 << WIDX_FILTER_RIDE_TAB_GENTLE) | (1 << WIDX_FILTER_RIDE_TAB_COASTER) |
                              (1 << WIDX_FILTER_RIDE_TAB_THRILL) | (1 << WIDX_FILTER_RIDE_TAB_WATER) |
...
static void window_editor_object_selection_paint(rct_window * w, rct_drawpixelinfo * dpi)
{
    ...
    const int ride_tabs[] = { SPR_TAB_RIDE_16,        0x20000000 | SPR_TAB_RIDES_TRANSPORT_0,
                              SPR_TAB_RIDES_GENTLE_0, 0x20000000 | SPR_TAB_RIDES_ROLLER_COASTERS_0,
    ...
    if (widget->type != WWT_EMPTY)
    {
        ...
        gfx_draw_string_left_clipped(dpi, STR_OBJECTS_SORT_TYPE, &stringId, w->colours[1], w->x + widget->left + 1,
    ...
    if (widget->type != WWT_EMPTY)
    {
        ...
        gfx_draw_string_left_clipped(dpi, STR_OBJECTS_SORT_RIDE, &stringId, w->colours[1], w->x + widget->left + 1,
                                     w->y + widget->top + 1, widget->right - widget->left);
    }
    ...
    if (description != NULL)
    {
        ...
        {
            gfx_draw_string_left_wrapped(dpi, gCommonFormatArgs, x, y, width, STR_OBJECT_SELECTION_DESCRIPTION_SCENARIO_TEXT,
                                         COLOUR_BLACK);
        }
        else
        {
    ...
    {
    case 8:
        stringId = STR_ROLLERCOASTER_TYCOON_2_DROPDOWN;
        break;
    case 1:
        stringId = STR_OBJECT_FILTER_WW;
        break;
    case 2:
        stringId = STR_OBJECT_FILTER_TT;
        break;
    default:
        stringId = STR_OBJECT_FILTER_CUSTOM;
        break;
    }
    ...
    gfx_draw_string_right(dpi, STR_WINDOW_COLOUR_2_STRINGID, gCommonFormatArgs, COLOUR_BLACK, w->x + w->width - 5,
                          w->y + w->height - 3 - 12);
}
...
static bool filter_string(const ObjectRepositoryItem * item)
{
    ...
    return strstr(name_lower, filter_lower) != NULL ||
           (((item->ObjectEntry.flags & 0x0F) == OBJECT_TYPE_RIDE) && strstr(type_lower, filter_lower) != NULL);
}
...
static bool filter_source(const ObjectRepositoryItem * item)
{
    ...
    return (_FILTER_RCT2 && source == 8) || (_FILTER_WW && source == 1) || (_FILTER_TT && source == 2) ||
           (_FILTER_CUSTOM && source != 8 && source != 1 && source != 2);
}
...
static void filter_update_counts()
{
    if (!_FILTER_ALL || strlen(_filter_string) > 0)
    {
        ...
        {
            const ObjectRepositoryItem * item = &items[i];
            if (filter_source(item) && filter_string(item) && filter_chunks(item) && filter_selected(*selectionFlags))
            {
                uint8 objectType = item->ObjectEntry.flags & 0xF;
