...
static rct_widget window_title_command_editor_widgets[] = {
    ...
    { WWT_DROPDOWN_BUTTON, 1, WS + WHA + 3, WW - WS - 1, BY2 - 14, BY2 - 3, STR_TITLE_COMMAND_EDITOR_ACTION_GET_LOCATION,
      STR_NONE }, // Get location/zoom/etc
...
static rct_window_event_list window_title_command_editor_events = { NULL,
...
static rct_xy16 get_location()
{
    ...
    if (w != NULL)
    {
        ...
        get_map_coordinates_from_pos(w->viewport->view_width / 2, w->viewport->view_height / 2,
                                     VIEWPORT_INTERACTION_MASK_TERRAIN, &mapCoord.x, &mapCoord.y, &interactionType, &mapElement,
                                     NULL);
        mapCoord.x -= 16;
        mapCoord.x /= 32;
        mapCoord.y -= 16;
...
void window_title_command_editor_open(int index, bool insert)
{
    ...
    window = window_create_centred(WW, WH, &window_title_command_editor_events, WC_TITLE_COMMAND_EDITOR, WF_STICK_TO_FRONT);
    ...
    window->enabled_widgets = (1 << WIDX_CLOSE) | (1 << WIDX_COMMAND) | (1 << WIDX_COMMAND_DROPDOWN) |
                              (1 << WIDX_TEXTBOX_FULL) | (1 << WIDX_TEXTBOX_X) | (1 << WIDX_TEXTBOX_Y) | (1 << WIDX_INPUT) |
                              (1 << WIDX_INPUT_DROPDOWN) | (1 << WIDX_GET) | (1 << WIDX_OKAY) | (1 << WIDX_CANCEL);
...
static void window_title_command_editor_paint(rct_window * w, rct_drawpixelinfo * dpi)
{
    ...
    gfx_draw_string_left_clipped(dpi, get_command_info(command.command).nameStringId, NULL, w->colours[1],
                                 w->x + w->widgets[WIDX_COMMAND].left + 1, w->y + w->widgets[WIDX_COMMAND].top,
                                 w->widgets[WIDX_COMMAND_DROPDOWN].left - w->widgets[WIDX_COMMAND].left - 4);

    if (command.command == TITLE_SCRIPT_SPEED)
    {
        gfx_draw_string_left_clipped(dpi, SpeedNames[command.speed - 1], NULL, w->colours[1],
    ...
    else if (command.command == TITLE_SCRIPT_LOAD)
    {
        ...
        {
            gfx_draw_string_left_clipped(dpi, STR_TITLE_COMMAND_EDITOR_NO_SAVE_SELECTED, NULL, w->colours[1],
                                         w->x + w->widgets[WIDX_INPUT].left + 1, w->y + w->widgets[WIDX_INPUT].top,
                                         w->widgets[WIDX_INPUT_DROPDOWN].left - w->widgets[WIDX_INPUT].left - 4);
        }
        else
        {
            set_format_arg(0, uintptr_t,
                           (uintptr_t)&gConfigTitleSequences.presets[gCurrentTitleSequence].saves[command.saveIndex]);
