...
static const rct_string_id ClimateNames[] = {
    STR_CLIMATE_COOL_AND_WET, STR_CLIMATE_WARM, STR_CLIMATE_HOT_AND_DRY, STR_CLIMATE_COLD,
};
...
// 0x009A9DF4
static rct_window_event_list window_objective_options_main_events = { NULL,
...
// 0x009A9F58
static rct_window_event_list window_objective_options_rides_events = { NULL,
...
static uint64 window_editor_objective_options_page_enabled_widgets[] = {
    (1 << WIDX_CLOSE) | (1 << WIDX_TAB_1) | (1 << WIDX_TAB_2) | (1 << WIDX_OBJECTIVE) | (1 << WIDX_OBJECTIVE_DROPDOWN) |
        ...
        (1 << WIDX_OBJECTIVE_ARG_2_DECREASE) | (1 << WIDX_CLIMATE) | (1 << WIDX_CLIMATE_DROPDOWN) | (1 << WIDX_PARK_NAME) |
        (1 << WIDX_SCENARIO_NAME) | (1 << WIDX_CATEGORY) | (1 << WIDX_CATEGORY_DROPDOWN) | (1 << WIDX_DETAILS),

    (1 << WIDX_CLOSE) | (1 << WIDX_TAB_1) | (1 << WIDX_TAB_2)
};

static uint64 window_editor_objective_options_page_hold_down_widgets[] = { (1 << WIDX_OBJECTIVE_ARG_1_INCREASE) |
...
static void window_editor_objective_options_main_mouseup(rct_window * w, int widgetIndex)
{
    ...
    case WIDX_SCENARIO_NAME:
        ...
        break;
    case WIDX_DETAILS:
        window_text_input_raw_open(w, WIDX_DETAILS, STR_PARK_SCENARIO_DETAILS, STR_ENTER_SCENARIO_DESCRIPTION, gS6Info.details,
                                   256);
        break;
    }
}
...
static void window_editor_objective_options_show_objective_dropdown(rct_window * w)
{
    ...
    window_dropdown_show_text_custom_width(w->x + dropdownWidget->left, w->y + dropdownWidget->top,
                                           ...
                                           DROPDOWN_FLAG_STAY_OPEN, numItems, dropdownWidget->right - dropdownWidget->left - 3);
...
static void window_editor_objective_options_show_climate_dropdown(rct_window * w)
{
    ...
    window_dropdown_show_text_custom_width(w->x + dropdownWidget->left, w->y + dropdownWidget->top,
                                           dropdownWidget->bottom - dropdownWidget->top + 1, w->colours[1],
                                           DROPDOWN_FLAG_STAY_OPEN, 4, dropdownWidget->right - dropdownWidget->left - 3);
...
static void window_editor_objective_options_show_category_dropdown(rct_window * w)
{
    ...
    window_dropdown_show_text_custom_width(w->x + dropdownWidget->left, w->y + dropdownWidget->top,
                                           dropdownWidget->bottom - dropdownWidget->top + 1, w->colours[1],
                                           DROPDOWN_FLAG_STAY_OPEN, 5, dropdownWidget->right - dropdownWidget->left - 3);
...
static void window_editor_objective_options_rides_paint(rct_window * w, rct_drawpixelinfo * dpi)
{
    ...
    gfx_draw_string_left(dpi, STR_WINDOW_PRESERVATION_ORDER, NULL, COLOUR_BLACK, w->x + 6,
                         w->y + w->widgets[WIDX_PAGE_BACKGROUND].top + 3);
}
