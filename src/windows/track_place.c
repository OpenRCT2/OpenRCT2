...
static rct_window_event_list window_track_place_events = { window_track_place_close,
...
static void window_track_place_attempt_placement(rct_track_td6 * td6, int x, int y, int z, int bl, money32 * cost,
                                                 uint8 * rideIndex);

static void window_track_place_clear_mini_preview();
static void window_track_place_draw_mini_preview(rct_track_td6 * td6);
static void window_track_place_draw_mini_preview_track(rct_track_td6 * td6, int pass, rct_xy16 origin, rct_xy16 * min,
                                                       rct_xy16 * max);
static void window_track_place_draw_mini_preview_maze(rct_track_td6 * td6, int pass, rct_xy16 origin, rct_xy16 * min,
                                                      rct_xy16 * max);
...
static void window_track_place_clear_provisional()
{
    ...
    {
        sub_6D01B3(_trackDesign, PTD_OPERATION_CLEAR_OUTLINES, _window_track_place_ride_index, _window_track_place_last_valid_x,
        ...
        while (trackBlock->index != 255)
        {
            ...
            else
            {
                ...
                if (draw_mini_preview_is_pixel_in_bounds(pixelPosition))
                {
                    ...
                    uint8 colour = TrackSequenceProperties[trackType][0] & TRACK_SEQUENCE_FLAG_ORIGIN
    ...
    while (mazeElement->all != 0)
    {
        ...
        else
        {
            ...
            if (draw_mini_preview_is_pixel_in_bounds(pixelPosition))
            {
                ...
                uint8 colour = mazeElement->type == 8 || mazeElement->type == 128 ? PALETTE_INDEX_PRIMARY_LIGHTEST
                                                                                  : PALETTE_INDEX_PRIMARY_MID_DARK;
...
static rct_xy16 draw_mini_preview_get_pixel_position(sint16 x, sint16 y)
{
    return (rct_xy16){ 80 + ((y / 32) - (x / 32)) * 4, 38 + ((y / 32) + (x / 32)) * 2 };
}
