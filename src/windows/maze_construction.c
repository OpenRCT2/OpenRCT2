...
static rct_widget window_maze_construction_widgets[] = {
    ...
    { WWT_FLATBTN, 1, 83, 127, 125, 153, SPR_CONSTRUCTION_DIRECTION_SE,
...
// 0x993F6C
static rct_window_event_list window_maze_construction_events = { window_maze_construction_close,
...
static void window_maze_construction_resize(rct_window * w)
{
    ...
    {
        disabledWidgets |= ((1ULL << WIDX_MAZE_BUILD_MODE) | (1ULL << WIDX_MAZE_MOVE_MODE) | (1ULL << WIDX_MAZE_FILL_MODE) |
                            (1ULL << WIDX_MAZE_DIRECTION_NW) | (1ULL << WIDX_MAZE_DIRECTION_NE) |
...
static void window_maze_construction_entrance_tooldown(int x, int y, rct_window * w)
{
    ...
    gGameCommandErrorTitle = entranceExitType ? STR_CANT_BUILD_MOVE_ENTRANCE_FOR_THIS_RIDE_ATTRACTION
                                              : STR_CANT_BUILD_MOVE_EXIT_FOR_THIS_RIDE_ATTRACTION;

    money32 cost = game_do_command(x, GAME_COMMAND_FLAG_APPLY | ((direction ^ 2) << 8), y, rideIndex | (entranceExitType << 8),
...
static void window_maze_construction_construct(int direction)
{
    ...
    money32 cost =
        game_do_command(x, flags | (direction << 8), y, _currentRideIndex | (mode << 8), GAME_COMMAND_SET_MAZE_TRACK, z, 0);
    if (cost == MONEY32_UNDEFINED)
    {
        return;
    }
