...
rct_widget window_game_bottom_toolbar_widgets[] = {
    ...
    { WWT_FLATBTN, 2, 0x007D + WIDTH_MOD, 0x0094 + WIDTH_MOD, 5, 28, 0xFFFFFFFF,
      STR_SHOW_SUBJECT_TIP }, // Associated news item window
    { WWT_FLATBTN, 2, 0x01EB - WIDTH_MOD, 0x0202 - WIDTH_MOD, 5, 28, SPR_LOCATE,
      STR_LOCATE_SUBJECT_TIP }, // Scroll to news item target
...
static void window_game_bottom_toolbar_draw_park_rating(rct_drawpixelinfo * dpi, rct_window * w, int colour, int x, int y,
                                                        uint8 factor);
 ...
 *  rct2: 0x0097BFDC
 */
static rct_window_event_list window_game_bottom_toolbar_events = { NULL,
...
void window_game_bottom_toolbar_open()
{
    ...
    window = window_create(0, gScreenHeight - 32, gScreenWidth, 32, &window_game_bottom_toolbar_events, WC_BOTTOM_TOOLBAR,
    ...
    window->enabled_widgets |= (1 << WIDX_LEFT_OUTSET) | (1 << WIDX_MONEY) | (1 << WIDX_GUESTS) | (1 << WIDX_PARK_RATING) |
                               (1 << WIDX_MIDDLE_OUTSET) | (1 << WIDX_MIDDLE_INSET) | (1 << WIDX_NEWS_SUBJECT) |
                               (1 << WIDX_NEWS_LOCATE) | (1 << WIDX_RIGHT_OUTSET) | (1 << WIDX_DATE);
...
static void window_game_bottom_toolbar_paint(rct_window * w, rct_drawpixelinfo * dpi)
{
    ...
    gfx_filter_rect(dpi, w->x + window_game_bottom_toolbar_widgets[WIDX_LEFT_OUTSET].left,
    ...
    gfx_filter_rect(dpi, w->x + window_game_bottom_toolbar_widgets[WIDX_RIGHT_OUTSET].left,
...
static void window_game_bottom_toolbar_draw_left_panel(rct_drawpixelinfo * dpi, rct_window * w)
{
    ...
    gfx_fill_rect_inset(dpi, w->x + window_game_bottom_toolbar_widgets[WIDX_LEFT_OUTSET].left + 1,
    ...
    x = (window_game_bottom_toolbar_widgets[WIDX_LEFT_OUTSET].left +
         window_game_bottom_toolbar_widgets[WIDX_LEFT_OUTSET].right) /
            2 +
        w->x;
    ...
    if (!(gParkFlags & PARK_FLAGS_NO_MONEY))
    {
        ...
        gfx_draw_string_centred(
            dpi, (cash < 0 ? STR_BOTTOM_TOOLBAR_CASH_NEGATIVE : STR_BOTTOM_TOOLBAR_CASH), x, y - 3,
    ...
    static const rct_string_id guestCountFormats[] = {
        STR_BOTTOM_TOOLBAR_NUM_GUESTS_STABLE, STR_BOTTOM_TOOLBAR_NUM_GUESTS_DECREASE, STR_BOTTOM_TOOLBAR_NUM_GUESTS_INCREASE,
    };

    // Draw guests
    gfx_draw_string_centred(dpi, guestCountFormats[gGuestChangeModifier], x, y,
    ...
    window_game_bottom_toolbar_draw_park_rating(
        dpi, w, w->colours[3], w->x + window_game_bottom_toolbar_widgets[WIDX_PARK_RATING].left + 11,
        w->y + window_game_bottom_toolbar_widgets[WIDX_PARK_RATING].top, max(10, ((gParkRating / 4) * 263) / 256));
}
...
static void window_game_bottom_toolbar_draw_right_panel(rct_drawpixelinfo * dpi, rct_window * w)
{
    ...
    // Draw green inset rectangle on panel
    gfx_fill_rect_inset(dpi, w->x + window_game_bottom_toolbar_widgets[WIDX_RIGHT_OUTSET].left + 1,
                        ...
                        w->y + window_game_bottom_toolbar_widgets[WIDX_RIGHT_OUTSET].bottom - 1, w->colours[1],
                        INSET_RECT_F_30);

    x = (window_game_bottom_toolbar_widgets[WIDX_RIGHT_OUTSET].left +
    ...
    gfx_draw_string_centred(dpi, stringId, x, y,
...
static void window_game_bottom_toolbar_draw_news_item(rct_drawpixelinfo * dpi, rct_window * w)
{
    ...
    // Current news item
    gfx_fill_rect_inset(dpi, w->x + middleOutsetWidget->left + 1, w->y + middleOutsetWidget->top + 1,
                        w->x + middleOutsetWidget->right - 1, w->y + middleOutsetWidget->bottom - 1, w->colours[2],
                        INSET_RECT_F_30);
    ...
    gfx_draw_string_centred_wrapped_partial(dpi, x, y, width, COLOUR_BRIGHT_GREEN, STR_BOTTOM_TOOLBAR_NEWS_TEXT, &newsItemText,
                                            newsItem->ticks);
