...
static rct_window_event_list window_loadsave_events = { window_loadsave_close,
...
static void window_loadsave_mouseup(rct_window * w, int widgetIndex)
{
    ...
    case WIDX_UP:
        ...
        break;
    case WIDX_NEW_FILE:
        window_text_input_open(w, WIDX_NEW_FILE, STR_NONE, STR_FILEBROWSER_FILE_NAME_PROMPT, STR_STRING,
                               (uintptr_t)&_defaultName, 64);
        break;
...
static void window_loadsave_paint(rct_window * w, rct_drawpixelinfo * dpi)
{
    ...
    if (gConfigGeneral.load_save_sort == SORT_NAME_ASCENDING)
        ...
        id = STR_DOWN;
    gfx_draw_string_centred_clipped(dpi, STR_NAME, &id, COLOUR_GREY, w->x + 4 + (w->width - 8) / 4, w->y + 50,
                                    (w->width - 8) / 2);
    // Date button text
    if (gConfigGeneral.load_save_sort == SORT_DATE_ASCENDING)
        ...
        id = STR_NONE;
    gfx_draw_string_centred_clipped(dpi, STR_DATE, &id, COLOUR_GREY, w->x + 4 + (w->width - 8) * 3 / 4, w->y + 50,
                                    (w->width - 8) / 2);
}
    ...
    case (LOADSAVETYPE_LOAD | LOADSAVETYPE_GAME):
        ...
        if (gLoadSaveTitleSequenceSave)
        {
            ...
            if (title_sequence_save_exists(gCurrentTitleSequence, newName))
            {
                ...
                window_text_input_open(w, WIDX_SCROLL, STR_FILEBROWSER_RENAME_SAVE_TITLE, STR_ERROR_EXISTING_NAME, STR_STRING,
                                       (uintptr_t)_listItems[w->selected_list_item].name, TITLE_SEQUENCE_MAX_SAVE_LENGTH - 1);
            }
            else
            {
        ...
        else if ((extension == FILE_EXTENSION_SV6 && game_load_save(path)) ||
...
static rct_widget window_overwrite_prompt_widgets[] = {
    ...
    { WWT_DROPDOWN_BUTTON, 0, OVERWRITE_WW - 95, OVERWRITE_WW - 11, OVERWRITE_WH - 20, OVERWRITE_WH - 9, STR_SAVE_PROMPT_CANCEL,
      STR_NONE },
    { WIDGETS_END }
};
...
static rct_window_event_list window_overwrite_prompt_events = { NULL,
...
static rct_window * window_overwrite_prompt_open(const char * name, const char * path)
{
    ...
    w = window_create_centred(OVERWRITE_WW, OVERWRITE_WH, &window_overwrite_prompt_events, WC_LOADSAVE_OVERWRITE_PROMPT,
                              WF_STICK_TO_FRONT);
