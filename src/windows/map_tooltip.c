...
static rct_widget window_map_tooltip_widgets[] = { { WWT_IMGBTN, 0, 0, 199, 0, 29, 0xFFFFFFFF, STR_NONE }, { WIDGETS_END } };
...
static rct_window_event_list window_map_tooltip_events = { NULL,
                                                           NULL,
                                                           NULL,
                                                           NULL,
                                                           NULL,
...
void window_map_tooltip_update_visibility()
{
    ...
    if (_cursorHoldDuration < 25 || stringId == STR_NONE || (gInputPlaceObjectModifier & 3) ||
...
static void window_map_tooltip_open()
{
    ...
    if (w == NULL)
    {
        w = window_create(x, y, width, height, &window_map_tooltip_events, WC_MAP_TOOLTIP,
...
static void window_map_tooltip_paint(rct_window * w, rct_drawpixelinfo * dpi)
{
    ...
    gfx_draw_string_centred_wrapped(dpi, gMapTooltipFormatArgs, w->x + (w->width / 2), w->y + (w->height / 2), w->width,
