...
static void scenario_objective_check_10_rollercoasters()
{
    ...
    FOR_ALL_RIDES(i, ride)
    {
        ...
        if (rideType != NULL &&
            ...
            ride->status == RIDE_STATUS_OPEN && ride->excitement >= RIDE_RATING(6, 00) && type_already_counted[subtype_id] == 0)
        {
...
static void scenario_objective_check_10_rollercoasters_length()
{
    ...
    FOR_ALL_RIDES(i, ride)
    {
        ...
        if ((rideType->category[0] == RIDE_GROUP_ROLLERCOASTER || rideType->category[1] == RIDE_GROUP_ROLLERCOASTER) &&
            ride->status == RIDE_STATUS_OPEN && ride->excitement >= RIDE_RATING(7, 00) && type_already_counted[subtype_id] == 0)
        {
...
static void scenario_objective_check_monthly_food_income()
{
    ...
    sint32    lastMonthProfit =
        expenditureLastMonth[RCT_EXPENDITURE_TYPE_SHOP_SHOP_SALES] + expenditureLastMonth[RCT_EXPENDITURE_TYPE_SHOP_STOCK] +
        expenditureLastMonth[RCT_EXPENDITURE_TYPE_FOODDRINK_SALES] + expenditureLastMonth[RCT_EXPENDITURE_TYPE_FOODDRINK_STOCK];
