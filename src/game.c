...
void game_update()
{
    ...
    for (i = 0; i < numUpdates; i++)
    {
        ...
        if (gInputState == INPUT_STATE_RESET || gInputState == INPUT_STATE_NORMAL)
        {
...
int game_do_command_p(int command, int * eax, int * ebx, int * ecx, int * edx, int * esi, int * edi, int * ebp)
{
    ...
    *esi         = command;
    ...
    if ((flags & GAME_COMMAND_FLAG_NETWORKED) && !(flags & GAME_COMMAND_FLAG_GHOST) &&
        (command == GAME_COMMAND_PLACE_FENCE || command == GAME_COMMAND_PLACE_SCENERY ||
                ...
                gGameCommandNestLevel == 1 /* Send only top-level commands */)
            {
                ...
                { // Disable these commands over the network
                    network_send_gamecmd(*eax, *ebx, *ecx, *edx, *esi, *edi, *ebp,
                                         game_command_callback_get_index(game_command_callback));
...
void game_autosave()
{
    ...
    utf8 timeName[34];
    snprintf(timeName, 34, "autosave_%d-%02d-%02d_%02d-%02d-%02d%s", currentDate.year, currentDate.month, currentDate.day,
             currentTime.hour, currentTime.minute, currentTime.second, fileExtension);
