    ...
    char flags, uint16_t sprite)
{
    ...
    if (flags & VIEWPORT_FOCUS_TYPE_SPRITE)
    {
        ...
        auto* centre_sprite = get_sprite(sprite);
...
void viewport_update_sprite_follow(rct_window* window)
{
    ...
    {
        auto* sprite = get_sprite(window->viewport_target_sprite);
...
{
    auto* sprite = try_get_sprite(window->viewport_smart_follow_sprite);
    if (sprite == nullptr)
    {
        window->viewport_smart_follow_sprite = SPRITE_INDEX_NULL;
        window->viewport_target_sprite = SPRITE_INDEX_NULL;
    }
    else if (sprite->sprite_identifier == SPRITE_IDENTIFIER_PEEP)
    {
    ...
    else if (sprite->sprite_identifier == SPRITE_IDENTIFIER_VEHICLE)
    {
