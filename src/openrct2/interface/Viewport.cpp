...
namespace OpenRCT2
{
    ...
    void ViewportCreate(WindowBase& w, const ScreenCoordsXY& screenCoords, int32_t width, int32_t height, const Focus& focus)
    {
        ...
        if (Config::Get().general.alwaysShowGridlines)
            viewport->flags |= ViewportFlag::GRIDLINES;
        w.viewport = viewport;
    ...
    static void ViewportSetUndergroundFlag(int32_t underground, WindowBase* window, Viewport* viewport)
    {
            ...
            || (window->classification == WindowClass::mainWindow && !window->viewportSmartFollowSprite.IsNull()))
        {
            if (!underground)
            {
                int32_t bit = viewport->flags & ViewportFlag::UNDERGROUND_INSIDE;
                viewport->flags &= ~ViewportFlag::UNDERGROUND_INSIDE;
                if (!bit)
                    return;
            }
            else
    ...
    void ViewportRotateAll(int32_t direction)
    {
        WindowVisitEach([direction](WindowBase* w) {
            ...
            if (viewport == nullptr)
                return;
            if (viewport->flags & ViewportFlag::INDEPENDENT_ROTATION)
                return;
    ...
    {
        if (viewport->flags & ViewportFlag::RENDERING_INHIBITED)
            return;
    ...
    static void ViewportPaintColumn(PaintSession& session)
    {
        ...
        if (session.ViewFlags
                & (ViewportFlag::HIDE_VERTICAL | ViewportFlag::HIDE_BASE | ViewportFlag::UNDERGROUND_INSIDE
                   | ViewportFlag::CLIP_VIEW)
            && (~session.ViewFlags & ViewportFlag::TRANSPARENT_BACKGROUND))
        {
            PaletteIndex colour = PaletteIndex::pi10;
            if (session.ViewFlags & ViewportFlag::HIDE_ENTITIES)
            {
        ...
        if (Config::Get().general.renderWeatherGloom && !gTrackDesignSaveMode
            && !(session.ViewFlags & ViewportFlag::HIDE_ENTITIES) && !(session.ViewFlags & ViewportFlag::HIGHLIGHT_PATH_ISSUES))
        {
    ...
    void ShowGridlines()
    {
        if (gShowGridLinesRefCount == 0)
        {
            ...
            if (mainWindow != nullptr)
            {
                if (!(mainWindow->viewport->flags & ViewportFlag::GRIDLINES))
                {
    ...
    void ShowLandRights()
    {
        if (gShowLandRightsRefCount == 0)
        {
            ...
            if (mainWindow != nullptr)
            {
                if (!(mainWindow->viewport->flags & ViewportFlag::LAND_OWNERSHIP))
                {
                    mainWindow->viewport->flags |= ViewportFlag::LAND_OWNERSHIP;
                    mainWindow->invalidate();
                }
            }
        }
    ...
    void ShowConstructionRights()
    {
        if (gShowConstructionRightsRefCount == 0)
        {
            ...
            if (mainWindow != nullptr)
            {
                if (!(mainWindow->viewport->flags & ViewportFlag::CONSTRUCTION_RIGHTS))
                {
                    mainWindow->viewport->flags |= ViewportFlag::CONSTRUCTION_RIGHTS;
                    mainWindow->invalidate();
                }
            }
        }
        ...
        if (window != nullptr)
        {
            ...
            switch (mode)
            {
                ...
                { // Set all these flags to 0, and invalidate if any were active
                    uint32_t mask = ViewportFlag::UNDERGROUND_INSIDE | ViewportFlag::HIDE_RIDES | ViewportFlag::HIDE_SCENERY
                ...
                case ViewportVisibility::undergroundViewGhostOn: // 6CB7C4
                    ...
                    invalidate += !(vp->flags & ViewportFlag::UNDERGROUND_INSIDE);
                    vp->flags |= ViewportFlag::UNDERGROUND_INSIDE;
                    break;
                case ViewportVisibility::trackHeights: // 6CB7EB
                    // Set track heights on, invalidate if off
                    invalidate += !(vp->flags & ViewportFlag::TRACK_HEIGHTS);
                    vp->flags |= ViewportFlag::TRACK_HEIGHTS;
                    break;
                ...
                case ViewportVisibility::undergroundViewGhostOff: // 6CB7D8
                    ...
                    invalidate += vp->flags & ViewportFlag::UNDERGROUND_INSIDE;
                    vp->flags &= ~(static_cast<uint16_t>(ViewportFlag::UNDERGROUND_INSIDE));
                    break;
            }
            if (invalidate != 0)
                window->invalidate();
    ...
    VisibilityKind GetPaintStructVisibility(const PaintStruct* ps, uint32_t viewFlags)
    {
        ...
        auto cutAwayViewWithTransparency = (viewFlags & ViewportFlag::CLIP_VIEW)
            && (viewFlags & ViewportFlag::CLIP_VIEW_SEE_THROUGH);
        ...
        switch (ps->InteractionItem)
        {
            case ViewportInteractionItem::entity:
                if (ps->Entity != nullptr)
                {
                    switch (ps->Entity->type)
                    {
                        case EntityType::vehicle:
                        {
                            if (viewFlags & ViewportFlag::HIDE_VEHICLES || clipped)
                            {
                                return (viewFlags & ViewportFlag::INVISIBLE_VEHICLES) ? VisibilityKind::hidden
                                                                                      : VisibilityKind::partial;
                            }
                            // Rides without track can technically have a 'vehicle':
                            // these should be hidden if 'hide rides' is enabled
                            if (viewFlags & ViewportFlag::HIDE_RIDES || clipped)
                            {
                                ...
                                {
                                    return (viewFlags & ViewportFlag::INVISIBLE_RIDES) ? VisibilityKind::hidden
                                                                                       : VisibilityKind::partial;
                                }
                            }
                            break;
                        ...
                        case EntityType::guest:
                            if (viewFlags & ViewportFlag::HIDE_GUESTS)
                            {
                            ...
                            break;
                        case EntityType::staff:
                            if (viewFlags & ViewportFlag::HIDE_STAFF)
                            {
                                return VisibilityKind::hidden;
                            }
                            else if (clipped)
            ...
            case ViewportInteractionItem::ride:
                if (viewFlags & ViewportFlag::HIDE_RIDES || clipped)
                {
                    return (viewFlags & ViewportFlag::INVISIBLE_RIDES) ? VisibilityKind::hidden : VisibilityKind::partial;
                }
                break;
            case ViewportInteractionItem::footpath:
            case ViewportInteractionItem::pathAddition:
            case ViewportInteractionItem::banner:
                if (viewFlags & ViewportFlag::HIDE_PATHS || clipped)
                {
                    return (viewFlags & ViewportFlag::INVISIBLE_PATHS) ? VisibilityKind::hidden : VisibilityKind::partial;
                }
                break;
            ...
            case ViewportInteractionItem::wall:
                if (ps->Element != nullptr)
                {
                    ...
                    {
                        if (viewFlags & ViewportFlag::HIDE_VEGETATION || clipped)
                        {
                            return (viewFlags & ViewportFlag::INVISIBLE_VEGETATION) ? VisibilityKind::hidden
                                                                                    : VisibilityKind::partial;
                        }
                    }
                    else
                    {
                        if (viewFlags & ViewportFlag::HIDE_SCENERY || clipped)
                        {
                            return (viewFlags & ViewportFlag::INVISIBLE_SCENERY) ? VisibilityKind::hidden
                                                                                 : VisibilityKind::partial;
                        }
                    }
                }
                if (ps->InteractionItem == ViewportInteractionItem::wall
                    && (viewFlags & ViewportFlag::UNDERGROUND_INSIDE || clipped))
                {
                    return VisibilityKind::partial;
                }
                break;
