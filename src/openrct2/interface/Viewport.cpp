...
void ViewportUpdatePosition(WindowBase* window)
{
    ...
    if (mapCoord.x < MAP_MINIMUM_X_Y - MAP_Z_PARALLAX_OFFSET)
    {
        mapCoord.x = MAP_MINIMUM_X_Y - MAP_Z_PARALLAX_OFFSET;
        at_map_edge = 1;
    }
    if (mapCoord.y < MAP_MINIMUM_X_Y - MAP_Z_PARALLAX_OFFSET)
    {
        mapCoord.y = MAP_MINIMUM_X_Y - MAP_Z_PARALLAX_OFFSET;
        at_map_edge = 1;
    }

    // Clamp to the map maximum value (scenario specific)
    auto mapSizeMinus2 = GetMapSizeMinus2();
    if (mapCoord.x > mapSizeMinus2.x + MAP_Z_PARALLAX_OFFSET)
    {
        mapCoord.x = mapSizeMinus2.x + MAP_Z_PARALLAX_OFFSET;
        at_map_edge = 1;
    }
    if (mapCoord.y > mapSizeMinus2.y + MAP_Z_PARALLAX_OFFSET)
    {
        mapCoord.y = mapSizeMinus2.y + MAP_Z_PARALLAX_OFFSET;
        at_map_edge = 1;
    }
