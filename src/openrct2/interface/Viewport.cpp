...
namespace OpenRCT2
{
    ...
    static void ViewportRedrawAfterShift(
        DrawPixelInfo* dpi, WindowBase* window, Viewport* viewport, const ScreenCoordsXY& coords)
    {
        ...
        std::vector<RecordedPaintSession>* recorded_sessions)
    {
        ...
        const uint32_t bitmask = viewport->zoom >= ZoomLevel{ 0 } ? 0xFFFFFFFF & (viewport->zoom.ApplyTo(0xFFFFFFFF))
                                                                  : 0xFFFFFFFF;
