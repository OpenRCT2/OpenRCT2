 ...
 * @param class (cl)
 */
rct_window * window_create(sint32 x, sint32 y, sint32 width, sint32 height, rct_window_event_list * event_handlers,
                           rct_windowclass cls, uint16 flags)
{
 ...
 * @param class (cl)
 */
rct_window * window_create_auto_pos(sint32 width, sint32 height, rct_window_event_list * event_handlers, rct_windowclass cls,
                                    uint16 flags)
{
    uint16 screenWidth  = context_get_width();
    uint16 screenHeight = context_get_height();
...
rct_window * window_create_centred(sint32 width, sint32 height, rct_window_event_list * event_handlers, rct_windowclass cls,
                                   uint16 flags)
{
...
{
    rct_xyz16 location_3d = { .x = x, .y = y, .z = z };
...
void window_draw_widgets(rct_window * w, rct_drawpixelinfo * dpi)
{
    ...
    if (w->flags & WF_WHITE_BORDER_MASK)
    {
        gfx_fill_rect_inset(dpi, w->x, w->y, w->x + w->width - 1, w->y + w->height - 1, COLOUR_WHITE,
...
void tool_cancel()
{
    if (input_test_flag(INPUT_FLAG_TOOL_ACTIVE))
    {
        ...
        if (gCurrentToolWidget.widget_index != -1)
        {
            ...
            widget_invalidate_by_number(gCurrentToolWidget.window_classification, gCurrentToolWidget.window_number,
...
void window_start_textbox(rct_window * call_w, rct_widgetindex call_widget, rct_string_id existing_text, char * existing_args,
                          sint32 maxLength)
{
