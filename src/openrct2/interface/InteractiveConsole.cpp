...
#define SET_FLAG(variable, flag, value)                                                                                        \
        ...
        if (!argv.empty())
        {
            if (argv[0] == "list")
            {
                for (auto peep : EntityList<Staff>())
                {
                    ...
                    console.WriteFormatLine(
                        "staff id %03d type: %02u energy %03u name %s", peep->sprite_index, peep->AssignedStaffType,
    ...
    static int32_t ConsoleCommandGet(InteractiveConsole& console, const arguments_t& argv)
    {
        if (!argv.empty())
        {
            ...
            {
                console.WriteFormatLine(
                    "forbid_marketing_campaigns %d", (gParkFlags & PARK_FLAGS_FORBID_MARKETING_CAMPAIGN) != 0);
            }
            ...
            {
                console.WriteFormatLine(
                    "difficult_guest_generation %d", (gParkFlags & PARK_FLAGS_DIFFICULT_GUEST_GENERATION) != 0);
            }
            ...
            else if (argv[0] == "location")
            {
                ...
                if (w != nullptr)
                {
                    ...
                    auto info = GetMapCoordinatesFromPos(
                        ...
                        EnumsToFlags(ViewportInteractionItem::Terrain));
        ...
        if (argv.size() > 1)
        {
            ...
            else if (argv[0] == "guest_initial_happiness" && InvalidArguments(&invalidArgs, int_valid[0]))
            {
                auto scenarioSetSetting = ScenarioSetSettingAction(
                    ...
                    CalculateGuestInitialHappiness(static_cast<uint8_t>(int_val[0])));
                scenarioSetSetting.SetCallback([&console](const GameAction*, const GameActions::Result* res) {
                    if (res->Error != GameActions::Status::Ok)
            ...
            {
                auto scenarioSetSetting = ScenarioSetSettingAction(
                    ScenarioSetSetting::GuestsPreferLessIntenseRides, int_val[0]);
                scenarioSetSetting.SetCallback([&console](const GameAction*, const GameActions::Result* res) {
                    if (res->Error != GameActions::Status::Ok)
                        console.WriteLineError(
                            "set guest_prefer_less_intense_rides command failed, likely due to permissions.");
                    else
            ...
            {
                auto scenarioSetSetting = ScenarioSetSettingAction(
                    ScenarioSetSetting::GuestsPreferMoreIntenseRides, int_val[0]);
                scenarioSetSetting.SetCallback([&console](const GameAction*, const GameActions::Result* res) {
                    if (res->Error != GameActions::Status::Ok)
                        console.WriteLineError(
                            "set guest_prefer_more_intense_rides command failed, likely due to permissions.");
                    else
            ...
            {
                auto scenarioSetSetting = ScenarioSetSettingAction(
                    ScenarioSetSetting::ParkRatingHigherDifficultyLevel, int_val[0]);
                scenarioSetSetting.SetCallback([&console](const GameAction*, const GameActions::Result* res) {
                    if (res->Error != GameActions::Status::Ok)
    ...
    static int32_t ConsoleCommandForceDate(
        [[maybe_unused]] InteractiveConsole& console, [[maybe_unused]] const arguments_t& argv)
    {
        ...
        {
            day = std::clamp(
                gDateMonthTicks / (TICKS_PER_MONTH / days_in_month[month - 1]) + 1, 1,
                static_cast<int>(days_in_month[month - 1]));
        }
    ...
    static int32_t ConsoleCommandLoadPark(
        [[maybe_unused]] InteractiveConsole& console, [[maybe_unused]] const arguments_t& argv)
    {
        if (argv.size() < 1)
        {
    ...
    static int32_t ConsoleCommandSavePark(
        [[maybe_unused]] InteractiveConsole& console, [[maybe_unused]] const arguments_t& argv)
    {
        if (argv.size() < 1)
        {
    ...
    static int32_t ConsoleCommandReplayStart(InteractiveConsole& console, const arguments_t& argv)
    {
        ...
        if (replayManager->StartPlayback(name))
        {
            ...
            console.WriteFormatLine(
                logFmt, info.FilePath.c_str(), recordingDate, info.Ticks, info.NumCommands, info.NumChecksums);
...
#pragma warning(disable : 4702) // unreachable code
    ...
    static int32_t ConsoleCommandDereference(
        [[maybe_unused]] InteractiveConsole& console, [[maybe_unused]] const arguments_t& argv)
    {
...
#pragma GCC diagnostic pop
    ...
    static int32_t ConsoleCommandTerminate(
        [[maybe_unused]] InteractiveConsole& console, [[maybe_unused]] const arguments_t& argv)
    {
        std::terminate();
        return 0;
...
#pragma warning(pop)
    ...
    static int32_t ConsoleCommandAddNewsItem(
        [[maybe_unused]] InteractiveConsole& console, [[maybe_unused]] const arguments_t& argv)
    {
        if (argv.size() < 2)
        {
...
static constexpr const utf8* console_window_table[] = {
    ...
    static constexpr const ConsoleCommand console_command_table[] = {
        ...
        { "object_count", ConsoleCommandCountObjects, "Shows the number of objects of each type in the scenario.",
          "object_count" },
        { "open", ConsoleCommandOpen, "Opens the window with the give name.", "open <window>." },
        { "quit", ConsoleCommandClose, "Closes the console.", "quit" },
        { "remove_park_fences", ConsoleCommandRemoveParkFences, "Removes all park fences from the surface",
          "remove_park_fences" },
        { "remove_unused_objects", ConsoleCommandRemoveUnusedObjects,
          "Removes all the unused objects from the object selection.", "remove_unused_objects" },
        { "remove_floating_objects", ConsoleCommandRemoveFloatingObjects, "Removes floating objects",
          "remove_floating_objects" },
