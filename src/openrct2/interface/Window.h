...
#define TOP_TOOLBAR_HEIGHT 27
    ...
    struct Viewport
    {
        ...
        {
            return (
                vpos.y >= viewPos.y && vpos.y < viewPos.y + view_height && vpos.x >= viewPos.x
                && vpos.x < viewPos.x + view_width);
        }
...
#define WINDOW_LIMIT_RESERVED 4 // Used to reserve room for the main viewport, toolbars, etc.
    ...
    WindowBase* WindowCreate(
        const ScreenCoordsXY& pos, int32_t width, int32_t height, WindowEventList* event_handlers, WindowClass cls,
        uint32_t flags);
