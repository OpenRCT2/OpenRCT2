...
static void viewport_interaction_remove_park_wall(rct_map_element * mapElement, sint32 x, sint32 y)
{
    ...
    {
        gGameCommandErrorTitle = STR_CANT_REMOVE_THIS;
        game_do_command(x, GAME_COMMAND_FLAG_APPLY, y, map_element_get_direction(mapElement) | (mapElement->base_height << 8),
                        GAME_COMMAND_REMOVE_WALL, 0, 0);
    }
}
...
static void viewport_interaction_remove_large_scenery(rct_map_element * mapElement, sint32 x, sint32 y)
{
    ...
    {
        sint32 id = (mapElement->type & 0xC0) | ((mapElement->properties.scenerymultiple.colour[0] & 0xE0) >> 2) |
    ...
    {
        gGameCommandErrorTitle = STR_CANT_REMOVE_THIS;
        game_do_command(x, 1 | (map_element_get_direction(mapElement) << 8), y,
