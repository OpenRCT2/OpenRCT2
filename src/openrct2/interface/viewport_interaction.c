...
int viewport_interaction_get_item_left(int x, int y, viewport_interaction_info * info)
{
    ...
    rct_xy16 mapCoord = { 0 };
    get_map_coordinates_from_pos(x, y, VIEWPORT_INTERACTION_MASK_SPRITE & VIEWPORT_INTERACTION_MASK_RIDE &
                                           VIEWPORT_INTERACTION_MASK_PARK,
                                 &mapCoord.x, &mapCoord.y, &info->type, &info->mapElement, NULL);
    info->x    = mapCoord.x;
    info->y    = mapCoord.y;
        ...
        case SPRITE_IDENTIFIER_MISC:
            if (game_is_not_paused())
            {
                ...
                case SPRITE_MISC_BALLOON:
                    game_do_command(info.sprite->balloon.sprite_index, GAME_COMMAND_FLAG_APPLY, 0, 0,
...
int viewport_interaction_get_item_right(int x, int y, viewport_interaction_info * info)
{
    ...
    rct_xy16 mapCoord = { 0 };
    get_map_coordinates_from_pos(x, y, ~(VIEWPORT_INTERACTION_MASK_TERRAIN & VIEWPORT_INTERACTION_MASK_WATER), &mapCoord.x,
                                 &mapCoord.y, &info->type, &info->mapElement, NULL);
    info->x    = mapCoord.x;
    info->y    = mapCoord.y;
    mapElement = info->mapElement;
...
static void viewport_interaction_remove_scenery(rct_map_element * mapElement, int x, int y)
{
    ...
    game_do_command(x, (mapElement->type << 8) | 1, y, (mapElement->properties.scenery.type << 8) | mapElement->base_height,
...
static void viewport_interaction_remove_footpath_item(rct_map_element * mapElement, int x, int y)
{
    ...
    gGameCommandErrorTitle = STR_CANT_REMOVE_THIS;
    game_do_command(x, ((mapElement->properties.path.type & 7) << 8) | 1, y, (type << 8) | mapElement->base_height,
...
static void viewport_interaction_remove_park_wall(rct_map_element * mapElement, int x, int y)
{
    ...
    {
        gGameCommandErrorTitle = STR_CANT_REMOVE_THIS;
        game_do_command(x, GAME_COMMAND_FLAG_APPLY, y, (mapElement->type & 0x3) | (mapElement->base_height << 8),
...
static void viewport_interaction_remove_large_scenery(rct_map_element * mapElement, int x, int y)
{
    ...
    {
        int id = (mapElement->type & 0xC0) | ((mapElement->properties.scenerymultiple.colour[0] & 0xE0) >> 2) |
    ...
    {
        gGameCommandErrorTitle = STR_CANT_REMOVE_THIS;
        game_do_command(x, 1 | ((mapElement->type & 0x3) << 8), y,
...
void sub_68A15E(int screenX, int screenY, short * x, short * y, int * direction, rct_map_element ** mapElement)
{
    ...
    rct_viewport *    viewport;
    get_map_coordinates_from_pos(screenX, screenY, VIEWPORT_INTERACTION_MASK_TERRAIN & VIEWPORT_INTERACTION_MASK_WATER, &my_x,
                                 &my_y, &interactionType, &myMapElement, &viewport);
