...
#    define BACKTRACE_TOKEN L"9d89dffbeb1c81deb554c848513c0fc57a3b91a35271ce44639b6748cf7b76f3"
    ...
    static bool UploadMinidump(const std::map<std::wstring, std::wstring>& files, int& error, std::wstring& response)
    {
        ...
        int timeout = 10000;
        bool success = google_breakpad::HTTPUpload::SendMultipartPostRequest(
            url, parameters, files, &timeout, &response, &error);
        ...
        MDRawAssertionInfo* assertion, bool succeeded)
    {
        if (!succeeded)
        {
            constexpr const wchar_t* DumpFailedMessage = L"Failed to create the dump. Please file an issue with OpenRCT2 on "
                                                         L"GitHub "
        ...
        // By the time we reach this point, OpenGL context is already lost causing *any* call to gl* to stall or fail in
        // unexpected way. Implementing a proof of concept with glGetGraphicsResetStatus in
        // https://github.com/OpenRCT2/OpenRCT2/commit/3974594fc36e24d14549921d378251242e3a23e2 yielded no additional
        // information, while potentially significantly raising the required OpenGL version. There are (at least) two ways out
        // of this:
        ...
        constexpr const wchar_t* MessageFormat = L"A crash has occurred and a dump was created at\n%s.\n\nPlease file an issue "
                                                 ...
                                                 L"We would like to upload the crash dump for automated analysis, do you "
                                                 L"agree?\n"
...
namespace OpenRCT2
{
    CExceptionHandler CrashInit()
    {
        ...
        auto exHandler = new google_breakpad::ExceptionHandler(
            GetDumpDirectory(), 0, OnCrash, 0, google_breakpad::ExceptionHandler::HANDLER_ALL, MiniDumpWithDataSegs, PipeName,
            0);
