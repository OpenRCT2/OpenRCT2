...
#    define BACKTRACE_TOKEN "880330550e07896abb0de50eca685eec04e0383e57393f315b977bf88688fcf2"
    ...
    MDRawAssertionInfo* assertion, bool succeeded)
{
    ...
    wprintf(L"Dump Id: %s\n", miniDumpId);
    wprintf(L"Version: %s\n", WSZ(std::wstring(OPENRCT2_VERSION, OPENRCT2_VERSION + strlen(OPENRCT2_VERSION)));
    wprintf(L"Commit: %s\n", _wszCommitSha1Short);
    ...
    swprintf_s(message, MessageFormat, dumpFilePath, WSZ(std::wstring(OPENRCT2_VERSION, OPENRCT2_VERSION + strlen(OPENRCT2_VERSION))), _wszCommitSha1Short);

    // Cannot use platform_show_messagebox here, it tries to set parent window already dead.
    int answer = MessageBoxW(nullptr, message, WSZ(std::wstring(OPENRCT2_NAME, OPENRCT2_NAME + strlen(OPENRCT2_NAME))), MB_YESNO | MB_ICONERROR);
    if (answer == IDYES)
    {
        int error;
        std::wstring response;
        ...
        if (!ok)
        {
            ...
            MessageBoxW(
                nullptr, message, WSZ(std::wstring(OPENRCT2_NAME, OPENRCT2_NAME + strlen(OPENRCT2_NAME))),
                MB_OK | MB_ICONERROR);
        }
        else
        {
            MessageBoxW(
                ...
                WSZ(std::wstring(OPENRCT2_NAME, OPENRCT2_NAME + strlen(OPENRCT2_NAME))), MB_OK | MB_ICONINFORMATION);
        }
    }
