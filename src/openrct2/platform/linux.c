...
void platform_posix_sub_resolve_openrct_data_path(utf8 * out, size_t size)
{
    static const utf8 * searchLocations[] = {
        ...
        "/usr/local/share/openrct2", "/var/lib/openrct2", "/usr/share/openrct2",
    };
...
bool platform_open_common_file_dialog(utf8 * outFilename, file_dialog_desc * desc, size_t outSize)
{
    ...
    case DT_ZENITY:
        ...
        for (int j = 0; j < countof(desc->filters); j++)
        {
            if (desc->filters[j].pattern && desc->filters[j].name)
            {
                ...
                char filterTemp[100] = { 0 };
                snprintf(filterTemp, countof(filterTemp), " --file-filter=\"%s | %s\"", desc->filters[j].name,
                         filterPatternRegex);
                safe_strcat(filter, filterTemp, countof(filter));
            }
        }
    ...
    else if (desc->type == FD_SAVE && access(result, F_OK) != -1 && dtype == DT_KDIALOG)
    {
