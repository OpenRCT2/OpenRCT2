...
{
    const auto& gameState = getGameState();
    return !FinanceCheckMoneyRequired(flags) || cost <= 0 || cost <= gameState.parks[0].cash;
}
...
void FinancePayRideUpkeep()
{
    ...
    for (auto& ride : RideManager(gameState))
    {
        ...
        if (ride.status != RideStatus::closed && !(gameState.parks[0].flags & PARK_FLAGS_NO_MONEY))
        {
...
{
    auto& gameState = getGameState();
    gameState.parks[0].cash = gameState.scenarioOptions.initialCash;
}
...
money64 FinanceGetLastMonthShopProfit()
{
    ...
    {
        const auto& gameState = getGameState();
        const auto* lastMonthExpenditure = gameState.parks[0].expenditureTable[1];
