...
static bool award_is_deserved_most_untidy(int32_t activeAwardTypes)
{
    ...
    for (auto peep : EntityList<Guest>(EntityListId::Peep))
    {
        ...
        if (peep->Thoughts[0].type == PeepThoughtType::PEEP_THOUGHT_TYPE_BAD_LITTER
            || peep->Thoughts[0].type == PeepThoughtType::PEEP_THOUGHT_TYPE_PATH_DISGUSTING
            || peep->Thoughts[0].type == PeepThoughtType::PEEP_THOUGHT_TYPE_VANDALISM)
        {
            negativeCount++;
        }
    }
...
static bool award_is_deserved_most_tidy(int32_t activeAwardTypes)
{
    ...
    for (auto peep : EntityList<Guest>(EntityListId::Peep))
    {
        ...
        if (peep->Thoughts[0].type == PeepThoughtType::PEEP_THOUGHT_TYPE_VERY_CLEAN)
            positiveCount++;
...
static bool award_is_deserved_most_beautiful(int32_t activeAwardTypes)
{
    ...
    for (auto peep : list)
    {
        ...
        if (peep->Thoughts[0].type == PeepThoughtType::PEEP_THOUGHT_TYPE_SCENERY)
            positiveCount++;
...
static bool award_is_deserved_safest([[maybe_unused]] int32_t activeAwardTypes)
{
    ...
    for (auto peep : EntityList<Guest>(EntityListId::Peep))
    {
        if (peep->OutsideOfPark)
            continue;
        if (peep->Thoughts[0].freshness <= 5 && peep->Thoughts[0].type == PeepThoughtType::PEEP_THOUGHT_TYPE_VANDALISM)
...
static bool award_is_deserved_best_food(int32_t activeAwardTypes)
{
    ...
    for (auto peep : EntityList<Guest>(EntityListId::Peep))
    {
        ...
        if (peep->Thoughts[0].freshness <= 5 && peep->Thoughts[0].type == PeepThoughtType::PEEP_THOUGHT_TYPE_HUNGRY)
            hungryPeeps++;
    }
...
static bool award_is_deserved_best_restrooms([[maybe_unused]] int32_t activeAwardTypes)
{
    ...
    for (auto peep : EntityList<Guest>(EntityListId::Peep))
    {
        ...
        if (peep->Thoughts[0].freshness <= 5 && peep->Thoughts[0].type == PeepThoughtType::PEEP_THOUGHT_TYPE_TOILET)
...
static bool award_is_deserved_most_confusing_layout([[maybe_unused]] int32_t activeAwardTypes)
{
    ...
    for (auto peep : EntityList<Guest>(EntityListId::Peep))
    {
        ...
        peepsCounted++;
        if (peep->Thoughts[0].freshness <= 5
            && (peep->Thoughts[0].type == PeepThoughtType::PEEP_THOUGHT_TYPE_LOST
                || peep->Thoughts[0].type == PeepThoughtType::PEEP_THOUGHT_TYPE_CANT_FIND))
            peepsLost++;
    }
