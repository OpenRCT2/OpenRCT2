...
Guest* Peep::AsGuest()
{
    return AssignedPeepType == Crowd::Type::Guest ? static_cast<Guest*>(this) : nullptr;
}

Staff* Peep::AsStaff()
{
    return AssignedPeepType == Crowd::Type::Staff ? static_cast<Staff*>(this) : nullptr;
}
...
std::unique_ptr<GameActionResult> Peep::Place(const TileCoordsXYZ& location, bool apply)
{
    ...
    if (apply)
    {
        ...
        if (AssignedPeepType == Crowd::Type::Guest)
        {
...
void peep_sprite_remove(Peep* peep)
{
    ...
    bool wasGuest = peep->AssignedPeepType == Crowd::Type::Guest;
    if (peep->AssignedPeepType == Crowd::Type::Guest)
    {
    ...
    else
    {
        ...
        peep->AssignedPeepType = Crowd::Type::Invalid;
        staff_update_greyed_patrol_areas();
        peep->AssignedPeepType = Crowd::Type::Staff;
...
Peep* Peep::Generate(const CoordsXYZ& coords)
{
    ...
    peep->AssignedPeepType = Crowd::Type::Guest;
...
void Peep::FormatNameTo(Formatter& ft) const
{
    if (Name == nullptr)
    {
        if (AssignedPeepType == Crowd::Type::Staff)
        {
...
void Peep::PerformNextAction(uint8_t& pathing_result, TileElement*& tile_result)
{
    ...
    if (AssignedPeepType == Crowd::Type::Staff || (GetNextIsSurface()))
    {
        int16_t height = abs(tile_element_height(newLoc) - z);
        if (height <= 3 || (AssignedPeepType == Crowd::Type::Staff && height <= 32))
        {
