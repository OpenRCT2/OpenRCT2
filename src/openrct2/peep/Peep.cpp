            ...
            else if (tile_element->GetType() == TILE_ELEMENT_TYPE_SURFACE)
            {
                ...
                if (tile_element->AsSurface()->GetWaterHeight() > 0)
                {
                    ...
                    if (height - 4 >= z && height < z + 20)
                    {
                        ...
                        InsertNewThought(PeepThoughtType::PEEP_THOUGHT_TYPE_DROWNING, PEEP_THOUGHT_ITEM_NONE);
...
void Peep::UpdatePicked()
{
    ...
    if (SubState == 13)
    {
        InsertNewThought(PeepThoughtType::PEEP_THOUGHT_TYPE_HELP, PEEP_THOUGHT_ITEM_NONE);
    }
}
...
static void peep_update_thoughts(Peep* peep)
{
    ...
    {
        if (peep->Thoughts[i].type == PeepThoughtType::PEEP_THOUGHT_TYPE_NONE)
            break;
        ...
        else if (peep->Thoughts[i].freshness > 1)
        {
            if (++peep->Thoughts[i].fresh_timeout == 0)
            {
                ...
                if (++peep->Thoughts[i].freshness >= 28)
                {
                    ...
                    peep->Thoughts[PEEP_MAX_THOUGHTS - 1].type = PeepThoughtType::PEEP_THOUGHT_TYPE_NONE;
                }
            }
        }
        else
...
void peep_problem_warnings_update()
{
    ...
    for (auto peep : EntityList<Guest>(EntityListId::Peep))
    {
        ...
        {
            case PeepThoughtType::PEEP_THOUGHT_TYPE_LOST: // 0x10
                lost_counter++;
                break;

            case PeepThoughtType::PEEP_THOUGHT_TYPE_HUNGRY: // 0x14
            ...
            case PeepThoughtType::PEEP_THOUGHT_TYPE_BAD_LITTER: // 0x1a
                litter_counter++;
                break;
            case PeepThoughtType::PEEP_THOUGHT_TYPE_CANT_FIND_EXIT: // 0x1b
                noexit_counter++;
                break;
            case PeepThoughtType::PEEP_THOUGHT_TYPE_PATH_DISGUSTING: // 0x1f
                disgust_counter++;
                break;
            case PeepThoughtType::PEEP_THOUGHT_TYPE_VANDALISM: // 0x21
                vandalism_counter++;
                break;
            default:
                break;
...
Peep* Peep::Generate(const CoordsXYZ& coords)
{
    ...
    peep->Thoughts->type = PeepThoughtType::PEEP_THOUGHT_TYPE_NONE;
...
static void peep_footpath_move_forward(Peep* peep, const CoordsXYE& coords, bool vandalism)
{
    ...
    if (vandalism)
    {
        ...
        if (vandalisedTiles & 0x3E && (vandalThoughtTimeout == 0))
        {
            ...
            {
                peep->InsertNewThought(PeepThoughtType::PEEP_THOUGHT_TYPE_VANDALISM, PEEP_THOUGHT_ITEM_NONE);
    ...
    {
        peep->InsertNewThought(PeepThoughtType::PEEP_THOUGHT_TYPE_CROWDED, PEEP_THOUGHT_ITEM_NONE);
    ...
    else
    {
        ...
        {
            peep->InsertNewThought(PeepThoughtType::PEEP_THOUGHT_TYPE_PATH_DISGUSTING, PEEP_THOUGHT_ITEM_NONE);
            peep->HappinessTarget = std::max(0, peep->HappinessTarget - 17);
    ...
    else
    {
        ...
        {
            peep->InsertNewThought(PeepThoughtType::PEEP_THOUGHT_TYPE_BAD_LITTER, PEEP_THOUGHT_ITEM_NONE);
            peep->HappinessTarget = std::max(0, peep->HappinessTarget - 17);
            // Reset litter time
