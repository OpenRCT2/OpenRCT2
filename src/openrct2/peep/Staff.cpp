...
int32_t Staff::HandymanDirectionRandSurface(uint8_t validDirections)
{
    ...
    // to make it back to the first SomePeepDirection and
...
bool Staff::DoHandymanPathFinding()
{
    ...
    if (newDirection == INVALID_DIRECTION)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    pathDirections &= ~(1 << direction_reverse(SomePeepDirection));
                    if (pathDirections == 0)
                    {
                        pathDirections |= 1 << direction_reverse(SomePeepDirection);
                    }
    ...
    SomePeepDirection = newDirection;
...
static uint8_t staff_direction_surface(Peep* peep, uint8_t initialDirection)
{
    ...
    {
        // Looks left and right from initial SomePeepDirection
        switch (i)
        {
            case 1:
                direction++;
...
static uint8_t staff_mechanic_direction_path_rand(Peep* peep, uint8_t pathDirections)
{
    if (scenario_rand() & 1)
    {
        if (pathDirections & (1 << peep->SomePeepDirection))
            return peep->SomePeepDirection;
    }
...
static uint8_t staff_mechanic_direction_path(Peep* peep, uint8_t validDirections, PathElement* pathElement)
{
    ...
    // Check if this is dead end - i.e. only way out is the reverse SomePeepDirection.
    pathDirections &= ~(1 << direction_reverse(peep->SomePeepDirection));
    if (pathDirections == 0)
    {
        pathDirections |= (1 << direction_reverse(peep->SomePeepDirection));
    }
...
void Staff::UpdateHeadingToInspect()
{
    ...
    if (SubState <= 3)
    {
        ...
        SomePeepDirection = rideEntranceExitElement->GetDirection();

        int32_t destX = NextLoc.x + 16 + DirectionOffsets[SomePeepDirection].x * 53;
        int32_t destY = NextLoc.y + 16 + DirectionOffsets[SomePeepDirection].y * 53;
...
bool Staff::UpdateFixingMoveToBrokenDownVehicle(bool firstRun, Ride* ride)
{
    if (!firstRun)
    {
        ...
        CoordsXY offset = DirectionOffsets[SomePeepDirection];
...
bool Staff::UpdateFixingMoveToStationExit(bool firstRun, Ride* ride)
{
    if (!firstRun)
    {
        ...
        CoordsXY stationPlatformDirection = DirectionOffsets[SomePeepDirection];
...
bool Staff::UpdateFixingLeaveByEntranceExit(bool firstRun, Ride* ride)
{
    if (!firstRun)
    {
        ...
        CoordsXY ebx_direction = DirectionOffsets[SomePeepDirection];
