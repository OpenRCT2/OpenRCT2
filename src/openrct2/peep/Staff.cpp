...
{
    auto peep = As<Peep>();
    return peep && peep->AssignedPeepType == PeepE::Type::Staff;
}
...
bool staff_can_ignore_wide_flag(Peep* staff, const CoordsXYZ& staffPos, TileElement* path)
{
    ...
    if (staff->AssignedPeepType != PeepE::Type::Staff)
        return false;
...
{
    return (
        sprite_identifier == SPRITE_IDENTIFIER_PEEP && AssignedPeepType == PeepE::Type::Staff && StaffType == STAFF_TYPE_MECHANIC);
}
