...
static void peep_update_ride_sub_state_2_rejoin_queue(rct_peep * peep, rct_ride * ride)
{
    ...
    rct_peep * queue_peep;
    for (queue_peep = GET_PEEP(current_last); queue_peep->next_in_queue != SPRITE_INDEX_NULL;
...
static void peep_pathfind_heuristic_search(sint16 x, sint16 y, uint8 z, rct_peep * peep, rct_map_element * currentMapElement,
                                           bool inPatrolArea, uint8 counter, uint16 * endScore, int test_edge,
 ...
 * Continue to the next map element without updating the parameters (best result so far). */
        ...
        do
        {
            ...
            peep_pathfind_heuristic_search(x, y, height, peep, mapElement, nextInPatrolArea, counter, endScore, test_edge,
