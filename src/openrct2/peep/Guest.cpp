...
void Guest::Tick128UpdateGuest(int32_t index)
{
    if (static_cast<uint32_t>(index & 0x1FF) == (gCurrentTicks & 0x1FF))
    {
        ...
        if (State == PEEP_STATE_WALKING && OutsideOfPark == 0 && !(peep_flags & PEEP_FLAGS_LEAVING_PARK) && NoOfRides == 0
        ...
        if (static_cast<uint32_t>(index & 0x3FF) == (gCurrentTicks & 0x3FF))
        {
            ...
            if (OutsideOfPark == 0 && (State == PEEP_STATE_WALKING || State == PEEP_STATE_SITTING))
            {
                ...
                else
                {
                    ...
                    if (!(gParkFlags & PARK_FLAGS_NO_MONEY) && CashInPocket <= MONEY(9, 00) && Happiness >= 105
                        && Energy >= 70)
                    {
...
void Guest::UpdateBuying()
{
    ...
    if (SubState == 1)
    {
        ...
        SomePeepDirection = direction_reverse(SomePeepDirection);
...
void Guest::UpdateRideLeaveExit()
{
    ...
    if (auto loc = UpdateAction())
    {
        if (ride != nullptr)
        {
            MoveTo((*loc).x, (*loc).y, ride->stations[CurrentRideStation].GetBaseZ());
        }
        return;
    }
