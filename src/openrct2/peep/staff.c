...
static money32 staff_hire_new_staff_member(uint8 staff_type, uint8 flags, sint16 command_x, sint16 command_y, sint16 command_z,
                                           int autoposition, int * newPeep_sprite_index)
{
    ...
    if (staff_type != STAFF_TYPE_HANDYMAN && staff_type != STAFF_TYPE_MECHANIC && staff_type != STAFF_TYPE_SECURITY &&
    ...
    else
    {
        ...
        static const rct_string_id staffNames[] = {
            STR_HANDYMAN_X, STR_MECHANIC_X, STR_SECURITY_GUARD_X, STR_ENTERTAINER_X,
        };

        /* rct2: 0x009929FC */
        static const uint8 spriteTypes[] = {
            PEEP_SPRITE_TYPE_HANDYMAN, PEEP_SPRITE_TYPE_MECHANIC, PEEP_SPRITE_TYPE_SECURITY, PEEP_SPRITE_TYPE_ENTERTAINER_PANDA,
        };
    ...
    if ((staff_type == STAFF_TYPE_HANDYMAN) && gConfigGeneral.handymen_mow_default)
    {
        ...
        game_do_command(newPeep->x, ((int)new_orders << 8) | 1, newPeep->y, newPeep->sprite_index, GAME_COMMAND_SET_STAFF_ORDER,
                        0, 0);
    }
...
uint16 hire_new_staff_member(uint8 staffType)
{
    ...
    int result = game_do_command_p(GAME_COMMAND_HIRE_NEW_STAFF_MEMBER, &command_x, &ebx, &command_y, &command_z, &esi,
...
static uint8 staff_handyman_direction_to_nearest_litter(rct_peep * peep)
{
    ...
    rct_xy16 litterTile = {.x = nearestLitter->x & 0xFFE0, .y = nearestLitter->y & 0xFFE0 };
    ...
    rct_xy16 nextTile = {.x = (nearestLitter->x & 0xFFE0) - TileDirectionDelta[nextDirection].x,
...
static int staff_handyman_direction_rand_surface(rct_peep * peep, uint8 validDirections)
{
    ...
    for (int i = 0; i < 4; ++i, ++direction)
    {
        ...
        rct_xy16 chosenTile = {
            .x = peep->next_x + TileDirectionDelta[direction].x, .y = peep->next_y + TileDirectionDelta[direction].y,
        };
...
static int staff_path_finding_handyman(rct_peep * peep)
{
    ...
    rct_xy16 chosenTile = {.x = peep->next_x + TileDirectionDelta[direction].x,
...
static uint8 staff_mechanic_direction_surface(rct_peep * peep)
{
    ...
    if ((peep->state == PEEP_STATE_ANSWERING || peep->state == PEEP_STATE_HEADING_TO_INSPECTION) && scenario_rand() & 1)
    {
        ...
        rct_xy16 chosenTile = {.x = (location & 0xFF) * 32, .y = (location >> 8) * 32 };
...
void game_command_set_staff_name(int * eax, int * ebx, int * ecx, int * edx, int * esi, int * edi, int * ebp)
{
    ...
    *ebx = set_peep_name(*ebx & 0xFF, *eax & 0xFFFF, sprite_index, (uint8 *)edx, (uint8 *)ebp, (uint8 *)edi);
}
