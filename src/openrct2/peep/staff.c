...
uint32 gStaffPatrolAreas[RCT2_MAX_STAFF * RCT12_PATROL_AREA_SIZE];
uint8 gStaffModes[RCT2_MAX_STAFF];
...
static money32 staff_hire_new_staff_member(uint8 staff_type, uint8 flags, sint16 command_x, sint16 command_y, sint16 command_z, sint32 autoposition, sint32 *newPeep_sprite_index)
{
    ...
    if (flags & GAME_COMMAND_FLAG_APPLY)
    {
        ...
        if (flags == 0) {
            ...
            for (i = 0; i < RCT12_PATROL_AREA_SIZE; i++) {
                gStaffPatrolAreas[newStaffId * RCT12_PATROL_AREA_SIZE + i] = 0;
            }
        }
...
void game_command_set_staff_patrol(sint32 *eax, sint32 *ebx, sint32 *ecx, sint32 *edx, sint32 *esi, sint32 *edi, sint32 *ebp)
{
    if(*ebx & GAME_COMMAND_FLAG_APPLY){
        ...
        sint32 patrolOffset = peep->staff_id * RCT12_PATROL_AREA_SIZE;
...
void staff_update_greyed_patrol_areas()
{
    ...
    {
        sint32 staffPatrolOffset = (staff_type + STAFF_MAX_COUNT) * RCT12_PATROL_AREA_SIZE;
        ...
        for (uint16 sprite_index = gSpriteListHead[SPRITE_LIST_PEEP]; sprite_index != SPRITE_INDEX_NULL; sprite_index = peep->next)
        {
            ...
            {
                sint32 peepPatrolOffset = peep->staff_id * RCT12_PATROL_AREA_SIZE;
                for (sint32 i = 0; i < RCT12_PATROL_AREA_SIZE; i++) {
...
bool staff_is_patrol_area_set(sint32 staffIndex, sint32 x, sint32 y)
{
    ...
    sint32 peepOffset = staffIndex * RCT12_PATROL_AREA_SIZE;
    sint32 offset = (x | y) >> 5;
