    ...
    TileCoordsXYZ* endXYZ, uint8_t* endSteps)
{
    ...
    if (peep->AssignedPeepType == PeepE::Type::Staff && peep->StaffType == STAFF_TYPE_MECHANIC)
    {
...
Direction peep_pathfind_choose_direction(const TileCoordsXYZ& loc, Peep* peep)
{
    ...
    int32_t maxTilesChecked = (peep->AssignedPeepType == PeepE::Type::Staff) ? 50000 : 15000;
    // Used to allow walking through no entry banners
    _peepPathFindIsStaff = (peep->AssignedPeepType == PeepE::Type::Staff);
