...
const rct_xy16 dword_97F020[] = { { 32, 0 }, { 0, 0 }, { 0, 32 }, { 32, 32 }, { 32, 32 }, { 32, 0 }, { 0, 0 }, { 0, 32 } };
...
void jumping_fountain_begin(int type, int x, int y, rct_map_element * mapElement)
{
    ...
    case PATTERN_CYCLIC_SQUARES:
        ...
        for (i = 0; i < 4; i++)
        {
            jumping_fountain_create(type, x + dword_97F020[i].x, y + dword_97F020[i].y, z, _fountainDirections[i],
    ...
    case PATTERN_RACING_PAIRS:
        ...
        jumping_fountain_create(type, x + dword_97F020[randomIndex].x, y + dword_97F020[randomIndex].y, z,
...
static void jumping_fountain_continue(rct_jumping_fountain * jumpingFountain)
{
    ...
    int type = jumpingFountain->misc_identifier == SPRITE_MISC_JUMPING_FOUNTAIN_SNOW ? JUMPING_FOUNTAIN_TYPE_SNOW
...
static void jumping_fountain_split(rct_jumping_fountain * jumpingFountain, int x, int y, int z, int availableDirections)
{
    ...
    for (direction = 0; direction < 8; direction++)
    {
        if (availableDirections & (1 << direction))
        {
            jumping_fountain_create(type, x, y, z, direction >> 1, jumpingFountain->fountain_flags & ~FOUNTAIN_FLAG_7,
