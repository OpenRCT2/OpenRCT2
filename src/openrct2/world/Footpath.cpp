...
namespace OpenRCT2
{
    ...
    {
        auto& objMgr = OpenRCT2::GetContext()->GetObjectManager();
        return static_cast<FootpathRailingsObject*>(
            objMgr.GetLoadedObject(ObjectType::FootpathRailings, GetRailingsEntryIndex()));
    }
    ...
    {
        auto pathElement = MapGetFirstTileElementWithBaseHeightBetween<PathElement>(
            { position, position.z + PATH_HEIGHT_STEP });
        ...
        for (uint8_t direction = 0; direction < NumOrthogonalDirections; direction++)
        {
            ...
            if (!TileElementWantsPathConnectionTowards({ TileCoordsXY{ footpathPos }, z1, direction }, tileElement))
            {
                ...
                FootpathRemoveEdgesTowards(
                    { footpathPos + CoordsDirectionDelta[direction], z0 * COORDS_Z_STEP, z1 * COORDS_Z_STEP }, direction,
                    isQueue);
            }
            else
            {
