 ...
 * then return the location of the ground tile. Besides the location it also computes the PeepDirection of the yellow arrow
...
/**
 * Returns true if the edge of tile x, y specified by PeepDirection is occupied by a fence
 ...
 * The function will search for a path in the PeepDirection given, then check clockwise to see if it there is a path and
...
// TODO: Change this into a simple check that validates if the PeepDirection should be fully checked with loc_6A6D7E and
    ...
    int32_t direction)
{
    ...
    if (rideIndex != RIDE_ID_NULL && lastPathElement != nullptr)
    {
        if (lastPathElement->AsPath()->IsQueue())
        {
            ...
            lastPathElement->AsPath()->SetQueueBannerDirection(lastPathDirection); // set the ride sign PeepDirection
...
searchFromFootpath:
    // Exclude PeepDirection we came from
    targetPos.z = tileElement->GetBaseZ();
    edges &= ~(1 << direction);

    // Find next PeepDirection to go
    ...
    if (edges == 0)
    {
        // Only possible PeepDirection to go
        if (tileElement->AsPath()->IsSloped() && tileElement->AsPath()->GetSlopeDirection() == direction)
        {
...
// PeepDirection (ride entrances and exits, shops, paths).
    ...
    do
    {
        ...
        switch (tileElement->GetType())
        {
            case TILE_ELEMENT_TYPE_PATH:
                ...
                {
                    if (!tileElement->AsPath()->IsSloped())
                        // The footpath is flat, it can be connected to from any PeepDirection
                        return true;
            ...
            case TILE_ELEMENT_TYPE_TRACK:
                if (tileElement->base_height == coords.z)
                {
                    ...
                    if (FlatRideTrackSequenceProperties[trackType][trackSequence] & TRACK_SEQUENCE_FLAG_CONNECTS_TO_PATH)
                    {
                        ...
                        if (FlatRideTrackSequenceProperties[trackType][trackSequence] & (1 << dx))
                        {
                            // Track element has the flags required for the given PeepDirection
                            return true;
                        }
                    }
                }
            ...
            case TILE_ELEMENT_TYPE_ENTRANCE:
                if (tileElement->base_height == coords.z)
                {
                    ...
                    {
                        // Entrance wants to be connected towards the given PeepDirection
                        return true;
                    }
                }
                break;
