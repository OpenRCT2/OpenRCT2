...
static const rct_xy16 _moneyEffectMoveOffset[] = { { 1, -1 }, { 1, 1 }, { -1, 1 }, { -1, -1 } };
...
{
    rct_xyz16 mapPosition = {.x = gCommandPosition.x, .y = gCommandPosition.y, .z = gCommandPosition.z };
    ...
    if (mapPosition.x == MAP_LOCATION_NULL)
    {
        ...
        screen_get_map_xy(mainViewport->x + (mainViewport->width / 2), mainViewport->y + (mainViewport->height / 2),
                          &mapPosition.x, &mapPosition.y, NULL);
        if (mapPosition.x == MAP_LOCATION_NULL)
            return;
