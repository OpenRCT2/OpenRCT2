...
sint32 map_element_iterator_next(map_element_iterator *it)
{
	...
	if (it->x < (MAX_MAP_EDGE_SIZE - 1)) {
		it->x++;
		it->element = map_get_first_element_at(it->x, it->y);
		return 1;
	}

	if (it->y < (MAX_MAP_EDGE_SIZE - 1)) {
		it->x = 0;
		it->y++;
		it->element = map_get_first_element_at(it->x, it->y);
		return 1;
...
{
	if (x < 0 || y < 0 || x > (MAX_MAP_EDGE_SIZE - 1) || y > (MAX_MAP_EDGE_SIZE - 1)) {
		log_error("Trying to access element outside of range");
		return NULL;
	}
	return gMapElementTilePointers[x + y * MAX_MAP_EDGE_SIZE];
}
...
void map_set_tile_elements(sint32 x, sint32 y, rct_map_element *elements)
{
	...
	gMapElementTilePointers[x + y * MAX_MAP_EDGE_SIZE] = elements;
}
...
void map_count_remaining_land_rights()
{
	...
	for (sint32 x = 0; x < MAX_MAP_EDGE_SIZE; x++) {
		for (sint32 y = 0; y < MAX_MAP_EDGE_SIZE; y++) {
...
void map_update_tile_pointers()
{
	...
	for (y = 0; y < MAX_MAP_EDGE_SIZE; y++) {
		for (x = 0; x < MAX_MAP_EDGE_SIZE; x++) {
			*tile++ = mapElement;
			while (!map_element_is_last_for_tile(mapElement++));
		}
	}
...
{
	if (x < (MAX_MAP_EDGE_SIZE * 32) && x >= 0 && y < (MAX_MAP_EDGE_SIZE * 32) && y >= 0) {
		return true;
	}
	return false;
}
...
static money32 smooth_land(sint32 flags, sint32 centreX, sint32 centreY, sint32 mapLeft, sint32 mapTop, sint32 mapRight, sint32 mapBottom, sint32 command)
{
	...
	mapRight = clamp(0, mapRight, (MAX_MAP_EDGE_SIZE - 1) * 32);
	mapBottom = clamp(0, mapBottom, (MAX_MAP_EDGE_SIZE - 1) * 32);
	...
	for (; size <= MAX_MAP_EDGE_SIZE; size += 2) {
		initialMinZ += 2;
...
{
	return x < 32 || y < 32 || x >= ((MAX_MAP_EDGE_SIZE - 1) * 32) || y >= ((MAX_MAP_EDGE_SIZE - 1) * 32);
}
...
void map_reorganise_elements()
{
	...
	rct_map_element* new_map_elements = malloc(3 * (MAX_MAP_EDGE_SIZE * MAX_MAP_EDGE_SIZE) * sizeof(rct_map_element));
	...
	memset(gMapElements + num_elements, 0, (3 * (MAX_MAP_EDGE_SIZE * MAX_MAP_EDGE_SIZE) - num_elements) * sizeof(rct_map_element));
...
rct_map_element *map_element_insert(sint32 x, sint32 y, sint32 z, sint32 flags)
{
	...
	originalMapElement = gMapElementTilePointers[y * MAX_MAP_EDGE_SIZE + x];

	// Set tile index pointer to point to new element block
	gMapElementTilePointers[y * MAX_MAP_EDGE_SIZE + x] = newMapElement;
...
void map_remove_out_of_range_elements()
{
	...
	for (sint32 y = 0; y < (MAX_MAP_EDGE_SIZE * 32); y += 32) {
		for (sint32 x = 0; x < (MAX_MAP_EDGE_SIZE * 32); x += 32) {
