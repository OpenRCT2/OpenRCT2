    ...
    if (flags & GAME_COMMAND_FLAG_APPLY)
    {
        ...
        if ((surfaceElement->properties.surface.slope & 0x0F) == 0x0F)
        {
            ...
            if (diagonal)
            {
                ...
                {
                case 0b1011:
                    surfaceElement->properties.surface.slope |= (1 << 0);
                    break;
                case 0b0111:
                    surfaceElement->properties.surface.slope |= (1 << 1);
                    break;
                case 0b1110:
                    surfaceElement->properties.surface.slope |= (1 << 2);
                    break;
                case 0b1101:
                    surfaceElement->properties.surface.slope |= (1 << 3);
                    break;
                }
            }
...
sint32 tile_inspector_track_base_height_offset(sint32 x, sint32 y, sint32 elementIndex, sint8 offset, sint32 flags)
{
    ...
    if (flags & GAME_COMMAND_FLAG_APPLY)
    {
        ...
        for (; trackBlock->index != 255; trackBlock++)
        {
            ...
            openrct2_assert(map_get_surface_element_at(elemX >> 5, elemY >> 5) != NULL, "No surface at %d,%d", elemX >> 5,
                            elemY >> 5);
