...
static struct
{
    uint32  width, height;
    uint8 * mono_bitmap;
} _heightMapData = { .width = 0, .height = 0, .mono_bitmap = NULL };
...
static void mapgen_blobs(sint32 count, sint32 lowSize, sint32 highSize, sint32 lowHeight, sint32 highHeight)
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        mapgen_blob(border + (util_rand() % borderRange), border + (util_rand() % borderRange),
                    (sint32)(M_PI * radius * radius), lowHeight + (util_rand() % heightRange));
    }
}
...
static void mapgen_blob_fill(sint32 height)
{
    ...
    sint32 left = 0, top = 0, right = _heightSize - 1, bottom = _heightSize - 1;
