...
void sprite_position_tween_all(float nudge)
{
    for (uint16 i = 0; i < MAX_SPRITES; i++)
    {
        ...
        if (sprite_should_tween(sprite))
        {
            ...
            sprite_set_coordinates(posB.x + (sint16)((posA.x - posB.x) * nudge), posB.y + (sint16)((posA.y - posB.y) * nudge),
                                   posB.z + (sint16)((posA.z - posB.z) * nudge), sprite);
            invalidate_sprite_2(sprite);
        }
    }
}
