...
static money32 SmallSceneryRemove(sint16 x, sint16 y, sint8 baseHeight, uint8 quadrant, uint8 sceneryType, uint8 flags)
{
    ...
    do {
        if ((map_element->type >> 6) != quadrant)
            continue;
        if (map_element->base_height != baseHeight)
            continue;
        if (map_element->properties.scenery.type != sceneryType)
            continue;
...
static money32 SmallScenerySetColour(sint16 x, sint16 y, sint8 baseHeight, uint8 quadrant, uint8 sceneryType, uint8 primaryColour, uint8 secondaryColour, uint8 flags)
{
    ...
    rct_map_element *map_element = map_get_small_scenery_element_at(x, y, baseHeight, sceneryType, quadrant);
    ...
    if (flags & GAME_COMMAND_FLAG_APPLY) {
        ...
        map_element->properties.scenery.colour_1 |= primaryColour;
        map_element->properties.scenery.colour_2 &= 0xE0;
        map_element->properties.scenery.colour_2 |= secondaryColour;
        map_invalidate_tile_full(x, y);
    }
    ...
    uint8 flags)
{
    ...
    money32 cost = (scenery_entry->small_scenery.price * 10) + clearCost;
    ...
    new_map_element->properties.scenery.type = sceneryType;
    new_map_element->properties.scenery.age = 0;
    new_map_element->properties.scenery.colour_1 = primaryColour;
    new_map_element->properties.scenery.colour_2 = secondaryColour;
