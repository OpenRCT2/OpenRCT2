    ...
    for (uint32 light = 0; light < LightListCurrentCountFront; light++)
    {
        ...
        if (true)
        {
            ...
            for (sint32 pat = startSamplePoint; pat < totalSamplePoints; pat++)
            {
                ...
                //  &interactionType, &mapElement, NULL);
                    ...
                    // if (screenX >= 0 && screenX < (sint32)myviewport->width && screenY >= 0 && screenY <
                    // (sint32)myviewport->height)
                    //{
...
void lightfx_add_3d_light_magic_from_drawing_tile(rct_xy16 mapPosition, sint16 offsetX, sint16 offsetY, sint16 offsetZ,
                                                  uint8 lightType)
{
...
void lightfx_render_to_texture(void * dstPixels, uint32 dstPitch, uint8 * bits, uint32 width, uint32 height, uint32 * palette,
                               uint32 * lightPalette)
{
