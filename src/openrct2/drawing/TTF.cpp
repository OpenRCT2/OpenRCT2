...
void TTF::ttf_toggle_hinting(bool)
{
...
TTF_Font* TTF::ttf_open_font(const utf8* fontPath, int32_t ptSize)
{
    return TTF_OpenFont(fontPath, ptSize);
}

void TTF::ttf_close_font(TTF_Font* font)
{
    TTF_CloseFont(font);
}

void TTF::ttf_surface_cache_dispose(ttf_cache_entry* entry)
{
...
void TTF::ttf_surface_cache_dispose_all()
{
...
void TTF::ttf_toggle_hinting()
{
    FontLockHelper<std::mutex> lock(_mutex);
    ttf_toggle_hinting(true);
}

TTFSurface* TTF::ttf_surface_cache_get_or_add(TTF_Font* font, std::string_view text)
{
...
void TTF::ttf_getwidth_cache_dispose(ttf_getwidth_cache_entry* entry)
{
...
void TTF::ttf_getwidth_cache_dispose_all()
{
...
uint32_t TTF::ttf_getwidth_cache_get_or_add(TTF_Font* font, std::string_view text)
{
...
TTFFontDescriptor* TTF::ttf_get_font_from_sprite_base(FontSpriteBase spriteBase)
{
    FontLockHelper<std::mutex> lock(_mutex);
    return &gCurrentTTFFontSet->size[font_get_size_from_sprite_base(spriteBase)];
}

bool TTF::ttf_provides_glyph(const TTF_Font* font, codepoint_t codepoint)
{
    return TTF_GlyphIsProvided(font, codepoint);
}

bool TTF::ttf_get_size(TTF_Font* font, std::string_view text, int32_t* outWidth, int32_t* outHeight)
{
...
TTFSurface* TTF::ttf_render(TTF_Font* font, std::string_view text)
{
    thread_local std::string buffer;
    buffer.assign(text);
...
void TTF::ttf_free_surface(TTFSurface* surface)
{
