...
void misc_paint(paint_session * session, rct_sprite * misc, sint32 imageDirection)
{
    ...
    case SPRITE_MISC_MONEY_EFFECT: // 1
    {
        ...
        paint_floating_money_effect(session, value, stringId, moneyEffect->y, moneyEffect->z,
                                    (sint8 *)&money_wave[moneyEffect->wiggle % 22], moneyEffect->offset_x,
    ...
    case SPRITE_MISC_CRASHED_VEHICLE_PARTICLE: // 2
    {
        ...
        imageId =
            imageId | (particle.colour[0] << 19) | (particle.colour[1] << 24) | IMAGE_TYPE_REMAP | IMAGE_TYPE_REMAP_2_PLUS;
        sub_98196C(session, imageId, 0, 0, 1, 1, 0, misc->unknown.z, get_current_rotation());
        break;
    }
