    ...
    const TileDescriptor& neighbour, bool isWater)
{
    ...
    if (session.ViewFlags.has(ViewportFlag::UNDERGROUND_INSIDE))
    {
...
void PaintSurface(PaintSession& session, uint8_t direction, uint16_t height, const SurfaceElement& tileElement)
{
    ...
    else
    {
        ...
        else if (surfaceObject != nullptr)
        {
            ...
            {
                if (!session.ViewFlags.hasAny(ViewportFlag::HIDE_BASE, ViewportFlag::UNDERGROUND_INSIDE))
                {
        ...
        if (session.ViewFlags.hasAny(ViewportFlag::UNDERGROUND_INSIDE, ViewportFlag::HIDE_BASE))
        {
    ...
    if ((gLegacyScene == LegacyScene::scenarioEditor || gameState.cheats.sandboxMode)
        && session.ViewFlags.has(ViewportFlag::LAND_OWNERSHIP))
    {
    ...
    if (session.ViewFlags.has(ViewportFlag::LAND_OWNERSHIP))
    {
        PaintSurfaceLandOwnership(session, tileElement, height, surfaceShape);
    }

    if (session.ViewFlags.has(ViewportFlag::CONSTRUCTION_RIGHTS) && !(tileElement.hasOwnership(OwnershipFlag::landOwned)))
    {
