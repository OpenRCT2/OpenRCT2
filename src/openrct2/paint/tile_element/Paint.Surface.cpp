    ...
    const TileDescriptor& neighbour, bool isWater)
{
    ...
    bool neighbourIsClippedAway = (session.ViewFlags & ViewportFlag::CLIP_VIEW) && !TileIsInsideClipView(neighbour);
    ...
    if (session.ViewFlags & ViewportFlag::UNDERGROUND_INSIDE)
    {
    ...
    PaintSession& session, const SurfaceElement& tileElement, uint16_t height, uint8_t surfaceShape)
{
    ...
    // Loc660E9A:
    if (tileElement.hasOwnership(OwnershipFlag::landOwned))
    {
    ...
    else if (tileElement.hasOwnership(OwnershipFlag::landForSale))
    {
    ...
    PaintSession& session, const SurfaceElement& tileElement, uint16_t height, uint8_t surfaceShape)
{
    ...
    else if (tileElement.hasOwnership(OwnershipFlag::constructionRightsForSale))
    {
        const auto pos = CoordsXYZ(session.MapPosition.x + 16, session.MapPosition.y + 16, aboveWaterHeight);
        const auto height2 = TileElementHeight(pos, aboveWaterSurfaceShape) + ForSaleSignZOffset;
...
void PaintSurface(PaintSession& session, uint8_t direction, uint16_t height, const SurfaceElement& tileElement)
{
    ...
    if (PaintShouldShowHeightMarkers(session, ViewportFlag::LAND_HEIGHTS))
    {
    ...
    {
        const bool showGridlines = (session.ViewFlags & ViewportFlag::GRIDLINES);
        ...
        else if (surfaceObject != nullptr)
        {
            ...
            {
                if ((session.ViewFlags & (ViewportFlag::HIDE_BASE | ViewportFlag::UNDERGROUND_INSIDE)) == 0)
                {
        ...
        if (session.ViewFlags & (ViewportFlag::UNDERGROUND_INSIDE | ViewportFlag::HIDE_BASE))
        {
    ...
    if ((gLegacyScene == LegacyScene::scenarioEditor || gameState.cheats.sandboxMode)
        && session.ViewFlags & ViewportFlag::LAND_OWNERSHIP)
    {
    ...
    if (session.ViewFlags & ViewportFlag::LAND_OWNERSHIP)
    {
        PaintSurfaceLandOwnership(session, tileElement, height, surfaceShape);
    }

    if (session.ViewFlags & ViewportFlag::CONSTRUCTION_RIGHTS && !(tileElement.hasOwnership(OwnershipFlag::landOwned)))
    {
    ...
    if (zoomLevel <= ZoomLevel{ 0 } && has_surface && !(session.ViewFlags & ViewportFlag::UNDERGROUND_INSIDE)
        && !(session.ViewFlags & ViewportFlag::HIDE_BASE) && Config::Get().general.landscapeSmoothing)
    {
    ...
    if ((session.ViewFlags & ViewportFlag::UNDERGROUND_INSIDE) && !(session.ViewFlags & ViewportFlag::HIDE_BASE)
    ...
    if (!(session.ViewFlags & ViewportFlag::HIDE_VERTICAL))
    {
    ...
    const bool waterGetsClipped = (session.ViewFlags & ViewportFlag::CLIP_VIEW) && (waterHeight > gClipHeight * kCoordsZStep);
    const bool waterIsTransparent = Config::Get().general.transparentWater
        || (session.ViewFlags & ViewportFlag::UNDERGROUND_INSIDE);
