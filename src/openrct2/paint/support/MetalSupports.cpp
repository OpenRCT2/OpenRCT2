    ...
    int32_t height, ImageId imageTemplate)
{
    return MetalSupportsPaintSetup<false>(
        session, RotateMetalSupportGraphic(supportType, 0), placement, heightExtra, height, imageTemplate);
}
    ...
    const int32_t heightExtra, int32_t height, ImageId imageTemplate)
{
    return MetalSupportsPaintSetup<false>(
        ...
        kMetalSupportPlacementRotated[EnumValue(placement)][direction], heightExtra, height, imageTemplate);
}
    ...
    const int32_t heightExtra)
{
    ...
    if (direction & 1)
    {
        MetalSupportsPaintSetup<false>(session, graphic, MetalSupportPlace::TopRightSide, heightExtra, height, colour);
        MetalSupportsPaintSetup<false>(session, graphic, MetalSupportPlace::BottomLeftSide, heightExtra, height, colour);
    }
    else
    {
        MetalSupportsPaintSetup<false>(session, graphic, MetalSupportPlace::TopLeftSide, heightExtra, height, colour);
        MetalSupportsPaintSetup<false>(session, graphic, MetalSupportPlace::BottomRightSide, heightExtra, height, colour);
    }
}
