 ...
 */
bool wooden_a_supports_paint_setup(paint_session * session, sint32 supportType, sint32 special, sint32 height,
                                   uint32 imageColourFlags, bool * underground)
{
    if (underground != NULL)
    {
        *underground = false;
    ...
    if (special != 0)
    {
        ...
        if (imageId != 0 && byte_97B23C[special].var_7 != 0)
        {
            ...
            {
                sub_98197C(session, imageId, 0, 0, bBox.length.x, bBox.length.y, bBox.length.z, z, bBox.offset.x, bBox.offset.y,
            ...
            {
                hasSupports       = true;
                paint_struct * ps = sub_98198C(session, imageId, 0, 0, bBox.length.x, bBox.length.y, bBox.length.z, z,
                                               bBox.offset.x, bBox.offset.y, bBox.offset.z + z, rotation);
                if (ps != NULL)
                {
 ...
 */
bool wooden_b_supports_paint_setup(paint_session * session, sint32 supportType, sint32 special, sint32 height,
                                   uint32 imageColourFlags, bool * underground)
{
#ifndef NO_RCT2
    ...
    if (!skipTo663004)
    {
        while (heightSteps > 0)
        {
            ...
            {
                sub_98196C(session, WoodenSupportImageIds[supportType].half | imageColourFlags, 0, 0, 32, 32,
            ...
            {
                sub_98196C(session, WoodenSupportImageIds[supportType].full | imageColourFlags, 0, 0, 32, 32,
    ...
    if (special != 0)
    {
        ...
        { // byte_97B23C[special].var_7 is never 0
            ...
            {
                sub_98197C(session, imageId | imageColourFlags, 0, 0, boundBox.length.x, boundBox.length.y, boundBox.length.z,
            ...
            else
            {
                paint_struct * paintStruct = sub_98198C(
                    ...
                    baseHeight, boundBox.offset.x, boundBox.offset.y, boundBox.offset.z + baseHeight, get_current_rotation());
                _9E32B1 = true;
                if (paintStruct != NULL)
                {
 ...
 *  rct2: 0x00663105
 */
bool metal_a_supports_paint_setup(paint_session * session, uint8 supportType, uint8 segment, sint32 special, sint32 height,
                                  uint32 imageColourFlags)
{
 ...
 * @return (Carry Flag)
 */
bool metal_b_supports_paint_setup(paint_session * session, uint8 supportType, uint8 segment, sint32 special, sint32 height,
                                  uint32 imageColourFlags)
{
#ifndef NO_RCT2
    if (gUseOriginalRidePaint)
    ...
    if (height < supportSegments[segment].height)
    {
        ...
        sub_98196C(session, _metalSupportTypeToCrossbeamImages[supportType][ebp] | imageColourFlags,
                   loc_97AF20[originalSegment].x + loc_97B052[ebp].x, loc_97AF20[originalSegment].y + loc_97B052[ebp].y,
                   _97B062[ebp].x, _97B062[ebp].y, 1, baseHeight, get_current_rotation());
    }
    ...
    else
    {
        ...
        sub_98196C(session, imageId | imageColourFlags, loc_97AF20[segment].x, loc_97AF20[segment].y, 0, 0, 5,
                   supportSegments[segment].height, get_current_rotation());
    ...
    if (heightDiff > 0)
    {
        sub_98196C(session, (_97B15C[supportType].beam_id + (heightDiff - 1)) | imageColourFlags, loc_97AF20[segment].x,
    ...
    while (true)
    {
        ...
        sub_98196C(session, imageId | imageColourFlags, loc_97AF20[segment].x, loc_97AF20[segment].y, 0, 0, beamLength - 1,
                   baseHeight, get_current_rotation());
    ...
    if (special != 0)
    {
        ...
        while (true)
        {
            ...
            uint32 imageId = _97B15C[supportType].beam_id + (beamLength - 1);
            sub_98197C(session, imageId | imageColourFlags, loc_97AF20[originalSegment].x, loc_97AF20[originalSegment].y, 0, 0,
                       0, baseHeight, loc_97AF20[originalSegment].x, loc_97AF20[originalSegment].y, height,
                       get_current_rotation());
            baseHeight += beamLength;
        }
    }
 ...
 */
bool path_a_supports_paint_setup(paint_session * session, sint32 supportType, sint32 special, sint32 height,
                                 uint32 imageColourFlags, rct_footpath_entry * pathEntry, bool * underground)
{
#ifndef NO_RCT2
    if (gUseOriginalRidePaint)
    {
    ...
    else if (session->Support.slope & 0x0F)
    {
        ...
        sub_98197C(session, ebx | imageColourFlags, 0, 0, 32, 32, 11, baseHeight, 0, 0, baseHeight + 2, get_current_rotation());
    ...
    if (special != 0)
    {
        ...
        if (supportsDesc.var_6 == 0 || session->WoodenSupportsPrependTo == NULL)
        {
            sub_98197C(session, imageId | imageColourFlags, 0, 0, boundBox.length.y, boundBox.length.x, boundBox.length.z,
                       baseHeight, boundBox.offset.x, boundBox.offset.y, baseHeight + boundBox.offset.z,
                       get_current_rotation());
            hasSupports = true;
        }
        else
        {
            paint_struct * paintStruct = sub_98198C(session, imageId | imageColourFlags, 0, 0, boundBox.length.y,
                                                    boundBox.length.x, boundBox.length.z, baseHeight, boundBox.offset.x,
                                                    boundBox.offset.y, baseHeight + boundBox.offset.z, get_current_rotation());
            hasSupports                = true;
            if (paintStruct != NULL)
            {
                session->WoodenSupportsPrependTo->var_20 = paintStruct;
 ...
 * @return Whether supports were drawn
 */
bool path_b_supports_paint_setup(paint_session * session, sint32 segment, sint32 special, sint32 height,
                                 uint32 imageColourFlags, rct_footpath_entry * pathEntry)
{
#ifndef NO_RCT2
    if (gUseOriginalRidePaint)
    {
    ...
    else
    {
        ...
        sub_98196C(session, (pathEntry->bridge_image + 37 + imageOffset) | imageColourFlags, loc_97AF20[segment].x,
    ...
    if (heightDiff > 0)
    {
        sub_98196C(session, (pathEntry->bridge_image + 20 + (heightDiff - 1)) | imageColourFlags, loc_97AF20[segment].x,
                   loc_97AF20[segment].y, 0, 0, heightDiff - 1, baseHeight, get_current_rotation());
    }
    ...
    while (keepGoing)
    {
        ...
        for (sint32 i = 0; i < 4; ++i)
        {
            ...
            sub_98196C(session, (pathEntry->bridge_image + 20 + (z - 1)) | imageColourFlags, loc_97AF20[segment].x,
        ...
        sub_98196C(session, imageId | imageColourFlags, loc_97AF20[segment].x, loc_97AF20[segment].y, 0, 0, (z - 1), baseHeight,
                   get_current_rotation());
    ...
    if (special != 0)
    {
        ...
        while (true)
        {
            ...
            uint32 imageId = pathEntry->bridge_image + 20 + (z - 1);
            sub_98197C(session, imageId | imageColourFlags, loc_97AF20[segment].x, loc_97AF20[segment].y, 0, 0, 0, baseHeight,
