...
{
    return (vehicle->HasFlag(VehicleFlag::CarIsReversed)) ? RollInvertTable[EnumValue(vehicle->roll)] : vehicle->roll;
}
...
static void VehiclePitchFlatBankedLeft67(
    ...
    uint32_t boundingBoxIndex)
{
    if (vehicle->HasFlag(VehicleFlag::CarIsInverted))
    {
        carEntry--;
    }
    boundingBoxIndex = boundingBoxIndex != kBoundBoxIndexUndefined ? boundingBoxIndex
...
void Vehicle::Paint(PaintSession& session, int32_t imageDirection) const
{
    ...
    if (HasFlag(VehicleFlag::Crashed))
    {
        PaintAddImageAsParent(
    ...
    else
    {
        ...
        if (HasFlag(VehicleFlag::CarIsInverted))
        {
            carEntryIndex++;
            zOffset += 16;
        }
