...
namespace WoodenRC
{
    ...
    struct SpriteBoundBox2 // TODO: Should this be renamed?
    {
        ImageIndex ImageIdA;
        ImageIndex ImageIdB;
        CoordsXYZ offset;
    ...
    ImageId GetTrackColour(const PaintSession& session)
    {
        if (isClassic)
    ...
    ImageId GetRailsColour(PaintSession& session);
        ...
        const BoundBoxXYZ& boundBox)
    {
        ...
        else
        {
            ...
            const ImageId railsImageId = GetRailsColour(session).WithIndex(imageIdRails);
    ...
    template<bool isClassic>
    void TrackPaintBb(PaintSession& session, const SpriteBoundBox2* bb, int16_t height)
    {
        if (bb->ImageIdA == 0)
            return;

        ImageId imageId = GetTrackColour<isClassic>(session).WithIndex(bb->ImageIdA);
        ...
        if (bb->ImageIdB != 0)
        {
            ImageId railsImageId = GetRailsColour(session).WithIndex(bb->ImageIdB);
