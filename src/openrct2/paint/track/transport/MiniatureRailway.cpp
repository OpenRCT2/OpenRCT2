    ...
    const TrackElement& trackElement, SupportType supportType)
{
    ...
    const auto imageId = session.TrackColours.WithIndex(miniature_railway_track_pieces_flat[direction]);
    if (!paintAsGravel)
    {
    ...
    {
        const auto imageIdAlt = MiniatureRailwayTrackToGravel(imageId);
        PaintAddImageAsParentRotated(session, direction, imageIdAlt, { 0, 6, height }, { { 0, 6, height }, { 32, 20, 2 } });
    }
    if (paintGrooved)
    {
        auto imageIdAlt = MiniatureRailwayTrackToGrooved(imageId);
