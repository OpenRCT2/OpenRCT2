...
void EntityPaintSetup(PaintSession& session, const CoordsXY& pos)
{
    ...
    if (gTrackDesignSaveMode || (session.ViewFlags & ViewportFlag::HIDE_ENTITIES))
    {
        return;
    }
    ...
    const bool highlightPathIssues = (session.ViewFlags & ViewportFlag::HIGHLIGHT_PATH_ISSUES);
