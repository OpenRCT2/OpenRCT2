 ...
 */
static paint_struct * sub_9819_c(paint_session * session, uint32 image_id, rct_xyz16 offset, rct_xyz16 boundBoxSize,
                                 rct_xyz16 boundBoxOffset, uint8 rotation)
{
...
paint_struct * sub_98196C(paint_session * session, uint32 image_id, sint8 x_offset, sint8 y_offset, sint16 bound_box_length_x,
                          sint16 bound_box_length_y, sint8 bound_box_length_z, sint16 z_offset, uint32 rotation)
{
    ...
    rct_xyz16 coord_3d = { .x = x_offset, // ax
                           .y = y_offset, // cx
                           .z = z_offset };
 ...
 * @return (ebp) paint_struct on success (CF == 0), NULL on failure (CF == 1)
 */
paint_struct * sub_98197C(paint_session * session, uint32 image_id, sint8 x_offset, sint8 y_offset, sint16 bound_box_length_x,
                          ...
                          sint16 bound_box_offset_y, sint16 bound_box_offset_z, uint32 rotation)
{
    ...
    rct_xy16 attach = { .x = ps->bound_box_x, .y = ps->bound_box_y };
 ...
 * @param rotation (ebp)
 */
void paint_floating_money_effect(paint_session * session, money32 amount, rct_string_id string_id, sint16 y, sint16 z,
                                 sint8 y_offsets[], sint16 offset_x, uint32 rotation)
{
    if (session->NextFreePaintStruct >= session->EndOfPaintStructArray)
    {
        return;
    ...
    rct_xyz16 position = { .x = session->SpritePosition.x, .y = session->SpritePosition.y, .z = z };
...
{
    rct_drawpixelinfo * dpi     = session->Unk140E9A8;
    rct_xy16            mapTile = { .x = dpi->x & 0xFFE0, .y = (dpi->y - 16) & 0xFFE0 };
                                 ...
                                 const paint_struct_bound_box * currentBBox)
{
    ...
    {
    case 0:
        if ((*initialBBox).z_end >= (*currentBBox).z && (*initialBBox).y_end >= (*currentBBox).y &&
            (*initialBBox).x_end >= (*currentBBox).x &&
            !((*initialBBox).z < (*currentBBox).z_end && (*initialBBox).y < (*currentBBox).y_end &&
              (*initialBBox).x < (*currentBBox).x_end))
            result = true;
        break;
    case 1:
        if ((*initialBBox).z_end >= (*currentBBox).z && (*initialBBox).y_end >= (*currentBBox).y &&
...
paint_struct * paint_arrange_structs_helper(paint_struct * ps_next, uint16 quadrantIndex, uint8 flag)
{
    ...
    while (true)
    {
        ...
        const paint_struct_bound_box initialBBox = { .x     = ps_next->bound_box_x,
        ...
        while (true)
        {
            ...
            const paint_struct_bound_box currentBBox = { .x     = ps_next->bound_box_x,
                                                         .y     = ps_next->bound_box_y,
                                                         .z     = ps_next->bound_box_z,
                                                         .x_end = ps_next->bound_box_x_end,
                                                         .y_end = ps_next->bound_box_y_end,
...
paint_struct paint_session_arrange(paint_session * session)
{
    ...
    if (quadrantIndex != UINT32_MAX)
    {
        ...
        paint_struct * ps_cache =
            paint_arrange_structs_helper(&psHead, session->QuadrantBackIndex & 0xFFFF, PAINT_QUADRANT_FLAG_NEXT);
...
static void paint_ps_image_with_bounding_boxes(rct_drawpixelinfo * dpi, paint_struct * ps, uint32 imageId, sint16 x, sint16 y)
{
    ...
    // bottom square
    gfx_draw_line(dpi, screenCoordFrontBottom.x, screenCoordFrontBottom.y, screenCoordLeftBottom.x, screenCoordLeftBottom.y,
                  colour);
    gfx_draw_line(dpi, screenCoordBackBottom.x, screenCoordBackBottom.y, screenCoordLeftBottom.x, screenCoordLeftBottom.y,
                  colour);
    gfx_draw_line(dpi, screenCoordBackBottom.x, screenCoordBackBottom.y, screenCoordRightBottom.x, screenCoordRightBottom.y,
                  colour);
    gfx_draw_line(dpi, screenCoordFrontBottom.x, screenCoordFrontBottom.y, screenCoordRightBottom.x, screenCoordRightBottom.y,
                  colour);
    ...
    gfx_draw_line(dpi, screenCoordRightTop.x, screenCoordRightTop.y, screenCoordRightBottom.x, screenCoordRightBottom.y,
                  colour);
    ...
    // vertical front
    gfx_draw_line(dpi, screenCoordFrontTop.x, screenCoordFrontTop.y, screenCoordFrontBottom.x, screenCoordFrontBottom.y,
                  colour);
