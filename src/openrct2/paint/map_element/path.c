...
void loc_6A37C9(rct_map_element * mapElement, int height, rct_footpath_entry * footpathEntry, bool hasFences, uint32 imageFlags,
                uint32 sceneryImageFlags);

void loc_6A3B57(rct_map_element * mapElement, sint16 height, rct_footpath_entry * footpathEntry, bool hasFences,
                uint32 imageFlags, uint32 sceneryImageFlags);

/* rct2: 0x006A5AE5 */
static void path_bit_lights_paint(rct_scenery_entry * pathBitEntry, rct_map_element * mapElement, int height, uint8 edges,
...
/* rct2: 0x006A5C94 */
static void path_bit_bins_paint(rct_scenery_entry * pathBitEntry, rct_map_element * mapElement, int height, uint8 edges,
                                uint32 pathBitImageFlags)
{
    if (footpath_element_is_sloped(mapElement))
        height += 8;
...
/* rct2: 0x006A5E81 */
static void path_bit_benches_paint(rct_scenery_entry * pathBitEntry, rct_map_element * mapElement, int height, uint8 edges,
                                   uint32 pathBitImageFlags)
{
    uint32 imageId;
...
/* rct2: 0x006A6008 */
static void path_bit_jumping_fountains_paint(rct_scenery_entry * pathBitEntry, rct_map_element * mapElement, int height,
                                             uint8 edges, uint32 pathBitImageFlags, rct_drawpixelinfo * dpi)
{
    if (dpi->zoom_level != 0)
        return;
 ...
 */
static void sub_6A4101(rct_map_element * map_element, uint16 height, uint32 ebp, bool word_F3F038,
                       rct_footpath_entry * footpathEntry, uint32 base_image_id, uint32 imageFlags)
{
    if (map_element->type & 1)
    {
        ...
        else
        {
            ...
            case 3:
                sub_98197C(90 + base_image_id, 0, 4, 28, 1, 7, height, 0, 4, height + 2, get_current_rotation());
                sub_98197C(91 + base_image_id, 28, 0, 1, 28, 7, height, 28, 4, height + 2,
            ...
            case 12:
                sub_98197C(89 + base_image_id, 4, 0, 1, 28, 7, height, 4, 0, height + 2, get_current_rotation());
                sub_98197C(92 + base_image_id, 0, 28, 28, 1, 7, height, 4, 28, height + 2,
        ...
        sub_98197C(imageId, 0, 0, 1, 1, 21, height, boundBoxOffsets.x, boundBoxOffsets.y, boundBoxOffsets.z,
        ...
        if (direction < 2 && map_element->properties.path.ride_index != 255 && imageFlags == 0)
        {
            ...
            sub_98199C(scrolling_text_setup(string_id, scroll, scrollingMode), 0, 0, 1, 1, 21, height + 7, boundBoxOffsets.x,
                       boundBoxOffsets.y, boundBoxOffsets.z, get_current_rotation());
        }
    ...
    else
    {
        ...
        case 3:
            sub_98197C(76 + base_image_id, 0, 4, 28, 1, 7, height, 0, 4, height + 2, get_current_rotation());
            sub_98197C(77 + base_image_id, 28, 0, 1, 28, 7, height, 28, 4, height + 2,
                       get_current_rotation()); // bound_box_offset_y seems to be a bug
        ...
        case 12:
            sub_98197C(75 + base_image_id, 4, 0, 1, 28, 7, height, 4, 0, height + 2, get_current_rotation());
            sub_98197C(78 + base_image_id, 0, 28, 28, 1, 7, height, 4, 28, height + 2,
                       get_current_rotation()); // bound_box_offset_x seems to be a bug
 ...
 */
static void sub_6A3F61(rct_map_element * map_element, uint16 bp, uint16 height, rct_footpath_entry * footpathEntry,
                       uint32 imageFlags, uint32 sceneryImageFlags, bool word_F3F038)
{
    // eax --
    // ebx --
    // ecx
                ...
                uint32 sceneryImageFlags)
{
    ...
    {
        sub_98197C(imageId | imageFlags, 0, 0, boundBoxSize.x, boundBoxSize.y, 0, height, boundBoxOffset.x, boundBoxOffset.y,
                   height + 1, get_current_rotation());
    }
    else
    {
        uint32 image_id;
        ...
        sub_98197C(image_id | imageFlags, 0, 0, boundBoxSize.x, boundBoxSize.y, 0, height, boundBoxOffset.x, boundBoxOffset.y,
                   height + 1, get_current_rotation());
        ...
        {
            sub_98199C(imageId | imageFlags, 0, 0, boundBoxSize.x, boundBoxSize.y, 0, height, boundBoxOffset.x,
    ...
    if ((mapElement->type & 1) || (mapElement->properties.path.edges != 0xFF && hasFences))
    {
                ...
                uint32 imageFlags, uint32 sceneryImageFlags)
{
    ...
    else
    {
        ...
        sub_98197C(bridgeImage | imageFlags, 0, 0, boundBoxSize.x, boundBoxSize.y, 0, height, boundBoxOffset.x,
                   boundBoxOffset.y, height + 1, get_current_rotation());
