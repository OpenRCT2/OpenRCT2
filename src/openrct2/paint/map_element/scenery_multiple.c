...
// 6B8172:
static void scenery_multiple_paint_supports(paint_session * session, uint8 direction, uint16 height,
                                            rct_map_element * mapElement, uint32 dword_F4387C, rct_large_scenery_tile * tile)
{
    if (tile->var_7 & 0x20)
    {
        return;
...
static void scenery_multiple_sign_paint_line(paint_session * session, const utf8 * str, rct_large_scenery_text * text,
                                             sint32 textImage, sint32 textColour, uint8 direction, sint32 y_offset)
{
...
void scenery_multiple_paint(paint_session * session, uint8 direction, uint16 height, rct_map_element * mapElement)
{
    ...
    image_id |= ((mapElement->properties.scenerymultiple.colour[0] & 0x1F) << 19) |
                ((mapElement->properties.scenerymultiple.colour[1] & 0x1F) << 24) | IMAGE_TYPE_REMAP | IMAGE_TYPE_REMAP_2_PLUS;
    rct_xyz16 boxlength;
    rct_xyz16 boxoffset;
    ...
    if (entry->large_scenery.flags & LARGE_SCENERY_FLAG_3D_TEXT)
    {
        ...
        if (text->flags & LARGE_SCENERY_TEXT_FLAG_VERTICAL)
        {
            ...
            while ((codepoint = utf8_get_next(fitStrPtr, &fitStrPtr)) != 0)
            {
                ...
                scenery_multiple_sign_paint_line(session, str, entry->large_scenery.text, entry->large_scenery.text_image,
                                                 textColour, direction, y_offset - height2);
            ...
            if (text->flags & LARGE_SCENERY_TEXT_FLAG_TWO_LINE)
            {
                ...
                if (width > text->max_width)
                {
                    ...
                    for (sint32 i = 0; i < 2; i++)
                    {
                        ...
                        } while (w <= text->max_width && (dst = utf8_write_codepoint(dst, codepoint)) != NULL &&
                                 (srcold = src) != NULL && (codepoint = utf8_get_next(src, (const utf8 **)&src)) != '\0');
                        src = srcold;
                        ...
                        scenery_multiple_sign_paint_line(session, str1, entry->large_scenery.text,
                                                         entry->large_scenery.text_image, textColour, direction, y_offset);
                        y_offset += (scenery_multiple_sign_get_glyph(text, 'A')->height + 1) * 2;
                    }
                }
                else
                {
                    scenery_multiple_sign_paint_line(session, signString, entry->large_scenery.text,
                                                     entry->large_scenery.text_image, textColour, direction, y_offset);
                }
            }
            else
    ...
    sub_98199C(session, scrolling_text_setup(session, stringId, scroll, scrollMode), 0, 0, 1, 1, 21, height + 25, boxoffset.x,
               boxoffset.y, boxoffset.z, get_current_rotation());
