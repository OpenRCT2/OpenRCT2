...
void banner_paint(paint_session * session, uint8 direction, sint32 height, rct_map_element * map_element)
{
    ...
    {
        image_id |= (gBanners[map_element->properties.banner.index].colour << 19) | IMAGE_TYPE_REMAP;
    }
    ...
    sub_98199C(session, scrolling_text_setup(session, string_id, scroll, scrollingMode), 0, 0, 1, 1, 0x15, height + 22,
               boundBoxOffsetX, boundBoxOffsetY, boundBoxOffsetZ, get_current_rotation());
}
