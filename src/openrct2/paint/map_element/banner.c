...
const rct_xy16 BannerBoundBoxes[][2] = {
    { { 1, 2 }, { 1, 29 } }, { { 2, 32 }, { 29, 32 } }, { { 32, 2 }, { 32, 29 } }, { { 2, 1 }, { 29, 1 } },
};
...
void banner_paint(uint8 direction, int height, rct_map_element * map_element)
{
    ...
    if (dpi->zoom_level > 1 || gTrackDesignSaveMode)
        return;
    ...
    {
        image_id |= (gBanners[map_element->properties.banner.index].colour << 19) | 0x20000000;
    }
    ...
    if (direction >= 2 || (map_element->flags & MAP_ELEMENT_FLAG_GHOST))
        return;
    ...
    sub_98199C(scrolling_text_setup(string_id, scroll, scrollingMode), 0, 0, 1, 1, 0x15, height + 22, boundBoxOffsetX,
               boundBoxOffsetY, boundBoxOffsetZ, get_current_rotation());
}
