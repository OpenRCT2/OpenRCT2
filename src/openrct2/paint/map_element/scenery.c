...
void scenery_paint(uint8 direction, int height, rct_map_element * mapElement)
{
    ...
    {
        sub_98197C(baseImageid, x_offset, y_offset, boxlength.x, boxlength.y, boxlength.z - 1, height, boxoffset.x, boxoffset.y,
                   boxoffset.z, rotation);
    }
    ...
    if (entry->small_scenery.flags & SMALL_SCENERY_FLAG_HAS_GLASS)
    {
        if (dword_F64EB0 == 0)
        {
            ...
            int image_id = (baseImageid & 0x7FFFF) + (GlassPaletteIds[(mapElement->properties.scenery.colour_1 & 0x1F)] << 19) +
                           0x40000004;
            sub_98199C(image_id, x_offset, y_offset, boxlength.x, boxlength.y, boxlength.z - 1, height, boxoffset.x,
    ...
    if (entry->small_scenery.flags & SMALL_SCENERY_FLAG_ANIMATED)
    {
        ...
        if ((entry->small_scenery.flags & SMALL_SCENERY_FLAG21) || (dpi->zoom_level <= 1))
        {
            ...
            else if (entry->small_scenery.flags & SMALL_SCENERY_FLAG13)
            {
                // 6E043B:
            ...
            else if (entry->small_scenery.flags & SMALL_SCENERY_FLAG15)
            {
                // 6E02F6:
            ...
            else
            {
                ...
                if (entry->small_scenery.flags & SMALL_SCENERY_FLAG21)
                {
                    sub_98197C(image_id, x_offset, y_offset, boxlength.x, boxlength.y, boxlength.z - 1, height, boxoffset.x,
                               boxoffset.y, boxoffset.z, rotation);
                }
                else
                {
