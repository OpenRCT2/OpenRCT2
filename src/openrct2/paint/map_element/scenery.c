...
static const rct_xy16 offsets[] = { { 3, 3 }, { 3, 17 }, { 17, 3 }, { 3, 3 } };

static const rct_xy16 lengths[] = { { 12, 26 }, { 26, 12 }, { 12, 26 }, { 26, 12 } };
...
void scenery_paint(paint_session * session, uint8 direction, sint32 height, rct_map_element * mapElement)
{
    ...
    {
        sub_98197C(session, baseImageid, x_offset, y_offset, boxlength.x, boxlength.y, boxlength.z - 1, height, boxoffset.x,
                   boxoffset.y, boxoffset.z, rotation);
    }
    ...
    if (entry->small_scenery.flags & SMALL_SCENERY_FLAG_HAS_GLASS)
    {
        if (dword_F64EB0 == 0)
        {
            ...
            sint32 image_id =
                (baseImageid & 0x7FFFF) + (GlassPaletteIds[scenery_small_get_primary_colour(mapElement)] << 19) + 0x40000004;
            sub_98199C(session, image_id, x_offset, y_offset, boxlength.x, boxlength.y, boxlength.z - 1, height, boxoffset.x,
                       boxoffset.y, boxoffset.z, rotation);
        }
    }
    ...
    if (entry->small_scenery.flags & SMALL_SCENERY_FLAG_ANIMATED)
    {
        ...
        if ((entry->small_scenery.flags & SMALL_SCENERY_FLAG_VISIBLE_WHEN_ZOOMED) || (dpi->zoom_level <= 1))
        {
            ...
            if (entry->small_scenery.flags & SMALL_SCENERY_FLAG_FOUNTAIN_SPRAY_1)
            {
                ...
                sub_98199C(session, image_id, x_offset, y_offset, boxlength.x, boxlength.y, boxlength.z - 1, height,
                           boxoffset.x, boxoffset.y, boxoffset.z, rotation);
            }
            else if (entry->small_scenery.flags & SMALL_SCENERY_FLAG_FOUNTAIN_SPRAY_4)
            {
                // 6E043B:
            ...
            else if (entry->small_scenery.flags & SMALL_SCENERY_FLAG_IS_CLOCK)
            {
                // 6E035C:
            ...
            else if (entry->small_scenery.flags & SMALL_SCENERY_FLAG_SWAMP_GOO)
            {
                // 6E02F6:
            ...
            else if (entry->small_scenery.flags & SMALL_SCENERY_FLAG_HAS_FRAME_OFFSETS)
            {
                ...
                if (entry->small_scenery.flags & SMALL_SCENERY_FLAG_VISIBLE_WHEN_ZOOMED)
                {
                    sub_98197C(session, image_id, x_offset, y_offset, boxlength.x, boxlength.y, boxlength.z - 1, height,
                               boxoffset.x, boxoffset.y, boxoffset.z, rotation);
                }
                else
                {
