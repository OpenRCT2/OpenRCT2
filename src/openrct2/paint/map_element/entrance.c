...
static void ride_entrance_exit_paint(uint8 direction, int height, rct_map_element * map_element)
{
    ...
    sub_98197C(image_id, 0, 0, lengthX, lengthY, ah, height, (direction & 1) ? 28 : 2, (direction & 1) ? 2 : 28, height,
               get_current_rotation());

    if (transparant_image_id)
    {
        transparant_image_id += 4;
        sub_98199C(transparant_image_id, 0, 0, lengthX, lengthY, ah, height, (direction & 1) ? 28 : 2, (direction & 1) ? 2 : 28,
    ...
    if (!is_exit && !(map_element->flags & MAP_ELEMENT_FLAG_GHOST) && map_element->properties.entrance.ride_index != 0xFF)
    {
        ...
        sub_98199C(scrolling_text_setup(string_id, scroll, style->scrolling_mode), 0, 0, 0x1C, 0x1C, 0x33,
...
static void park_entrance_paint(uint8 direction, int height, rct_map_element * map_element)
{
    ...
    case 0:
        ...
        sub_98199C(scrolling_text_setup(park_text_id, scroll, entrance->scrolling_mode + direction / 2), 0, 0, 0x1C, 0x1C, 0x2F,
                   height + entrance->text_height, 2, 2, height + entrance->text_height, get_current_rotation());
        break;
    case 1:
    case 2:
        entrance = (rct_entrance_type *)object_entry_groups[OBJECT_TYPE_PARK_ENTRANCE].chunks[0];
