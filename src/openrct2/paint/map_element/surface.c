...
#define DEFINE_EDGE_SPRITES(base)                                                                                              \
    {                                                                                                                          \
        (base) + 0, (base) + 20, (base) + 10, (base) + 30,                                                                     \
    }
#define DEFINE_EDGE_TUNNEL_SPRITES(base)                                                                                       \
    {                                                                                                                          \
        (base) + 36, (base) + 40, (base) + 44, (base) + 48, (base) + 52, (base) + 56, (base) + 60, (base) + 64, (base) + 68,   \
            (base) + 72, (base) + 76, (base) + 80, (base) + 36, (base) + 48, (base) + 60, (base) + 72,                         \
    }
...
const uint32 dword_97B750[][2] = {
    ...
    { SPR_TERRAIN_GRID | COLOUR_BRIGHT_RED << 19 | IMAGE_TYPE_REMAP,
 ...
 */
static void viewport_surface_smoothen_edge(paint_session * session, enum edge_t edge, struct tile_descriptor self,
 ...
 */
static void viewport_surface_draw_land_side_top(paint_session * session, enum edge_t edge, uint8 height, uint8 terrain,
                                                struct tile_descriptor self, struct tile_descriptor neighbour)
{
    registers regs;
 ...
 */
static void viewport_surface_draw_land_side_bottom(paint_session * session, enum edge_t edge, uint8 height, uint8 edgeStyle,
                                                   struct tile_descriptor self, struct tile_descriptor neighbour)
{
    registers regs;
    ...
    while (true)
    {
        ...
        sub_98197C(session, image_id, offset.x, offset.y, tunnelBounds.x, tunnelBounds.y, boundBoxLength - 1, zOffset, 0, 0,
        ...
        sub_98197C(session, image_id, offset.x, offset.y, tunnelBounds.x, tunnelBounds.y, boundBoxLength - 1, curHeight * 16,
 ...
 */
static void viewport_surface_draw_water_side_top(paint_session * session, enum edge_t edge, uint8 height, uint8 terrain,
                                                 struct tile_descriptor self, struct tile_descriptor neighbour)
{
    registers regs;
 ...
 */
static void viewport_surface_draw_water_side_bottom(paint_session * session, enum edge_t edge, uint8 height, uint8 edgeStyle,
                                                    struct tile_descriptor self, struct tile_descriptor neighbour)
{
    registers regs;
...
void surface_paint(paint_session * session, uint8 direction, uint16 height, rct_map_element * mapElement)
{
    ...
    rct_xy16 base = { .x = session->SpritePosition.x, .y = session->SpritePosition.y };
    ...
    if (!(gCurrentViewportFlags & VIEWPORT_FLAG_HIDE_VERTICAL))
    {
        ...
        viewport_surface_draw_land_side_top(session, EDGE_TOPLEFT, height / 16, eax / 32, tileDescriptors[0],
                                            tileDescriptors[3]);
        viewport_surface_draw_land_side_top(session, EDGE_TOPRIGHT, height / 16, eax / 32, tileDescriptors[0],
                                            tileDescriptors[4]);
        viewport_surface_draw_land_side_bottom(session, EDGE_BOTTOMLEFT, height / 16, eax / 32, tileDescriptors[0],
                                               tileDescriptors[1]);
        viewport_surface_draw_land_side_bottom(session, EDGE_BOTTOMRIGHT, height / 16, eax / 32, tileDescriptors[0],
                                               tileDescriptors[2]);
    ...
    if (map_get_water_height(mapElement) > 0)
    {
        ...
        if (!gTrackDesignSaveMode)
        {
            ...
            viewport_surface_draw_water_side_top(session, EDGE_TOPLEFT, waterHeight / 16, eax / 32, tileDescriptors[0],
                                                 tileDescriptors[3]);
            viewport_surface_draw_water_side_top(session, EDGE_TOPRIGHT, waterHeight / 16, eax / 32, tileDescriptors[0],
                                                 tileDescriptors[4]);
            viewport_surface_draw_water_side_bottom(session, EDGE_BOTTOMLEFT, waterHeight / 16, eax / 32, tileDescriptors[0],
                                                    tileDescriptors[1]);
            viewport_surface_draw_water_side_bottom(session, EDGE_BOTTOMRIGHT, waterHeight / 16, eax / 32, tileDescriptors[0],
                                                    tileDescriptors[2]);
        }
    }
    ...
    if ((mapElement->properties.surface.ownership & 0x0F) && !gTrackDesignSaveMode)
    {
        ...
        for (sint32 i = 0; i < 4; i++)
        {
            ...
            sub_98197C(session, image_id, offset.x, offset.y, box_size.x, box_size.y, 9, local_height, box_offset.x,
                       box_offset.y, local_height + 1, rotation);
        }
    }
    ...
    default:
        ...
        paint_util_set_segment_support_height(session,
                                              SEGMENT_B4 | SEGMENT_B8 | SEGMENT_BC | SEGMENT_C0 | SEGMENT_C4 | SEGMENT_C8 |
