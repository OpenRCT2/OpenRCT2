    ...
    int32_t z)
{
    ...
    if (ride->type == RIDE_TYPE_MAZE)
    {
        ...
        viewport_set_visibility((0 & TRACK_ELEMENT_LOCATION_IS_UNDERGROUND) ? 1 : 3);
    ...
    else
    {
        ...
        _unkF440C5.direction = trackDirection;
        _currentTrackSelectionFlags |= TRACK_SELECTION_FLAG_TRACK;
        viewport_set_visibility((dynamic_cast<TrackPlaceActionResult*>(res.get())->GroundFlag & TRACK_ELEMENT_LOCATION_IS_UNDERGROUND) ? 1 : 3);
        if (_currentTrackSlopeEnd != 0)
            viewport_set_visibility(2);
