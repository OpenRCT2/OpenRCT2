...
#define MAIN_PARK_WIDGETS                                                                                                      \
    ...
    {                                                                                                                          \
        WWT_TAB, 1, 189, 219, 17, 43, 0x20000000 | SPR_TAB, STR_PARK_AWARDS_TAB_TIP                                            \
    } /* tab 7			  */
...
static rct_widget window_park_entrance_widgets[] = {
    ...
    //{ WWT_FLATBTN,			1,	205,	228,	97,		120,	SPR_BUY_CONSTRUCTION_RIGHTS,
    // STR_BUY_CONSTRUCTION_RIGHTS_TIP },	// buy construction rights
...
static rct_widget * window_park_page_widgets[] = { window_park_entrance_widgets, window_park_rating_widgets,
                                                   window_park_guests_widgets,   window_park_price_widgets,
                                                   window_park_stats_widgets,    window_park_objective_widgets,
...
static rct_window_event_list window_park_entrance_events = { window_park_entrance_close,
...
static rct_window_event_list window_park_rating_events = { NULL,
...
static rct_window_event_list window_park_guests_events = { NULL,
...
static rct_window_event_list window_park_price_events = { NULL,
...
static rct_window_event_list window_park_stats_events = { NULL,
...
static rct_window_event_list window_park_objective_events = { NULL,
...
static rct_window_event_list window_park_awards_events = { NULL,
...
static rct_window_event_list * window_park_page_events[] = { &window_park_entrance_events, &window_park_rating_events,
                                                             &window_park_guests_events,   &window_park_price_events,
                                                             &window_park_stats_events,    &window_park_objective_events,
...
static uint32 window_park_page_enabled_widgets[] = {
    (1 << WIDX_CLOSE) | (1 << WIDX_TAB_1) | (1 << WIDX_TAB_2) | (1 << WIDX_TAB_3) | (1 << WIDX_TAB_4) | (1 << WIDX_TAB_5) |
        (1 << WIDX_TAB_6) | (1 << WIDX_TAB_7) | (1 << WIDX_OPEN_OR_CLOSE) | (1 << WIDX_BUY_LAND_RIGHTS) |
        //(1 << WIDX_BUY_CONSTRUCTION_RIGHTS) |
        (1 << WIDX_LOCATE) | (1 << WIDX_RENAME) | (1 << WIDX_CLOSE_LIGHT) | (1 << WIDX_OPEN_LIGHT),
    ...
    (1 << WIDX_CLOSE) | (1 << WIDX_TAB_1) | (1 << WIDX_TAB_2) | (1 << WIDX_TAB_3) | (1 << WIDX_TAB_4) | (1 << WIDX_TAB_5) |
        (1 << WIDX_TAB_6) | (1 << WIDX_TAB_7) | (1 << WIDX_INCREASE_PRICE) | (1 << WIDX_DECREASE_PRICE),

    (1 << WIDX_CLOSE) | (1 << WIDX_TAB_1) | (1 << WIDX_TAB_2) | (1 << WIDX_TAB_3) | (1 << WIDX_TAB_4) | (1 << WIDX_TAB_5) |
        (1 << WIDX_TAB_6) | (1 << WIDX_TAB_7),

    (1 << WIDX_CLOSE) | (1 << WIDX_TAB_1) | (1 << WIDX_TAB_2) | (1 << WIDX_TAB_3) | (1 << WIDX_TAB_4) | (1 << WIDX_TAB_5) |
        (1 << WIDX_TAB_6) | (1 << WIDX_TAB_7) | (1 << WIDX_ENTER_NAME),

    (1 << WIDX_CLOSE) | (1 << WIDX_TAB_1) | (1 << WIDX_TAB_2) | (1 << WIDX_TAB_3) | (1 << WIDX_TAB_4) | (1 << WIDX_TAB_5) |
        (1 << WIDX_TAB_6) | (1 << WIDX_TAB_7)
};

static uint32 window_park_page_hold_down_widgets[] = { 0,
                                                       0,
                                                       0,

                                                       (1 << WIDX_INCREASE_PRICE) | (1 << WIDX_DECREASE_PRICE),
...
static void window_park_entrance_tool_update_land_rights(sint16 x, sint16 y)
{
    ...
    gLandRightsCost = game_do_command(gMapSelectPositionA.x, 0x4, gMapSelectPositionA.y, LandRightsMode ? 0x00E : 0x20F,
    ...
    case WIDX_BUY_LAND_RIGHTS:
        if (LandRightsMode)
        {
            if (x != MAP_LOCATION_NULL)
            {
                ...
                game_do_command(gMapSelectPositionA.x, 1, gMapSelectPositionA.y, 0x00E, GAME_COMMAND_BUY_LAND_RIGHTS,
        ...
        else
        {
            if (x != MAP_LOCATION_NULL)
            {
                ...
                game_do_command(gMapSelectPositionA.x, 1, gMapSelectPositionA.y, 0x20F, GAME_COMMAND_BUY_LAND_RIGHTS,
                                gMapSelectPositionB.x, gMapSelectPositionB.y);
            }
        }
        break;
...
static void window_park_entrance_invalidate(rct_window * w)
{
    ...
    if (gScenarioObjectiveType == OBJECTIVE_GUESTS_AND_RATING || (gParkFlags & PARK_FLAGS_NO_MONEY))
...
static void window_park_entrance_paint(rct_window * w, rct_drawpixelinfo * dpi)
{
    ...
    gfx_draw_string_centred_clipped(dpi, STR_BLACK_STRING, gCommonFormatArgs, COLOUR_BLACK,
                                    w->x + (labelWidget->left + labelWidget->right) / 2, w->y + labelWidget->top,
    ...
    if (zr != 0xFFFF)
    {
        ...
        if (w->viewport == NULL)
        {
            ...
            viewport_create(w, w->x + viewportWidget->left + 1, w->y + viewportWidget->top + 1,
                            ...
                            (viewportWidget->bottom - viewportWidget->top) - 2, 0, x, y, z,
                            w->viewport_focus_sprite.type & VIEWPORT_FOCUS_TYPE_MASK, -1);
            w->flags |= (1 << 2);
            window_invalidate(w);
        }
    }
...
static void window_park_rating_paint(rct_window * w, rct_drawpixelinfo * dpi)
{
    ...
    // Graph border
    gfx_fill_rect_inset(dpi, x + widget->left + 4, y + widget->top + 15, x + widget->right - 4, y + widget->bottom - 4,
...
static void window_park_guests_paint(rct_window * w, rct_drawpixelinfo * dpi)
{
    ...
    // Current value
    gfx_draw_string_left(dpi, STR_GUESTS_IN_PARK_LABEL, &gNumGuestsInPark, COLOUR_BLACK, x + widget->left + 3,
                         y + widget->top + 2);
...
static void window_park_price_invalidate(rct_window * w)
{
    ...
    if ((gParkFlags & PARK_FLAGS_NO_MONEY) || ((gParkFlags & PARK_FLAGS_PARK_FREE_ENTRY) && !gCheatsUnlockAllPrices))
    {
...
static void window_park_objective_mouseup(rct_window * w, int widgetIndex)
{
    ...
    case WIDX_ENTER_NAME:
        window_text_input_open(w, WIDX_ENTER_NAME, STR_ENTER_NAME, STR_PLEASE_ENTER_YOUR_NAME_FOR_THE_SCENARIO_CHART, 0, 0, 32);
        break;
    }
}
...
static void window_park_set_page(rct_window * w, int page)
{
    ...
    listen = 0;
    if (page == WINDOW_PARK_PAGE_ENTRANCE && w->page == WINDOW_PARK_PAGE_ENTRANCE && w->viewport != NULL &&
...
static void window_park_draw_tab_images(rct_drawpixelinfo * dpi, rct_window * w)
{
    ...
    if (!(w->disabled_widgets & (1 << WIDX_TAB_3)))
    {
        ...
        gfx_draw_sprite(dpi, sprite_idx, w->x + (w->widgets[WIDX_TAB_3].left + w->widgets[WIDX_TAB_3].right) / 2,
