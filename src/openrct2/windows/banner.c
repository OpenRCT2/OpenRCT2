...
static const rct_string_id BannerColouredTextFormats[] = {
    STR_TEXT_COLOR_BLACK,     STR_TEXT_COLOR_GREY,         STR_TEXT_COLOR_WHITE,    STR_TEXT_COLOR_RED,
    STR_TEXT_COLOR_GREEN,     STR_TEXT_COLOR_YELLOW,       STR_TEXT_COLOR_TOPAZ,    STR_TEXT_COLOR_CELADON,
    STR_TEXT_COLOR_BABYBLUE,  STR_TEXT_COLOR_PALELAVENDER, STR_TEXT_COLOR_PALEGOLD, STR_TEXT_COLOR_LIGHTPINK,
    STR_TEXT_COLOR_PEARLAQUA, STR_TEXT_COLOR_PALESILVER,
};
...
static rct_window_event_list window_banner_events = { NULL,
...
void window_banner_open(rct_windownumber number)
{
    ...
    w->enabled_widgets = (1 << WIDX_CLOSE) | (1 << WIDX_BANNER_TEXT) | (1 << WIDX_BANNER_NO_ENTRY) |
    ...
    viewport_create(w, w->x + viewportWidget->left + 1, w->y + viewportWidget->top + 1,
                    (viewportWidget->right - viewportWidget->left) - 2, (viewportWidget->bottom - viewportWidget->top) - 2, 0,
                    view_x, view_y, view_z, 0, -1);
...
static void window_banner_mouseup(rct_window * w, int widgetIndex)
{
    ...
    case WIDX_BANNER_DEMOLISH:
        game_do_command(x, 1, y, map_element->base_height | (map_element->properties.banner.position << 8),
                        GAME_COMMAND_REMOVE_BANNER, 0, 0);
        break;
    case WIDX_BANNER_TEXT:
        window_text_input_open(w, WIDX_BANNER_TEXT, STR_BANNER_TEXT, STR_ENTER_BANNER_TEXT, gBanners[w->number].string_idx, 0,
                               32);
        break;
    case WIDX_BANNER_NO_ENTRY:
        textinput_cancel();
        game_do_command(1, GAME_COMMAND_FLAG_APPLY, w->number, banner->colour, GAME_COMMAND_SET_BANNER_STYLE,
                        banner->text_colour, banner->flags ^ BANNER_FLAG_NO_ENTRY);
        break;
    }
}
...
static void window_banner_mousedown(int widgetIndex, rct_window * w, rct_widget * widget)
{
    ...
    case WIDX_TEXT_COLOUR_DROPDOWN_BUTTON:
        ...
        window_dropdown_show_text_custom_width(widget->left + w->x, widget->top + w->y, widget->bottom - widget->top + 1,
                                               w->colours[1], DROPDOWN_FLAG_STAY_OPEN, 13, widget->right - widget->left - 3);
...
static void window_banner_dropdown(rct_window * w, int widgetIndex, int dropdownIndex)
{
    ...
    case WIDX_MAIN_COLOUR:
        ...
        game_do_command(1, GAME_COMMAND_FLAG_APPLY, w->number, dropdownIndex, GAME_COMMAND_SET_BANNER_STYLE,
                        banner->text_colour, banner->flags);
        break;
    case WIDX_TEXT_COLOUR_DROPDOWN_BUTTON:
        if (dropdownIndex == -1)
            break;

        game_do_command(1, GAME_COMMAND_FLAG_APPLY, w->number, banner->colour, GAME_COMMAND_SET_BANNER_STYLE, dropdownIndex + 1,
                        banner->flags);
        break;
    }
}
...
static void window_banner_textinput(rct_window * w, int widgetIndex, char * text)
{
    ...
    {
        game_do_command(1, GAME_COMMAND_FLAG_APPLY, w->number, *((int *)(text + 0)), GAME_COMMAND_SET_BANNER_NAME,
                        *((int *)(text + 8)), *((int *)(text + 4)));
        game_do_command(2, GAME_COMMAND_FLAG_APPLY, w->number, *((int *)(text + 12)), GAME_COMMAND_SET_BANNER_NAME,
                        *((int *)(text + 20)), *((int *)(text + 16)));
        game_do_command(0, GAME_COMMAND_FLAG_APPLY, w->number, *((int *)(text + 24)), GAME_COMMAND_SET_BANNER_NAME,
                        *((int *)(text + 32)), *((int *)(text + 28)));
    }
}
