...
void window_scenery_open()
{
    ...
    window = window_create(gScreenWidth - WINDOW_SCENERY_WIDTH, 0x1D, WINDOW_SCENERY_WIDTH, WINDOW_SCENERY_HEIGHT,
...
void window_scenery_paint(rct_window * w, rct_drawpixelinfo * dpi)
{
    ...
    gfx_draw_sprite(dpi, imageId, w->x + window_scenery_widgets[selectedWidgetId].left,
                    w->y + window_scenery_widgets[selectedWidgetId].top, selectedWidgetId);
    ...
    {
        // -14
        gfx_draw_string_right(dpi, STR_COST_LABEL, gCommonFormatArgs, COLOUR_BLACK, w->x + w->width - 0x1A,
                              w->y + w->height - 13);
    }

    set_format_arg(0, rct_string_id, sceneryEntry->name);
    gfx_draw_string_left_clipped(dpi, STR_BLACK_STRING, gCommonFormatArgs, COLOUR_BLACK, w->x + 3, w->y + w->height - 13,
                                 w->width - 19);
}
...
void window_scenery_scrollpaint(rct_window * w, rct_drawpixelinfo * dpi, int scrollIndex)
{
    ...
    while ((currentSceneryGlobalId = window_scenery_tab_entries[tabIndex][sceneryTabItemIndex]) != -1)
    {
        ...
        if (gWindowSceneryPaintEnabled == 1)
        {
            ...
            {
                gfx_fill_rect_inset(dpi, left, top, left + SCENERY_BUTTON_WIDTH - 1, top + SCENERY_BUTTON_HEIGHT - 1,
