...
static rct_window_event_list window_land_events = { window_land_close,
                                                    window_land_mouseup,
                                                    NULL,
...
static char window_land_floor_texture_order[] = { TERRAIN_SAND_DARK,    TERRAIN_SAND_LIGHT, TERRAIN_DIRT,
                                                  TERRAIN_GRASS_CLUMPS, TERRAIN_GRASS,      TERRAIN_ROCK,
                                                  TERRAIN_SAND,         TERRAIN_MARTIAN,    TERRAIN_CHECKERBOARD,
                                                  TERRAIN_ICE,          TERRAIN_GRID_RED,   TERRAIN_GRID_YELLOW,
                                                  TERRAIN_GRID_BLUE,    TERRAIN_GRID_GREEN };

static char window_land_wall_texture_order[] = {
    TERRAIN_EDGE_ROCK, TERRAIN_EDGE_WOOD_RED, TERRAIN_EDGE_WOOD_BLACK, TERRAIN_EDGE_ICE, 0, 0
};
...
static void window_land_mousedown(int widgetIndex, rct_window * w, rct_widget * widget)
{
    ...
    case WIDX_FLOOR:
        ...
        window_dropdown_show_image(w->x + widget->left, w->y + widget->top, widget->bottom - widget->top, w->colours[2], 0, 14,
                                   47, 36, gAppropriateImageDropdownItemsPerRow[14]);
    ...
    case WIDX_WALL:
        ...
        window_dropdown_show_image(w->x + widget->left, w->y + widget->top, widget->bottom - widget->top, w->colours[2], 0, 4,
                                   47, 36, gAppropriateImageDropdownItemsPerRow[4]);
        gDropdownDefaultIndex = defaultIndex;
        break;
    case WIDX_PREVIEW:
        window_land_inputsize(w);
...
static void window_land_dropdown(rct_window * w, int widgetIndex, int dropdownIndex)
{
    ...
    case WIDX_FLOOR:
        ...
        type =
            (dropdownIndex == -1) ? _selectedFloorTexture : (uint32)gDropdownItemsArgs[dropdownIndex] - SPR_FLOOR_TEXTURE_GRASS;
    ...
    case WIDX_WALL:
        ...
        type = (dropdownIndex == -1) ? _selectedWallTexture : (uint32)gDropdownItemsArgs[dropdownIndex] - SPR_WALL_TEXTURE_ROCK;
...
static void window_land_invalidate(rct_window * w)
{
    ...
    window_land_widgets[WIDX_PREVIEW].image = gLandToolSize <= 7 ? SPR_LAND_TOOL_SIZE_0 + gLandToolSize : 0xFFFFFFFF;
}
