...
static rct_widget window_editor_inventions_list_drag_widgets[] = { { WWT_IMGBTN, 0, 0, 149, 0, 13, STR_NONE, STR_NONE },
                                                                   { WIDGETS_END } };
...
// 0x0098177C
static rct_window_event_list window_editor_inventions_list_events = { window_editor_inventions_list_close,
...
// 0x009817EC
static rct_window_event_list window_editor_inventions_list_drag_events = { NULL,
                                                                           NULL,
                                                                           NULL,
                                                                           NULL,
                                                                           NULL,
...
static const rct_string_id EditorInventionsResearchCategories[] = {
    STR_RESEARCH_NEW_TRANSPORT_RIDES,     STR_RESEARCH_NEW_GENTLE_RIDES, STR_RESEARCH_NEW_ROLLER_COASTERS,
    STR_RESEARCH_NEW_THRILL_RIDES,        STR_RESEARCH_NEW_WATER_RIDES,  STR_RESEARCH_NEW_SHOPS_AND_STALLS,
...
{
    return (researchItem->entryIndex &
            (RESEARCH_ENTRY_FLAG_RIDE_ALWAYS_RESEARCHED | RESEARCH_ENTRY_FLAG_SCENERY_SET_ALWAYS_RESEARCHED)) != 0;
}
    ...
    for (rct_research_item * research = gResearchItems; research->entryIndex != RESEARCHED_ITEMS_END; research++)
    {
        ...
        if (!(ride_entry->flags & RIDE_ENTRY_FLAG_SEPARATE_RIDE))
        {
            ...
            for (uint8 rideType = 0; rideType < object_entry_group_counts[OBJECT_TYPE_RIDE]; rideType++)
            {
                ...
                if (ride_base_type == master_ride->ride_type[0] || ride_base_type == master_ride->ride_type[1] ||
        ...
        if (!master_found)
        {
            ...
            if (!(gEditorSelectedObjects[OBJECT_TYPE_RIDE][object_index] & (1 << 0)))
                continue;
            if (ride_base_type != ride_entry->ride_type[0] && ride_base_type != ride_entry->ride_type[1] &&
...
static void research_items_make_all_unresearched()
{
    ...
    do
    {
        ...
        for (researchItem = gResearchItems; researchItem->entryIndex != RESEARCHED_ITEMS_SEPARATOR; researchItem++)
        {
            ...
            if (nextResearchItem->entryIndex == RESEARCHED_ITEMS_SEPARATOR ||
...
static rct_research_item * window_editor_inventions_list_get_item_from_scroll_y(int scrollIndex, int y)
{
    ...
    for (; researchItem->entryIndex != RESEARCHED_ITEMS_SEPARATOR && researchItem->entryIndex != RESEARCHED_ITEMS_END;
         researchItem++)
    {
        y -= 10;
        if (y < 0)
...
void window_editor_inventions_list_open()
{
    ...
    w = window_create_centred(600, 400, &window_editor_inventions_list_events, WC_EDITOR_INVENTION_LIST, WF_NO_SCROLLING);
    w->widgets         = window_editor_inventions_list_widgets;
    w->enabled_widgets = (1 << WIDX_CLOSE) | (1 << WIDX_TAB_1) | (1 << WIDX_RANDOM_SHUFFLE) | (1 << WIDX_MOVE_ITEMS_TO_BOTTOM) |
...
static void window_editor_inventions_list_drag_open(rct_research_item * researchItem)
{
    ...
    w = window_create(gTooltipCursorX - (stringWidth / 2), gTooltipCursorY - 7, stringWidth, 14,
