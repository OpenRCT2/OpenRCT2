...
static rct_window_event_list window_editor_bottom_toolbar_events = { NULL,
...
void window_editor_bottom_toolbar_open()
{
    ...
    window = window_create(0, gScreenHeight - 32, gScreenWidth, 32, &window_editor_bottom_toolbar_events, WC_BOTTOM_TOOLBAR,
...
{
    colour_scheme_update_by_class(w, (gScreenFlags & SCREEN_FLAGS_SCENARIO_EDITOR) ? WC_EDITOR_SCENARIO_BOTTOM_TOOLBAR
...
void window_editor_bottom_toolbar_paint(rct_window * w, rct_drawpixelinfo * dpi)
{
    ...
    if (!(gScreenFlags & SCREEN_FLAGS_TRACK_MANAGER))
    {
        ...
        {
            gfx_filter_rect(dpi, window_editor_bottom_toolbar_widgets[WIDX_PREVIOUS_IMAGE].left + w->x,
        ...
        {
            gfx_filter_rect(dpi, window_editor_bottom_toolbar_widgets[WIDX_NEXT_IMAGE].left + w->x,
    ...
    if (!(gScreenFlags & SCREEN_FLAGS_TRACK_MANAGER))
    {
        ...
        if (drawPreviousButton)
        {
            gfx_fill_rect_inset(dpi, window_editor_bottom_toolbar_widgets[WIDX_PREVIOUS_IMAGE].left + 1 + w->x,
                                ...
                                window_editor_bottom_toolbar_widgets[WIDX_PREVIOUS_IMAGE].bottom - 1 + w->y, w->colours[1],
                                INSET_RECT_F_30);
        }

        if ((drawPreviousButton || drawNextButton) && gS6Info.editor_step != EDITOR_STEP_ROLLERCOASTER_DESIGNER)
        {
            gfx_fill_rect_inset(dpi, window_editor_bottom_toolbar_widgets[WIDX_NEXT_IMAGE].left + 1 + w->x,
                                ...
                                window_editor_bottom_toolbar_widgets[WIDX_NEXT_IMAGE].bottom - 1 + w->y, w->colours[1],
                                INSET_RECT_F_30);
        }

        short stateX = (window_editor_bottom_toolbar_widgets[WIDX_PREVIOUS_IMAGE].right +
                        window_editor_bottom_toolbar_widgets[WIDX_NEXT_IMAGE].left) /
                           2 +
                       w->x;
        short stateY = w->height - 0x0C + w->y;
        gfx_draw_string_centred(dpi, EditorStepNames[gS6Info.editor_step], stateX, stateY,
                                NOT_TRANSLUCENT(w->colours[2]) | COLOUR_FLAG_OUTLINE, 0);

        if (drawPreviousButton)
        {
            gfx_draw_sprite(dpi, SPR_PREVIOUS, window_editor_bottom_toolbar_widgets[WIDX_PREVIOUS_IMAGE].left + 6 + w->x,
        ...
        if ((drawPreviousButton || drawNextButton) && gS6Info.editor_step != EDITOR_STEP_ROLLERCOASTER_DESIGNER)
        {
            gfx_draw_sprite(dpi, SPR_NEXT, window_editor_bottom_toolbar_widgets[WIDX_NEXT_IMAGE].right - 29 + w->x,
