...
static void window_title_editor_mouseup(rct_window * w, int widgetIndex)
{
    ...
    case WIDX_TITLE_EDITOR_DUPLICATE_BUTTON:
        ...
        {
            window_text_input_open(w, widgetIndex, STR_TITLE_EDITOR_ACTION_DUPLICATE, STR_TITLE_EDITOR_ENTER_NAME_FOR_SEQUENCE,
                                   STR_STRING, (uintptr_t)_editingTitleSequence->Name, 64);
        }
        break;
    ...
    case WIDX_TITLE_EDITOR_RENAME_SAVE:
        if (window_title_editor_check_can_edit())
        {
            if (w->selected_list_item != -1)
            {
                window_text_input_open(w, widgetIndex, STR_FILEBROWSER_RENAME_SAVE_TITLE, STR_TITLE_EDITOR_ENTER_NAME_FOR_SAVE,
                                       STR_STRING, (uintptr_t)_editingTitleSequence->Saves[w->selected_list_item], 52 - 1);
            }
        }
        break;
...
static void window_title_editor_scrollgetsize(rct_window * w, int scrollIndex, int * width, int * height)
{
    ...
    *height = (int)(lineCount * ROW_HEIGHT);
    int i   = *height - window_title_editor_widgets[WIDX_TITLE_EDITOR_LIST].bottom +
...
static void window_title_editor_draw_tab_images(rct_drawpixelinfo * dpi, rct_window * w)
{
    for (int i = 0; i < WINDOW_TITLE_EDITOR_TAB_COUNT; i++)
    {
        ...
        gfx_draw_sprite(dpi, spriteId, w->x + w->widgets[WIDX_TITLE_EDITOR_PRESETS_TAB + i].left + x,
...
static void window_title_editor_add_park_callback(int result, const utf8 * path)
{
    ...
    if (save_filename_exists(filename))
    {
        ...
        window_text_input_open(w, WIDX_TITLE_EDITOR_RENAME_SAVE, STR_FILEBROWSER_RENAME_SAVE_TITLE, STR_ERROR_EXISTING_NAME,
                               STR_STRING, (uintptr_t)_renameSavePath, 52 - 1);
        return;
    }
