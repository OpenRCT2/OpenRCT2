...
static rct_widget window_themes_widgets[] = {
    ...
    { WWT_DROPDOWN_BUTTON, 1, 10, 100, 82, 93, STR_TITLE_EDITOR_ACTION_DUPLICATE,
      STR_THEMES_ACTION_DUPLICATE_TIP },                                                                 // Duplicate button
...
static int window_themes_tab_animation_loops[]   = { 32, 32, 1, 1, 64, 32, 8, 14, 38 };
static int window_themes_tab_animation_divisor[] = { 4, 4, 1, 1, 4, 2, 2, 2, 2 };
static int window_themes_tab_sprites[]           = { SPR_TAB_PAINT_0,
...
static rct_windowclass window_themes_tab_1_classes[] = { WC_TOP_TOOLBAR,
                                                         WC_BOTTOM_TOOLBAR,
...
static rct_windowclass window_themes_tab_2_classes[] = { WC_PARK_INFORMATION, WC_FINANCES, WC_NEW_CAMPAIGN,
                                                         WC_RESEARCH,         WC_MAP,      WC_VIEWPORT,
                                                         WC_RECENT_NEWS };

static rct_windowclass window_themes_tab_3_classes[] = { WC_LAND,
                                                         WC_WATER,
                                                         WC_CLEAR_SCENERY,
                                                         WC_LAND_RIGHTS,
                                                         WC_SCENERY,
...
static rct_windowclass window_themes_tab_4_classes[] = { WC_RIDE,  WC_RIDE_LIST,  WC_PEEP,  WC_GUEST_LIST,
                                                         WC_STAFF, WC_STAFF_LIST, WC_BANNER };

static rct_windowclass window_themes_tab_5_classes[] = {
    WC_EDITOR_OBJECT_SELECTION, WC_EDITOR_INVENTION_LIST, WC_EDITOR_SCENARIO_OPTIONS, WC_EDTIOR_OBJECTIVE_OPTIONS, WC_MAPGEN,
    WC_MANAGE_TRACK_DESIGN,     WC_INSTALL_TRACK
};
...
static rct_windowclass window_themes_tab_7_classes[] = {
    WC_SAVE_PROMPT,         WC_DEMOLISH_RIDE_PROMPT,      WC_FIRE_PROMPT,
    WC_TRACK_DELETE_PROMPT, WC_LOADSAVE_OVERWRITE_PROMPT, WC_NETWORK_STATUS,
};
...
static void window_themes_draw_tab_images(rct_drawpixelinfo * dpi, rct_window * w)
{
    for (int i = 0; i < WINDOW_THEMES_TAB_COUNT; i++)
    {
        ...
        gfx_draw_sprite(dpi, sprite_idx, w->x + w->widgets[WIDX_THEMES_SETTINGS_TAB + i].left,
...
void window_themes_open()
{
    ...
    window->enabled_widgets = (1 << WIDX_THEMES_CLOSE) | (1 << WIDX_THEMES_SETTINGS_TAB) | (1 << WIDX_THEMES_MAIN_UI_TAB) |
                              ...
                              (1 << WIDX_THEMES_PRESETS_DROPDOWN) | (1 << WIDX_THEMES_DUPLICATE_BUTTON) |
                              (1 << WIDX_THEMES_DELETE_BUTTON) | (1 << WIDX_THEMES_RENAME_BUTTON) |
                              (1 << WIDX_THEMES_RCT1_RIDE_LIGHTS) | (1 << WIDX_THEMES_RCT1_PARK_LIGHTS) |
...
static void window_themes_mouseup(rct_window * w, int widgetIndex)
{
    ...
    case WIDX_THEMES_DUPLICATE_BUTTON:;
        ...
        window_text_input_open(w, widgetIndex, STR_TITLE_EDITOR_ACTION_DUPLICATE, STR_THEMES_PROMPT_ENTER_THEME_NAME,
                               STR_STRING, (uintptr_t)activeThemeName, 64);
        break;
    ...
    case WIDX_THEMES_RENAME_BUTTON:
        ...
        else
        {
            activeAvailableThemeIndex = theme_manager_get_active_available_theme_index();
            activeThemeName           = theme_manager_get_available_theme_name(activeAvailableThemeIndex);
            window_text_input_open(w, widgetIndex, STR_TRACK_MANAGE_RENAME, STR_THEMES_PROMPT_ENTER_THEME_NAME, STR_STRING,
...
void window_themes_scrollmousedown(rct_window * w, int scrollIndex, int x, int y)
{
    if (y / _row_height < get_colour_scheme_tab_count())
    {
        ...
        if (_colour_index_2 < numColours)
        {
            if (x >= _button_offset_x && x < _button_offset_x + 12 * 6 && y2 >= _button_offset_y && y2 < _button_offset_y + 11)
            {
                ...
                else
                {
                    ...
                    window_themes_widgets[WIDX_THEMES_COLOURBTN_MASK].top = _colour_index_1 * _row_height + _button_offset_y -
                                                                            w->scrolls[0].v_top +
                                                                            window_themes_widgets[WIDX_THEMES_LIST].top;
            ...
            else if (x >= _button_offset_x && x < _button_offset_x + 12 * 6 - 1 && y2 >= _check_offset_y &&
...
void window_themes_invalidate(rct_window * w)
{
    ...
    else if (_selected_tab == WINDOW_THEMES_TAB_FEATURES)
    {
        ...
        widget_set_checkbox_value(w, WIDX_THEMES_RCT1_SCENARIO_FONT,
...
void window_themes_paint(rct_window * w, rct_drawpixelinfo * dpi)
{
    ...
    if (_selected_tab == WINDOW_THEMES_TAB_SETTINGS)
    {
        ...
        gfx_draw_string_left_clipped(dpi, STR_STRING, gCommonFormatArgs, w->colours[1],
                                     ...
                                     w->x + window_themes_widgets[WIDX_THEMES_PRESETS_DROPDOWN].left -
        ...
        if (y + _row_height >= dpi->y)
        {
            if (i + 1 < get_colour_scheme_tab_count())
            {
                ...
                if (colour & COLOUR_FLAG_TRANSLUCENT)
                {
                    ...
                    gfx_filter_rect(dpi, 0, y + _row_height - 2, window_themes_widgets[WIDX_THEMES_LIST].right,
                                    y + _row_height - 2, windowPalette.highlight);
                    gfx_filter_rect(dpi, 0, y + _row_height - 1, window_themes_widgets[WIDX_THEMES_LIST].right,
                                    y + _row_height - 1, windowPalette.shadow);
                }
                else
                {
                    colour = ColourMapA[w->colours[1]].mid_dark;
                    gfx_fill_rect(dpi, 0, y + _row_height - 2, window_themes_widgets[WIDX_THEMES_LIST].right,
                                  y + _row_height - 2, colour);
                    colour = ColourMapA[w->colours[1]].lightest;
                    gfx_fill_rect(dpi, 0, y + _row_height - 1, window_themes_widgets[WIDX_THEMES_LIST].right,
                                  y + _row_height - 1, colour);
                }
            }
            ...
            for (uint8 j = 0; j < numColours; j++)
            {
                ...
                gfx_fill_rect_inset(dpi, _button_offset_x + 12 * j, y + _check_offset_y, _button_offset_x + 12 * j + 9,
                                    y + _check_offset_y + 10, w->colours[1], INSET_RECT_F_E0);
                if (colour & COLOUR_FLAG_TRANSLUCENT)
                {
                    gCurrentFontSpriteBase = FONT_SPRITE_BASE_MEDIUM_DARK;
                    gfx_draw_string(dpi, (char *)CheckBoxMarkString, w->colours[1] & 0x7F, _button_offset_x + 12 * j,
                                    y + _check_offset_y);
                }
            }
        }
