...
int gAppropriateImageDropdownItemsPerRow[] = { 1, 1, 1, 1, 2, 2, 3, 3, 4, 3, 5, 4, 4, 5, 5, 5, 4,
...
static rct_widget window_dropdown_widgets[] = {
    { WWT_IMGBTN, 0, 0, 0, 0, 0, SPR_NONE, STR_NONE }, { WIDGETS_END },
};
...
static rct_window_event_list window_dropdown_events = { NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
                                                        NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
                                                        NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, window_dropdown_paint,
                                                        NULL };
...
void window_dropdown_show_text_custom_width(int x, int y, int extray, uint8 colour, uint8 flags, int num_items, int width)
{
    ...
    // Create the window
    w = window_create(x, y + extray, window_dropdown_widgets[WIDX_BACKGROUND].right + 1,
                      window_dropdown_widgets[WIDX_BACKGROUND].bottom + 1, &window_dropdown_events, WC_DROPDOWN,
                      WF_STICK_TO_FRONT);
 ...
 * @param numColumns (bl)
 */
void window_dropdown_show_image(int x, int y, int extray, uint8 colour, uint8 flags, int numItems, int itemWidth,
                                int itemHeight, int numColumns)
{
    int          width, height;
    rct_window * w;
    ...
    for (int i = 0; i < gDropdownNumItems; i++)
    {
        ...
        else
        {
            ...
            if (item == (uint16)-1 || item == (uint16)-2)
            {
                ...
                gfx_draw_sprite(dpi, image, w->x + 2 + (cell_x * _dropdown_item_width),
            ...
            else
            {
                ...
                // Draw item string
                gfx_draw_string_left_clipped(dpi, item, (void *)(&gDropdownItemsArgs[i]), colour,
                                           ...
                                           uint32 availableColours)
{
    ...
    // Show dropdown
    window_dropdown_show_image(w->x + widget->left, w->y + widget->top, widget->bottom - widget->top + 1, dropdownColour,
                               DROPDOWN_FLAG_STAY_OPEN, numItems, 12, 12, gAppropriateImageDropdownItemsPerRow[numItems]);
