...
// rct2: 0x00981BBC
const rct_xy16 MiniMapOffsets[] = { { 256 - 8, 0 }, { 512 - 8, 256 }, { 256 - 8, 512 }, { 0 - 8, 256 } };
...
static rct_window_event_list window_map_events = { window_map_close,
                                                   window_map_mouseup,
                                                   window_map_resize,
...
void window_map_open()
{
    ...
    w->enabled_widgets = (1 << WIDX_CLOSE) | (1 << WIDX_PEOPLE_TAB) | (1 << WIDX_RIDES_TAB) | (1 << WIDX_MAP_SIZE_SPINNER) |
                         ...
                         (1 << WIDX_LAND_OWNED_CHECKBOX) | (1 << WIDX_CONSTRUCTION_RIGHTS_OWNED_CHECKBOX) |
                         (1 << WIDX_LAND_SALE_CHECKBOX) | (1 << WIDX_CONSTRUCTION_RIGHTS_SALE_CHECKBOX) |
                         (1 << WIDX_BUILD_PARK_ENTRANCE) | (1 << WIDX_ROTATE_90) | (1 << WIDX_PEOPLE_STARTING_POSITION) |
                         (1 << WIDX_MAP_GENERATOR);

    w->hold_down_widgets = (1 << WIDX_MAP_SIZE_SPINNER_UP) | (1 << WIDX_MAP_SIZE_SPINNER_DOWN);
...
static void window_map_tooldrag(rct_window * w, int widgetIndex, int x, int y)
{
    ...
    case WIDX_SET_LAND_RIGHTS:
        if (gMapSelectFlags & MAP_SELECT_FLAG_ENABLE)
        {
            ...
            game_do_command(gMapSelectPositionA.x, GAME_COMMAND_FLAG_APPLY, gMapSelectPositionA.y, _activeTool,
...
static void window_map_scrollmousedown(rct_window * w, int scrollIndex, int x, int y)
{
    ...
    if (land_tool_is_active())
    {
        ...
        game_do_command(gMapSelectPositionA.x, GAME_COMMAND_FLAG_APPLY, gMapSelectPositionA.y,
...
static void window_map_invalidate(rct_window * w)
{
    ...
    if ((gScreenFlags & SCREEN_FLAGS_SCENARIO_EDITOR) || gCheatsSandboxMode)
    {
        ...
        if ((gInputFlags & INPUT_FLAG_TOOL_ACTIVE) && gCurrentToolWidget.window_classification == WC_MAP &&
        ...
        if ((gInputFlags & INPUT_FLAG_TOOL_ACTIVE) && gCurrentToolWidget.window_classification == WC_MAP)
        {
            ...
            { // if in set land rights mode: show land tool buttons + modes
                ...
                w->widgets[WIDX_LAND_TOOL].image = gLandToolSize <= 7 ? SPR_LAND_TOOL_SIZE_0 + gLandToolSize : 0xFFFFFFFF;
            }
        }
        else
        {
...
static void window_map_place_park_entrance_tool_down(int x, int y)
{
    ...
    if (mapX != MAP_LOCATION_NULL)
    {
        ...
        money32 price          = game_do_command(mapX, GAME_COMMAND_FLAG_APPLY | (direction << 8), mapY, mapZ,
                                        GAME_COMMAND_PLACE_PARK_ENTRANCE, 0, 0);
