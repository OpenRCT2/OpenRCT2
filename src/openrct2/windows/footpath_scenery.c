...
static rct_widget window_footpath_scenery_widgets[] = {
    ...
    { WWT_CAPTION, 0, 1, WW - 2, 1, 14, STR_PATH_SCENERY_TITLE, STR_WINDOW_TITLE_TIP },
    { WWT_CLOSEBOX, 0, WW - 13, WW - 3, 2, 13, STR_CLOSE_X, STR_CLOSE_WINDOW_TIP },

    { WWT_SCROLL, 1, PADDING, LIST_WIDTH, YPL(1), OHPL(9), SCROLL_VERTICAL, STR_NONE },
    { WWT_FLATBTN, 1, LIST_BUTTON_X, WIDGET_WIDTH, YPL(1), FHPL(1), SPR_DEMOLISH, STR_PATH_SCENERY_DELETE_RULES },

    { WWT_DROPDOWN, 2, PADDING, DROPDOWN_RIGHT, YPL(11), OHPL(11), STR_STRINGID, STR_NONE },
    { WWT_DROPDOWN_BUTTON, 2, DROPDOWN_RIGHT - 11, DROPDOWN_RIGHT - 1, YPL(11) + 1, OHPL(11) - 1, STR_DROPDOWN_GLYPH,
      STR_NONE },

    { WWT_CLOSEBOX, 1, APPEND_BUTTON_LEFT, APPEND_BUTTON_RIGHT, YPL(13), HPL(13), STR_PATH_SCENERY_APPEND, STR_NONE },
...
static void window_footpath_scenery_invalidate(rct_window * w)
{
    ...
    {
        set_format_arg(0, rct_string_id, STR_PATH_SCENERY_NOTHING);
    }
}
...
static void window_footpath_scenery_paint(rct_window * w, rct_drawpixelinfo * dpi)
{
    ...
    gfx_draw_string_left(dpi, STR_PATH_SCENERY_ON_PATH, NULL, COLOUR_BLACK, w->x + PADDING, w->y + YPL(10) + 3);
}
...
static void window_footpath_scenery_path_scenery_dialog(rct_window * w, rct_widget * widget, int widgetIndex)
{
    ...
    for (uint16 sceneryId = 0x100; sceneryId < 0x10F; sceneryId++)
    {
...
static void window_footpath_scenery_object_dropdown(rct_window * w, int widgetIndex, int dropdownIndex)
{
    ...
    else if (dropdownIndex != -1)
    {
        ...
        for (sceneryId = 0x100; sceneryId < 0x10F; sceneryId++)
        {
            int pathBitIndex = sceneryId - 0x100;
...
static void window_footpath_scenery_scrollpaint(rct_window * w, rct_drawpixelinfo * dpi, int scrollIndex)
{
    ...
    while (c != NULL)
    {
        ...
        {
            rct_string_id nothing = STR_PATH_SCENERY_NOTHING;
