...
static rct_widget window_ride_list_widgets[] = {
    ...
    { WWT_DROPDOWN_BUTTON, 1, 262, 272, 47, 56, STR_DROPDOWN_GLYPH,
      STR_RIDE_LIST_INFORMATION_TYPE_TIP },                                              // information type dropdown button
...
static rct_window_event_list window_ride_list_events = { NULL,
...
rct_string_id ride_info_type_string_mapping[DROPDOWN_LIST_COUNT] = { STR_STATUS,
                                                                     STR_POPULARITY,
                                                                     STR_SATISFACTION,
                                                                     STR_PROFIT,
...
static const rct_string_id ride_list_statusbar_count_strings[PAGE_COUNT] = {
    STR_NUMBER_RIDES, STR_NUMBER_SHOPS_AND_STALLS, STR_NUMBER_RESTROOMS_AND_INFORMATION_KIOSKS,
};

bool ride_info_type_money_mapping[DROPDOWN_LIST_COUNT] = { false, false, false, true,  false, true,  false, false,
...
void window_ride_list_open()
{
    ...
    if (window == NULL)
    {
        ...
        window->enabled_widgets = (1 << WIDX_CLOSE) | (1 << WIDX_OPEN_CLOSE_ALL) | (1 << WIDX_CURRENT_INFORMATION_TYPE) |
                                  (1 << WIDX_INFORMATION_TYPE_DROPDOWN) | (1 << WIDX_SORT) | (1 << WIDX_TAB_1) |
                                  (1 << WIDX_TAB_2) | (1 << WIDX_TAB_3) | (1 << WIDX_CLOSE_LIGHT) | (1 << WIDX_OPEN_LIGHT);
...
static void window_ride_list_mousedown(int widgetIndex, rct_window * w, rct_widget * widget)
{
    ...
    else if (widgetIndex == WIDX_INFORMATION_TYPE_DROPDOWN)
    {
        ...
        window_dropdown_show_text_custom_width(w->x + widget->left, w->y + widget->top, widget->bottom - widget->top,
                                               w->colours[1], DROPDOWN_FLAG_STAY_OPEN, numItems,
...
static void window_ride_list_paint(rct_window * w, rct_drawpixelinfo * dpi)
{
    ...
    gfx_draw_string_left(dpi, ride_list_statusbar_count_strings[w->page], &w->no_list_items, COLOUR_BLACK, w->x + 4,
