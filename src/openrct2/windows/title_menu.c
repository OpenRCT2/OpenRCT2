...
static rct_window_event_list window_title_menu_events = { NULL,
...
void window_title_menu_open()
{
    ...
    window = window_create(0, gScreenHeight - 142, 0, 100, &window_title_menu_events, WC_TITLE_MENU,
                           WF_STICK_TO_BACK | WF_TRANSPARENT | WF_NO_BACKGROUND);
    window->widgets         = window_title_menu_widgets;
    window->enabled_widgets = ((1 << WIDX_START_NEW_GAME) | (1 << WIDX_CONTINUE_SAVED_GAME) |
...
static void window_title_menu_mousedown(int widgetIndex, rct_window * w, rct_widget * widget)
{
    if (widgetIndex == WIDX_GAME_TOOLS)
    {
        ...
        window_dropdown_show_text(w->x + widget->left, w->y + widget->top, widget->bottom - widget->top + 1,
                                  TRANSLUCENT(w->colours[0]), DROPDOWN_FLAG_STAY_OPEN, 4);
    }
}
