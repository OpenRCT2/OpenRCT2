...
typedef enum { DDIDX_MULTIPLAYER = 0 } TOP_TOOLBAR_NETWORK_DDIDX;
...
static const int left_aligned_widgets_order[] = {
    WIDX_PAUSE,     WIDX_FASTFORWARD, WIDX_FILE_MENU, WIDX_NETWORK,   WIDX_CHEATS, WIDX_DEBUG,

    WIDX_SEPARATOR,

    WIDX_ZOOM_OUT,  WIDX_ZOOM_IN,     WIDX_ROTATE,    WIDX_VIEW_MENU, WIDX_MAP,
};

// from right to left
static const int right_aligned_widgets_order[] = { WIDX_NEWS,           WIDX_GUESTS,       WIDX_STAFF,    WIDX_PARK,
                                                   WIDX_RIDES,          WIDX_RESEARCH,     WIDX_FINANCES,

                                                   WIDX_SEPARATOR,

                                                   WIDX_CONSTRUCT_RIDE, WIDX_PATH,         WIDX_SCENERY,  WIDX_WATER,
                                                   WIDX_LAND,           WIDX_CLEAR_SCENERY };
...
static rct_widget window_top_toolbar_widgets[] = {
    ...
    { WWT_TRNBTN, 0, 0x001E + 30, 0x003B + 30, 0, 27, 0x20000000 | SPR_TOOLBAR_FILE,
      STR_DISC_AND_GAME_OPTIONS_TIP },                                                                       // File menu
    ...
    { WWT_TRNBTN, 1, 0x00A0 + 30, 0x00BD + 30, 0, 27, 0x20000000 | SPR_TOOLBAR_VIEW,
      STR_VIEW_OPTIONS_TIP },                                                                           // Transparancy menu
...
void window_top_toolbar_open()
{
    ...
    window = window_create(0, 0, gScreenWidth, 28, &window_top_toolbar_events, WC_TOP_TOOLBAR,
...
static void window_top_toolbar_mousedown(int widgetIndex, rct_window * w, rct_widget * widget)
{
    ...
    case WIDX_FILE_MENU:
        ...
        window_dropdown_show_text(w->x + widget->left, w->y + widget->top, widget->bottom - widget->top + 1,
...
static void window_top_toolbar_dropdown(rct_window * w, int widgetIndex, int dropdownIndex)
{
    ...
    case WIDX_CHEATS:
        ...
        case DDIDX_DISABLE_CLEARANCE_CHECKS:
            game_do_command(0, GAME_COMMAND_FLAG_APPLY, CHEAT_DISABLECLEARANCECHECKS, !gCheatsDisableClearanceChecks,
                            GAME_COMMAND_CHEAT, 0, 0);
            break;
        case DDIDX_DISABLE_SUPPORT_LIMITS:
            game_do_command(0, GAME_COMMAND_FLAG_APPLY, CHEAT_DISABLESUPPORTLIMITS, !gCheatsDisableSupportLimits,
                            GAME_COMMAND_CHEAT, 0, 0);
            break;
        }
        break;
...
static void repaint_scenery_tool_down(sint16 x, sint16 y, sint16 widgetIndex)
{
    ...
    uint16            flags = VIEWPORT_INTERACTION_MASK_SCENERY & VIEWPORT_INTERACTION_MASK_WALL &
    ...
    case VIEWPORT_INTERACTION_ITEM_SCENERY:
    {
        ...
        // If can't repaint
        if (!(scenery_entry->small_scenery.flags & (SMALL_SCENERY_FLAG_HAS_PRIMARY_COLOUR | SMALL_SCENERY_FLAG_HAS_GLASS)))
            return;

        gGameCommandErrorTitle = STR_CANT_REPAINT_THIS;
        game_do_command(grid_x, 1 | (map_element->type << 8), grid_y,
                        map_element->base_height | (map_element->properties.scenery.type << 8), GAME_COMMAND_SET_SCENERY_COLOUR,
                        0, gWindowSceneryPrimaryColour | (gWindowScenerySecondaryColour << 8));
        break;
    }
    case VIEWPORT_INTERACTION_ITEM_WALL:
    {
        ...
        // If can't repaint
        if (!(scenery_entry->wall.flags & (WALL_SCENERY_FLAG1 | WALL_SCENERY_FLAG2)))
            return;

        gGameCommandErrorTitle = STR_CANT_REPAINT_THIS;
        game_do_command(grid_x, 1 | (gWindowSceneryPrimaryColour << 8), grid_y,
                        (map_element->type & MAP_ELEMENT_DIRECTION_MASK) | (map_element->base_height << 8),
                        GAME_COMMAND_SET_FENCE_COLOUR, 0, gWindowScenerySecondaryColour | (gWindowSceneryTertiaryColour << 8));
        break;
    }
    case VIEWPORT_INTERACTION_ITEM_LARGE_SCENERY:
    {
        ...
        gGameCommandErrorTitle = STR_CANT_REPAINT_THIS;
        game_do_command(grid_x, 1 | ((map_element->type & MAP_ELEMENT_DIRECTION_MASK) << 8), grid_y,
                        map_element->base_height | ((map_element->properties.scenerymultiple.type >> 10) << 8),
                        GAME_COMMAND_SET_LARGE_SCENERY_COLOUR, 0,
    ...
    case VIEWPORT_INTERACTION_ITEM_BANNER:
    {
        ...
        // If can't repaint
        if (!(scenery_entry->banner.flags & (1 << 0)))
            return;

        gGameCommandErrorTitle = STR_CANT_REPAINT_THIS;
        game_do_command(grid_x, 1, grid_y, map_element->base_height | ((map_element->properties.banner.position & 0x3) << 8),
 ...
 * edi : parameter_3
 */
static void sub_6E1F34(sint16 x, sint16 y, uint16 selected_scenery, sint16 * grid_x, sint16 * grid_y, uint32 * parameter_1,
                       uint32 * parameter_2, uint32 * parameter_3)
{
    ...
    else
    {
        if (!gSceneryCtrlPressed)
        {
            if (gInputPlaceObjectModifier & PLACE_OBJECT_MODIFIER_COPY_Z)
            {
                // CTRL pressed
                rct_map_element * map_element;
                uint16            flags = VIEWPORT_INTERACTION_MASK_TERRAIN & VIEWPORT_INTERACTION_MASK_RIDE &
                               VIEWPORT_INTERACTION_MASK_SCENERY & VIEWPORT_INTERACTION_MASK_FOOTPATH &
                               VIEWPORT_INTERACTION_MASK_WALL & VIEWPORT_INTERACTION_MASK_LARGE_SCENERY;
    ...
    case 0:
    {
        ...
        if (!gSceneryCtrlPressed)
        {
            uint16            flags            = VIEWPORT_INTERACTION_MASK_TERRAIN & VIEWPORT_INTERACTION_MASK_WATER;
    ...
    case 1:
    {
        ...
        uint16            flags            = VIEWPORT_INTERACTION_MASK_FOOTPATH & VIEWPORT_INTERACTION_MASK_FOOTPATH_ITEM;
        int               interaction_type = 0;
        rct_map_element * map_element;
...
static void window_top_toolbar_scenery_tool_down(short x, short y, rct_window * w, short widgetIndex)
{
    ...
    case SCENERY_TYPE_SMALL:
    {
        int  quantity = 1;
        bool isCluster =
            gWindowSceneryClusterEnabled && (network_get_mode() != NETWORK_MODE_CLIENT ||
        ...
        for (int q = 0; q < quantity; q++)
        {
            ...
            for (; zAttemptRange != 0; zAttemptRange--)
            {
                ...
                int cost = game_do_command(cur_grid_x, flags, cur_grid_y, parameter_2, GAME_COMMAND_PLACE_SCENERY,
    ...
    case SCENERY_TYPE_LARGE:
    {
        ...
        for (; zAttemptRange != 0; zAttemptRange--)
        {
            ...
            gDisableErrorWindowSound = true;
            gGameCommandErrorTitle   = STR_CANT_POSITION_THIS_HERE;
            int cost = game_do_command(gridX, flags, gridY, parameter_2, GAME_COMMAND_PLACE_LARGE_SCENERY, parameter_3,
                                       gSceneryPlaceZ);
            gDisableErrorWindowSound = false;
...
static void top_toolbar_tool_update_water(sint16 x, sint16 y)
{
    ...
    rct_xy16 mapTile          = { 0 };
    int      interaction_type = 0;
    get_map_coordinates_from_pos(x, y, VIEWPORT_INTERACTION_MASK_TERRAIN & VIEWPORT_INTERACTION_MASK_WATER, &mapTile.x,
                                 &mapTile.y, &interaction_type, NULL, NULL);
 ...
 */
static money32 try_place_ghost_scenery(rct_xy16 map_tile, uint32 parameter_1, uint32 parameter_2, uint32 parameter_3,
                                       uint16 selected_tab)
{
    scenery_remove_ghost_tool_placement();
    ...
    case 0:
        // Small Scenery
        // 6e252b
        cost = game_do_command(map_tile.x, parameter_1 | 0x69, map_tile.y, parameter_2, GAME_COMMAND_PLACE_SCENERY, parameter_3,
                               gSceneryPlaceZ);
    ...
    case 1:
        // Path Bits
        // 6e265b
        cost = game_do_command(map_tile.x,
                               (parameter_1 & 0xFF00) | (GAME_COMMAND_FLAG_APPLY | GAME_COMMAND_FLAG_ALLOW_DURING_PAUSED |
                                                         GAME_COMMAND_FLAG_5 | GAME_COMMAND_FLAG_GHOST | GAME_COMMAND_FLAG_7),
    ...
    case 2:
        // Walls
        // 6e26b0
        cost = game_do_command(map_tile.x, parameter_1 | 0x69, map_tile.y, parameter_2, GAME_COMMAND_PLACE_FENCE,
                               gSceneryPlaceZ, _unkF64F15);
    ...
    case 3:
        // Large Scenery
        // 6e25a7
        cost = game_do_command(map_tile.x, parameter_1 | 0x69, map_tile.y, parameter_2, GAME_COMMAND_PLACE_LARGE_SCENERY,
                               parameter_3, gSceneryPlaceZ);
    ...
    case 4:
        ...
        // 6e2612
        cost =
            game_do_command(map_tile.x, parameter_1 | 0x69, map_tile.y, parameter_2, GAME_COMMAND_PLACE_BANNER, parameter_3, 0);
...
static void top_toolbar_tool_update_scenery(sint16 x, sint16 y)
{
    ...
    case 0:
        ...
        if ((gSceneryGhostType & (1 << 0)) && mapTile.x == gSceneryGhostPosition.x && mapTile.y == gSceneryGhostPosition.y &&
    ...
    case 1:
        ...
        // If no change in ghost placement
        if ((gSceneryGhostType & (1 << 1)) && mapTile.x == gSceneryGhostPosition.x && mapTile.y == gSceneryGhostPosition.y &&
    ...
    case 2:
        ...
        // If no change in ghost placement
        if ((gSceneryGhostType & (1 << 2)) && mapTile.x == gSceneryGhostPosition.x && mapTile.y == gSceneryGhostPosition.y &&
    ...
    case 3:
        ...
        {
            rct_xy16 tileLocation = {.x = tile->x_offset, .y = tile->y_offset };
        ...
        // If no change in ghost placement
        if ((gSceneryGhostType & (1 << 3)) && mapTile.x == gSceneryGhostPosition.x && mapTile.y == gSceneryGhostPosition.y &&
            gSceneryPlaceZ == _unkF64F0A && (sint16)(parameter3 & 0xFFFF) == gSceneryPlaceObject)
        {
            return;
        }
    ...
    case 4:
        ...
        // If no change in ghost placement
        if ((gSceneryGhostType & (1 << 4)) && mapTile.x == gSceneryGhostPosition.x && mapTile.y == gSceneryGhostPosition.y &&
            (sint16)(parameter2 & 0xFF) == gSceneryGhostPosition.z && ((parameter2 >> 8) & 0xFF) == gSceneryPlaceRotation)
        {
            return;
        }
...
static void window_top_toolbar_tool_down(rct_window * w, int widgetIndex, int x, int y)
{
    ...
    case WIDX_CLEAR_SCENERY:
        ...
        game_do_command(gMapSelectPositionA.x, 1, gMapSelectPositionA.y,
                        (gClearSmallScenery | gClearLargeScenery << 1 | gClearFootpath << 2), GAME_COMMAND_CLEAR_SCENERY,
    ...
    case WIDX_LAND:
        if (gMapSelectFlags & MAP_SELECT_FLAG_ENABLE)
        {
            ...
            game_do_command(gMapSelectPositionA.x, 1, gMapSelectPositionA.y,
                            gLandToolTerrainSurface | (gLandToolTerrainEdge << 8), GAME_COMMAND_CHANGE_SURFACE_STYLE,
                            gMapSelectPositionB.x, gMapSelectPositionB.y);
            gCurrentToolId = 3;
        }
        break;
...
money32 selection_lower_land(uint8 flags)
{
    ...
    if (gLandMountainMode)
    {
        return game_do_command(centreX, flags, centreY, xBounds, GAME_COMMAND_EDIT_LAND_SMOOTH, 0x8000 + gMapSelectType,
                               yBounds);
    }
    else
    {
...
static void window_top_toolbar_water_tool_drag(short x, short y)
{
    ...
    if (y <= dx)
    {
        ...
        game_do_command(gMapSelectPositionA.x, 1, gMapSelectPositionA.y, dx, GAME_COMMAND_RAISE_WATER, gMapSelectPositionB.x,
    ...
    if (y >= dx)
    {
        ...
        game_do_command(gMapSelectPositionA.x, 1, gMapSelectPositionA.y, dx, GAME_COMMAND_LOWER_WATER, gMapSelectPositionB.x,
                        gMapSelectPositionB.y);
        gWaterToolRaiseCost = MONEY32_UNDEFINED;
        gWaterToolLowerCost = MONEY32_UNDEFINED;
...
void top_toolbar_init_fastforward_menu(rct_window * w, rct_widget * widget)
{
    ...
    window_dropdown_show_text(w->x + widget->left, w->y + widget->top, widget->bottom - widget->top + 1, w->colours[0] | 0x80,
                              0, num_items);
...
void top_toolbar_init_rotate_menu(rct_window * w, rct_widget * widget)
{
    ...
    window_dropdown_show_text(w->x + widget->left, w->y + widget->top, widget->bottom - widget->top + 1, w->colours[1] | 0x80,
                              0, 2);
...
void toggle_land_window(rct_window * topToolbar, int widgetIndex)
{
    if ((gInputFlags & INPUT_FLAG_TOOL_ACTIVE) && gCurrentToolWidget.window_classification == WC_TOP_TOOLBAR &&
        gCurrentToolWidget.widget_index == 7)
    {
        tool_cancel();
    }
    else
...
{
    if ((gInputFlags & INPUT_FLAG_TOOL_ACTIVE) && gCurrentToolWidget.window_classification == WC_TOP_TOOLBAR &&
        gCurrentToolWidget.widget_index == 16)
    {
        tool_cancel();
    }
    else
...
{
    if ((gInputFlags & INPUT_FLAG_TOOL_ACTIVE) && gCurrentToolWidget.window_classification == WC_TOP_TOOLBAR &&
        gCurrentToolWidget.widget_index == 8)
    {
        tool_cancel();
    }
    else
