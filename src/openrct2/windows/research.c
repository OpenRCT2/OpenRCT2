...
static rct_widget * window_research_page_widgets[] = { window_research_development_widgets, window_research_funding_widgets };
...
//
static rct_window_event_list window_research_development_events = { NULL,
...
// 0x009890E8
static rct_window_event_list window_research_funding_events = { NULL,
...
static uint32 window_research_page_enabled_widgets[] = {
    (1 << WIDX_CLOSE) | (1 << WIDX_TAB_1) | (1 << WIDX_TAB_2) | (1 << WIDX_LAST_DEVELOPMENT_BUTTON),

    (1 << WIDX_CLOSE) | (1 << WIDX_TAB_1) | (1 << WIDX_TAB_2) | (1 << WIDX_RESEARCH_FUNDING) |
        (1 << WIDX_RESEARCH_FUNDING_DROPDOWN_BUTTON) | (1 << WIDX_TRANSPORT_RIDES) | (1 << WIDX_GENTLE_RIDES) |
        (1 << WIDX_ROLLER_COASTERS) | (1 << WIDX_THRILL_RIDES) | (1 << WIDX_WATER_RIDES) | (1 << WIDX_SHOPS_AND_STALLS) |
...
static const rct_string_id ResearchCategoryNames[] = {
    STR_RESEARCH_CATEGORY_TRANSPORT,  STR_RESEARCH_CATEGORY_GENTLE, STR_RESEARCH_CATEGORY_ROLLERCOASTER,
    STR_RESEARCH_CATEGORY_THRILL,     STR_RESEARCH_CATEGORY_WATER,  STR_RESEARCH_CATEGORY_SHOP,
    STR_RESEARCH_CATEGORY_SCENERYSET,
};

static const rct_string_id ResearchStageNames[] = {
    STR_RESEARCH_STAGE_INITIAL_RESEARCH, STR_RESEARCH_STAGE_DESIGNING, STR_RESEARCH_STAGE_COMPLETING_DESIGN,
...
static void window_research_development_invalidate(rct_window * w)
{
    ...
    if (typeId != 0xFFFFFFFF)
    {
        ...
        window_research_development_widgets[WIDX_LAST_DEVELOPMENT_BUTTON].image =
            typeId >= 0x10000 ? SPR_NEW_RIDE : SPR_NEW_SCENERY;
    }
}
...
void window_research_development_page_paint(rct_window * w, rct_drawpixelinfo * dpi, int baseWidgetIndex)
{
        ...
        if (gResearchProgressStage != RESEARCH_STAGE_INITIAL_RESEARCH)
        {
            ...
            if (gResearchProgressStage != RESEARCH_STAGE_DESIGNING)
            {
                ...
                if (typeId >= 0x10000)
                {
                    ...
                    stringId =
                        (rideEntry->flags & RIDE_ENTRY_FLAG_SEPARATE_RIDE_NAME) ? rideEntry->name : ((typeId >> 8) & 0xFF) + 2;
                }
                else
                {
    ...
    if (typeId != 0xFFFFFFFF)
    {
        if (typeId >= 0x10000)
        {
            ...
            stringId = (rideEntry->flags & RIDE_ENTRY_FLAG_SEPARATE_RIDE_NAME) ? rideEntry->name : ((typeId >> 8) & 0xFF) + 2;
