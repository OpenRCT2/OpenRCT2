...
// 0x98E44C
static rct_window_event_list window_sign_events = { NULL,
                                                    window_sign_mouseup,
                                                    NULL,
...
// 0x9A410C
static rct_window_event_list window_sign_small_events = { NULL,
    ...
    while (1)
    {
        if (map_element_get_type(map_element) == MAP_ELEMENT_TYPE_SCENERY_MULTIPLE)
        {
            rct_scenery_entry * scenery_entry =
                get_large_scenery_entry(map_element->properties.scenerymultiple.type & MAP_ELEMENT_LARGE_TYPE_MASK);
            if (scenery_entry->large_scenery.var_11 != 0xFF)
            {
                int id = (map_element->type & 0xC0) | ((map_element->properties.scenerymultiple.colour[0] & 0xE0) >> 2) |
...
static void window_sign_mouseup(rct_window * w, int widgetIndex)
{
    ...
    case WIDX_SIGN_DEMOLISH:
        ...
        game_do_command(x, 1 | ((map_element->type & 0x3) << 8), y,
...
static void window_sign_dropdown(rct_window * w, int widgetIndex, int dropdownIndex)
{
    ...
    case WIDX_TEXT_COLOUR:
        ...
        game_do_command(1, GAME_COMMAND_FLAG_APPLY, w->number, w->list_information_type, GAME_COMMAND_SET_SIGN_STYLE,
                        dropdownIndex, 1);
        break;
    default:
        return;
...
static void window_sign_textinput(rct_window * w, int widgetIndex, char * text)
{
    ...
    {
        game_do_command(1, GAME_COMMAND_FLAG_APPLY, w->number, *((int *)(text + 0)), GAME_COMMAND_SET_SIGN_NAME,
                        *((int *)(text + 8)), *((int *)(text + 4)));
        game_do_command(2, GAME_COMMAND_FLAG_APPLY, w->number, *((int *)(text + 12)), GAME_COMMAND_SET_SIGN_NAME,
                        *((int *)(text + 20)), *((int *)(text + 16)));
        game_do_command(0, GAME_COMMAND_FLAG_APPLY, w->number, *((int *)(text + 24)), GAME_COMMAND_SET_SIGN_NAME,
...
static void window_sign_small_mouseup(rct_window * w, int widgetIndex)
{
    ...
    case WIDX_SIGN_DEMOLISH:
        ...
        game_do_command(x, 1 | ((map_element->type & 0x3) << 8), y, (map_element->base_height << 8) | (map_element->type & 0x3),
