...
static rct_window_event_list window_new_campaign_events = { NULL,
...
void window_new_campaign_open(sint16 campaignType)
{
    ...
    {
        if (!ride_type_has_flag(ride->type, RIDE_TYPE_FLAG_IS_SHOP | RIDE_TYPE_FLAG_SELLS_FOOD | RIDE_TYPE_FLAG_SELLS_DRINKS |
        ...
        if (w->campaign.campaign_type == ADVERTISING_CAMPAIGN_FOOD_OR_DRINK_FREE)
        {
            ...
            if (window_new_campaign_shop_items[0] != 255)
            {
                ...
                window_dropdown_show_text_custom_width(
                    ...
                    w->colours[1], DROPDOWN_FLAG_STAY_OPEN, numItems, dropdownWidget->right - dropdownWidget->left - 3);
            }
        }
        else
        {
...
static void window_new_campaign_paint(rct_window * w, rct_drawpixelinfo * dpi)
{
    ...
    gfx_draw_string_left(dpi, w->campaign.no_weeks == 1 ? STR_MARKETING_1_WEEK : STR_X_WEEKS, &w->campaign.no_weeks,
                         w->colours[0], w->x + spinnerWidget->left + 1, w->y + spinnerWidget->top);
