...
static rct_window_event_list window_footpath_events = { window_footpath_close,
...
void window_footpath_open()
{
    ...
    window->enabled_widgets = (1 << WIDX_CLOSE) | (1 << WIDX_FOOTPATH_TYPE) | (1 << WIDX_QUEUELINE_TYPE) |
                              (1 << WIDX_DIRECTION_NW) | (1 << WIDX_DIRECTION_NE) | (1 << WIDX_DIRECTION_SW) |
                              (1 << WIDX_DIRECTION_SE) | (1 << WIDX_SLOPEDOWN) | (1 << WIDX_LEVEL) | (1 << WIDX_SLOPEUP) |
...
static void window_footpath_show_footpath_types_dialog(rct_window * w, rct_widget * widget, int showQueues)
{
    ...
    // If the game is in sandbox mode, also show paths that are normally restricted to the scenario editor, but not their queues
    // (since these usually shouldn't have one)
...
static void window_footpath_set_provisional_path_at_point(int x, int y)
{
    ...
    rct_xy16          mapCoord = { 0 };
    get_map_coordinates_from_pos(x, y, VIEWPORT_INTERACTION_MASK_FOOTPATH & VIEWPORT_INTERACTION_MASK_TERRAIN, &mapCoord.x,
                                 &mapCoord.y, &interactionType, &mapElement, NULL);
    x = mapCoord.x;
    y = mapCoord.y;
    ...
    else
    {
        ...
        if ((gFootpathProvisionalFlags & PROVISIONAL_PATH_FLAG_1) && gFootpathProvisionalPosition.x == x &&
            gFootpathProvisionalPosition.y == y && gFootpathProvisionalPosition.z == mapElement->base_height)
        {
            return;
        }
...
static void window_footpath_construct()
{
    ...
    if (cost != MONEY32_UNDEFINED)
    {
        ...
        map_remove_intersecting_walls(x - TileDirectionDelta[direction].x, y - TileDirectionDelta[direction].y, z, z + 4,
                                      direction);
    }
    ...
    if (cost != MONEY32_UNDEFINED)
    {
        audio_play_sound_at_location(SOUND_PLACE_ITEM, gFootpathConstructFromPosition.x, gFootpathConstructFromPosition.y,
...
static void footpath_remove_map_element(rct_map_element * mapElement)
{
    ...
    footpath_remove(gFootpathConstructFromPosition.x, gFootpathConstructFromPosition.y, mapElement->base_height, 1);
...
static void window_footpath_set_enabled_and_pressed_widgets()
{
    ...
    else
    {
        ...
        disabledWidgets |= (1 << WIDX_DIRECTION_GROUP) | (1 << WIDX_DIRECTION_NW) | (1 << WIDX_DIRECTION_NE) |
                           (1 << WIDX_DIRECTION_SW) | (1 << WIDX_DIRECTION_SE) | (1 << WIDX_SLOPE_GROUP) |
                           (1 << WIDX_SLOPEDOWN) | (1 << WIDX_LEVEL) | (1 << WIDX_SLOPEUP) | (1 << WIDX_CONSTRUCT) |
                           (1 << WIDX_REMOVE);
    }
