...
static rct_window_event_list window_track_list_events = { window_track_list_close,
...
void window_track_list_open(ride_list_item item)
{
    ...
    rct_window * w     = window_create(x, y, 600, 400, &window_track_list_events, WC_TRACK_DESIGN_LIST, 0);
    ...
    *substituteElement = tmpElement;
    ...
    if (td6->track_flags & TRACK_DESIGN_FLAG_SCENERY_UNAVAILABLE)
    {
        if (!gTrackDesignSceneryToggle)
        {
            ...
            gfx_draw_string_centred_clipped(dpi, STR_DESIGN_INCLUDES_SCENERY_WHICH_IS_UNAVAILABLE, NULL, COLOUR_BLACK, x, y,
                                            368);
            y -= 10;
        }
    }
