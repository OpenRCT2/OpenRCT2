...
static rct_window_event_list window_server_list_events = { window_server_list_close,
...
void window_server_list_open()
{
    ...
    window->enabled_widgets = ((1 << WIDX_CLOSE) | (1 << WIDX_PLAYER_NAME_INPUT) | (1 << WIDX_FETCH_SERVERS) |
                               (1 << WIDX_ADD_SERVER) | (1 << WIDX_START_SERVER));
    ...
    case WIDX_LIST:
    {
        ...
        if (serverIndex >= 0 && serverIndex < _numServerEntries)
        {
            if (strcmp(_serverEntries[serverIndex].version, NETWORK_STREAM_ID) != 0 &&
                strcmp(_serverEntries[serverIndex].version, "") != 0)
            {
...
static void window_server_list_dropdown(rct_window * w, int widgetIndex, int dropdownIndex)
{
    ...
    if (serverIndex < 0)
        return;
    if (serverIndex >= _numServerEntries)
        return;
...
static void window_server_list_paint(rct_window * w, rct_drawpixelinfo * dpi)
{
    ...
    gfx_draw_string_left(dpi, STR_NETWORK_VERSION, (void *)&version, COLOUR_WHITE, w->x + 324,
                         w->y + w->widgets[WIDX_START_SERVER].top);
