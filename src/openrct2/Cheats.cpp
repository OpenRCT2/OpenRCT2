...
bool gCheatsKeepDisconnectedRideEntrances = false;
...
void CheatsReset()
{
    ...
    gCheatsKeepDisconnectedRideEntrances = false;
}
...
void CheatsSerialise(DataSerialiser& ds)
{
    ...
    if (ds.IsSaving())
    {
        ...
        CheatEntrySerialise(ds, CheatType::KeepDisconnectedRideEntrances, gCheatsKeepDisconnectedRideEntrances, count);
        ...
        for (uint16_t i = 0; i < count; i++)
        {
            ...
            switch (static_cast<CheatType>(type))
            {
                ...
                case CheatType::KeepDisconnectedRideEntrances:
                    ds << gCheatsKeepDisconnectedRideEntrances;
                    break;
                default:
                    break;
            }
...
const char* CheatsGetName(CheatType cheatType)
{
    switch (cheatType)
    {
        ...
        case CheatType::KeepDisconnectedRideEntrances:
            return language_get_string(STR_CHEAT_KEEP_DISCONNECTED_RIDE_ENTRANCES);
        default:
