...
static void DiagnosticPrint(DiagnosticLevel level, const std::string& prefix, const std::string& msg)
{
    ...
    if (stream == stdout)
        OpenRCT2::Console::WriteLine("%s%s", prefix.c_str(), msg.c_str());
    else
        OpenRCT2::Console::Error::WriteLine("%s%s", prefix.c_str(), msg.c_str());
}
...
void DiagnosticLog(DiagnosticLevel diagnosticLevel, const char* format, ...)
{
    ...
    {
        // Level
        auto prefix = OpenRCT2::String::StdFormat("%s: ", _level_strings[static_cast<uint8_t>(diagnosticLevel)]);

        // Message
        va_start(args, format);
        auto msg = OpenRCT2::String::Format_VA(format, args);
        va_end(args);
    ...
    DiagnosticLevel diagnosticLevel, const char* file, const char* function, int32_t line, const char* format, ...)
{
    ...
    if (_log_levels[static_cast<uint8_t>(diagnosticLevel)])
    {
        ...
        {
            prefix = OpenRCT2::String::StdFormat(
                "%s[%s:%d (%s)]: ", _level_strings[static_cast<uint8_t>(diagnosticLevel)], file, line, function);
        }
        else
        {
            prefix = OpenRCT2::String::StdFormat("%s: ", _level_strings[static_cast<uint8_t>(diagnosticLevel)]);
        }
