...
bool _log_levels[(uint8_t)DiagnosticLevel::Count] = { true, true, true, false, true };
...
int _android_log_priority[(uint8_t)DiagnosticLevel::Count] = { ANDROID_LOG_FATAL, ANDROID_LOG_ERROR, ANDROID_LOG_WARN,
...
void diagnostic_log(DiagnosticLevel diagnosticLevel, const char* format, ...)
{
    ...
    if (!_log_levels[(uint8_t)diagnosticLevel])
        return;

    va_start(args, format);
    __android_log_vprint(_android_log_priority[(uint8_t)diagnosticLevel], "OpenRCT2", format, args);
    va_end(args);
}
    ...
    DiagnosticLevel diagnosticLevel, const char* file, const char* function, int32_t line, const char* format, ...)
{
    ...
    va_start(args, format);
    __android_log_vprint(_android_log_priority[(uint8_t)diagnosticLevel], file, format, args);
    va_end(args);
}
...
void diagnostic_log(DiagnosticLevel diagnosticLevel, const char* format, ...)
{
    ...
    {
        // Level
        auto prefix = String::StdFormat("%s: ", _level_strings[(uint8_t)diagnosticLevel]);
    ...
    DiagnosticLevel diagnosticLevel, const char* file, const char* function, int32_t line, const char* format, ...)
{
    ...
    if (_log_levels[(uint8_t)diagnosticLevel])
    {
        ...
        {
            prefix = String::StdFormat("%s[%s:%d (%s)]: ", _level_strings[(uint8_t)diagnosticLevel], file, line, function);
        }
        else
        {
            prefix = String::StdFormat("%s: ", _level_strings[(uint8_t)diagnosticLevel]);
        }
