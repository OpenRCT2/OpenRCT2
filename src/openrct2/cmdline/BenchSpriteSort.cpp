...
static void fixup_pointers(std::vector<RecordedPaintSession>& s)
{
    for (size_t i = 0; i < s.size(); i++)
    {
        ...
        {
            if (entries[j].AsBasic()->NextQuadrantPs == reinterpret_cast<PaintStruct*>(-1))
            {
                entries[j].AsBasic()->NextQuadrantPs = nullptr;
            }
            else
            {
                auto nextQuadrantPs = reinterpret_cast<size_t>(entries[j].AsBasic()->NextQuadrantPs) / sizeof(PaintEntry);
                entries[j].AsBasic()->NextQuadrantPs = s[i].Entries[nextQuadrantPs].AsBasic();
            }
        }
...
{
    {
        ...
        {
            ps.AsBasic()->NextQuadrantPs = reinterpret_cast<PaintStruct*>(-1);
        }
