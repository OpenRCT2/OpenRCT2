...
void RideObject::ReadLegacy(IReadObjectContext* context, IStream* stream)
{
    ...
    for (int32_t i = 0; i < RCT2::Limits::RCT2_MAX_VEHICLES_PER_RIDE_ENTRY; i++)
    {
...
void RideObject::SetRepositoryItem(ObjectRepositoryItem* item) const
{
    ...
    for (int32_t i = 0; i < RCT2::Limits::RCT2_MAX_RIDE_TYPES_PER_RIDE_ENTRY; i++)
    {
        item->RideInfo.RideType[i] = _legacyType.ride_type[i];
    }
    for (int32_t i = 0; i < RCT2::Limits::RCT2_MAX_CATEGORIES_PER_RIDE; i++)
    {
