...
void RideObject::ReadLegacy(IReadObjectContext* context, IStream* stream)
{
    ...
    GetStringTable().Read(context, stream, static_cast<uint8_t>(ObjectStringID::DESCRIPTION));
    GetStringTable().Read(context, stream, static_cast<uint8_t>(ObjectStringID::CAPACITY));
...
{
    return GetString(static_cast<uint8_t>(ObjectStringID::DESCRIPTION));
}

std::string RideObject::GetCapacity() const
{
    return GetString(static_cast<uint8_t>(ObjectStringID::CAPACITY));
}
