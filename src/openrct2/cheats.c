...
static void cheat_explode_guests()
{
    ...
    FOR_ALL_GUESTS(sprite_index, peep)
    {
        ...
        // ride vehicle logic.
        if (peep->state == PEEP_STATE_ENTERING_RIDE || peep->state == PEEP_STATE_QUEUING_FRONT ||
            peep->state == PEEP_STATE_LEAVING_RIDE || peep->state == PEEP_STATE_ON_RIDE || peep->state == PEEP_STATE_QUEUING)
        {
            continue;
        }
...
void game_command_cheat(sint32 * eax, sint32 * ebx, sint32 * ecx, sint32 * edx, sint32 * esi, sint32 * edi, sint32 * ebp)
{
    ...
    if (*ebx & GAME_COMMAND_FLAG_APPLY)
    {
        ...
        case CHEAT_ENABLECHAINLIFTONALLTRACK:
            ...
            break;
        case CHEAT_UNLOCKALLPRICES:
            gCheatsUnlockAllPrices = *edx != 0;
            window_invalidate_by_class(WC_RIDE);
            window_invalidate_by_class(WC_PARK_INFORMATION);
            break;
...
const char * cheats_get_cheat_string(int cheat, int edx, int edi)
{
    ...
    case CHEAT_UNLOCKALLPRICES:
        return language_get_string(STR_CHEAT_UNLOCK_PRICES);
    case CHEAT_BUILDINPAUSEMODE:
