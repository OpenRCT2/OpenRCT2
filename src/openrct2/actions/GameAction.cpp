...
GameActions& GameActions::Instance()
{
    static GameActions gameActions;
    return gameActions;
}

void GameActions::Initialize()
{
...
GameActionFactory GameActions::Register(uint32 id, GameActionFactory factory)
{
...
std::unique_ptr<GameAction> GameActions::Create(uint32 id)
{
    Initialize();
...
GameActionResult::Ptr GameActions::Query(const GameAction * action)
{
...
GameActionResult::Ptr GameActions::InternalExecute(const GameAction * action)
{
    Guard::ArgumentNotNull(action);

    GameActionScopedDepth scopedDepth(_depth);

    uint16 curDepth = _depth;
    uint16 actionFlags = action->GetActionFlags();
    uint32 flags = action->GetFlags();

    // Nested commands will execute right away, don't add/take money and
    // will never get networked.
    bool nestedGameAction = curDepth > 1;
    ...
    if (result->Error == GA_ERROR::OK)
    {
        ...
        if (!(actionFlags & GA_FLAGS::CLIENT_ONLY) && nestedGameAction == false)
        {
    ...
    // TODO: Should nested commands also do this? In case of multiple errors
    // one would override each other.
