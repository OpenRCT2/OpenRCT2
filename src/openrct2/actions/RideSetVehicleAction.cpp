...
bool RideSetVehicleAction::ride_is_vehicle_type_valid(Ride* ride) const
{
...

    {
        ...
        if (gCheatsShowVehiclesFromOtherTrackTypes
            && !(
                ride->GetRideTypeDescriptor().HasFlag(RideTypeFlags::FlatRide) || rtd.HasFlag(RideTypeFlags::IsMaze)
    ...
    for (; rideTypeIterator <= rideTypeIteratorMax; rideTypeIterator++)
    {
        ...
        {
            if (GetRideTypeDescriptor(rideTypeIterator).HasFlag(RideTypeFlags::FlatRide))
                continue;

            const auto& rtd = GetRideTypeDescriptor(rideTypeIterator);
            if (rtd.HasFlag(RideTypeFlags::IsMaze) || rideTypeIterator == RIDE_TYPE_MINI_GOLF)
                continue;
        }
