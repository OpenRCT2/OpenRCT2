...
public:
    ...
    void Serialise(DataSerialiser & stream) override
    {
        ...
        stream << DS_TAG(_location.x) << DS_TAG(_location.y) << DS_TAG(_location.z) << DS_TAG(_location.direction) << DS_TAG(_tileIndex);
    }
    ...
    GameActionResult::Ptr Query() const override
    {
        ...
        int32_t z = tile_element_height(_location.x, _location.y);
        res->Position.x = _location.x + 16;
        res->Position.y = _location.y + 16;
        res->Position.z = z;
        ...
        {
            log_warning("Invalid game command for scenery removal, x = %d, y = %d", _location.x, _location.y);
        ...
        LocationXYZ16 firstTile = {
            ...
            static_cast<int16_t>((_location.z * 8) - scenery_entry->large_scenery.tiles[_tileIndex].z_offset)
        };

        rotate_map_coordinates(&firstTile.x, &firstTile.y, _location.direction);

        firstTile.x = _location.x - firstTile.x;
        firstTile.y = _location.y - firstTile.y;
        ...
        for (int32_t i = 0; scenery_entry->large_scenery.tiles[i].x_offset != -1; i++)
        {
            ...
            rotate_map_coordinates(&currentTile.x, &currentTile.y, _location.direction);
    ...
    GameActionResult::Ptr Execute() const override
    {
        ...
        for (int32_t i = 0; scenery_entry->large_scenery.tiles[i].x_offset != -1; i++)
        {
            ...
            do
            {
                ...
                if (sceneryElement->GetDirection() != _location.direction)
                    continue;
...
private:
    ...
    {
        TileElement* tileElement = map_get_first_element_at(_location.x / 32, _location.y / 32);
        if (tileElement == nullptr)
            return nullptr;
        ...
        do
        {
            ...
            if (tileElement->base_height != _location.z)
                continue;

            if (tileElement->AsLargeScenery()->GetSequenceIndex() != _tileIndex)
                continue;

            if (tileElement->GetDirection() != _location.direction)
                continue;
