...
public:
    ...
    void Serialise(DataSerialiser & stream) override
    {
        ...
        stream << DS_TAG(_location.x) << DS_TAG(_location.y) << DS_TAG(_location.z) << DS_TAG(_initialPlacement) << DS_TAG(_location.direction)
    ...
    GameActionResult::Ptr Query() const override
    {
        ...
        res->Position.x = _location.x + 8;
        res->Position.y = _location.y + 8;
        res->Position.z = _location.z;
        ...
        if (!map_is_location_owned(floor2(_location.x, 32), floor2(_location.y, 32), _location.z) && !gCheatsSandboxMode)
        {
        ...
        TileElement* tileElement = map_get_surface_element_at(_location.x / 32, _location.y / 32);
        ...
        uint8_t baseHeight = _location.z >> 3;
        uint8_t clearanceHeight = (_location.z + 32) >> 3;
        ...
        tileElement = map_get_track_element_at_of_type_from_ride(_location.x, _location.y, baseHeight, TRACK_ELEM_MAZE, _rideIndex);
        if (tileElement == nullptr)
        {
            ...
            if (!map_can_construct_at(floor2(_location.x, 32), floor2(_location.y, 32), baseHeight, clearanceHeight, { 0b1111, 0 }))
            {
    ...
    GameActionResult::Ptr Execute() const override
    {
        ...
        {
            footpath_remove_litter(_location.x, _location.y, _location.z);
            wall_remove_at(floor2(_location.x, 32), floor2(_location.y, 32), _location.z, _location.z + 32);
        }

        uint8_t baseHeight = _location.z >> 3;
        uint8_t clearanceHeight = (_location.z + 32) >> 3;

        TileElement* tileElement = map_get_track_element_at_of_type_from_ride(_location.x, _location.y, baseHeight, TRACK_ELEM_MAZE, _rideIndex);
        if (tileElement == nullptr)
        {
            Ride* ride = get_ride(_rideIndex);
            ...
            uint16_t flooredX = floor2(_location.x, 32);
            uint16_t flooredY = floor2(_location.y, 32);

            tileElement = tile_element_insert(_location.x / 32, _location.y / 32, baseHeight, 0xF);
        ...
        switch (_mode)
        {
            ...
            {
                uint8_t segmentOffset = MazeGetSegmentBit(_location.x, _location.y);

                tileElement->AsTrack()->MazeEntrySubtract(1 << segmentOffset);

                if (!_initialPlacement)
                {
                    segmentOffset = byte_993CE9[(_location.direction + segmentOffset)];
                    tileElement->AsTrack()->MazeEntrySubtract(1 << segmentOffset);

                    uint8_t temp_edx = byte_993CFC[segmentOffset];
                    if (temp_edx != 0xFF)
                    {
                        uint16_t previousElementX = floor2(_location.x, 32) - CoordsDirectionDelta[_location.direction].x;
                        uint16_t previousElementY = floor2(_location.y, 32) - CoordsDirectionDelta[_location.direction].y;
            ...
            case GC_SET_MAZE_TRACK_FILL:
                if (!_initialPlacement)
                {
                    uint16_t previousSegmentX = _location.x - CoordsDirectionDelta[_location.direction].x / 2;
                    uint16_t previousSegmentY = _location.y - CoordsDirectionDelta[_location.direction].y / 2;
        ...
        map_invalidate_tile(floor2(_location.x, 32), floor2(_location.y, 32), tileElement->base_height * 8, tileElement->clearance_height * 8);
