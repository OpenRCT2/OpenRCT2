    ...
    case NETWORK_STATUS_CONNECTING:
    {
        ...
        case SOCKET_STATUS_RESOLVING:
        {
            if (_lastConnectStatus != SOCKET_STATUS_RESOLVING)
            {
                ...
                window_network_status_open(str_resolving, []() -> void { gNetwork.Close(); });
            }
            break;
        }
        case SOCKET_STATUS_CONNECTING:
        {
            if (_lastConnectStatus != SOCKET_STATUS_CONNECTING)
            {
                ...
                window_network_status_open(str_connecting, []() -> void { gNetwork.Close(); });
        ...
        case SOCKET_STATUS_CONNECTED:
        {
            ...
            window_network_status_open(str_authenticating, []() -> void { gNetwork.Close(); });
            break;
        }
        default:
        {
...
{
    auto it = std::find_if(player_list.begin(), player_list.end(),
...
NetworkGroup * Network::AddGroup()
{
    ...
    {
        if (std::find_if(group_list.begin(), group_list.end(),
                         [&id](std::unique_ptr<NetworkGroup> const & group) { return group->Id == id; }) == group_list.end())
        {
            newid = id;
            break;
        }
...
uint8 Network::GetGroupIDByHash(const std::string & keyhash)
{
    ...
    if (networkUser != nullptr && networkUser->GroupId.HasValue())
    {
        ...
        {
            log_warning("User %s is assigned to non-existent group %u. Assigning to default group (%u)", keyhash.c_str(),
...
void Network::LoadGroups()
{
    ...
    else
    {
        ...
        for (size_t i = 0; i < groupCount; i++)
        {
            ...
            std::unique_ptr<NetworkGroup> newgroup(
                new NetworkGroup(NetworkGroup::FromJson(jsonGroup))); // change to make_unique in c++14
...
void Network::Server_Send_OBJECTS(NetworkConnection &                               connection,
                                  const std::vector<const ObjectRepositoryItem *> & objects) const
{
...
unsigned char * Network::save_for_network(SDL_RWops * rw_buffer, size_t & out_size,
                                          const std::vector<const ObjectRepositoryItem *> & objects) const
{
...
{
    std::unique_ptr<NetworkPacket> packet(NetworkPacket::Allocate());
    *packet << (uint32)NETWORK_COMMAND_GAMECMD << (uint32)gCurrentTicks << eax << (ebx | GAME_COMMAND_FLAG_NETWORKED) << ecx
            << edx << esi << edi << ebp << callback;
    server_connection.QueuePacket(std::move(packet));
}

void Network::Server_Send_GAMECMD(uint32 eax, uint32 ebx, uint32 ecx, uint32 edx, uint32 esi, uint32 edi, uint32 ebp,
                                  uint8 playerid, uint8 callback)
{
    std::unique_ptr<NetworkPacket> packet(NetworkPacket::Allocate());
    *packet << (uint32)NETWORK_COMMAND_GAMECMD << (uint32)gCurrentTicks << eax << (ebx | GAME_COMMAND_FLAG_NETWORKED) << ecx
            << edx << esi << edi << ebp << playerid << callback;
    SendPacketToClients(*packet);
}
...
void Network::ProcessGameCommandQueue()
{
    while (game_command_queue.begin() != game_command_queue.end() && game_command_queue.begin()->tick == gCurrentTicks)
    {
        ...
        int     command       = gc.esi;
        money32 cost          = game_do_command_p(command, (int *)&gc.eax, (int *)&gc.ebx, (int *)&gc.ecx, (int *)&gc.edx,
                                         (int *)&gc.esi, (int *)&gc.edi, (int *)&gc.ebp);
...
void Network::RemoveClient(std::unique_ptr<NetworkConnection> & connection)
{
    ...
    {
        char         text[256];
        const char * has_disconnected_args[2] = { (char *)connection_player->Name.c_str(),
        ...
        if (pickup_peep)
        {
            ...
            game_do_command(pickup_peep->sprite_index, GAME_COMMAND_FLAG_APPLY, 1, 0,
        ...
        gNetwork.Server_Send_EVENT_PLAYER_DISCONNECTED((char *)connection_player->Name.c_str(),
                                                       connection->GetLastDisconnectReason());
    }
    player_list.erase(std::remove_if(player_list.begin(), player_list.end(),
                                     [connection_player](std::unique_ptr<NetworkPlayer> & player) {
...
void Network::Client_Handle_OBJECTS(NetworkConnection & connection, NetworkPacket & packet)
{
    ...
    for (uint32 i = 0; i < size; i++)
    {
        ...
        {
            log_warning("Object %s has different checksum/flags (%x/%x) than server (%x/%x).", s.c_str(),
                        ori->ObjectEntry.checksum, ori->ObjectEntry.flags, checksum, flags);
        }
    }
...
void Network::Client_Handle_MAP(NetworkConnection & connection, NetworkPacket & packet)
{
    ...
    window_network_status_open(str_downloading_map, []() -> void { gNetwork.Close(); });
...
void Network::Server_Handle_GAMECMD(NetworkConnection & connection, NetworkPacket & packet)
{
    ...
    if (commandCommand == GAME_COMMAND_TOGGLE_PAUSE || commandCommand == GAME_COMMAND_LOAD_OR_QUIT)
    {
        return;
    }
