...
static int32_t _compressLevel = std::numeric_limits<int32_t>::min();
...
static exitcode_t HandleCommandConvert(CommandLineArgEnumerator* enumerator)
{
    ...
    int16_t compressLevel;
    if (_compressLevel >= std::numeric_limits<int16_t>::min() && _compressLevel <= std::numeric_limits<int16_t>::max())
    {
        compressLevel = static_cast<int16_t>(_compressLevel);
    }
    else
    {
        // treat invalid values as the default value from the config file
        ...
        compressLevel = config.SaveCompressLevelFile;
    }
    ...
    try
    {
        ...
        exporter->Export(gameState, destinationPath, compressLevel);
    }
    catch (const std::exception& ex)
    {
        Console::Error::WriteLine(ex.what());
