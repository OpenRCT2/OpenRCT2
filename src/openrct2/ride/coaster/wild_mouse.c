...
static const uint32 _wild_mouse_brakes_image_ids[4] = {
    SPR_WILD_MOUSE_BRAKES_SW_NE, SPR_WILD_MOUSE_BRAKES_NW_SE, SPR_WILD_MOUSE_BRAKES_SW_NE, SPR_WILD_MOUSE_BRAKES_NW_SE,
};

static const uint32 _wild_mouse_block_brakes_image_ids[4] = {
    SPR_WILD_MOUSE_BLOCK_BRAKES_SW_NE, SPR_WILD_MOUSE_BLOCK_BRAKES_NW_SE, SPR_WILD_MOUSE_BLOCK_BRAKES_SW_NE,
    SPR_WILD_MOUSE_BLOCK_BRAKES_NW_SE,
};

/** rct2: 0x0078B1E4 */
static void wild_mouse_track_flat(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height,
...
static void wild_mouse_track_station(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height,
                                     rct_map_element * mapElement)
{
    static const uint32 baseImageIds[4] = {
        SPR_STATION_BASE_B_SW_NE, SPR_STATION_BASE_B_NW_SE, SPR_STATION_BASE_B_SW_NE, SPR_STATION_BASE_B_NW_SE,
    };

    int trackType = mapElement->properties.track.type;
    sub_98197C_rotated(direction, baseImageIds[direction] | gTrackColours[SCHEME_MISC], 0, 0, 32, 28, 2, height - 2, 0, 2,
                       height);
    if (trackType == TRACK_ELEM_END_STATION)
    {
        sub_98199C_rotated(direction, _wild_mouse_block_brakes_image_ids[direction] | gTrackColours[SCHEME_TRACK], 0, 0, 32, 20,
                           2, height, 0, 0, height);
    }
    else
    {
        sub_98199C_rotated(direction, _wild_mouse_brakes_image_ids[direction] | gTrackColours[SCHEME_TRACK], 0, 0, 32, 20, 2,
                           height, 0, 0, height);
    }
...
/** rct2: 0x0078B1F4 */
static void wild_mouse_track_25_deg_up(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height,
                                       rct_map_element * mapElement)
{
    static const uint32 imageIds[4][2] = {
...
/** rct2: 0x0078B204 */
static void wild_mouse_track_60_deg_up(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height,
                                       rct_map_element * mapElement)
{
    static const uint32 imageIds[4][2] = {
...
/** rct2: 0x0078B214 */
static void wild_mouse_track_flat_to_25_deg_up(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height,
                                               rct_map_element * mapElement)
{
    static const uint32 imageIds[4][2] = {
...
/** rct2: 0x0078B224 */
static void wild_mouse_track_25_deg_up_to_60_deg_up(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height,
                                                    rct_map_element * mapElement)
{
    static const uint32 imageIds[4][2] = {
...
/** rct2: 0x0078B234 */
static void wild_mouse_track_60_deg_up_to_25_deg_up(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height,
                                                    rct_map_element * mapElement)
{
    static const uint32 imageIds[4][2] = {
...
/** rct2: 0x0078B244 */
static void wild_mouse_track_25_deg_up_to_flat(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height,
                                               rct_map_element * mapElement)
{
    static const uint32 imageIds[4][2] = {
...
/** rct2: 0x0078B254 */
static void wild_mouse_track_25_deg_down(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height,
                                         rct_map_element * mapElement)
{
...
/** rct2: 0x0078B264 */
static void wild_mouse_track_60_deg_down(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height,
                                         rct_map_element * mapElement)
{
...
/** rct2: 0x0078B274 */
static void wild_mouse_track_flat_to_25_deg_down(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height,
                                                 rct_map_element * mapElement)
{
...
/** rct2: 0x0078B284 */
static void wild_mouse_track_25_deg_down_to_60_deg_down(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height,
                                                        rct_map_element * mapElement)
{
...
/** rct2: 0x0078B294 */
static void wild_mouse_track_60_deg_down_to_25_deg_down(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height,
                                                        rct_map_element * mapElement)
{
...
/** rct2: 0x0078B2A4 */
static void wild_mouse_track_25_deg_down_to_flat(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height,
                                                 rct_map_element * mapElement)
{
    wild_mouse_track_flat_to_25_deg_up(rideIndex, trackSequence, (direction + 2) & 3, height, mapElement);
}

static void wild_mouse_track_right_quarter_turn_3(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height,
                                                  rct_map_element * mapElement)
{
...
static void wild_mouse_track_right_quarter_turn_3_25_deg_down(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height,
                                                              rct_map_element * mapElement)
{
    ...
    {
        int               part = trackSequence == 0 ? 0 : 1;
        const sprite_bb * sbb  = &imageIds[direction][part];
        sub_98196C(sbb->sprite_id | gTrackColours[SCHEME_TRACK], (sint8)sbb->offset.x, (sint8)sbb->offset.y, sbb->bb_size.x,
                   sbb->bb_size.y, (sint8)sbb->bb_size.z, height + (sint8)sbb->offset.z, get_current_rotation());
    }
...
static void wild_mouse_track_left_quarter_turn_3_25_deg_up(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height,
                                                           rct_map_element * mapElement)
{
...
/** rct2: 0x0078B314 */
static void wild_mouse_track_right_quarter_turn_3_25_deg_up(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height,
                                                            rct_map_element * mapElement)
{
    static const sprite_bb imageIds[4][2] = {
        {
...
/** rct2: 0x0078B324 */
static void wild_mouse_track_left_quarter_turn_3_25_deg_down(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height,
                                                             rct_map_element * mapElement)
{
    trackSequence = mapLeftQuarterTurn3TilesToRightQuarterTurn3Tiles[trackSequence];
...
static void wild_mouse_track_left_quarter_turn_3(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height,
                                                 rct_map_element * mapElement)
{
    trackSequence = mapLeftQuarterTurn3TilesToRightQuarterTurn3Tiles[trackSequence];
...
/** rct2: 0x0078B394 */
static void wild_mouse_track_left_quarter_turn_1(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height,
                                                 rct_map_element * mapElement)
{
    static const uint32 imageIds[4] = {
        SPR_WILD_MOUSE_QUARTER_TURN_1_SW_NE, SPR_WILD_MOUSE_QUARTER_TURN_1_NW_SE, SPR_WILD_MOUSE_QUARTER_TURN_1_NE_SW,
...
/** rct2: 0x0078B3A4 */
static void wild_mouse_track_right_quarter_turn_1(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height,
                                                  rct_map_element * mapElement)
{
...
/** rct2: 0x0078B354 */
static void wild_mouse_track_flat_to_60_deg_up(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height,
                                               rct_map_element * mapElement)
{
    static const uint32 imageIds[4][2] = {
...
/** rct2: 0x0078B364 */
static void wild_mouse_track_60_deg_up_to_flat(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height,
                                               rct_map_element * mapElement)
{
    static const uint32 imageIds[4][2] = {
...
/** rct2: 0x0078B374 */
static void wild_mouse_track_flat_to_60_deg_down(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height,
                                                 rct_map_element * mapElement)
{
...
/** rct2: 0x0078B384 */
static void wild_mouse_track_60_deg_down_to_flat(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height,
                                                 rct_map_element * mapElement)
{
...
/** rct2: 0x0078B344 */
static void wild_mouse_track_brakes(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height,
                                    rct_map_element * mapElement)
{
...
/** rct2: 0x0078B3C4 */
static void wild_mouse_track_rotation_control_toggle(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height,
                                                     rct_map_element * mapElement)
{
    static const uint32 imageIds[4] = {
        SPR_WILD_MOUSE_ROTATION_CONTROL_TOGGLE_SW_NE, SPR_WILD_MOUSE_ROTATION_CONTROL_TOGGLE_NW_SE,
        SPR_WILD_MOUSE_ROTATION_CONTROL_TOGGLE_SW_NE, SPR_WILD_MOUSE_ROTATION_CONTROL_TOGGLE_NW_SE,
    };
...
/** rct2: 0x0078B3B4 */
static void wild_mouse_track_block_brakes(uint8 rideIndex, uint8 trackSequence, uint8 direction, int height,
                                          rct_map_element * mapElement)
{
