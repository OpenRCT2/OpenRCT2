...
static void hybrid_rc_track_flat(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
...
static void hybrid_rc_track_station(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
                                  const rct_tile_element * tileElement)
{
...
static void hybrid_rc_track_25_deg_up(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                    int32_t height, const rct_tile_element * tileElement)
{
    if (track_element_is_lift_hill(tileElement))
    {
        switch (direction)
...
static void hybrid_rc_track_60_deg_up(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                    int32_t height, const rct_tile_element * tileElement)
{
    if (track_element_is_lift_hill(tileElement))
    {
...
static void hybrid_rc_track_flat_to_25_deg_up(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                            int32_t height, const rct_tile_element * tileElement)
{
    if (track_element_is_lift_hill(tileElement))
    {
...
static void hybrid_rc_track_25_deg_up_to_60_deg_up(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                                 int32_t height, const rct_tile_element * tileElement)
{
    if (track_element_is_lift_hill(tileElement))
    {
...
static void hybrid_rc_track_60_deg_up_to_25_deg_up(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                                 int32_t height, const rct_tile_element * tileElement)
{
    if (track_element_is_lift_hill(tileElement))
    {
...
static void hybrid_rc_track_25_deg_up_to_flat(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                            int32_t height, const rct_tile_element * tileElement)
{
    if (track_element_is_lift_hill(tileElement))
    {
...
static void hybrid_rc_track_25_deg_down(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                      int32_t height, const rct_tile_element * tileElement)
{
    hybrid_rc_track_25_deg_up(session, rideIndex, trackSequence, (direction + 2) & 3, height, tileElement);
}

static void hybrid_rc_track_60_deg_down(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                      int32_t height, const rct_tile_element * tileElement)
{
    hybrid_rc_track_60_deg_up(session, rideIndex, trackSequence, (direction + 2) & 3, height, tileElement);
}

static void hybrid_rc_track_flat_to_25_deg_down(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                              int32_t height, const rct_tile_element * tileElement)
{
    hybrid_rc_track_25_deg_up_to_flat(session, rideIndex, trackSequence, (direction + 2) & 3, height, tileElement);
}

static void hybrid_rc_track_25_deg_down_to_60_deg_down(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                     uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    hybrid_rc_track_60_deg_up_to_25_deg_up(session, rideIndex, trackSequence, (direction + 2) & 3, height, tileElement);
}

static void hybrid_rc_track_60_deg_down_to_25_deg_down(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                     uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    hybrid_rc_track_25_deg_up_to_60_deg_up(session, rideIndex, trackSequence, (direction + 2) & 3, height, tileElement);
}

static void hybrid_rc_track_25_deg_down_to_flat(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                              int32_t height, const rct_tile_element * tileElement)
{
    hybrid_rc_track_flat_to_25_deg_up(session, rideIndex, trackSequence, (direction + 2) & 3, height, tileElement);
}

static void hybrid_rc_track_90_deg_up(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                        int32_t height, const rct_tile_element * tileElement)
{
...
static void hybrid_rc_track_90_deg_down(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                          int32_t height, const rct_tile_element * tileElement)
{
    hybrid_rc_track_90_deg_up(session, rideIndex, trackSequence, (direction + 2) & 3, height, tileElement);
}

static void hybrid_rc_track_60_deg_up_to_90_deg_up(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                     uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_90_deg_down_to_60_deg_down(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                         uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    hybrid_rc_track_60_deg_up_to_90_deg_up(session, rideIndex, trackSequence, (direction + 2) & 3, height, tileElement);
}

static void hybrid_rc_track_90_deg_up_to_60_deg_up(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                     uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (direction)
    {
...
static void hybrid_rc_track_60_deg_down_to_90_deg_down(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                         uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_left_quarter_turn_3(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                              int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_right_quarter_turn_3(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                               int32_t height, const rct_tile_element * tileElement)
{
...
static void hybrid_rc_track_left_quarter_turn_5(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                              int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_right_quarter_turn_5(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                               int32_t height, const rct_tile_element * tileElement)
{
...
static void hybrid_rc_track_left_eighth_to_diag(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                              int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_right_eighth_to_diag(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                               int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_left_eighth_to_orthogonal(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                    uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
...
static void hybrid_rc_track_right_eighth_to_orthogonal(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                     uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    trackSequence = mapLeftEighthTurnToOrthogonal[trackSequence];
...
static void hybrid_rc_track_diag_flat(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                    int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_diag_25_deg_up(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                         int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_diag_25_deg_up_to_flat(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                                 int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_diag_flat_to_25_deg_up(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                                 int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_diag_25_deg_down(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                           int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_diag_flat_to_25_deg_down(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                   uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_diag_25_deg_down_to_flat(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                   uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_diag_60_deg_up(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                         int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_diag_25_deg_up_to_60_deg_up(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                      uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_diag_60_deg_up_to_25_deg_up(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                      uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_diag_60_deg_down(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                           int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_diag_25_deg_down_to_60_deg_down(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                          uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_diag_60_deg_down_to_25_deg_down(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                          uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_flat_to_left_bank(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                            int32_t height, const rct_tile_element * tileElement)
{
    switch (direction)
    {
...
static void hybrid_rc_track_flat_to_right_bank(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                             int32_t height, const rct_tile_element * tileElement)
{
    switch (direction)
    {
...
static void hybrid_rc_track_left_bank_to_flat(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                            int32_t height, const rct_tile_element * tileElement)
{
	hybrid_rc_track_flat_to_right_bank(session, rideIndex, trackSequence, (direction + 2) & 3, height, tileElement);
}

static void hybrid_rc_track_right_bank_to_flat(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                             int32_t height, const rct_tile_element * tileElement)
{
...
static void hybrid_rc_track_left_bank_to_25_deg_up(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                                 int32_t height, const rct_tile_element * tileElement)
{
    switch (direction)
    {
...
static void hybrid_rc_track_right_bank_to_25_deg_up(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                  uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (direction)
    {
...
static void hybrid_rc_track_25_deg_up_to_left_bank(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                                 int32_t height, const rct_tile_element * tileElement)
{
    switch (direction)
    {
...
static void hybrid_rc_track_25_deg_up_to_right_bank(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                  uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (direction)
    {
...
static void hybrid_rc_track_left_bank_to_25_deg_down(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                   uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    hybrid_rc_track_25_deg_up_to_right_bank(session, rideIndex, trackSequence, (direction + 2) & 3, height, tileElement);
}

static void hybrid_rc_track_right_bank_to_25_deg_down(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                    uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    hybrid_rc_track_25_deg_up_to_left_bank(session, rideIndex, trackSequence, (direction + 2) & 3, height, tileElement);
}

static void hybrid_rc_track_25_deg_down_to_left_bank(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                   uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    hybrid_rc_track_right_bank_to_25_deg_up(session, rideIndex, trackSequence, (direction + 2) & 3, height, tileElement);
}

static void hybrid_rc_track_25_deg_down_to_right_bank(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                    uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    hybrid_rc_track_left_bank_to_25_deg_up(session, rideIndex, trackSequence, (direction + 2) & 3, height, tileElement);
}

static void hybrid_rc_track_left_bank(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                    int32_t height, const rct_tile_element * tileElement)
{
    switch (direction)
    {
...
static void hybrid_rc_track_right_bank(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                     int32_t height, const rct_tile_element * tileElement)
{
...
static void hybrid_rc_track_diag_flat_to_left_bank(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                                 int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_diag_flat_to_right_bank(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                  uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_diag_left_bank_to_flat(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                                 int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_diag_right_bank_to_flat(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                  uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_diag_left_bank_to_25_deg_up(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                      uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_diag_right_bank_to_25_deg_up(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                       uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_diag_25_deg_up_to_left_bank(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                      uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_diag_25_deg_up_to_right_bank(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                       uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_diag_left_bank_to_25_deg_down(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                        uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_diag_right_bank_to_25_deg_down(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                         uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_diag_25_deg_down_to_left_bank(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                        uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_diag_25_deg_down_to_right_bank(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                         uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_diag_left_bank(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                         int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_diag_right_bank(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                          int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_left_quarter_turn_3_bank(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                   uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_right_quarter_turn_3_bank(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                    uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    trackSequence = mapLeftQuarterTurn3TilesToRightQuarterTurn3Tiles[trackSequence];
...
static void hybrid_rc_track_banked_left_quarter_turn_5(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                     uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_banked_right_quarter_turn_5(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                      uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    trackSequence = mapLeftQuarterTurn5TilesToRightQuarterTurn5Tiles[trackSequence];
...
static void hybrid_rc_track_left_eighth_bank_to_diag(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                   uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_right_eighth_bank_to_diag(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                    uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_left_eighth_bank_to_orthogonal(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                         uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    trackSequence = mapLeftEighthTurnToOrthogonal[trackSequence];
...
static void hybrid_rc_track_right_eighth_bank_to_orthogonal(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                          uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    trackSequence = mapLeftEighthTurnToOrthogonal[trackSequence];
...
static void hybrid_rc_track_left_quarter_turn_3_25_deg_up(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                        uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_right_quarter_turn_3_25_deg_up(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                         uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_left_quarter_turn_3_25_deg_down(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                          uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    trackSequence = mapLeftQuarterTurn3TilesToRightQuarterTurn3Tiles[trackSequence];
...
static void hybrid_rc_track_right_quarter_turn_3_25_deg_down(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                           uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    trackSequence = mapLeftQuarterTurn3TilesToRightQuarterTurn3Tiles[trackSequence];
...
static void hybrid_rc_track_left_quarter_turn_5_25_deg_up(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                        uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_right_quarter_turn_5_25_deg_up(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                         uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_left_quarter_turn_5_25_deg_down(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                          uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    trackSequence = mapLeftQuarterTurn5TilesToRightQuarterTurn5Tiles[trackSequence];
...
static void hybrid_rc_track_right_quarter_turn_5_25_deg_down(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                           uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    trackSequence = mapLeftQuarterTurn5TilesToRightQuarterTurn5Tiles[trackSequence];
...
static void hybrid_rc_track_left_quarter_turn_1_60_deg_up(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                        uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (direction)
    {
...
static void hybrid_rc_track_right_quarter_turn_1_60_deg_up(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                         uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (direction)
    {
...
static void hybrid_rc_track_left_quarter_turn_1_60_deg_down(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                          uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    hybrid_rc_track_right_quarter_turn_1_60_deg_up(session, rideIndex, trackSequence, (direction + 1) & 3, height, tileElement);
}

static void hybrid_rc_track_right_quarter_turn_1_60_deg_down(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                           uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    hybrid_rc_track_left_quarter_turn_1_60_deg_up(session, rideIndex, trackSequence, (direction - 1) & 3, height, tileElement);
}

static void hybrid_rc_track_left_quarter_turn_1_90_deg_up(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                            uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_right_quarter_turn_1_90_deg_up(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                             uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_left_quarter_turn_1_90_deg_down(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                              uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    hybrid_rc_track_right_quarter_turn_1_90_deg_up(session, rideIndex, trackSequence, (direction + 1) & 3, height,tileElement);
}

static void hybrid_rc_track_right_quarter_turn_1_90_deg_down(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                               uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
...
static void hybrid_rc_track_25_deg_up_to_left_banked_25_deg_up(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                             uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (direction)
    {
...
static void hybrid_rc_track_25_deg_up_to_right_banked_25_deg_up(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                              uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (direction)
    {
...
static void hybrid_rc_track_left_banked_25_deg_up_to_25_deg_up(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                             uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (direction)
    {
...
static void hybrid_rc_track_right_banked_25_deg_up_to_25_deg_up(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                              uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (direction)
    {
...
static void hybrid_rc_track_left_banked_25_deg_down_to_25_deg_down(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                                 uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
...
static void hybrid_rc_track_right_banked_25_deg_down_to_25_deg_down(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                                  uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
...
static void hybrid_rc_track_25_deg_down_to_left_banked_25_deg_down(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                                 uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
...
static void hybrid_rc_track_25_deg_down_to_right_banked_25_deg_down(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                                  uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
...
static void hybrid_rc_track_left_banked_flat_to_left_banked_25_deg_up(paint_session * session, uint8_t rideIndex,
...
static void hybrid_rc_track_right_banked_flat_to_right_banked_25_deg_up(paint_session * session, uint8_t rideIndex,
                                                                      uint8_t trackSequence, uint8_t direction, int32_t height,
                                                                      const rct_tile_element * tileElement)
{
    switch (direction)
...
static void hybrid_rc_track_left_banked_25_deg_up_to_left_banked_flat(paint_session * session, uint8_t rideIndex,
                                                                    uint8_t trackSequence, uint8_t direction, int32_t height,
                                                                    const rct_tile_element * tileElement)
{
    switch (direction)
...
static void hybrid_rc_track_right_banked_25_deg_up_to_right_banked_flat(paint_session * session, uint8_t rideIndex,
                                                                      uint8_t trackSequence, uint8_t direction, int32_t height,
                                                                      const rct_tile_element * tileElement)
{
    switch (direction)
...
static void hybrid_rc_track_left_banked_flat_to_left_banked_25_deg_down(paint_session * session, uint8_t rideIndex,
                                                                      uint8_t trackSequence, uint8_t direction, int32_t height,
                                                                      const rct_tile_element * tileElement)
{
...
static void hybrid_rc_track_right_banked_flat_to_right_banked_25_deg_down(paint_session * session, uint8_t rideIndex,
                                                                        uint8_t trackSequence, uint8_t direction, int32_t height,
                                                                        const rct_tile_element * tileElement)
{
...
static void hybrid_rc_track_left_banked_25_deg_down_to_left_banked_flat(paint_session * session, uint8_t rideIndex,
                                                                      uint8_t trackSequence, uint8_t direction, int32_t height,
                                                                      const rct_tile_element * tileElement)
{
...
static void hybrid_rc_track_right_banked_25_deg_down_to_right_banked_flat(paint_session * session, uint8_t rideIndex,
                                                                        uint8_t trackSequence, uint8_t direction, int32_t height,
                                                                        const rct_tile_element * tileElement)
{
...
static void hybrid_rc_track_25_deg_up_left_banked(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                                int32_t height, const rct_tile_element * tileElement)
{
    switch (direction)
    {
...
static void hybrid_rc_track_25_deg_up_right_banked(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                                 int32_t height, const rct_tile_element * tileElement)
{
    switch (direction)
    {
...
static void hybrid_rc_track_25_deg_down_left_banked(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                  uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    hybrid_rc_track_25_deg_up_right_banked(session, rideIndex, trackSequence, (direction + 2) & 3, height, tileElement);
}

static void hybrid_rc_track_25_deg_down_right_banked(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                   uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    hybrid_rc_track_25_deg_up_left_banked(session, rideIndex, trackSequence, (direction + 2) & 3, height, tileElement);
}

static void hybrid_rc_track_flat_to_left_banked_25_deg_up(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                        uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (direction)
    {
...
static void hybrid_rc_track_flat_to_right_banked_25_deg_up(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                         uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (direction)
    {
...
static void hybrid_rc_track_left_banked_25_deg_up_to_flat(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                        uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (direction)
    {
...
static void hybrid_rc_track_right_banked_25_deg_up_to_flat(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                         uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (direction)
    {
...
static void hybrid_rc_track_flat_to_left_banked_25_deg_down(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                          uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    hybrid_rc_track_right_banked_25_deg_up_to_flat(session, rideIndex, trackSequence, (direction + 2) & 3, height, tileElement);
}

static void hybrid_rc_track_flat_to_right_banked_25_deg_down(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                           uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    hybrid_rc_track_left_banked_25_deg_up_to_flat(session, rideIndex, trackSequence, (direction + 2) & 3, height, tileElement);
}

static void hybrid_rc_track_left_banked_25_deg_down_to_flat(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                          uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    hybrid_rc_track_flat_to_right_banked_25_deg_up(session, rideIndex, trackSequence, (direction + 2) & 3, height, tileElement);
}

static void hybrid_rc_track_right_banked_25_deg_down_to_flat(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                           uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    hybrid_rc_track_flat_to_left_banked_25_deg_up(session, rideIndex, trackSequence, (direction + 2) & 3, height, tileElement);
}

static void hybrid_rc_track_left_banked_quarter_turn_3_25_deg_up(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                               uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_right_banked_quarter_turn_3_25_deg_up(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                                uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_left_banked_quarter_turn_3_25_deg_down(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                                 uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    trackSequence = mapLeftQuarterTurn3TilesToRightQuarterTurn3Tiles[trackSequence];
...
static void hybrid_rc_track_right_banked_quarter_turn_3_25_deg_down(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                                  uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    trackSequence = mapLeftQuarterTurn3TilesToRightQuarterTurn3Tiles[trackSequence];
...
static void hybrid_rc_track_left_banked_quarter_turn_5_25_deg_up(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                               uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_right_banked_quarter_turn_5_25_deg_up(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                                uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_left_banked_quarter_turn_5_25_deg_down(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                                 uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    trackSequence = mapLeftQuarterTurn5TilesToRightQuarterTurn5Tiles[trackSequence];
...
static void hybrid_rc_track_right_banked_quarter_turn_5_25_deg_down(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                                  uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    trackSequence = mapLeftQuarterTurn5TilesToRightQuarterTurn5Tiles[trackSequence];
...
static void hybrid_rc_track_s_bend_left(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                      int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_s_bend_right(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                       int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_left_half_banked_helix_up_small(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                          uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
/** rct2: 0x008AD984 */
static void hybrid_rc_track_right_half_banked_helix_up_small(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                           uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
/** rct2: 0x008AD994 */
static void hybrid_rc_track_left_half_banked_helix_down_small(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                            uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    if (trackSequence >= 4)
    {
...
/** rct2: 0x008AD9A4 */
static void hybrid_rc_track_right_half_banked_helix_down_small(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                             uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    if (trackSequence >= 4)
    {
...
static void hybrid_rc_track_left_half_banked_helix_up_large(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                          uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
/** rct2: 0x008AD9C4 */
static void hybrid_rc_track_right_half_banked_helix_up_large(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                           uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_left_half_banked_helix_down_large(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                            uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    if (trackSequence >= 7)
    {
...
static void hybrid_rc_track_right_half_banked_helix_down_large(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                             uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    if (trackSequence >= 7)
    {
...
static void hybrid_rc_track_left_barrel_roll_up_to_down(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                          uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_right_barrel_roll_up_to_down(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                           uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
static void hybrid_rc_track_left_barrel_roll_down_to_up(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                          uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
...
static void hybrid_rc_track_right_barrel_roll_down_to_up(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                           uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
...
/** rct2: 0x008ADA34 */
static void hybrid_rc_track_brakes(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
                                 const rct_tile_element * tileElement)
{
    switch (direction)
    {
...
/** rct2: 0x008ADA44 */
static void hybrid_rc_track_on_ride_photo(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                        int32_t height, const rct_tile_element * tileElement)
{
    switch (direction)
    {
...
/** rct2: 0x008ADED4 */
static void hybrid_rc_track_flat_to_60_deg_up_long_base(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                      uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
/** rct2: 0x008ADEE4 */
static void hybrid_rc_track_60_deg_up_to_flat_long_base(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                      uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
/** rct2: 0x008ADEF4 */
static void hybrid_rc_track_flat_to_60_deg_down_long_base(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                        uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
...
/** rct2: 0x008ADF04 */
static void hybrid_rc_track_60_deg_up_to_flat_long_base122(paint_session * session, uint8_t rideIndex, uint8_t trackSequence,
                                                         uint8_t direction, int32_t height, const rct_tile_element * tileElement)
{
...
/** rct2: 0x008ADF14 */
static void hybrid_rc_track_cable_lift_hill(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                          int32_t height, const rct_tile_element * tileElement)
{
    switch (trackSequence)
    {
...
/** rct2: 0x008ADAA4 */
/** rct2: 0x008ADEC4 */
static void hybrid_rc_track_block_brakes(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction,
                                       int32_t height, const rct_tile_element * tileElement)
{
    switch (direction)
    {
...
static void hybrid_rc_track_booster(paint_session * session, uint8_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
                                  const rct_tile_element * tileElement)
{
