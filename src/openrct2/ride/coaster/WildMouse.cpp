    ...
    const TrackElement& trackElement)
{
    ...
    {
        MetalASupportsPaintSetup(session, MetalSupportsTubes, 4, -1, height, session.TrackColours[SCHEME_SUPPORTS]);
    }
    ...
    const TrackElement& trackElement)
{
    ...
    if (track_paint_util_should_paint_supports(session.MapPosition))
    {
        MetalASupportsPaintSetup(session, MetalSupportsTubes, 4, -9, height, session.TrackColours[SCHEME_SUPPORTS]);
    }
    ...
    const TrackElement& trackElement)
{
    ...
    if (track_paint_util_should_paint_supports(session.MapPosition))
    {
        if (direction == 0 || direction == 3)
        {
            MetalASupportsPaintSetup(session, MetalSupportsTubes, 4, -33, height, session.TrackColours[SCHEME_SUPPORTS]);
        }
        else
        {
    ...
    const TrackElement& trackElement)
{
    ...
    if (track_paint_util_should_paint_supports(session.MapPosition))
    {
        MetalASupportsPaintSetup(session, MetalSupportsTubes, 4, -4, height, session.TrackColours[SCHEME_SUPPORTS]);
    }
    if (direction == 0 || direction == 3)
    {
        PaintUtilPushTunnelRotated(session, direction, height, Tunnel0);
    ...
    const TrackElement& trackElement)
{
    ...
    if (track_paint_util_should_paint_supports(session.MapPosition))
    {
        MetalASupportsPaintSetup(session, MetalSupportsTubes, 4, -13, height, session.TrackColours[SCHEME_SUPPORTS]);
    }
    if (direction == 0 || direction == 3)
    {
        PaintUtilPushTunnelRotated(session, direction, height - 8, Tunnel1);
    ...
    const TrackElement& trackElement)
{
    ...
    if (track_paint_util_should_paint_supports(session.MapPosition))
    {
        MetalASupportsPaintSetup(session, MetalSupportsTubes, 4, -21, height, session.TrackColours[SCHEME_SUPPORTS]);
    }
    if (direction == 0 || direction == 3)
    {
        PaintUtilPushTunnelRotated(session, direction, height - 8, Tunnel1);
    ...
    const TrackElement& trackElement)
{
    ...
    if (track_paint_util_should_paint_supports(session.MapPosition))
    {
        MetalASupportsPaintSetup(session, MetalSupportsTubes, 4, -7, height, session.TrackColours[SCHEME_SUPPORTS]);
    }
    if (direction == 0 || direction == 3)
    {
    ...
    const TrackElement& trackElement)
{
    ...
    {
        case 0:
            blockedSegments = SegmentBC | SegmentC4 | SegmentCC | SegmentD0;
            break;
        case 2:
            blockedSegments = SegmentC0 | SegmentC4 | SegmentD0 | SegmentD4;
            break;
        case 3:
            blockedSegments = SegmentB8 | SegmentC4 | SegmentC8 | SegmentD4;
            break;
    }
    ...
    const TrackElement& trackElement)
{
    ...
    if (trackSequence == 0 || trackSequence == 3)
    {
        ...
        PaintAddImageAsParent(
            session, session.TrackColours[SCHEME_TRACK].WithIndex(sbb->SpriteId), { offset.x, offset.y, height + offset.z },
            sbb->BbSize);
    }

    track_paint_util_right_quarter_turn_3_tiles_25_deg_down_tunnel(session, height, direction, trackSequence, Tunnel2, Tunnel1);
    ...
    const TrackElement& trackElement)
{
    ...
    if (trackSequence == 0 || trackSequence == 3)
    {
        ...
        const SpriteBb* sbb = &imageIds[direction][part];
        PaintAddImageAsParent(
            session, session.TrackColours[SCHEME_TRACK].WithIndex(sbb->SpriteId),
            { sbb->Offset.x, sbb->Offset.y, height + sbb->Offset.z }, sbb->BbSize);
    }

    track_paint_util_right_quarter_turn_3_tiles_25_deg_up_tunnel(session, height, direction, trackSequence, Tunnel1, Tunnel2);
    ...
    switch (trackSequence)
    {
        ...
        case 3:
            if (direction == 2)
            {
                MetalASupportsPaintSetup(session, MetalSupportsTubes, 4, -11, height, session.TrackColours[SCHEME_SUPPORTS]);
            }
            else
            {
                MetalASupportsPaintSetup(session, MetalSupportsTubes, 4, -9, height, session.TrackColours[SCHEME_SUPPORTS]);
    ...
    const TrackElement& trackElement)
{
    ...
    if (track_paint_util_should_paint_supports(session.MapPosition))
    {
        MetalASupportsPaintSetup(session, MetalSupportsTubes, 4, -5, height, session.TrackColours[SCHEME_SUPPORTS]);
    }
    if (direction == 0 || direction == 3)
    {
        PaintUtilPushTunnelRotated(session, direction, height, Tunnel0);
    ...
    const TrackElement& trackElement)
{
    ...
    if (track_paint_util_should_paint_supports(session.MapPosition))
    {
        if (direction == 0 || direction == 3)
        {
            MetalASupportsPaintSetup(session, MetalSupportsTubes, 4, -17, height, session.TrackColours[SCHEME_SUPPORTS]);
        }
        else
        {
            MetalASupportsPaintSetup(session, MetalSupportsTubes, 4, 16, height, session.TrackColours[SCHEME_SUPPORTS]);
        }
    }
    if (direction == 0 || direction == 3)
    {
