...
#    define SET_FIELD(fieldname, ...) .fieldname = __VA_ARGS__
    ...
    enum ride_type_flags : uint64_t
    {
        ...
        RIDE_TYPE_FLAG_CANNOT_HAVE_GAPS = (1uLL << 8), // used by rides that can't have gaps, like those with a vertical tower,
        ...
        RIDE_TYPE_FLAG_VEHICLE_IS_INTEGRAL = (1uLL << 16), // Set by flat rides where the vehicle is integral to the structure,
                                                           // like
        ...
        RIDE_TYPE_FLAG_PEEP_SHOULD_GO_INSIDE_FACILITY = (1uLL << 21), // used by toilets and first aid to mark that peep should
                                                                      // go inside the building (rather than 'buying' at the
                                                                      // counter)
        ...
        RIDE_TYPE_FLAG_START_CONSTRUCTION_INVERTED = (1uLL << 47), // This is only set on the Flying RC and its alternative
                                                                   // type.
