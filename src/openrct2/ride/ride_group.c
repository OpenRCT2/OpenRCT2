...
ride_group corkscrew_rc_groups[2];
ride_group junior_rc_groups[2];
ride_group car_ride_groups[2];
ride_group twister_rc_groups[2];
static ride_group fallback_group;
...
ride_group * get_ride_group(uint8 trackType, rct_ride_entry * rideEntry)
{
    ...
    case RIDE_TYPE_JUNIOR_ROLLER_COASTER:
        if (ride_entry_get_supported_track_pieces(rideEntry) & (1ULL << TRACK_SLOPE_STEEP))
            return &ride_group_midi_coaster;
        else
    ...
    default:
        // Fall back to the track defaults.
        return ride_group_generate_fallback(trackType);
    }
}
...
void generate_ride_groups()
{
    corkscrew_rc_groups[0] = ride_group_corkscrew_rc;
    corkscrew_rc_groups[1] = ride_group_hypercoaster;
    
    junior_rc_groups[0] = ride_group_junior_rc;
    junior_rc_groups[1] = ride_group_midi_coaster;
    
    car_ride_groups[0] = ride_group_car_ride;
    car_ride_groups[1] = ride_group_monster_trucks;
    
    twister_rc_groups[0] = ride_group_steel_twister_rc;
    twister_rc_groups[1] = ride_group_hyper_twister;
}

ride_group * ride_group_generate_fallback(uint8 trackType)
{
    fallback_group.track_type = trackType;
    fallback_group.maximum_height = RideData5[trackType].max_height;
    fallback_group.available_track_pieces = RideTypePossibleTrackConfigurations[trackType];
    fallback_group.naming = RideNaming[trackType];
...
ride_group * ride_group_find(uint8 rideType, uint8 index)
{
    ...
    generate_ride_groups();
    
    switch(rideType)
    {
        case RIDE_TYPE_CORKSCREW_ROLLER_COASTER:
            rideGroup = &corkscrew_rc_groups[index];
            break;
        case RIDE_TYPE_JUNIOR_ROLLER_COASTER:
            rideGroup = &junior_rc_groups[index];
            break;
        case RIDE_TYPE_CAR_RIDE:
            rideGroup = &car_ride_groups[index];
            break;
        case RIDE_TYPE_TWISTER_ROLLER_COASTER:
            rideGroup = &twister_rc_groups[index];
            break;
        default:
            return NULL;
    }
...
bool ride_group_is_invented(ride_group * rideGroup)
{
    ...
    while (*rideEntryIndexPtr != 255)
    {
        ...
        ride_group * rideEntryRideGroup = get_ride_group(rideGroup->track_type, rideEntry);
        
        if (!ride_groups_are_equal(rideGroup, rideEntryRideGroup))
            continue;
