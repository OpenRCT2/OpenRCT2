                                                      ...
                                                      uint8 direction, sint32 height, rct_map_element * mapElement)
{
    ...
    else
    {
        ...
        imageId =
            miniature_railway_track_pieces_s_bend_left[direction & 1][trackSequence] | session->TrackColours[SCHEME_TRACK];
        sub_98199C_rotated(session, direction, imageId, (sint8)offset.x, (sint8)offset.y, bounds.x, bounds.y, 2, height,
                           offset.x, offset.y, height);
    }
                                                              ...
                                                              uint8 direction, sint32 height, rct_map_element * mapElement)
{
    ...
    else
    {
        ...
        if (index >= 0)
        {
            ...
            sub_98199C(session, imageId, 0, 0, bounds.x, bounds.y, (sint8)bounds.z, height, offset.x, offset.y, height,
                       get_current_rotation());
        }
    }
...
static bool wooden_supports_paint_setup(paint_session * session, sint32 woodType, sint32 supportType, sint32 special,
                                        sint32 height, uint32 imageColourFlags, bool * underground)
{
    switch (woodType)
    {
    case WOOD_A:
