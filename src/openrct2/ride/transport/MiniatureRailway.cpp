...
namespace OpenRCT2
{
        ...
        if (isSupported)
        {
            ...
            if (paintGrooved)
            {
                ...
                PaintAddImageAsChildRotated(
                    session, direction, imageIdAlt, { 0, 6, height }, { 32, 20, 2 }, { 0, 6, height + 2 });
        ...
        const TrackElement& trackElement)
    {
        auto imageId = session.TrackColours[SCHEME_TRACK].WithIndex(
            miniature_railway_track_pieces_flat_to_25_deg_up[direction]);
    ...
    {
        auto imageId = session.TrackColours[SCHEME_TRACK].WithIndex(
            miniature_railway_track_pieces_25_deg_up_to_flat[direction]);
