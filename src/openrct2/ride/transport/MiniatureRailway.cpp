...
static constexpr const uint32_t miniature_railway_track_floor[4] = { SPR_FLOOR_PLANKS, SPR_FLOOR_PLANKS_90_DEG, SPR_FLOOR_PLANKS,
...
static uint32_t
miniature_railway_track_to_grooved_indent(uint32_t imageId, const rct_tile_element * path, uint8_t direction, uint8_t rotation)
{
    if (!path)
    {
        return 0;
...
static constexpr const int8_t s_bend_left_supports_type[4][4] = { { 0, 5, 3, 0 }, { 1, 2, 4, 1 }, { 0, 5, 3, 0 }, { 1, 2, 4, 1 } };

static constexpr const uint32_t miniature_railway_s_bend_left_tiles_track_floor[2][4] = { { SPR_FLOOR_PLANKS, SPR_FLOOR_PLANKS_W_SEGMENT,
    ...
    const rct_tile_element * tileElement)
{
    ...
    {
        sub_98197C_rotated(session, direction, imageId, (int8_t)offset.x, (int8_t)offset.y, bounds.x, bounds.y, 2, height,
    ...
    else
    {
        ...
        imageId =
            miniature_railway_track_pieces_s_bend_left[direction & 1][trackSequence] | session->TrackColours[SCHEME_TRACK];
        sub_98199C_rotated(session, direction, imageId, (int8_t)offset.x, (int8_t)offset.y, bounds.x, bounds.y, 2, height,
                           offset.x, offset.y, height);
    }
...
static constexpr const int8_t s_bend_right_supports_type[4][4] = { { 0, 4, 2, 0 }, { 1, 5, 3, 1 }, { 0, 4, 2, 0 }, { 1, 5, 3, 1 } };

static constexpr const uint32_t miniature_railway_s_bend_right_tiles_track_floor[2][4] = { { SPR_FLOOR_PLANKS, SPR_FLOOR_PLANKS_S_SEGMENT,
...
static bool wooden_supports_paint_setup(paint_session * session, int32_t woodType, int32_t supportType, int32_t special,
                                        int32_t height, uint32_t imageColourFlags, bool * underground)
{
    switch (woodType)
    {
    case WOOD_A:
