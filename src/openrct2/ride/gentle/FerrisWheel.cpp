    ...
    const TrackElement& trackElement)
{
    ...
    if (edges & EdgeNW && track_paint_util_has_fence(EdgeNW, session.MapPosition, trackElement, ride, rotation))
    {
        imageId = colourFlags.WithIndex(SPR_FENCE_ROPE_NW);
        PaintAddImageAsChild(session, imageId, { 0, 0, height }, { 32, 1, 7 }, { 0, 2, height + 2 });
    }
    if (edges & EdgeNE && track_paint_util_has_fence(EdgeNE, session.MapPosition, trackElement, ride, rotation))
    {
        imageId = colourFlags.WithIndex(SPR_FENCE_ROPE_NE);
        PaintAddImageAsChild(session, imageId, { 0, 0, height }, { 1, 32, 7 }, { 2, 0, height + 2 });
    }
    if (edges & EdgeSE && track_paint_util_has_fence(EdgeSE, session.MapPosition, trackElement, ride, rotation))
    {
    ...
    if (edges & EdgeSW && track_paint_util_has_fence(EdgeSW, session.MapPosition, trackElement, ride, rotation))
    {
