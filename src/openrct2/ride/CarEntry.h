...
enum class CarEntryFlag : uint32_t
{
    /*
    ...
    /*
     * Used together with HAS_INVERTED_SPRITE_SET and RECALCULATE_SPRITE_BOUNDS and includes the inverted sprites into the
     * function that recalculates the sprite bounds. If the vehicle combines the spinning carriage and non-spinning
     * undercarriage in the same sprite.
     */
...
using CarEntryFlags = FlagHolder<uint32_t, CarEntryFlag>;
