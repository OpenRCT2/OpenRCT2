...
static void vehicle_visual_splash1_effect(PaintSession& session, int32_t z, const Vehicle* vehicle)
{
    ...
    int32_t image_id = SPR_SPLASH_EFFECT_1_NE_0 + ((((vehicle->SpriteData.Direction / 8) + session.CurrentRotation) & 3) * 8)
        + ((gCurrentTicks / 2) & 7);
...
{
    if (vehicle->SpriteData.Direction & 7)
    {
        return;
    }
    if (vehicle->Pitch != 0)
    ...
    int32_t image_id = SPR_SPLASH_EFFECT_3_NE_0 + ((((vehicle->SpriteData.Direction / 8) + session.CurrentRotation) & 3) * 8)
        + ((gCurrentTicks / 2) & 7);
    PaintAddImageAsChild(session, ImageId(image_id), { 0, 0, z }, { { 0, 0, z }, { 0, 0, 0 } });
}
...
static void vehicle_visual_splash4_effect(PaintSession& session, int32_t z, const Vehicle* vehicle)
{
    ...
    int32_t image_id = SPR_SPLASH_EFFECT_5_NE_0 + ((((vehicle->SpriteData.Direction / 8) + session.CurrentRotation) & 3) * 8)
        + ((gCurrentTicks / 2) & 7);
