...
rct_string_id OpenRCT2::RideModes::ForwardRotation::GetOperationErrorMessage(Ride* ride) const
{
    return STR_CANT_CHANGE_NUMBER_OF_ROTATIONS;
}

void OpenRCT2::RideModes::ForwardRotation::PeepChooseSeatFromCar(Peep* peep, Ride* ride, Vehicle* vehicle) const
{
    if (vehicle == nullptr)
    {
        return;
...
bool OpenRCT2::RideModes::ForwardRotation::FindVehicleToEnter(Guest* guest, Ride* ride, std::vector<uint8_t>& car_array) const
{
...
void OpenRCT2::RideModes::ForwardRotation::UpdateRideFreeVehicleCheck(Guest* guest) const
{
...
void OpenRCT2::RideModes::ForwardRotation::UpdateRideLeaveVehicle(Guest* guest) const
{
    auto ride = get_ride(guest->CurrentRide);
    if (ride == nullptr)
        return;
