...
rct_vehicle * cable_lift_segment_create(sint32 rideIndex, sint32 x, sint32 y, sint32 z, sint32 direction, uint16 var_44,
...
static bool sub_6DF01A_loop(rct_vehicle * vehicle)
{
    ...
    for (; vehicle->remaining_distance >= 13962; _vehicleUnkF64E10++)
    {
        ...
        rct_xyz16 unk           = { .x = moveInfo->x, .y = moveInfo->y, .z = moveInfo->z };
...
sint32 cable_lift_update_track_motion(rct_vehicle * cableLift)
{
    ...
    for (uint16 spriteId = cableLift->sprite_index; spriteId != SPRITE_INDEX_NULL;)
    {
        ...
        if ((INT32_MAX - abs(vehicle->acceleration)) < abs(var2CTotal) && (var2CTotal < 0) == (vehicle->acceleration < 0))
        {
