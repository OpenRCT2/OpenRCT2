    ...
    const TrackElement& trackElement)
{
    ...
    {
        auto imageId = session.TrackColours[SCHEME_TRACK].WithIndex(SplashBoats25DegUpImageIdChained[direction]);
    ...
    const TrackElement& trackElement)
{
    if (trackElement.HasChain())
    {
        auto imageId = session.TrackColours[SCHEME_TRACK].WithIndex(SplashBoatsFlatTo25DegUpImageIdChained[direction]);
        PaintAddImageAsParentRotated(session, direction, imageId, { 0, 0, height }, { { 0, 6, height }, { 32, 20, 2 } });
    }
    else
    {
    ...
    const TrackElement& trackElement)
{
    if (trackElement.HasChain())
    {
        auto imageId = session.TrackColours[SCHEME_TRACK].WithIndex(SplashBoats25DegUpToFlatImageIdChained[direction]);
        PaintAddImageAsParentRotated(session, direction, imageId, { 0, 0, height }, { { 0, 6, height }, { 32, 20, 2 } });
    }
    else
    {
