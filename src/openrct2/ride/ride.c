                                    ...
                                    rct_xy_element * output, sint32 * z, sint32 * direction)
{
    ...
    do
    {
        ...
        uint8 nextRotation = map_element_get_direction_with_offset(mapElement, nextTrackCoordinate->rotation_begin) |
                                        ...
                                        track_begin_end * outTrackBeginEnd)
{
    ...
    do
    {
        ...
        uint8 nextRotation = map_element_get_direction_with_offset(mapElement, nextTrackCoordinate->rotation_end) |
        ...
        nextRotation = map_element_get_direction_with_offset(mapElement, nextTrackCoordinate->rotation_begin) |
                       (nextTrackCoordinate->rotation_begin & (1 << 2));
        ...
        if (track_block_get_next_from_zero(x, y, z, rideIndex, direction, &next_track, &z, &direction))
        {
            ...
            game_do_command(next_track.x, flags | ((direction & 3) << 8), next_track.y,
                            next_track.element->properties.track.type |
                                (map_element_get_track_sequence(next_track.element) << 8),
...
static void ride_entrance_exit_connected(Ride * ride, sint32 ride_idx)
{
    for (sint32 i = 0; i < MAX_STATIONS; ++i)
    {
        rct_xy8 station_start = ride->station_starts[i], entrance = ride->entrances[i], exit = ride->exits[i];
...
static rct_vehicle * vehicle_create_car(sint32 rideIndex, sint32 vehicleEntryIndex, sint32 carIndex, sint32 vehicleIndex,
                                        sint32 x, sint32 y, sint32 z, sint32 * remainingDistance, rct_map_element * mapElement)
{
    registers regs = { 0 };
...
static train_ref vehicle_create_train(sint32 rideIndex, sint32 x, sint32 y, sint32 z, sint32 vehicleIndex,
                                      sint32 * remainingDistance, rct_map_element * mapElement)
{
    Ride * ride = get_ride(rideIndex);
        ...
        case TRACK_ELEM_DIAG_60_DEG_UP_TO_FLAT:
            if (track_element_is_lift_hill(trackElement))
            {
                ...
                do
                {
                    ...
                    if (map_element_get_track_sequence(mapElement) != 0)
                        continue;
                    if (mapElement->base_height != trackBeginEnd.begin_z / 8)
                        continue;
 ...
 *  rct2: 0x006B4EA6
 */
void game_command_set_ride_status(sint32 * eax, sint32 * ebx, sint32 * ecx, sint32 * edx, sint32 * esi, sint32 * edi,
                                  sint32 * ebp)
{
 ...
 *  rct2: 0x006B578B
 */
void game_command_set_ride_name(sint32 * eax, sint32 * ebx, sint32 * ecx, sint32 * edx, sint32 * esi, sint32 * edi,
                                sint32 * ebp)
{
    char        oldName[128];
    ...
    rct_string_id newUserStringId =
        user_string_allocate(USER_STRING_HIGH_ID_NUMBER | USER_STRING_DUPLICATION_PERMITTED, newName);
        ...
        if (type != TRACK_ELEM_INVERTED_90_DEG_UP_TO_FLAT_QUARTER_LOOP)
        {
            money32 removePrice =
                game_do_command(x, GAME_COMMAND_FLAG_APPLY | (rotation << 8), y,
                                type | (map_element_get_track_sequence(it.element) << 8), GAME_COMMAND_REMOVE_TRACK, z, 0);
...
void game_command_callback_ride_construct_new(sint32 eax, sint32 ebx, sint32 ecx, sint32 edx, sint32 esi, sint32 edi,
                                              sint32 ebp)
{
 ...
 *  rct2: 0x006B49D9
 */
void game_command_demolish_ride(sint32 * eax, sint32 * ebx, sint32 * ecx, sint32 * edx, sint32 * esi, sint32 * edi,
                                sint32 * ebp)
{
            ...
            if (mapX >= 0 && mapY >= 0 && mapX < (256 * 32) && mapY < (256 * 32))
            {
                ...
                do
                {
                    ...
                    if (FlatRideTrackSequenceProperties[mapElement->properties.track.type]
                                                       [map_element_get_track_sequence(mapElement)] &
                        (1 << eax))
                    {
                        gRideEntranceExitPlaceDirection = direction ^ 2;
                        *outDirection                   = direction ^ 2;
...
void sub_6CB945(sint32 rideIndex)
{
    ...
    if (ride->type != RIDE_TYPE_MAZE)
    {
        for (uint8 stationId = 0; stationId < MAX_STATIONS; ++stationId)
        {
            ...
            rct_xyz16 location  = { .x = ride->station_starts[stationId].x * 32,
