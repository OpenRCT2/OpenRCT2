        ...
        if (!_changedPluginFiles.empty())
        {
            ...
            for (const auto& path : _changedPluginFiles)
            {
                auto findResult = std::find_if(
                    _plugins.begin(), _plugins.end(),
                    [&path](const std::shared_ptr<Plugin>& plugin) { return Path::Equals(path, plugin->GetPath()); });
...
const static EnumMap<GameCommand> ActionNameToType = {
    ...
    IntervalHandle ScriptEngine::AddInterval(
        const std::shared_ptr<Plugin>& plugin, int32_t delay, bool repeat, DukValue&& callback)
    {
...
#    endif
    ...
    std::string Stringify(const DukValue& val)
    {
