...
utf8 *path_get_directory(const utf8 *path)
{
    ...
    char *filename = strrchr(path, *PREFERRED_PATH_SEPARATOR);
...
void path_end_with_separator(utf8 *path, size_t size) {
    ...
    if ((length == 0) || ((path[length - 1] != *PREFERRED_PATH_SEPARATOR) && path[length - 1] != '/')) {
        safe_strcat(path, PREFERRED_PATH_SEPARATOR, size);
    }
}
