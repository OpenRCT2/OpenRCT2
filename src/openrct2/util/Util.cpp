 ...
 */
std::vector<uint8_t> util_zlib_inflate(uint8_t* data, size_t data_in_size, size_t* data_out_size)
{
    int32_t ret = Z_OK;
    ...
    do
    {
        ...
        ret = uncompress(buffer.data(), &out_size, data, static_cast<uLong>(data_in_size));
    } while (ret != Z_OK);
