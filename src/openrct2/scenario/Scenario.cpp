...
{
    auto companyValue = gameState.parks[0].companyValue;
    ...
    {
        // Allow name entry
        gameState.parks[0].flags |= PARK_FLAGS_SCENARIO_COMPLETE_NAME_INPUT;
...
ResultWithMessage ScenarioPrepareForSave(GameState_t& gameState)
{
    ...
    if (gameState.scenarioOptions.objective.Type == ObjectiveType::guestsAndRating)
        gameState.parks[0].flags |= PARK_FLAGS_PARK_OPEN;
