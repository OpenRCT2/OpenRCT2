...
namespace OpenRCT2
{
    ...
    private:
        std::unique_ptr<rct1_s4> ReadAndDecodeS4(IStream* stream, bool isScenario)
        {
            ...
            auto data = std::unique_ptr<uint8_t, decltype(deleter_lambda)>(
                stream->ReadArray<uint8_t>(dataSize), deleter_lambda);
        ...
        void SetRideColourScheme(Ride* dst, rct1_ride* src)
        {
            ...
            else
            {
                for (int i = 0; i < RCT1_MAX_TRAINS_PER_RIDE; i++)
                {
                    ...
                    auto colourSchemeCopyDescriptor = RCT1::GetColourSchemeCopyDescriptor(src->vehicle_type);
            ...
            for (size_t i = 0; i < researchListCount; i++)
            {
                ...
                switch (researchItem->type)
                {
                    ...
                    case RCT1_RESEARCH_TYPE_RIDE:
                    {
                        ...
                        Guard::Assert(
                            ownRideEntryIndex != RIDE_ENTRY_INDEX_NULL, "ownRideEntryIndex was RIDE_ENTRY_INDEX_NULL");
...
{
    return std::make_unique<OpenRCT2::S4Importer>();
}

void load_from_sv4(const utf8* path)
{
    auto& objectMgr = OpenRCT2::GetContext()->GetObjectManager();
    auto s4Importer = std::make_unique<OpenRCT2::S4Importer>();
