...
namespace OpenRCT2
{
    ...
    public:
        ...
        TBase* Update(const void* data, size_t dataLen) override
        {
            auto status = BCryptHashData(
                _hHash, reinterpret_cast<PBYTE>(const_cast<void*>(data)), static_cast<ULONG>(dataLen), 0);
    ...
    private:
        ...
        void ImportKey(const RsaKeyParams& params)
        {
            ...
            _keyBlobType = params.GetMagic() == BCRYPT_RSAFULLPRIVATE_MAGIC ? BCRYPT_RSAFULLPRIVATE_BLOB
                                                                            : BCRYPT_RSAPUBLIC_BLOB;
            auto status = BCryptImportKeyPair(
                _hAlg, NULL, _keyBlobType, &_hKey, blob.data(), static_cast<ULONG>(blob.size()), 0);
