...
#    endif
        ...
        {
            // When passing no buffer and a size of 0, vsnprintf returns the numbers of chars it would have writte, excluding
            // the null terminator.
            va_list copy;
            va_copy(copy, args);
        ...
        std::string ConvertToUtf8(std::string_view src, int32_t srcCodePage)
        {
            ...
            std::string dst;
            {
                ...
                int sizeReq = WideCharToMultiByte(
                    OpenRCT2::CodePage::UTF8, 0, u16.data(), srcLen, nullptr, 0, nullptr, nullptr);
        ...
        std::string ToUpper(std::string_view src)
        {
            ...
            auto requiredSize = LCMapStringEx(
                LOCALE_NAME_USER_DEFAULT, LCMAP_UPPERCASE | LCMAP_LINGUISTIC_CASING, srcW.c_str(),
                static_cast<int>(srcW.length()), nullptr, 0, nullptr, nullptr, 0);
            ...
            auto result = LCMapStringEx(
                LOCALE_NAME_USER_DEFAULT, LCMAP_UPPERCASE | LCMAP_LINGUISTIC_CASING, srcW.c_str(),
                static_cast<int>(srcW.length()), dstW.data(), static_cast<int>(dstW.length()), nullptr, nullptr, 0);
            if (result == 0)
            {
                // Check the error
