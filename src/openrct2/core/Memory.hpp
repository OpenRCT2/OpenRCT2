...
namespace Memory
{
    ...
    template <typename T> static T * ReallocateArray(T * ptr, size_t count)
    {
        ...
        Guard::ArgumentNotNull(result, "Failed to reallocate array at %x (%s) to have %u entries", ptr, typeid(T).name(),
                               count);
        return result;
    }
    ...
    template <typename T> static T * Copy(T * dst, const T * src, size_t size)
    {
        if (size == 0)
            return (T *)dst;
#ifdef DEBUG
    ...
    template <typename T> static T * Move(T * dst, const T * src, size_t size)
    {
        if (size == 0)
            return (T *)dst;
        ...
        Guard::ArgumentNotNull(result, "Failed to move %u bytes of memory from %x to %x for %s", size, src, dst,
                               typeid(T).name());
        return result;
    }

    template <typename T> static T * Duplicate(const T * src, size_t size)
    {
        T * result = Allocate<T>(size);
        return Copy(result, src, size);
    }

    template <typename T> static T * Set(T * dst, uint8 value, size_t size)
    {
        return (T *)memset((void *)dst, (int)value, size);
    }

    template <typename T> static T * CopyArray(T * dst, const T * src, size_t count)
    {
    ...
    template <typename T> static T * DuplicateArray(const T * src, size_t count)
    {
