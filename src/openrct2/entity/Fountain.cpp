    ...
    const int32_t iteration)
{
    ...
    if (jumpingFountain != nullptr)
    {
        ...
        jumpingFountain->SpriteData.sprite_direction = direction << 3;
        jumpingFountain->SpriteData.sprite_width = 33;
        jumpingFountain->SpriteData.sprite_height_negative = 36;
        jumpingFountain->SpriteData.sprite_height_positive = 12;
...
void JumpingFountain::AdvanceAnimation()
{
    ...
    const int32_t direction = (SpriteData.sprite_direction >> 3) & 7;
...
{
    int32_t direction = (SpriteData.sprite_direction >> 3) << 1;
...
void JumpingFountain::Bounce(const CoordsXYZ& newLoc, const int32_t availableDirections)
{
    ...
    if (Iteration < 8)
    {
        int32_t direction = ((SpriteData.sprite_direction >> 3) ^ 2) << 1;
        if (availableDirections & (1 << direction))
        {
            CreateNext(newLoc, direction);
        }
...
void JumpingFountain::Paint(PaintSession& session, int32_t imageDirection) const
{
    ...
    // Fountain rotation
    bool rotated = (SpriteData.sprite_direction / 16) & 1;
