...
namespace OpenRCT2::Graph
{
    ...
    static void DrawMonths(DrawPixelInfo& dpi, const T* series, int32_t count, const ScreenRect& bounds, const int32_t xStep)
    {
        ...
        for (int32_t i = count - 1; i >= 0; i--)
        {
            if (series[i] != TkNoValue && yearOver32 % 4 == 0)
            {
                ...
                GfxFillRect(
                    dpi, { screenCoords - ScreenCoordsXY{ 0, 4 }, screenCoords - ScreenCoordsXY{ 0, 1 } }, PaletteIndex::_10);
            }
        ...
        const T minValue, const T maxValue, const_utf8string text, ColourWithFlags textCol)
    {
        ...
        GfxDrawDashedLine(
            ...
            kDashLength, PaletteIndex::_10);
        GfxDrawDashedLine(dpi, { { bounds.GetLeft(), coords.y }, coords }, kDashLength, PaletteIndex::_10);

        DrawText(dpi, coords - ScreenCoordsXY{ 0, 16 }, { textCol, TextAlignment::CENTRE }, text);

        GfxFillRect(dpi, { { coords - ScreenCoordsXY{ 2, 2 } }, coords + ScreenCoordsXY{ 2, 2 } }, PaletteIndex::_10);
        GfxFillRect(dpi, { { coords - ScreenCoordsXY{ 1, 1 } }, { coords + ScreenCoordsXY{ 1, 1 } } }, PaletteIndex::_21);
    }
            ...
            if (value != TkNoValue)
            {
                ...
                if constexpr (TbackgroundLine)
                {
                    if (lastCoordsValid)
                    {
                        ...
                        GfxDrawLine(dpi, { leftTop1, rightBottom1 }, PaletteIndex::_10);
                        GfxDrawLine(dpi, { leftTop2, rightBottom2 }, PaletteIndex::_10);
                    }
                    if (i == 0)
                    {
                        GfxFillRect(dpi, { coords, coords + ScreenCoordsXY{ 2, 2 } }, PaletteIndex::_10);
                    }
                }
                else
                {
                    if (lastCoordsValid)
                    {
                        ...
                        GfxDrawLine(dpi, { leftTop, rightBottom }, PaletteIndex::_21);
                    }
                    if (i == 0)
                    {
                        GfxFillRect(
                            dpi, { coords - ScreenCoordsXY{ 1, 1 }, coords + ScreenCoordsXY{ 1, 1 } }, PaletteIndex::_21);
                    }
                }
