...
namespace
{
    ...
    const SDL_Scancode kRawToScancode[128] = {
        ...
        SDL_SCANCODE_KP_5,
        SDL_SCANCODE_KP_6,
        /* 30 */ SDL_SCANCODE_NONUSBACKSLASH,
        SDL_SCANCODE_Z,
        SDL_SCANCODE_X,
        SDL_SCANCODE_C,
        SDL_SCANCODE_V,
    ...
    constexpr int kIEQ_LSHIFT = 0x0001, kIEQ_RSHIFT = 0x0002, kIEQ_CAPSLOCK = 0x0004, kIEQ_CONTROL = 0x0008, kIEQ_LALT = 0x0010,
                  kIEQ_RALT = 0x0020, kIEQ_LCOMMAND = 0x0040, kIEQ_RCOMMAND = 0x0080, kIEQ_NUMERICPAD = 0x0100,
                  kIEQ_REPEAT = 0x0200;
...
int SDL_ShowMessageBox(const SDL_MessageBoxData* data, int* buttonid)
{
    ...
    if (gadgets.empty())
        gadgets = "OK";
    int r = amiga_ui_request(
        data->title != nullptr ? data->title : "OpenRCT2", data->message != nullptr ? data->message : "", gadgets.c_str());
    if (buttonid != nullptr)
