...
void TextureCache::GeneratePaletteTexture()
{
    ...
    glCall(glBindTexture, GL_TEXTURE_2D, _paletteTexture);
    glCall(glTexImage2D, GL_TEXTURE_2D, 0, GL_R8UI, width, height, 0, GL_RED_INTEGER, GL_UNSIGNED_BYTE, dpi.bits);
    DeleteDPI(dpi);
}
...
AtlasTextureInfo TextureCache::LoadImageTexture(const ImageId imageId)
{
    ...
    glCall(glBindTexture, GL_TEXTURE_2D_ARRAY, _atlasesTexture);
    glCall(
        glTexSubImage3D, GL_TEXTURE_2D_ARRAY, 0, cacheInfo.bounds.x, cacheInfo.bounds.y, cacheInfo.index, dpi.width, dpi.height,
