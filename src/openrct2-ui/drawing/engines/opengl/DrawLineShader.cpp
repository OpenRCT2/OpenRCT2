    ...
    : OpenGLShaderProgram("drawline")
{
    ...
    glCall(glBufferData, GL_ARRAY_BUFFER, GLsizei(sizeof(kVertexData)), kVertexData, GL_STATIC_DRAW);

    glCall(glBindVertexArray, _vao);
    glCall(
        glVertexAttribPointer, vVertMat + 0, 2, GL_FLOAT, GL_FALSE, GLsizei(sizeof(VDStruct)),
        reinterpret_cast<void*>(offsetof(VDStruct, mat[0])));
    glCall(
        glVertexAttribPointer, vVertMat + 1, 2, GL_FLOAT, GL_FALSE, GLsizei(sizeof(VDStruct)),
        reinterpret_cast<void*>(offsetof(VDStruct, mat[1])));
    glCall(
        glVertexAttribPointer, vVertMat + 2, 2, GL_FLOAT, GL_FALSE, GLsizei(sizeof(VDStruct)),
        reinterpret_cast<void*>(offsetof(VDStruct, mat[2])));
    glCall(
        glVertexAttribPointer, vVertMat + 3, 2, GL_FLOAT, GL_FALSE, GLsizei(sizeof(VDStruct)),
        reinterpret_cast<void*>(offsetof(VDStruct, mat[3])));

    glCall(glBindBuffer, GL_ARRAY_BUFFER, _vboInstances);
    glCall(
        glVertexAttribIPointer, vBounds, 4, GL_INT, GLsizei(sizeof(DrawLineCommand)),
        reinterpret_cast<void*>(offsetof(DrawLineCommand, bounds)));
    glCall(
        glVertexAttribIPointer, vColour, 1, GL_UNSIGNED_INT, GLsizei(sizeof(DrawLineCommand)),
        reinterpret_cast<void*>(offsetof(DrawLineCommand, colour)));
    glCall(
        glVertexAttribIPointer, vDepth, 1, GL_INT, GLsizei(sizeof(DrawLineCommand)),
