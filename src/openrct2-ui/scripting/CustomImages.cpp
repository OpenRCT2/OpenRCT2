...
namespace OpenRCT2::Scripting
{
    ...
    JSValue JSGetImageInfo(JSContext* ctx, ImageIndex id)
    {
        ...
        JS_SetPropertyStr(ctx, obj, "offset", ToJSValue(ctx, ScreenCoordsXY{ g1->x_offset, g1->y_offset }));
        JS_SetPropertyStr(ctx, obj, "width", JS_NewInt32(ctx, g1->width));
        JS_SetPropertyStr(ctx, obj, "height", JS_NewInt32(ctx, g1->height));

        JS_SetPropertyStr(ctx, obj, "hasTransparent", JS_NewBool(ctx, (g1->flags & G1_FLAG_HAS_TRANSPARENCY) != 0));
        JS_SetPropertyStr(ctx, obj, "isRLE", JS_NewBool(ctx, (g1->flags & G1_FLAG_RLE_COMPRESSION) != 0));
        JS_SetPropertyStr(ctx, obj, "isPalette", JS_NewBool(ctx, (g1->flags & G1_FLAG_PALETTE) != 0));
        JS_SetPropertyStr(ctx, obj, "noZoom", JS_NewBool(ctx, (g1->flags & G1_FLAG_NO_ZOOM_DRAW) != 0));

        if (g1->flags & G1_FLAG_HAS_ZOOM_SPRITE)
        {
            JS_SetPropertyStr(ctx, obj, "nextZoomId", JS_NewInt32(ctx, id - g1->zoomed_offset));
        }
        else
        {
