...
static void WindowParkObjectivePaint(rct_window* w, rct_drawpixelinfo* dpi)
{
    ...
    // Objective
    ft = Formatter();
    if (gScenarioObjective.LegacyType == OBJECTIVE_BUILD_THE_BEST)
    {
        rct_string_id rideTypeString = STR_NONE;
        auto rideTypeId = std::static_pointer_cast<ObjectiveSpecificRideGoal>(gScenarioObjective.PhasedGoals[0].goals[0])
                              ->GetBaseRideType();
    ...
    screenCoords.y += DrawTextWrapped(dpi, screenCoords, 221, ObjectiveNames[gScenarioObjective.LegacyType], ft);
    screenCoords.y += 5;
