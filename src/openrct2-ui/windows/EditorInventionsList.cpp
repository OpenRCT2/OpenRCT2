...
bool insert(
    const rct_research_item& researchItem, const rct_research_item& before, std::vector<rct_research_item>& researchItems)
{
    for (auto it = researchItems.begin(); it != researchItems.end(); ++it)
    {
...
static void research_scenery_groups_setup()
{
    for (size_t i = 0; i < Util::CountOf(RequiredSelectedObjects); i++)
    {
        ...
        if (find_object_in_entry_group(object, &entry_type, &entryIndex) && entry_type == OBJECT_TYPE_SCENERY_GROUP)
        {
 ...
 */
static void
    window_editor_inventions_list_cursor(rct_window* w, rct_widgetindex widgetIndex, sint32 x, sint32 y, sint32* cursorId)
{
    rct_research_item* researchItem;
    sint32 scrollIndex;
    ...
    switch (widgetIndex)
    {
        case WIDX_PRE_RESEARCHED_SCROLL: scrollIndex = 0; break;
        case WIDX_RESEARCH_ORDER_SCROLL: scrollIndex = 1; break;
        default: return;
    }
...
{
    // Draw background
    const uint8 paletteIndex = ColourMapA[w->colours[1]].mid_light;
    gfx_clear(dpi, paletteIndex);

    const auto& listToPaint = scrollIndex ? ResearchItem::gResearchable : ResearchItem::gResearched;

    const sint16 boxWidth = (w->widgets[WIDX_RESEARCH_ORDER_SCROLL].right - w->widgets[WIDX_RESEARCH_ORDER_SCROLL].left);
    const sint16 columnSplitOffset = boxWidth / 2;
    ...
    for (const rct_research_item& researchItem : listToPaint)
    {
        ...
        if (itemY + SCROLLABLE_ROW_HEIGHT >= dpi->y || itemY < dpi->y + dpi->height)
        {
            ...
            {
                sint32 top, bottom;
                if (_editorInventionsListDraggedItem)
                {
            ...
            if (&researchItem != _editorInventionsListDraggedItem)
            {
                ...
                const sint32 left = 1;
                const sint32 top = itemY;

                if (research_item_is_always_researched(&researchItem))
                {
                    gCurrentFontSpriteBase = FONT_SPRITE_BASE_MEDIUM_DARK;
                    gfx_draw_string_left_clipped_no_font_reset(
                        dpi, itemNameId, nullptr, COLOUR_FLAG_INSET | w->colours[1], left, top, boxWidth);
                }
                else if (
                    ...
                    && !RideGroupManager::RideTypeIsIndependent(researchItem.baseRideType))
                {
                    ...
                    // Group name
                    gfx_draw_string_left_clipped(dpi, STR_BLACK_STRING, &args, 0, left, top, columnSplitOffset);

                    // Vehicle name
                    gfx_draw_string_left_clipped(
                        dpi, STR_BLACK_STRING, &itemNameId, 0, left + columnSplitOffset, top, columnSplitOffset);
                }
                else
                {
                    gfx_draw_string_left_clipped(dpi, STR_BLACK_STRING, &itemNameId, 0, left, top, boxWidth);
                }
            }
        }
    }
 ...
 */
static void
    window_editor_inventions_list_drag_cursor(rct_window* w, rct_widgetindex widgetIndex, sint32 x, sint32 y, sint32* cursorId)
{
