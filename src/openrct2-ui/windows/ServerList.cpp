...
static void window_server_list_paint(rct_window* w, rct_drawpixelinfo* dpi)
{
    ...
    gfx_draw_string_left(
        ...
        { w->windowPos.x + 6, w->windowPos.y + w->widgets[WIDX_PLAYER_NAME_INPUT].top });
    ...
    gfx_draw_string_left(
        ...
        { w->windowPos.x + 324, w->windowPos.y + w->widgets[WIDX_START_SERVER].top + 1 });

    gfx_draw_string_left(
        dpi, _statusText, static_cast<void*>(&_numPlayersOnline), COLOUR_WHITE,
        { w->windowPos.x + 8, w->windowPos.y + w->height - 15 });
}
