...
private:
    ...
    void WindowFootpathShowFootpathTypesDialog(Widget* widget, bool showQueues)
    {
        ...
        for (ObjectEntryIndex i = 0; i < MAX_FOOTPATH_SURFACE_OBJECTS; i++)
        {
            ...
            if (gCheatsState.AllowRegularPathAsQueue && !showQueues && ((pathType->Flags & FOOTPATH_ENTRY_FLAG_IS_QUEUE) != 0))
            {
                continue;
            }
            // If the cheat is disabled, hide queues from the regular path view and vice versa.
            else if (
                !gCheatsState.AllowRegularPathAsQueue && showQueues != ((pathType->Flags & FOOTPATH_ENTRY_FLAG_IS_QUEUE) != 0))
            {
                continue;
            }
