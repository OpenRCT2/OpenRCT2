...
public:
    void Init(const OpenRCT2String& message, ScreenCoordsXY screenCoords)
    {
        int32_t textWidth = FormatTextForTooltip(message);
        width = textWidth + 3;
        height = ((_tooltipNumLines + 1) * font_get_line_height(FontSpriteBase::SMALL)) + 4;
        window_tooltip_widgets[WIDX_BACKGROUND].right = width;
        window_tooltip_widgets[WIDX_BACKGROUND].bottom = height;
        ...
        /*window_set_position(*this, screenCoords);
        window_resize(*this, _width, _height);*/
...
void WindowTooltipShow(const OpenRCT2String& message, ScreenCoordsXY screenCoords)
{
    ...
    auto window = WindowCreate<WindowTooltip>(WindowClass::Tooltip, 0, 0, WF_TRANSPARENT | WF_STICK_TO_FRONT);
    window->Init(message, screenCoords);
