...
#pragma region Vehicle
        ...
        void VehicleOnScrollDraw(RenderTarget& rt, int32_t scrollIndex)
        {
            ...
            // Background
            Rect::fill(rt, { { rt.x, rt.y }, { rt.x + rt.width, rt.y + rt.height } }, PaletteIndex::pi12);
...
#pragma region Colour
        ...
        void ColourOnScrollDraw(RenderTarget& rt, int32_t scrollIndex) const
        {
            ...
            // Background colour
            Rect::fill(rt, { { rt.x, rt.y }, { rt.x + rt.width - 1, rt.y + rt.height - 1 } }, PaletteIndex::pi12);
...
#pragma region Measurements
        ...
        void MeasurementsOnDraw(RenderTarget& rt)
        {
            ...
            if (widgets[WIDX_SAVE_DESIGN].type == WidgetType::button)
            {
                ...
                Rect::fillInset(
                    rt, { widgetCoords, { windowPos.x + 312, widgetCoords.y + 1 } }, colours[1], Rect::BorderStyle::inset);
            }
            else
            {
                auto ride = GetRide(rideId);
        ...
        void GraphsOnScrollDraw(RenderTarget& rt, int32_t scrollIndex)
        {
            ...
            for (int32_t x = 0; x < rt.x + rt.width; x += 80)
            {
                if (x + 80 >= rt.x)
                {
                    ...
                    Rect::fill(rt, { coord1, coord2 }, lightColour);
                    Rect::fill(rt, { coord1 + ScreenCoordsXY{ 16, 0 }, coord2 + ScreenCoordsXY{ 16, 0 } }, darkColour);
                    Rect::fill(rt, { coord1 + ScreenCoordsXY{ 32, 0 }, coord2 + ScreenCoordsXY{ 32, 0 } }, darkColour);
                    Rect::fill(rt, { coord1 + ScreenCoordsXY{ 48, 0 }, coord2 + ScreenCoordsXY{ 48, 0 } }, darkColour);
                    Rect::fill(rt, { coord1 + ScreenCoordsXY{ 64, 0 }, coord2 + ScreenCoordsXY{ 64, 0 } }, darkColour);
                }
                time += 5;
            }
            ...
            for (int32_t y = widget->height() - 13; y >= 8; y -= yInterval, yUnit += yUnitInterval)
            {
                ...
                Rect::fill(rt, { { rt.x, y }, { rt.x + rt.width - 1, y } }, colour);
            ...
            for (int32_t graphWidth = 0; graphWidth < rt.width; graphWidth++, x++)
            {
                ...
                if (listType == GRAPH_VERTICAL || listType == GRAPH_LATERAL)
                {
                    ...
                    if (secondPoint >= intensityThresholdNegative)
                    {
                        ...
                        Rect::fill(rt, { redLineTop, redLineBottom }, redLineColour);
                    }
