...
static void window_ride_show_view_dropdown(rct_window* w, rct_widget* widget)
{
    ...
    // Vehicles
    int32_t name = RideComponentNames[EnumValue(RideTypeDescriptors[ride->type].NameConvention.vehicle)].number;
    ...
    // Stations
    name = RideComponentNames[EnumValue(RideTypeDescriptors[ride->type].NameConvention.station)].number;
...
static rct_string_id window_ride_get_status_vehicle(rct_window* w, Formatter& ft)
{
    ...
    const RideComponentName stationName = RideComponentNames[EnumValue(RideTypeDescriptors[ride->type].NameConvention.station)];
...
static void window_ride_main_paint(rct_window* w, rct_drawpixelinfo* dpi)
{
    ...
    if (w->ride.view != 0)
    {
        if (w->ride.view > ride->num_vehicles)
        {
            ft.Add<rct_string_id>(RideComponentNames[EnumValue(RideTypeDescriptors[ride->type].NameConvention.station)].number);
    ...
    rct_window* const w, const rct_widgetindex widgetIndex, rct_string_id fallback)
{
    ...
    switch (widgetIndex)
    {
        ...
        case WIDX_VEHICLE_TRAINS_INCREASE:
        {
            ...
            rct_string_id stringId = RideComponentNames[EnumValue(vehicleType)].count;
            if (ride->max_trains > 1)
            {
                stringId = RideComponentNames[EnumValue(vehicleType)].count_plural;
            }
            ft.Add<rct_string_id>(stringId);
        ...
        case WIDX_VEHICLE_CARS_PER_TRAIN_INCREASE:
        {
            ...
            rct_string_id stringId = RideComponentNames[EnumValue(RideComponentType::Car)].singular;
            if (ride->GetMaxCarsPerTrain() - rideEntry->zero_cars > 1)
            {
                stringId = RideComponentNames[EnumValue(RideComponentType::Car)].plural;
            }
            ft.Add<rct_string_id>(stringId);
            return { fallback, ft };
        }
...
static void window_ride_vehicle_invalidate(rct_window* w)
{
    ...
    RideComponentType vehicleType = RideTypeDescriptors[ride->type].NameConvention.vehicle;
    stringId = RideComponentNames[EnumValue(vehicleType)].count;
    if (ride->num_vehicles > 1)
    {
        stringId = RideComponentNames[EnumValue(vehicleType)].count_plural;
    }
