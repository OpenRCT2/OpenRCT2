        ...
        if (screenCoords.y + _row_height >= dpi->y)
        {
            ...
            for (uint8_t j = 0; j < numColours; j++)
            {
                ...
                if (colour & COLOUR_FLAG_TRANSLUCENT)
                {
                    ...
                    screenCoords.x = _button_offset_x + 12 * j;
                    screenCoords.y = screenCoords.y + _check_offset_y;
                    gfx_draw_string(
                        dpi, static_cast<const char*>(CheckBoxMarkString), w->colours[1] & 0x7F, screenCoords);
                }
            }
        }
