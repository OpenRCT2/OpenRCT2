...
static void window_editor_scenario_options_financial_paint(rct_window* w, rct_drawpixelinfo* dpi)
{
    ...
    {
        screenCoords = { w->windowPos.x + 8, w->windowPos.y + w->widgets[WIDX_INITIAL_CASH].top };
        gfx_draw_string_left(dpi, STR_INIT_CASH_LABEL, nullptr, COLOUR_BLACK, screenCoords);

        screenCoords = { w->windowPos.x + w->widgets[WIDX_INITIAL_CASH].left + 1,
                         w->windowPos.y + w->widgets[WIDX_INITIAL_CASH].top };
    ...
    {
        screenCoords = { w->windowPos.x + 8, w->windowPos.y + w->widgets[WIDX_INITIAL_LOAN].top };
        gfx_draw_string_left(dpi, STR_INIT_LOAN_LABEL, nullptr, COLOUR_BLACK, screenCoords);

        screenCoords = { w->windowPos.x + w->widgets[WIDX_INITIAL_LOAN].left + 1,
                         w->windowPos.y + w->widgets[WIDX_INITIAL_LOAN].top };
    ...
    {
        screenCoords = { w->windowPos.x + 8, w->windowPos.y + w->widgets[WIDX_MAXIMUM_LOAN].top };
        gfx_draw_string_left(dpi, STR_MAX_LOAN_LABEL, nullptr, COLOUR_BLACK, screenCoords);

        screenCoords = { w->windowPos.x + w->widgets[WIDX_MAXIMUM_LOAN].left + 1,
                         w->windowPos.y + w->widgets[WIDX_MAXIMUM_LOAN].top };
    ...
    {
        screenCoords = { w->windowPos.x + 8, w->windowPos.y + w->widgets[WIDX_INTEREST_RATE].top };
        gfx_draw_string_left(dpi, STR_INTEREST_RATE_LABEL, nullptr, COLOUR_BLACK, screenCoords);

        screenCoords = { w->windowPos.x + w->widgets[WIDX_INTEREST_RATE].left + 1,
                         w->windowPos.y + w->widgets[WIDX_INTEREST_RATE].top };
...
static void window_editor_scenario_options_guests_paint(rct_window* w, rct_drawpixelinfo* dpi)
{
    ...
    if (w->widgets[WIDX_CASH_PER_GUEST].type != WWT_EMPTY)
    {
        ...
        screenCoords = { w->windowPos.x + 8, w->windowPos.y + w->widgets[WIDX_CASH_PER_GUEST].top };
        gfx_draw_string_left(dpi, STR_CASH_PER_GUEST_LABEL, nullptr, COLOUR_BLACK, screenCoords);

        // Cash per guest value
        screenCoords = { w->windowPos.x + w->widgets[WIDX_CASH_PER_GUEST].left + 1,
                         w->windowPos.y + w->widgets[WIDX_CASH_PER_GUEST].top };
    ...
    screenCoords = { w->windowPos.x + 8, w->windowPos.y + w->widgets[WIDX_GUEST_INITIAL_HAPPINESS].top };
    gfx_draw_string_left(dpi, STR_GUEST_INIT_HAPPINESS, nullptr, COLOUR_BLACK, screenCoords);

    // Guest initial happiness value
    screenCoords = { w->windowPos.x + w->widgets[WIDX_GUEST_INITIAL_HAPPINESS].left + 1,
    ...
    screenCoords = { w->windowPos.x + 8, w->windowPos.y + w->widgets[WIDX_GUEST_INITIAL_HUNGER].top };
    gfx_draw_string_left(dpi, STR_GUEST_INIT_HUNGER, nullptr, COLOUR_BLACK, screenCoords);

    // Guest initial hunger value
    screenCoords = { w->windowPos.x + w->widgets[WIDX_GUEST_INITIAL_HUNGER].left + 1,
                     w->windowPos.y + w->widgets[WIDX_GUEST_INITIAL_HUNGER].top };
    arg = ((255 - gGuestInitialHunger) * 100) / 255;
    gfx_draw_string_left(dpi, STR_PERCENT_FORMAT_LABEL, &arg, COLOUR_BLACK, screenCoords);

    // Guest initial thirst label
    screenCoords = { w->windowPos.x + 8, w->windowPos.y + w->widgets[WIDX_GUEST_INITIAL_THIRST].top };
    gfx_draw_string_left(dpi, STR_GUEST_INIT_THIRST, nullptr, COLOUR_BLACK, screenCoords);

    // Guest initial thirst value
    screenCoords = { w->windowPos.x + w->widgets[WIDX_GUEST_INITIAL_THIRST].left + 1,
                     w->windowPos.y + w->widgets[WIDX_GUEST_INITIAL_THIRST].top };
...
static void window_editor_scenario_options_park_paint(rct_window* w, rct_drawpixelinfo* dpi)
{
    ...
    if (w->widgets[WIDX_LAND_COST].type != WWT_EMPTY)
    {
        ...
        screenCoords = { w->windowPos.x + 8, w->windowPos.y + w->widgets[WIDX_LAND_COST].top };
        gfx_draw_string_left(dpi, STR_LAND_COST_LABEL, nullptr, COLOUR_BLACK, screenCoords);

        // Cost to buy land value
        screenCoords = { w->windowPos.x + w->widgets[WIDX_LAND_COST].left + 1,
                         w->windowPos.y + w->widgets[WIDX_LAND_COST].top };
        arg = gLandPrice;
        gfx_draw_string_left(dpi, STR_CURRENCY_FORMAT_LABEL, &arg, COLOUR_BLACK, screenCoords);
    }
    ...
    if (w->widgets[WIDX_CONSTRUCTION_RIGHTS_COST].type != WWT_EMPTY)
    {
        ...
        screenCoords = { w->windowPos.x + 8, w->windowPos.y + w->widgets[WIDX_CONSTRUCTION_RIGHTS_COST].top };
        gfx_draw_string_left(dpi, STR_RIGHTS_COST_LABEL, nullptr, COLOUR_BLACK, screenCoords);

        // Cost to buy construction rights value
        screenCoords = { w->windowPos.x + w->widgets[WIDX_CONSTRUCTION_RIGHTS_COST].left + 1,
                         w->windowPos.y + w->widgets[WIDX_CONSTRUCTION_RIGHTS_COST].top };
    ...
    if (w->widgets[WIDX_PAY_FOR_PARK_OR_RIDES].type != WWT_EMPTY)
    {
        ...
        screenCoords = { w->windowPos.x + w->widgets[WIDX_PAY_FOR_PARK_OR_RIDES].left + 1,
                         w->windowPos.y + w->widgets[WIDX_PAY_FOR_PARK_OR_RIDES].top };
    ...
    {
        // Entry price label
        screenCoords = { w->windowPos.x + w->widgets[WIDX_PAY_FOR_PARK_OR_RIDES].right + 8,
                         w->windowPos.y + w->widgets[WIDX_ENTRY_PRICE].top };
        gfx_draw_string_left(dpi, STR_ENTRY_PRICE_LABEL, nullptr, COLOUR_BLACK, screenCoords);

        // Entry price value
        screenCoords = { w->windowPos.x + w->widgets[WIDX_ENTRY_PRICE].left + 1,
                         w->windowPos.y + w->widgets[WIDX_ENTRY_PRICE].top };
    ...
    // Climate label
    screenCoords = { w->windowPos.x + 8, w->windowPos.y + w->widgets[WIDX_CLIMATE].top };
    gfx_draw_string_left(dpi, STR_CLIMATE_LABEL, nullptr, COLOUR_BLACK, screenCoords);

    // Climate value
    screenCoords = { w->windowPos.x + w->widgets[WIDX_CLIMATE].left + 1, w->windowPos.y + w->widgets[WIDX_CLIMATE].top };
