...
#pragma region Objective
        ...
        void ShowObjectiveDropdown()
        {
            ...
            for (auto i = 0; i < EnumValue(Scenario::ObjectiveType::count); i++)
            {
                ...
                const bool objectiveAllowedByMoneyUsage = !(gameState.parks[0].flags & PARK_FLAGS_NO_MONEY)
        ...
        void ObjectiveOnPrepareDraw()
        {
            ...
            setWidgetPressed(WIDX_HARD_PARK_RATING, gameState.parks[0].flags & PARK_FLAGS_DIFFICULT_PARK_RATING);
        }
            ...
            switch (widgetIndex)
            {
                case WIDX_PARK_NAME:
                {
                    WindowTextInputRawOpen(
                        this, WIDX_PARK_NAME, STR_PARK_NAME, STR_ENTER_PARK_NAME, {}, gameState.parks[0].name.c_str(),
                        kParkNameMaxLength);
                    break;
                }
        ...
        void ScenarioDetailsOnTextInput(WidgetIndex widgetIndex, std::string_view text)
        {
            ...
            switch (widgetIndex)
            {
                case WIDX_PARK_NAME:
                {
                    ...
                    {
                        scenarioOptions.name = gameState.parks[0].name;
                    }
                    break;
                }
                case WIDX_SCENARIO_NAME:
        ...
        void FinancialMouseDown(WidgetIndex widgetIndex)
        {
            ...
            switch (widgetIndex)
            {
                ...
                case WIDX_INITIAL_LOAN_INCREASE:
                    if (gameState.parks[0].bankLoan < 5000000.00_GBP)
                    {
                        auto scenarioSetSetting = GameActions::ScenarioSetSettingAction(
                ...
                case WIDX_INITIAL_LOAN_DECREASE:
                    if (gameState.parks[0].bankLoan > 0.00_GBP)
                    {
                        auto scenarioSetSetting = GameActions::ScenarioSetSettingAction(
                            GameActions::ScenarioSetSetting::initialLoan, park.bankLoan - 1000.00_GBP);
                        GameActions::Execute(&scenarioSetSetting, gameState);
                ...
                case WIDX_MAXIMUM_LOAN_INCREASE:
                    if (gameState.parks[0].maxBankLoan < 5000000.00_GBP)
                    {
                        auto scenarioSetSetting = GameActions::ScenarioSetSettingAction(
                ...
                case WIDX_MAXIMUM_LOAN_DECREASE:
                    if (gameState.parks[0].maxBankLoan > 0.00_GBP)
                    {
                        auto scenarioSetSetting = GameActions::ScenarioSetSettingAction(
                            GameActions::ScenarioSetSetting::maximumLoanSize, park.maxBankLoan - 1000.00_GBP);
                        GameActions::Execute(&scenarioSetSetting, gameState);
                ...
                case WIDX_INTEREST_RATE_INCREASE:
                    if (gameState.parks[0].bankLoanInterestRate < kMaxBankLoanInterestRate)
                    {
                        auto scenarioSetSetting = GameActions::ScenarioSetSettingAction(
                ...
                case WIDX_INTEREST_RATE_DECREASE:
                    if (gameState.parks[0].bankLoanInterestRate > 0)
                    {
                ...
                case WIDX_ENTRY_PRICE_INCREASE:
                    if (gameState.parks[0].entranceFee < kMaxEntranceFee)
                    {
                        auto scenarioSetSetting = GameActions::ScenarioSetSettingAction(
                ...
                case WIDX_ENTRY_PRICE_DECREASE:
                    if (gameState.parks[0].entranceFee > 0.00_GBP)
                    {
                        auto scenarioSetSetting = GameActions::ScenarioSetSettingAction(
                            GameActions::ScenarioSetSetting::parkChargeEntryFee, park.entranceFee - 1.00_GBP);
                        GameActions::Execute(&scenarioSetSetting, gameState);
...
#pragma region Guests
        ...
        void GuestsDraw(RenderTarget& rt)
        {
            ...
            // Guests' intensity value
            {
                ...
                const auto preferLess = gameState.parks[0].flags & PARK_FLAGS_PREF_LESS_INTENSE_RIDES;
                const auto preferMore = gameState.parks[0].flags & PARK_FLAGS_PREF_MORE_INTENSE_RIDES;
