...
#pragma region Widgets
    ...
    static constexpr Widget window_editor_scenario_options_scenario_info_widgets[] = {
        MAIN_OPTIONS_WIDGETS(STR_OBJECTIVE_SELECTION, WW_SCENARIO_INFO, WH_SCENARIO_INFO),
        MakeWidget        ({370,  99-51}, { 75,  12}, WindowWidgetType::Button,   WindowColour::Secondary, STR_CHANGE,         STR_CHANGE_NAME_OF_PARK_TIP                    ),
        MakeWidget        ({370, 116-51}, { 75,  12}, WindowWidgetType::Button,   WindowColour::Secondary, STR_CHANGE,         STR_CHANGE_NAME_OF_SCENARIO_TIP                ),
        MakeWidget        ({ 98, 133-51}, {180,  12}, WindowWidgetType::DropdownMenu, WindowColour::Secondary, kStringIdNone,           STR_SELECT_WHICH_GROUP_THIS_SCENARIO_APPEARS_IN),
        MakeWidget        ({266, 134-51}, { 11,  10}, WindowWidgetType::Button,   WindowColour::Secondary, STR_DROPDOWN_GLYPH, STR_SELECT_WHICH_GROUP_THIS_SCENARIO_APPEARS_IN),
        MakeWidget        ({370, 150-51}, { 75,  12}, WindowWidgetType::Button,   WindowColour::Secondary, STR_CHANGE,         STR_CHANGE_DETAIL_NOTES_ABOUT_PARK_SCENARIO_TIP),
    };
    ...
    static constexpr Widget window_editor_scenario_options_guests_widgets[] = {
        MAIN_OPTIONS_WIDGETS(STR_SCENARIO_OPTIONS_GUESTS, WW_GUESTS, WH_GUESTS),
...
#pragma region ScenarioInfo

        /**
         *
         *  rct2: 0x006719CA
         */
        void ScenarioInfoOnMouseUp(WidgetIndex widgetIndex)
        {
            const auto& gameState = getGameState();
            switch (widgetIndex)
            {
        ...
        void ScenarioInfoOnMouseDown(WidgetIndex widgetIndex)
        {
            switch (widgetIndex)
            {
        ...
        void ScenarioInfoOnDropdown(WidgetIndex widgetIndex, int32_t dropdownIndex)
        {
            if (dropdownIndex == -1)
                return;
        ...
        void ScenarioInfoOnPrepareDraw()
        {
            SetPressedTab();
            AnchorBorderWidgets();
        }

        void ScenarioInfoOnDraw(DrawPixelInfo& dpi)
        {
            DrawWidgets(dpi);
            DrawTabImages(dpi);
         ...
         *  rct2: 0x00671A73
         */
        void ScenarioInfoOnTextInput(WidgetIndex widgetIndex, std::string_view text)
        {
            if (text.empty())
                return;
