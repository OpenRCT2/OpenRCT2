...
void window_staff_list_paint(rct_window* w, rct_drawpixelinfo* dpi)
{
    ...
    if (!(gParkFlags & PARK_FLAGS_NO_MONEY))
    {
        ...
        gfx_draw_string_left(
            dpi, STR_COST_PER_MONTH, gCommonFormatArgs, COLOUR_BLACK,
            { w->windowPos.x + w->width - 155, w->windowPos.y + 0x20 });
    }
    ...
    if (selectedTab < 3)
    {
        gfx_draw_string_left(
            ...
            { w->windowPos.x + 6, window_staff_list_widgets[WIDX_STAFF_LIST_UNIFORM_COLOUR_PICKER].top + w->windowPos.y + 1 });
    }
    ...
    gfx_draw_string_left(
        ...
        { w->windowPos.x + 4, window_staff_list_widgets[WIDX_STAFF_LIST_LIST].bottom + w->windowPos.y + 2 });
}
