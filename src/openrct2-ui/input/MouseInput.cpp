...
namespace OpenRCT2
{
    ...
    // Only ever set when the input originated from a touchscreen; a mouse can never set this.
...
#pragma region Mouse input
    ...
    /**
     * Whether the input being handled came from a finger rather than a mouse. False for every real
     * mouse event, which is what keeps all of the touch behaviour below off on the desktop.
     */
            ...
            case InputState::viewportLeft:
                ...
                switch (state)
                {
                    case MouseState::released:
                        ...
                        if (IsTouchInput() && !w->flags.has(WindowFlag::noScrolling)
                            && gLegacyScene != LegacyScene::titleSequence)
                        {
