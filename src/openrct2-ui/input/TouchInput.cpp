...
namespace
{
    // Two taps closer together than this are treated as a double tap, which maps to a right click.
    constexpr uint32_t kTouchDoubleTimeout = 300;
    ...
    {
        return Config::Get().interface.touchGestures;
    }

    bool SecondaryActionSuppressed()
    {
        return Config::Get().interface.touchNoSecondaryAction;
    }
} // namespace
...
void TouchInputManager::OnFingerDown(const SDL_Event& e, CursorState& cursorState, const ScreenCoordsXY& pos)
{
    ...
    if (SecondaryActionSuppressed())
    {
        ...
        cursorState.touchIsDouble = false;
    }
    else
    {
        cursorState.touchIsDouble
            = (!cursorState.touchIsDouble && e.tfinger.timestamp - cursorState.touchDownTimestamp < kTouchDoubleTimeout);
    }

    if (cursorState.touchIsDouble)
    {
        StoreMouseInput(MouseState::rightPress, pos);
        cursorState.right = CURSOR_PRESSED;
        cursorState.old = 2;
    }
    else
    {
    ...
    cursorState.touchDownTimestamp = e.tfinger.timestamp;
}
...
void TouchInputManager::OnFingerUp(const SDL_Event& e, CursorState& cursorState, const ScreenCoordsXY& pos)
{
    ...
    if (cursorState.touchIsDouble)
    {
        ...
        cursorState.right = CURSOR_RELEASED;
        cursorState.old = 4;
    }
    else
    {
...
bool TouchInputManager::HandleMultiGesture(const SDL_Event& e, int32_t width)
{
    ...
    // With gesture handling on, pinch zoom is driven by the finger events above, which can
    // zoom about the pinch centre rather than about wherever the cursor happens to be.
    if (GesturesEnabled() && _pinching)
        return true;
